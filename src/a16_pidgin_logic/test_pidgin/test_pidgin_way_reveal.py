...
def test_WayMap_reveal_inx_ReturnsObjAndSetsAttr_way_Scenario2_With_tag():
    # ESTABLISH
    otx_r_bridge = "/"
    inx_r_bridge = ":"
    otx_accord45_way = to_way("accord45", otx_r_bridge)
    ...
    clean_otx_way = f"{otx_accord45_way}{otx_r_bridge}{clean_otx_str}"
    way_waymap = waymap_shop(otx_bridge=otx_r_bridge, inx_bridge=inx_r_bridge)
    way_waymap.set_tag(clean_otx_str, clean_inx_str)
    assert way_waymap.otx_exists(otx_accord45_way) is False
    assert way_waymap.otx_exists(clean_otx_way) is False
...
def test_WayMap_reveal_inx_AddsMissingObjsTo_otx2inx_WayUnit():
    ...
    x_waymap.set_tag(otx_a45_str, inx_a87_str)
    x_waymap.set_tag(casa_otx_str, casa_inx_str)
    x_waymap.set_tag(clean_otx_str, clean_inx_str)
    print(f"{x_waymap.tagmap.otx2inx=}")

...
def test_LessonFileHandler_get_max_lesson_file_number_ReturnsObj(env_dir_setup_cleanup):
    ...
    six_int = 6
    save_file(
        sue_lessonfilehandler._lessons_dir,
...
def test_LessonFileHandler_save_lesson_file_SaveCorrectObj(env_dir_setup_cleanup):
    ...
    sue_lesson2_path = create_path(sue_lessonfilehandler._lessons_dir, two_filename)
    sue_lesson6_path = create_path(sue_lessonfilehandler._lessons_dir, six_filename)
    print(f"{sue_lesson2_path=}")
    print(f"{sue_lesson6_path=}")
    sue_lessonunit = lessonunit_shop(
        belief_name=sue_str,
        _lesson_id=two_int,
        _atoms_dir=sue_lessonfilehandler._atoms_dir,
        _lessons_dir=sue_lessonfilehandler._lessons_dir,
    )
    ...
    assert sue_lessonfilehandler.hub_lesson_file_exists(six_int) is False
    two_file_dict = open_json(sue_lessonfilehandler._lessons_dir, two_filename)
...
def test_LessonFileHandler_save_lesson_file_RaisesErrorIfLessonUnit_atoms_dir_IsWrong(
    ...
    sue_lesson0_path = create_path(sue_lessonfilehandler._lessons_dir, six_filename)
    ...
    expected_exception_str = f"LessonUnit file cannot be saved because lessonunit._atoms_dir is incorrect: {sue_lessonunit._atoms_dir}. It must be {sue_lessonfilehandler._atoms_dir}."
...
def test_LessonFileHandler_save_lesson_file_RaisesErrorIfLessonUnit_lessons_dir_IsWrong(
    ...
    sue_lessonunit = lessonunit_shop(
        ...
        _lesson_id=x_lesson_id,
        _atoms_dir=sue_lessonfilehandler._atoms_dir,
        _lessons_dir="src\\incorrect_directory",
    )
    ...
    expected_exception_str = f"LessonUnit file cannot be saved because lessonunit._lessons_dir is incorrect: {sue_lessonunit._lessons_dir}. It must be {sue_lessonfilehandler._lessons_dir}."
    assert str(excinfo.value) == expected_exception_str
...
def test_LessonFileHandler_validate_lessonunit_ReturnsObjWithAttributesFixed(
    ...
    invalid_sue_lessonunit = lessonunit_shop(
        ...
        _atoms_dir=create_path(sue_lessonfilehandler._atoms_dir, "swimming"),
        _lessons_dir=create_path(sue_lessonfilehandler._atoms_dir, "swimming"),
    )
    valid_lessonunit = sue_lessonfilehandler.validate_lessonunit(invalid_sue_lessonunit)

    # THEN
    assert valid_lessonunit._atoms_dir == sue_lessonfilehandler._atoms_dir
    assert valid_lessonunit._lessons_dir == sue_lessonfilehandler._lessons_dir
    assert (
...
def test_LessonFileHandler_save_lesson_file_SaveCorrectObj_correct_invalid_attrs_IsTrue(
    ...
    sue_lesson2_path = create_path(sue_lessonfilehandler._lessons_dir, next_filename)
    print(f"{sue_lesson2_path=}")
    ...
    two_file_json = open_file(sue_lessonfilehandler._lessons_dir, next_filename)
...
def test_LessonFileHandler_default_lessonunit_ReturnsObjWithCorrect_lesson_id_WhenNoLessonFilesExist(
    ...
    assert sue_lessonunit._atoms_dir == sue_lessonfilehandler._atoms_dir
    assert sue_lessonunit._lessons_dir == sue_lessonfilehandler._lessons_dir
...
def test_LessonFileHandler_default_lessonunit_ReturnsObjWithCorrect_lesson_id_WhenLessonFilesExist(
    ...
    zero_lessonunit._atoms_dir = sue_lessonfilehandler._atoms_dir
    zero_lessonunit._lessons_dir = sue_lessonfilehandler._lessons_dir
...
def test_LessonFileHandler_del_lesson_file_DeleteslessonjsonAndNotBeliefAtomjsons(
    ...
    print(f"{get_dir_file_strs(sue_lessonfilehandler._atoms_dir)}")
    assert sue_lessonfilehandler.hub_lesson_file_exists(six_int)
...
def test_LessonFileHandler_save_lesson_file_CanCreateAndModify3lessonunits(
    ...
    sue_lessonfilehandler = lessonfilehandler_shop(env_dir(), "amy23", sue_str)
    delete_dir(sue_lessonfilehandler._lessons_dir)
    delete_dir(sue_lessonfilehandler._atoms_dir)
    set_dir(sue_lessonfilehandler._lessons_dir)
    set_dir(sue_lessonfilehandler._atoms_dir)
    assert len(get_dir_file_strs(sue_lessonfilehandler._lessons_dir)) == 0
    assert len(get_dir_file_strs(sue_lessonfilehandler._atoms_dir)) == 0
    ...
    # THEN
    assert len(get_dir_file_strs(sue_lessonfilehandler._lessons_dir)) == 3
...
def test_LessonFileHandler_save_lesson_file_ReturnsValidObj(env_dir_setup_cleanup):
    ...
    sue2_lessonunit._atoms_dir = create_path(
        sue_lessonfilehandler._atoms_dir, "swimming"
    )
    sue2_lessonunit._lessons_dir = create_path(
        sue_lessonfilehandler._atoms_dir, "swimming"
    )
    ...
    # THEN
    assert valid_lessonunit._lessons_dir != prev_sue2_lessonunit._lessons_dir
    assert valid_lessonunit._lessons_dir == sue_lessonfilehandler._lessons_dir
    assert valid_lessonunit._atoms_dir == sue_lessonfilehandler._atoms_dir

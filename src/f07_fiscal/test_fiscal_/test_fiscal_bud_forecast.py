...
def test_FiscalUnit_generate_forecast_bud_Sets_forecast_BudFile(env_dir_setup_cleanup):
    # ESTABLISH
    ...
    # WHEN
    sue_forecast = accord_fiscal.generate_forecast_bud(sue_str)

    # THEN
    example_bud = budunit_shop(sue_str, accord45_str)
    assert sue_forecast.fiscal_title == example_bud.fiscal_title
    assert sue_forecast.owner_name == example_bud.owner_name


def test_FiscalUnit_generate_forecast_bud_ReturnsRegeneratedObj(env_dir_setup_cleanup):
    # ESTABLISH
    ...
    # WHEN
    after_sue_bud = accord_fiscal.generate_forecast_bud(sue_str)

    # THEN method should wipe over forecast bud
    assert after_sue_bud.acct_exists(bob_str) is False


def test_FiscalUnit_generate_forecast_bud_SetsCorrectFileWithout_healerlink(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    accord_fiscal = fiscalunit_shop("accord45", get_test_fiscals_dir(), True)
    ...
    before_bob_forecast_bud = accord_fiscal.generate_forecast_bud(bob_str)
    sue_str = "Sue"
    ...
    # WHEN
    after_bob_forecast_bud = accord_fiscal.generate_forecast_bud(bob_str)

    # THEN
    assert after_bob_forecast_bud.acct_exists(sue_str)


def test_FiscalUnit_generate_forecast_bud_SetsFileWith_healerlink(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    accord_fiscal = fiscalunit_shop("accord45", get_test_fiscals_dir(), True)
...
def test_FiscalUnit_generate_all_forecast_buds_SetsCorrectFiles(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    accord_fiscal = fiscalunit_shop("accord45", get_test_fiscals_dir(), True)
    ...
    bob_voice_bud = accord_fiscal.generate_forecast_bud(bob_str)
    sue_voice_bud = accord_fiscal.generate_forecast_bud(sue_str)
    ...
    before_bob_forecast_bud = accord_fiscal.get_forecast_file_bud(bob_str)
    before_sue_forecast_bud = accord_fiscal.get_forecast_file_bud(sue_str)
    assert before_bob_forecast_bud.acct_exists(bob_str) is False
    assert before_sue_forecast_bud.acct_exists(sue_str) is False

    # WHEN
    accord_fiscal.generate_all_forecast_buds()

    # THEN
    after_bob_forecast_bud = accord_fiscal.get_forecast_file_bud(bob_str)
    after_sue_forecast_bud = accord_fiscal.get_forecast_file_bud(sue_str)
    assert after_bob_forecast_bud.acct_exists(bob_str)

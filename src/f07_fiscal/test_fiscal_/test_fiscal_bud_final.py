...
def test_FiscalUnit_generate_final_bud_Sets_final_BudFile(env_dir_setup_cleanup):
    ...
    # WHEN
    sue_final = accord_fiscal.generate_final_bud(sue_str)

    # THEN
    example_bud = budunit_shop(sue_str, accord45_str)
    assert sue_final.fiscal_title == example_bud.fiscal_title
...
def test_FiscalUnit_generate_final_bud_ReturnsRegeneratedObj(env_dir_setup_cleanup):
    ...
    # WHEN
    after_sue_bud = accord_fiscal.generate_final_bud(sue_str)
...
def test_FiscalUnit_generate_final_bud_SetsCorrectFileWithout_healerlink(
    ...
    before_bob_final_bud = accord_fiscal.generate_final_bud(bob_str)
    sue_str = "Sue"
    ...
    # WHEN
    after_bob_final_bud = accord_fiscal.generate_final_bud(bob_str)
...
def test_FiscalUnit_generate_all_final_buds_SetsCorrectFiles(
    ...
    bob_voice_bud = accord_fiscal.generate_final_bud(bob_str)
    sue_voice_bud = accord_fiscal.generate_final_bud(sue_str)
    ...
    before_bob_final_bud = accord_fiscal.get_final_file_bud(bob_str)
    before_sue_final_bud = accord_fiscal.get_final_file_bud(sue_str)
    assert before_bob_final_bud.acct_exists(bob_str) is False
    assert before_sue_final_bud.acct_exists(sue_str) is False

    # WHEN
    accord_fiscal.generate_all_final_buds()

    # THEN
    after_bob_final_bud = accord_fiscal.get_final_file_bud(bob_str)
    after_sue_final_bud = accord_fiscal.get_final_file_bud(sue_str)
    assert after_bob_final_bud.acct_exists(bob_str)

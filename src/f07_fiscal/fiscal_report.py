...
def get_fiscal_forecasts_accts_dataframe(x_fiscal: FiscalUnit) -> DataFrame:
    # get list of all owner paths
    owner_hubunits = x_fiscal.get_owner_hubunits()
...
def get_fiscal_forecasts_accts_plotly_fig(x_fiscal: FiscalUnit) -> plotly_Figure:
    column_header_list = [
        "owner_name",
        "acct_name",
        "credit_belief",
    ...
    df = get_fiscal_forecasts_accts_dataframe(x_fiscal)
    header_dict = dict(
        values=column_header_list, fill_color="paleturquoise", align="left"
    )
    x_table = plotly_Table(
    ...
    fig = plotly_Figure(data=[x_table])
    fig_title = f"fiscal '{x_fiscal.fiscal_title}', forecast accts metrics"
    fig.update_xaxes(showgrid=False)
    fig.update_yaxes(showgrid=False, zeroline=True, showticklabels=False)
    fig.update_layout(plot_bgcolor="white", title=fig_title, title_font_size=20)
...
def get_fiscal_forecasts_agenda_dataframe(x_fiscal: FiscalUnit) -> DataFrame:
    # get list of all owner paths
    owner_hubunits = x_fiscal.get_owner_hubunits()
    # for all owners get forecast
    forecast_dfs = []
...
def get_fiscal_forecasts_agenda_plotly_fig(x_fiscal: FiscalUnit) -> plotly_Figure:
    column_header_list = [
        "owner_name",
        "fund_ratio",
        "_item_title",
    ...
    df = get_fiscal_forecasts_agenda_dataframe(x_fiscal)
    header_dict = dict(
        values=column_header_list, fill_color="paleturquoise", align="left"
    )
    x_table = plotly_Table(
    ...
    fig = plotly_Figure(data=[x_table])
    fig_title = f"fiscal '{x_fiscal.fiscal_title}', forecast agenda metrics"
    fig.update_xaxes(showgrid=False)
    fig.update_yaxes(showgrid=False, zeroline=True, showticklabels=False)
    fig.update_layout(plot_bgcolor="white", title=fig_title, title_font_size=20)

...
def test_get_contacts_view_dict_ReturnsObj_Scenario1_contacts():
    ...
    sue_believer.add_contactunit(exx.bob, bob_cred_mass, bob_debt_mass)
    sue_believer.thinkout()
    ...
    contact_cred_mass_readable_key = add_readable(kw.contact_cred_mass)
    contact_debt_mass_readable_key = add_readable(kw.contact_debt_mass)
    ...
    irrational_contact_debt_mass_readable_key = add_readable(
        kw.irrational_contact_debt_mass
    )
    inallocable_contact_debt_mass_readable_key = add_readable(
    ...
    assert set(yao_contact_dict.keys()) == {
        ...
        contact_cred_mass_readable_key,
        contact_debt_mass_readable_key,
        memberships_readable_key,
        credor_pool_readable_key,
        debtor_pool_readable_key,
        irrational_contact_debt_mass_readable_key,
        inallocable_contact_debt_mass_readable_key,
    ...
    assert ypu.contact_cred_mass == yp_dict.get(kw.contact_cred_mass)
    assert ypu.contact_debt_mass == yp_dict.get(kw.contact_debt_mass)
    assert ypu.credor_pool == yp_dict.get(kw.credor_pool)
    assert ypu.debtor_pool == yp_dict.get(kw.debtor_pool)
    assert ypu.irrational_contact_debt_mass == yp_dict.get(
        kw.irrational_contact_debt_mass
    )
    assert ypu.inallocable_contact_debt_mass == yp_dict.get(
        kw.inallocable_contact_debt_mass
    )
    ...
    expected_contact_cred_mass_readable = f"contact_cred_mass: {ypu.contact_cred_mass}"
    expected_contact_debt_mass_readable = f"contact_debt_mass: {ypu.contact_debt_mass}"
    ...
    expected_irrational_contact_debt_mass_readable = (
        f"irrational_contact_debt_mass: {ypu.irrational_contact_debt_mass}"
    )
    expected_inallocable_contact_debt_mass_readable = (
        f"inallocable_contact_debt_mass: {ypu.inallocable_contact_debt_mass}"
    )
    ...
    assert (
        yp_dict.get(contact_cred_mass_readable_key)
        == expected_contact_cred_mass_readable
    )
    assert (
        yp_dict.get(contact_debt_mass_readable_key)
        == expected_contact_debt_mass_readable
    )
    ...
    assert (
        yp_dict.get(irrational_contact_debt_mass_readable_key)
        == expected_irrational_contact_debt_mass_readable
    )
    assert (
        yp_dict.get(inallocable_contact_debt_mass_readable_key)
        == expected_inallocable_contact_debt_mass_readable
    )
...
def test_get_contacts_view_dict_ReturnsObj_Scenario2_memberships():
    ...
    expected_group_cred_mass_readable = add_small_dot(expected_group_cred_mass_readable)
    expected_group_debt_mass_readable = add_small_dot(expected_group_debt_mass_readable)

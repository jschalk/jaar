...
from src.a01_term_logic.term import GroupTitle, PersonName, default_knot_if_None
...
class GroupUnit(GroupCore):
    _memberships: dict[PersonName, MemberShip] = (
        None  # set by BelieverUnit.set_personunit()
    )
    ...
    def set_membership(self, x_membership: MemberShip):
        ...
        self._memberships[x_membership.person_name] = x_membership
    ...
    def get_membership(self, x_person_name: PersonName) -> MemberShip:
        return self._memberships.get(x_person_name)

    def membership_exists(self, x_person_name: PersonName) -> bool:
        return self.get_membership(x_person_name) is not None

    def del_membership(self, person_name):
        self._memberships.pop(person_name)
    ...
    def _set_membership_fund_give_fund_take(self):
        credit_ledger = {}
        debt_ledger = {}
        for x_person_name, x_membership in self._memberships.items():
            credit_ledger[x_person_name] = x_membership.group_cred_points
            debt_ledger[x_person_name] = x_membership.group_debt_points
        ...
        for person_name, x_membership in self._memberships.items():
            x_membership._fund_give = fund_give_allot.get(person_name)
            x_membership._fund_take = fund_take_allot.get(person_name)
        ...
        for person_name, x_membership in self._memberships.items():
            x_membership._fund_agenda_give = fund_agenda_give_allot.get(person_name)
            x_membership._fund_agenda_take = fund_agenda_take_allot.get(person_name)

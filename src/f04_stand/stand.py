...
from src.f01_road.jaar_config import get_init_stand_id_if_None, get_json_filename
...
from src.f04_stand.atom import BudAtom, get_from_json as budatom_get_from_json
from src.f04_stand.delta import (
    BudDelta,
    buddelta_shop,
...
class stand_bud_conflict_Exception(Exception):
    pass
...
class StandUnit:
    ...
    def set_delta_start(self, x_delta_start: int):
        self._delta_start = get_init_stand_id_if_None(x_delta_start)
    ...
    def get_delta_atom_numbers(self, standunit_dict: list[str]) -> int:
        delta_dict = standunit_dict.get("delta")
    ...
    def _save_stand_file(self):
        x_filename = self._get_num_filename(self._stand_id)
        save_file(self._stands_dir, x_filename, self.get_deltametric_json())

    def stand_file_exists(self) -> bool:
        x_filename = self._get_num_filename(self._stand_id)
        return os_path_exists(create_path(self._stands_dir, x_filename))
    ...
    def get_edited_bud(self, before_bud: BudUnit) -> BudUnit:
        if (
            ...
            raise stand_bud_conflict_Exception(
                f"stand bud conflict {self.fisc_title} != {before_bud.fisc_title} or {self.owner_name} != {before_bud.owner_name}"
            )
...
def standunit_shop(
    ...
    x_standunit = StandUnit(
        ...
        _stand_id=get_init_stand_id_if_None(_stand_id),
        _buddelta=_buddelta,
        _stands_dir=_stands_dir,
        _atoms_dir=_atoms_dir,
        event_int=event_int,
    )
    x_standunit.set_delta_start(_delta_start)
    return x_standunit


def create_standunit_from_files(
    stands_dir: str,
    stand_id: str,
    atoms_dir: str,
) -> StandUnit:
    stand_filename = get_json_filename(stand_id)
    stand_dict = open_json(stands_dir, stand_filename)
    x_owner_name = stand_dict.get("owner_name")
    x_fisc_title = stand_dict.get("fisc_title")
    x_face_name = stand_dict.get("face_name")
    delta_atom_numbers_list = stand_dict.get("delta_atom_numbers")
    ...
    x_standunit._create_buddelta_from_atom_files(delta_atom_numbers_list)
    return x_standunit


def get_standunit_from_json(x_json: str) -> StandUnit:
    stand_dict = get_dict_from_json(x_json)
    if stand_dict.get("event_int") is None:
        x_event_int = None
    else:
        x_event_int = int(stand_dict.get("event_int"))
    x_standunit = standunit_shop(
        face_name=stand_dict.get("face_name"),
        owner_name=stand_dict.get("owner_name"),
        fisc_title=stand_dict.get("fisc_title"),
        _stand_id=stand_dict.get("stand_id"),
        _atoms_dir=stand_dict.get("atoms_dir"),
        event_int=x_event_int,
    )
    x_buddelta = get_buddelta_from_ordered_dict(stand_dict.get("delta"))
    x_standunit.set_buddelta(x_buddelta)
    return x_standunit

...
def get_owner_accts_plotly_fig(x_owner: OwnerUnit) -> plotly_Figure:
    column_header_list = [
        "acct_name",
        "_credor_respect",
        "acct_cred_points",
    ...
    df = get_owner_acctunits_dataframe(x_owner)
    df.insert(1, "_credor_respect", x_owner.credor_respect)
    df.insert(4, "_debtor_respect", x_owner.debtor_respect)
    header_dict = dict(
    ...
    fig_label = f"OwnerName '{x_owner.owner_name}' owner accts metrics"
...
def get_owner_agenda_plotly_fig(x_owner: OwnerUnit) -> plotly_Figure:
    column_header_list = [
        "owner_name",
        "fund_ratio",
        "plan_label",
    ...
    fig = plotly_Figure(data=[x_table])
    fig_label = f"OwnerName '{x_owner.owner_name}' owner agenda"
    fig.update_xaxes(showgrid=False)
    fig.update_yaxes(showgrid=False, zeroline=True, showticklabels=False)
    fig.update_layout(plot_bgcolor="white", title=fig_label, title_font_size=20)
...
def get_ownerunit_base_fig() -> plotly_Figure:
    fig = plotly_Figure()
...
def ownerunit_graph0(graphics_bool) -> plotly_Figure:
    if not graphics_bool:
        return
    fig = get_ownerunit_base_fig()
    ...
    fig.add_trace(
        plotly_Scatter(
            x=[2.0],
            y=[3.75],
            text=["What Owners Are Made Of: Graph 0"],
            mode="text",
        )
    )
...
def ownerunit_graph1(graphics_bool) -> plotly_Figure:
    fig = get_ownerunit_base_fig()
...
def ownerunit_graph2(graphics_bool) -> plotly_Figure:
    fig = get_ownerunit_base_fig()
...
def ownerunit_graph3(graphics_bool) -> plotly_Figure:
    fig = get_ownerunit_base_fig()
...
def ownerunit_graph4(graphics_bool) -> plotly_Figure:
    fig = get_ownerunit_base_fig()

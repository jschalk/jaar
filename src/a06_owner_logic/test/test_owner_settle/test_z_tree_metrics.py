...
def test_OwnerUnit_get_tree_metrics_exists():
    # ESTABLISH
    zia_owner = ownerunit_shop(owner_name="Zia")

    # WHEN
    zia_owner_tree_metrics = zia_owner.get_tree_metrics()

    # THEN
    assert zia_owner_tree_metrics.label_count is not None
    assert zia_owner_tree_metrics.reason_rcontexts is not None
    assert zia_owner_tree_metrics.level_count is not None
...
def test_OwnerUnit_get_tree_metrics_TracksReasonsThatHaveNoFactRcontexts():
    ...
    # WHEN
    yao_tree_metrics = yao_ownerunit.get_tree_metrics()
...
def test_OwnerUnit_get_missing_fact_rcontexts_ReturnsAllRcontextsNotCoveredByFacts():
    # ESTABLISH
    yao_ownerunit = ownerunit_v001()
    missing_rcontexts = yao_ownerunit.get_missing_fact_rcontexts()
    ...
    yao_ownerunit.add_fact(
        yao_ownerunit.make_l1_rope("day_minute"),
        fstate=yao_ownerunit.make_l1_rope("day_minute"),
        fopen=0,
        fnigh=1439,
    )
...
def test_OwnerUnit_3AdvocatesNoconceptunit_shop():
    ...
    zia_ownerunit = ownerunit_shop("Zia")
    ...
    accts_metrics = zia_ownerunit.get_awardlinks_metrics()

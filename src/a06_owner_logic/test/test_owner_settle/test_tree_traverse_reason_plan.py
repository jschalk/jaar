...
def test_OwnerUnit_ReasonUnits_create():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    casa_str = "casa"
    casa_rope = sue_owner.make_l1_rope(casa_str)
    wkday_str = "wkdays"
    wkday_rope = sue_owner.make_l1_rope(wkday_str)
    wed_str = "Wednesday"
    wed_rope = sue_owner.make_rope(wkday_rope, wed_str)
    ...
    # WHEN
    sue_owner.edit_plan_attr(casa_rope, reason=casa_wk_reason)
...
def test_OwnerUnit_edit_plan_attr_reasonunit_CorrectlySets_knot():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    ...
    # WHEN
    sue_owner.edit_plan_attr(casa_rope, reason=before_wk_reason)

    # THEN
    casa_plan = sue_owner.get_plan_obj(casa_rope)
    wk_reasonunit = casa_plan.reasonunits.get(wk_rope)
    assert wk_reasonunit.knot != slash_str
    assert wk_reasonunit.knot == sue_owner.knot


def test_OwnerUnit_edit_plan_attr_reason_rcontext_CorrectlySets_knot():
    # ESTABLISH
    slash_str = "/"
    ...
    assert wk_reasonunit.knot == bob_owner.knot


def test_OwnerUnit_set_reasonunits_status():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    casa_str = "casa"
    casa_rope = sue_owner.make_l1_rope(casa_str)
...
def test_agenda_returned_WhenNoReasonsExist():
    ...
    # THEN
    casa_rope = sue_owner.make_l1_rope("casa")
    assert sue_owner.get_plan_obj(casa_rope)._chore is True
    cat_rope = sue_owner.make_l1_rope("cat have dinner")
    assert sue_owner.get_plan_obj(cat_rope)._chore is True


def test_OwnerUnit_reasonheirs_AreCorrectlyInherited_v1():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    casa_rope = sue_owner.make_l1_rope("casa")
    wk_rope = sue_owner.make_l1_rope("wkdays")
    ...
    sue_owner.edit_plan_attr(casa_rope, reason=casa_wk_build_reasonunit)
    casa_plan = sue_owner.get_plan_obj(casa_rope)
...
def test_OwnerUnit_reasonheirs_AreCorrectlyInheritedTo4LevelsFromRoot():
    # ESTABLISH
    a4_owner = get_ownerunit_with_4_levels()
    casa_str = "casa"
    casa_rope = a4_owner.make_l1_rope(casa_str)
    wk_str = "wkdays"
    wk_rope = a4_owner.make_l1_rope(wk_str)
    wed_str = "Wednesday"
    wed_rope = a4_owner.make_rope(wk_rope, wed_str)
    ...
    a4_owner.edit_plan_attr(casa_rope, reason=casa_wk_build_reasonunit)

    # WHEN
    rla_str = "hp"
    rla_rope = a4_owner.make_rope(casa_rope, rla_str)
    a4_owner.set_plan(planunit_shop(rla_str), parent_rope=rla_rope)
    cost_str = "cost_quantification"
    cost_rope = a4_owner.make_rope(rla_rope, cost_str)
    a4_owner.set_plan(planunit_shop(cost_str), parent_rope=cost_rope)
    a4_owner.settle_owner()

    # THEN
    casa_plan = a4_owner.planroot._kids[casa_str]
    ...
    assert (
        rla_wk_reasonheir.rplan_active_requisite
        == casa_wk_built_reasonheir.rplan_active_requisite
    )
    ...
    assert (
        cost_wk_reasonheir.rplan_active_requisite
        == casa_wk_built_reasonheir.rplan_active_requisite
    )
...
def test_OwnerUnit_reasonheirs_AreCorrectlyInheritedTo4LevelsFromLevel2():
    a4_owner = get_ownerunit_with_4_levels()
    casa_str = "casa"
    casa_rope = a4_owner.make_l1_rope(casa_str)
    wk_plan_label = "wkdays"
    wk_rope = a4_owner.make_l1_rope(wk_plan_label)
    wed_str = "Wednesday"
    wed_rope = a4_owner.make_rope(wk_rope, wed_str)
    ...
    casa_plan = a4_owner.planroot.get_kid(casa_str)
    rla_plan = casa_plan.get_kid(rla_str)
    cost_plan = rla_plan.get_kid(cost_str)

    assert a4_owner.planroot._reasonheirs == {}
    ...
    # THEN
    assert a4_owner.planroot._reasonheirs == {}  # casa_wk_built_reasonheir
...
def test_OwnerUnit_ReasonUnits_set_UnCoupledMethod():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    casa_str = "casa"
    casa_rope = sue_owner.make_l1_rope(casa_str)
    ...
    # THEN
    casa_plan1 = sue_owner.get_plan_obj(casa_rope)
    assert casa_plan1.reasonunits is not None
    print(casa_plan1.reasonunits)
    assert casa_plan1.reasonunits[wk_rope] is not None
    assert casa_plan1.reasonunits[wk_rope].premises[wed_rope].popen is None
    assert casa_plan1.reasonunits[wk_rope].premises[wed_rope].pnigh is None
    ...
    # THEN
    assert casa_plan1.reasonunits[wk_rope].premises[wed_rope].popen == 12
    assert casa_plan1.reasonunits[wk_rope].premises[wed_rope].pnigh == 12
    ...
    # WHEN
    thu_str = "Thursday"
    thu_rope = sue_owner.make_rope(wk_rope, thu_str)
    sue_owner.edit_plan_attr(
        casa_rope,
        reason_rcontext=wk_rope,
...
def test_OwnerUnit_ReasonUnits_set_premisePlanWithDenomSetsPremiseDivision():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    casa_str = "casa"
    casa_rope = sue_owner.make_l1_rope(casa_str)
    ...
    wk_str = "wk"
    wk_rope = sue_owner.make_rope(time_rope, wk_str)
    sue_owner.set_l1_plan(planunit_shop(time_str, begin=100, close=2000))
    sue_owner.set_plan(planunit_shop(wk_str, denom=7), parent_rope=time_rope)
    ...
    assert casa_plan1.reasonunits[time_rope].premises[wk_rope].pdivisor == 7
    assert casa_plan1.reasonunits[time_rope].premises[wk_rope].popen == 2
    assert casa_plan1.reasonunits[time_rope].premises[wk_rope].pnigh == 5


def test_OwnerUnit_ReasonUnits_set_premisePlanWithBeginCloseSetsPremisePopen_Pnigh():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    casa = "casa"
    casa_rope = sue_owner.make_l1_rope(casa)
    ...
    rus_war = "rus_war"
    rus_war_rope = sue_owner.make_rope(time_rope, rus_war)
    sue_owner.set_plan(
        planunit_shop(time, begin=100, close=2000), sue_owner.belief_label
    )
    sue_owner.set_plan(planunit_shop(rus_war, begin=22, close=34), time_rope)
    ...
    casa_plan1 = sue_owner.get_plan_obj(casa_rope)
    assert casa_plan1.reasonunits[time_rope] is not None
    assert casa_plan1.reasonunits[time_rope].premises[rus_war_rope].pdivisor is None
    assert casa_plan1.reasonunits[time_rope].premises[rus_war_rope].popen == 22
    assert casa_plan1.reasonunits[time_rope].premises[rus_war_rope].pnigh == 34


def test_OwnerUnit_ReasonUnits_edit_plan_attr_CorrectlyDeletes_ReasonUnits_And_PremiseUnits():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    casa_rope = sue_owner.make_l1_rope("casa")
    wkday_rope = sue_owner.make_l1_rope("wkdays")
    wed_rope = sue_owner.make_rope(wkday_rope, "Wednesday")

    sue_owner.edit_plan_attr(
        casa_rope, reason_rcontext=wkday_rope, reason_premise=wed_rope
    )
    thu_rope = sue_owner.make_rope(wkday_rope, "Thursday")
    sue_owner.edit_plan_attr(
        casa_rope,
...
def test_OwnerUnit_ReasonUnits_del_reason_premise_UncoupledMethod2():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    casa_rope = sue_owner.make_l1_rope("casa")
    ...
    with pytest_raises(Exception) as excinfo:
        casa_plan1.del_reasonunit_rcontext(wkdays_rope)
    assert str(excinfo.value) == f"No ReasonUnit at '{wkdays_rope}'"


def test_OwnerUnit_edit_plan_attr_ownerIsAbleToEdit_rplan_active_requisite_AnyPlanIfInvaildThrowsError():
    # _rplan_active_requisite: str = None
    ...
    run_str = "run to casa"
    run_rope = sue_owner.make_l1_rope(run_str)
    sue_owner.set_plan(planunit_shop(run_str), sue_owner.belief_label)
    sue_owner.settle_owner()  # set tree metrics
    run_plan = sue_owner.get_plan_obj(run_rope)
    ...
    assert reasonunit_casa.rplan_active_requisite is True
    ...
    assert reasonunit_casa.rcontext == casa_rope
    assert len(reasonunit_casa.premises) == 0
    assert reasonunit_casa.rplan_active_requisite is False

    # WHEN
    sue_owner.edit_plan_attr(
        run_rope,
        reason_rcontext=casa_rope,
        reason_rplan_active_requisite="Set to Ignore",
    )
    ...
    assert reasonunit_casa.rcontext == casa_rope
    assert len(reasonunit_casa.premises) == 0
    assert reasonunit_casa.rplan_active_requisite is None


def test_OwnerUnit_ReasonUnits_PlanUnit_active_InfluencesReasonUnitStatus():
    # ESTABLISH an Owner with 5 plans, 1 Fact:
    ...
    casa_rope = sue_owner.make_l1_rope(casa_str)
    wkdays_str = "wkdays"
    wkdays_rope = sue_owner.make_l1_rope(wkdays_str)
    wed_str = "Wednesday"
    wed_rope = sue_owner.make_rope(wkdays_rope, wed_str)
    thu_str = "Thursday"
    thu_rope = sue_owner.make_rope(wkdays_rope, thu_str)
    ...
    # 5.1. ReasonUnit: plan(rcontext=...,casa) has .rplan_active_requisite = True
    ...
    sue_owner.add_fact(fcontext=wkdays_rope, fstate=wed_rope)
    sue_owner.settle_owner()
    ...
    sue_owner.add_fact(fcontext=wkdays_rope, fstate=thu_rope)
...
def test_OwnerUnit_settle_owner_SetsRationalAttrToFalseWhen_max_tree_traverse_Is1():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    assert sue_owner._rational is False
    # sue_owner.settle_owner()
    sue_owner._rational = True
    assert sue_owner._rational

    # WHEN
    # hack owner to set _max_tree_traverse = 1 (not allowed, should be 2 or more)
    sue_owner.max_tree_traverse = 1
    sue_owner.settle_owner()
...
def test_OwnerUnit_tree_traverse_count_SetByTotalNumberOfTreeTraversesEndsStatusIsDetected():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    assert sue_owner.max_tree_traverse != 2

    # WHEN
    sue_owner.settle_owner()
    # for plan_key in sue_owner._plan_dict.keys():
    #     print(f"{plan_key=}")

    # THEN
    assert sue_owner._tree_traverse_count == 2


def test_OwnerUnit_tree_traverse_count_CorrectlyCountsTreeTraversesForIrrationalOwners():
    # ESTABLISH irrational owner
    sue_owner = get_ownerunit_irrational_example()
    sue_owner.settle_owner()
    assert sue_owner._tree_traverse_count == 3

    # WHEN
    sue_owner.set_max_tree_traverse(21)
    sue_owner.settle_owner()

    # THEN
    assert sue_owner._tree_traverse_count == 21

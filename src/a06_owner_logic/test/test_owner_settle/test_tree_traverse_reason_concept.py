...
def test_OwnerUnit_ReasonUnits_create():
    ...
    # WHEN
    sue_owner.edit_concept_attr(casa_rope, reason=casa_wk_reason)
...
def test_OwnerUnit_edit_concept_attr_reasonunit_CorrectlySets_knot():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    ...
    # WHEN
    sue_owner.edit_concept_attr(casa_rope, reason=before_wk_reason)
...
def test_OwnerUnit_edit_concept_attr_reason_rcontext_CorrectlySets_knot():
    # ESTABLISH
    slash_str = "/"
...
def test_agenda_returned_WhenNoReasonsExist():
    ...
    # THEN
    casa_rope = sue_owner.make_l1_rope("casa")
    assert sue_owner.get_concept_obj(casa_rope)._chore is True
    cat_rope = sue_owner.make_l1_rope("cat have dinner")
    assert sue_owner.get_concept_obj(cat_rope)._chore is True
...
def test_OwnerUnit_reasonheirs_AreCorrectlyInherited_v1():
    ...
    sue_owner.edit_concept_attr(casa_rope, reason=casa_wk_build_reasonunit)
    casa_concept = sue_owner.get_concept_obj(casa_rope)
...
def test_OwnerUnit_reasonheirs_AreCorrectlyInheritedTo4LevelsFromRoot():
    ...
    a4_owner.edit_concept_attr(casa_rope, reason=casa_wk_build_reasonunit)

    # WHEN
    rla_str = "hp"
    rla_rope = a4_owner.make_rope(casa_rope, rla_str)
    a4_owner.set_concept(conceptunit_shop(rla_str), parent_rope=rla_rope)
    cost_str = "cost_quantification"
    cost_rope = a4_owner.make_rope(rla_rope, cost_str)
    a4_owner.set_concept(conceptunit_shop(cost_str), parent_rope=cost_rope)
    a4_owner.settle_owner()

    # THEN
    casa_concept = a4_owner.conceptroot._kids[casa_str]
...
def test_OwnerUnit_reasonheirs_AreCorrectlyInheritedTo4LevelsFromLevel2():
    ...
    wk_rope = a4_owner.make_l1_rope(wk_concept_label)
    wed_str = "Wednesday"
    wed_rope = a4_owner.make_rope(wk_rope, wed_str)
    ...
    casa_concept = a4_owner.conceptroot.get_kid(casa_str)
    rla_concept = casa_concept.get_kid(rla_str)
    cost_concept = rla_concept.get_kid(cost_str)

    assert a4_owner.conceptroot._reasonheirs == {}
    ...
    # THEN
    assert a4_owner.conceptroot._reasonheirs == {}  # casa_wk_built_reasonheir
...
def test_OwnerUnit_ReasonUnits_set_UnCoupledMethod():
    ...
    # THEN
    casa_concept1 = sue_owner.get_concept_obj(casa_rope)
...
def test_OwnerUnit_ReasonUnits_set_premiseConceptWithDenomSetsPremiseDivision():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    casa_str = "casa"
    casa_rope = sue_owner.make_l1_rope(casa_str)
    ...
    sue_owner.set_l1_concept(conceptunit_shop(time_str, begin=100, close=2000))
    sue_owner.set_concept(conceptunit_shop(wk_str, denom=7), parent_rope=time_rope)
...
def test_OwnerUnit_ReasonUnits_set_premiseConceptWithBeginCloseSetsPremisePopen_Pnigh():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    casa = "casa"
    casa_rope = sue_owner.make_l1_rope(casa)
    ...
    sue_owner.set_concept(
        conceptunit_shop(time, begin=100, close=2000), sue_owner.belief_label
    )
    sue_owner.set_concept(conceptunit_shop(rus_war, begin=22, close=34), time_rope)
...
def test_OwnerUnit_ReasonUnits_edit_concept_attr_CorrectlyDeletes_ReasonUnits_And_PremiseUnits():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    casa_rope = sue_owner.make_l1_rope("casa")
...
def test_OwnerUnit_edit_concept_attr_ownerIsAbleToEdit_rconcept_active_requisite_AnyConceptIfInvaildThrowsError():
    ...
    sue_owner.set_concept(conceptunit_shop(run_str), sue_owner.belief_label)
    sue_owner.settle_owner()  # set tree metrics
    run_concept = sue_owner.get_concept_obj(run_rope)
...
def test_OwnerUnit_ReasonUnits_ConceptUnit_active_InfluencesReasonUnitStatus():
    # ESTABLISH an Owner with 5 concepts, 1 Fact:
...
def test_OwnerUnit_tree_traverse_count_SetByTotalNumberOfTreeTraversesEndsStatusIsDetected():
    ...
    # WHEN
    sue_owner.settle_owner()
    # for concept_key in sue_owner._concept_dict.keys():
    #     print(f"{concept_key=}")

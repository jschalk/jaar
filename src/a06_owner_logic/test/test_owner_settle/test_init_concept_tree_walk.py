...
from src.a06_owner_logic.owner import get_sorted_concept_list, ownerunit_shop
...
def test_OwnerUnit_set_concept_dict_Scenario0():
    # ESTABLISH
    ...
    assert yao_owner._concept_dict == {}
    ...
    assert not root_concept._gogo_calc
    assert not root_concept._stop_calc
    assert yao_owner._concept_dict == {root_concept.get_concept_rope(): root_concept}
    assert yao_owner._reason_rcontexts == set()


def test_OwnerUnit_set_concept_dict_Scenario1():
    # ESTABLISH
    yao_owner = ownerunit_shop("Yao")
    time0_begin = 7
    time0_close = 31
...
def test_OwnerUnit_set_concept_dict_Clears_gogo_calc_stop_calc():
    # ESTABLISH
    ...
    texas_str = "Texas"
    texas_rope = sue_owner.make_rope(usa_rope, texas_str)
...
def test_OwnerUnit_set_concept_dict_Sets_reason_rcontexts():
    # ESTABLISH
    sue_owner = ownerunit_shop("Sue")
    nation_str = "nation"
    nation_rope = sue_owner.make_l1_rope(nation_str)
    polis_str = "polis"
    polis_rope = sue_owner.make_l1_rope(polis_str)
    sue_owner.add_concept(polis_rope)
    sue_owner.add_concept(nation_rope)
    ...
    # THEN
    assert sue_owner._reason_rcontexts == {polis_rope}


def test_OwnerUnit_set_concept_CreatesConceptUnitsUsedBy_reasonunits():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    casa_rope = sue_owner.make_l1_rope("casa")
    cleaning_rope = sue_owner.make_rope(casa_rope, "cleaning")
    ...
    buildings_rope = sue_owner.make_l1_rope(buildings_str)
    cookery_room_str = "cookery"
    cookery_room_rope = sue_owner.make_rope(buildings_rope, cookery_room_str)
    ...
    assert sue_owner.concept_exists(buildings_rope) is False
    ...
    # THEN
    assert sue_owner.concept_exists(buildings_rope)
...
def test_get_sorted_concept_list_ReturnsObj():
    ...
    x_sorted_concept_list = get_sorted_concept_list(
        list(sue_owner._concept_dict.values())
    )

    # THEN
    assert x_sorted_concept_list is not None
    assert len(x_sorted_concept_list) == 17
    assert x_sorted_concept_list[0] == sue_owner.conceptroot
    assert x_sorted_concept_list[1] == sue_owner.get_concept_obj(casa_rope)
    assert x_sorted_concept_list[11] == sue_owner.get_concept_obj(mon_rope)

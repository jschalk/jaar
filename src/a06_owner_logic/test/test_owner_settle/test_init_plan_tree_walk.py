...
from src.a06_owner_logic.owner import get_sorted_plan_list, ownerunit_shop
...
def test_OwnerUnit_set_plan_dict_Scenario0():
    # ESTABLISH
    ...
    assert not root_plan._gogo_calc
    assert not root_plan._stop_calc
    assert yao_owner._plan_dict == {root_plan.get_plan_rope(): root_plan}
    assert yao_owner._reason_rcontexts == set()


def test_OwnerUnit_set_plan_dict_Scenario1():
    # ESTABLISH
    yao_owner = ownerunit_shop("Yao")
    time0_begin = 7
    time0_close = 31
...
def test_OwnerUnit_set_plan_dict_Clears_gogo_calc_stop_calc():
    # ESTABLISH
...
def test_OwnerUnit_set_plan_dict_Sets_reason_rcontexts():
    # ESTABLISH
    ...
    sue_owner.add_plan(polis_rope)
    sue_owner.add_plan(nation_rope)
...
def test_OwnerUnit_set_plan_CreatesPlanUnitsUsedBy_reasonunits():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    ...
    assert sue_owner.plan_exists(buildings_rope) is False

    # WHEN
    sue_owner.set_plan(clean_cookery_plan, cleaning_rope, create_missing_plans=True)

    # THEN
    assert sue_owner.plan_exists(buildings_rope)
...
def test_get_sorted_plan_list_ReturnsObj():
    ...
    # WHEN
    x_sorted_plan_list = get_sorted_plan_list(list(sue_owner._plan_dict.values()))

    # THEN
    assert x_sorted_plan_list is not None
    assert len(x_sorted_plan_list) == 17
    assert x_sorted_plan_list[0] == sue_owner.planroot
    assert x_sorted_plan_list[1] == sue_owner.get_plan_obj(casa_rope)
    assert x_sorted_plan_list[11] == sue_owner.get_plan_obj(mon_rope)

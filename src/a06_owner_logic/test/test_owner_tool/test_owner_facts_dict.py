from copy import deepcopy as copy_deepcopy
from src.a01_term_logic.rope import create_rope
from src.a06_owner_logic.owner import ownerunit_shop
from src.a06_owner_logic.owner_tool import (
    clear_factunits_from_owner,
    get_owner_root_facts_dict,
    set_factunits_to_owner,
)


def test_get_owner_root_facts_dict_ReturnsObj_Scenario0_No_factunits():
    # ESTABLISH
    sue_str = "Sue"
    sue_owner = ownerunit_shop(sue_str)
    # WHEN / THEN
    assert get_owner_root_facts_dict(sue_owner) == {}
    assert get_owner_root_facts_dict(sue_owner) == sue_owner.get_factunits_dict()


def test_get_owner_root_facts_dict_ReturnsObj_Scenario1_factunits_Exist():
    # ESTABLISH
    sue_str = "Sue"
    sue_owner = ownerunit_shop(sue_str)
    casa_rope = sue_owner.make_l1_rope("case")
    clean_rope = sue_owner.make_l1_rope("clean")
    dirty_rope = sue_owner.make_l1_rope("dirty")
    sue_owner.add_fact(casa_rope, dirty_rope, create_missing_plans=True)

    # WHEN
    sue_fact_dict = get_owner_root_facts_dict(sue_owner)

    # THEN
    assert sue_fact_dict.get(casa_rope) != None
    casa_fact_dict = sue_fact_dict.get(casa_rope)
    assert casa_fact_dict.get("fcontext") == casa_rope
    assert casa_fact_dict.get("fstate") == dirty_rope
    expected_sue_fact_dict = {casa_rope: {"fcontext": casa_rope, "fstate": dirty_rope}}
    print(f"{sue_fact_dict=}")
    print(f"{expected_sue_fact_dict=}")
    assert sue_fact_dict == expected_sue_fact_dict


def test_get_owner_root_facts_dict_ReturnsObj_Scenario2_factunits_Exist():
    # ESTABLISH
    sue_str = "Sue"
    sue_owner = ownerunit_shop(sue_str)
    casa_rope = sue_owner.make_l1_rope("case")
    clean_rope = sue_owner.make_l1_rope("clean")
    dirty_rope = sue_owner.make_l1_rope("dirty")
    dirty_popen = 10
    dirty_pnigh = 13
    sue_owner.add_fact(casa_rope, dirty_rope, dirty_popen, dirty_pnigh, True)

    # WHEN
    sue_fact_dict = get_owner_root_facts_dict(sue_owner)

    # THEN
    assert sue_fact_dict.get(casa_rope) != None
    casa_fact_dict = sue_fact_dict.get(casa_rope)
    assert casa_fact_dict.get("fcontext") == casa_rope
    assert casa_fact_dict.get("fstate") == dirty_rope
    assert casa_fact_dict.get("fopen") == dirty_popen
    assert casa_fact_dict.get("fnigh") == dirty_pnigh
    expected_sue_fact_dict = {
        casa_rope: {
            "fcontext": casa_rope,
            "fstate": dirty_rope,
            "fopen": dirty_popen,
            "fnigh": dirty_pnigh,
        }
    }
    print(f"{sue_fact_dict=}")
    print(f"{expected_sue_fact_dict=}")
    assert sue_fact_dict == expected_sue_fact_dict


def test_set_factunits_to_owner_ReturnsObj_Scenario0_OwnerEmptyNoFacts():
    # ESTABLISH
    yao_owner = ownerunit_shop("Yao", "amy23")
    before_yao_owner = copy_deepcopy(yao_owner)
    facts_dict = {}
    assert yao_owner.get_dict() == before_yao_owner.get_dict()

    # WHEN
    set_factunits_to_owner(yao_owner, facts_dict)

    # THEN
    assert yao_owner.get_dict() == before_yao_owner.get_dict()


def test_set_factunits_to_owner_ReturnsObj_Scenario1_Owner1FactsChanged():
    # ESTABLISH
    bob_owner = ownerunit_shop("Bob", "amy23")
    casa_str = "casa"
    floor_str = "floor status"
    clean_str = "clean"
    dirty_str = "dirty"
    mop_str = "mop"
    casa_rope = bob_owner.make_l1_rope(casa_str)
    floor_rope = bob_owner.make_rope(casa_rope, floor_str)
    clean_rope = bob_owner.make_rope(floor_rope, clean_str)
    dirty_rope = bob_owner.make_rope(floor_rope, dirty_str)
    mop_rope = bob_owner.make_rope(casa_rope, mop_str)
    bob_owner.add_plan(floor_rope)
    bob_owner.add_plan(clean_rope)
    bob_owner.add_plan(dirty_rope)
    bob_owner.add_plan(mop_rope, task=True)
    bob_owner.edit_plan_attr(
        mop_rope, reason_rcontext=floor_rope, reason_premise=dirty_rope
    )
    dirty_facts_dict = {floor_rope: {"fcontext": floor_rope, "fstate": dirty_rope}}
    before_bob_owner = copy_deepcopy(bob_owner)
    assert bob_owner.get_factunits_dict() != dirty_facts_dict
    assert bob_owner.get_factunits_dict() == {}
    assert bob_owner.get_dict() == before_bob_owner.get_dict()

    # WHEN
    set_factunits_to_owner(bob_owner, dirty_facts_dict)

    # THEN
    assert bob_owner.get_dict() != before_bob_owner.get_dict()
    assert bob_owner.get_factunits_dict() == dirty_facts_dict


def test_set_factunits_to_owner_ReturnsObj_Scenario2_FactUnit_rcontext_DoesNotExistInOwner():
    # ESTABLISH
    bob_owner = ownerunit_shop("Bob", "amy23")
    casa_str = "casa"
    floor_str = "floor status"
    clean_str = "clean"
    dirty_str = "dirty"
    mop_str = "mop"
    casa_rope = bob_owner.make_l1_rope(casa_str)
    floor_rope = bob_owner.make_rope(casa_rope, floor_str)
    clean_rope = bob_owner.make_rope(floor_rope, clean_str)
    dirty_rope = bob_owner.make_rope(floor_rope, dirty_str)
    mop_rope = bob_owner.make_rope(casa_rope, mop_str)
    bob_owner.add_plan(floor_rope)
    # bob_owner.add_plan(clean_rope)
    bob_owner.add_plan(dirty_rope)
    bob_owner.add_plan(mop_rope, task=True)
    bob_owner.edit_plan_attr(
        mop_rope, reason_rcontext=floor_rope, reason_premise=dirty_rope
    )
    clean_facts_dict = {floor_rope: {"fcontext": floor_rope, "fstate": clean_rope}}
    before_bob_owner = copy_deepcopy(bob_owner)
    assert bob_owner.get_factunits_dict() != clean_facts_dict
    assert bob_owner.get_factunits_dict() == {}
    assert bob_owner.get_dict() == before_bob_owner.get_dict()

    # WHEN
    set_factunits_to_owner(bob_owner, clean_facts_dict)

    # THEN
    assert bob_owner.get_dict() != before_bob_owner.get_dict()
    assert bob_owner.get_factunits_dict() == clean_facts_dict
    assert bob_owner.get_plan_obj(clean_rope)


def test_set_factunits_to_owner_ReturnsObj_Scenario3_FactUnit_rcontext_WithoutRcontextNotAddedToOwner():
    # ESTABLISH
    bob_owner = ownerunit_shop("Bob", "amy23")
    casa_str = "casa"
    floor_str = "floor status"
    clean_str = "clean"
    dirty_str = "dirty"
    mop_str = "mop"
    casa_rope = bob_owner.make_l1_rope(casa_str)
    floor_rope = bob_owner.make_rope(casa_rope, floor_str)
    clean_rope = bob_owner.make_rope(floor_rope, clean_str)
    dirty_rope = bob_owner.make_rope(floor_rope, dirty_str)
    mop_rope = bob_owner.make_rope(casa_rope, mop_str)
    bob_owner.add_plan(floor_rope)
    # bob_owner.add_plan(clean_rope)
    bob_owner.add_plan(dirty_rope)
    bob_owner.add_plan(mop_rope, task=True)
    bob_owner.edit_plan_attr(
        mop_rope, reason_rcontext=floor_rope, reason_premise=dirty_rope
    )

    weather_str = "weather"
    raining_str = "raining"
    weather_rope = bob_owner.make_l1_rope(weather_str)
    rain_rope = bob_owner.make_rope(weather_rope, raining_str)

    two_facts_dict = {
        floor_rope: {"fcontext": floor_rope, "fstate": clean_rope},
        weather_rope: {"fcontext": weather_rope, "fstate": rain_rope},
    }
    before_bob_owner = copy_deepcopy(bob_owner)
    assert bob_owner.get_factunits_dict() != two_facts_dict
    assert bob_owner.get_factunits_dict() == {}
    assert bob_owner.get_dict() == before_bob_owner.get_dict()

    # WHEN
    set_factunits_to_owner(bob_owner, two_facts_dict)

    # THEN
    assert floor_rope in set(bob_owner.get_factunits_dict().keys())
    assert weather_rope not in set(bob_owner.get_factunits_dict().keys())
    assert bob_owner.get_dict() != before_bob_owner.get_dict()


def test_clear_factunits_from_owner_ReturnsObj_Scenario1_FactUnit_Exist():
    # ESTABLISH
    bob_owner = ownerunit_shop("Bob", "amy23")
    casa_str = "casa"
    floor_str = "floor status"
    clean_str = "clean"
    dirty_str = "dirty"
    mop_str = "mop"
    casa_rope = bob_owner.make_l1_rope(casa_str)
    floor_rope = bob_owner.make_rope(casa_rope, floor_str)
    clean_rope = bob_owner.make_rope(floor_rope, clean_str)
    dirty_rope = bob_owner.make_rope(floor_rope, dirty_str)
    mop_rope = bob_owner.make_rope(casa_rope, mop_str)
    bob_owner.add_plan(floor_rope)
    # bob_owner.add_plan(clean_rope)
    bob_owner.add_plan(dirty_rope)
    bob_owner.add_plan(mop_rope, task=True)
    bob_owner.edit_plan_attr(
        mop_rope, reason_rcontext=floor_rope, reason_premise=dirty_rope
    )
    bob_owner.add_fact(floor_rope, dirty_rope)
    floor_facts_dict = {floor_rope: {"fcontext": floor_rope, "fstate": dirty_rope}}
    assert bob_owner.get_factunits_dict() == floor_facts_dict
    assert bob_owner.get_factunits_dict() != {}

    # WHEN
    clear_factunits_from_owner(bob_owner)

    # THEN
    assert bob_owner.get_factunits_dict() != floor_facts_dict
    assert bob_owner.get_factunits_dict() == {}

...
def test_display_plantree_Scenario0(graphics_bool):
    ...
    print(f"Owner {a_owner.belief_label}: Labels ({len(a_owner._plan_dict)})")

    # WHEN / THEN
    x_fig = display_plantree(a_owner, graphics_bool)
...
def test_display_plantree_Scenario1_shows_Chores(graphics_bool):
    ...
    # WHEN / THEN
    display_plantree(a_owner, mode="Chore", graphics_bool=graphics_bool)
...
def test_OwnerUnit_fund_flow(graphics_bool):
    ...
    sue_owner.add_plan(casa_rope, mass=30)
    sue_owner.add_plan(cat_rope, mass=30)
    sue_owner.add_plan(hun_n_rope, mass=30)
    sue_owner.add_plan(hun_y_rope, mass=30)
    sue_owner.add_plan(clean_rope, mass=30)
    sue_owner.add_plan(sweep_rope, mass=30, task=True)
    sue_owner.add_plan(dish_rope, mass=30, task=True)
    dinner_str = "cat have dinner"
    dinner_rope = sue_owner.make_l1_rope(dinner_str)
    sue_owner.add_plan(dinner_rope, mass=30, task=True)

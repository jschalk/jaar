...
def test_OwnerUnit_set_acctunit_SetObjCorrectly():
    # ESTABLISH
    yao_str = "Yao"
    ...
    # WHEN
    bob_owner.set_acctunit(yao_acctunit)

    # THEN
    assert bob_owner.accts.get(yao_str).knot == slash_str
    x_accts = {yao_acctunit.acct_name: deepcopy_yao_acctunit}
    assert bob_owner.accts != x_accts
    deepcopy_yao_acctunit.knot = bob_owner.knot
    assert bob_owner.accts == x_accts


def test_OwnerUnit_set_acct_DoesNotSet_acct_name_membership():
    # ESTABLISH
    x_respect_bit = 5
    yao_owner = ownerunit_shop("Yao", respect_bit=x_respect_bit)
    zia_str = "Zia"

    # WHEN
    yao_owner.set_acctunit(acctunit_shop(zia_str), auto_set_membership=False)

    # THEN
    assert yao_owner.get_acct(zia_str).get_membership(zia_str) is None


def test_OwnerUnit_set_acct_DoesSet_acct_name_membership():
    # ESTABLISH
    x_respect_bit = 5
    yao_owner = ownerunit_shop("Yao", respect_bit=x_respect_bit)
    zia_str = "Zia"
    ...
    # THEN
    zia_zia_membership = yao_owner.get_acct(zia_str).get_membership(zia_str)
...
def test_OwnerUnit_set_acct_DoesNotOverRide_acct_name_membership():
    # ESTABLISH
    x_respect_bit = 5
    yao_owner = ownerunit_shop("Yao", respect_bit=x_respect_bit)
    zia_str = "Zia"
    ...
    # THEN
    zia_ohio_membership = yao_owner.get_acct(zia_str).get_membership(ohio_str)
...
def test_OwnerUnit_add_acctunit_CorrectlySets_accts():
    # ESTABLISH
    x_respect_bit = 6
    yao_owner = ownerunit_shop("Yao", respect_bit=x_respect_bit)
    zia_str = "Zia"
    ...
    # WHEN
    yao_owner.add_acctunit(zia_str, acct_cred_points=13, acct_debt_points=8)
    yao_owner.add_acctunit(sue_str, acct_debt_points=5)
    yao_owner.add_acctunit(xio_str, acct_cred_points=17)

    # THEN
    assert len(yao_owner.accts) == 3
    assert len(yao_owner.get_acctunit_group_titles_dict()) == 3
    assert yao_owner.accts.get(xio_str).acct_cred_points == 17
    assert yao_owner.accts.get(sue_str).acct_debt_points == 5
    assert yao_owner.accts.get(xio_str).respect_bit == x_respect_bit


def test_OwnerUnit_acct_exists_ReturnsObj():
    # ESTABLISH
    bob_owner = ownerunit_shop("Bob")
    yao_str = "Yao"

    # WHEN / THEN
    assert bob_owner.acct_exists(yao_str) is False

    # ESTABLISH
    bob_owner.add_acctunit(yao_str)

    # WHEN / THEN
    assert bob_owner.acct_exists(yao_str)


def test_OwnerUnit_set_acct_Creates_membership():
    # ESTABLISH
    yao_owner = ownerunit_shop("Yao")
    zia_str = "Zia"
    before_zia_credit = 7
    before_zia_debt = 17
    yao_owner.add_acctunit(zia_str, before_zia_credit, before_zia_debt)
    ...
    after_zia_credit = 11
    after_zia_debt = 13
    yao_owner.set_acctunit(acctunit_shop(zia_str, after_zia_credit, after_zia_debt))
...
def test_OwnerUnit_edit_acct_RaiseExceptionWhenAcctDoesNotExist():
    # ESTABLISH
    yao_owner = ownerunit_shop("Yao")
    zia_str = "Zia"
    ...
    with pytest_raises(Exception) as excinfo:
        yao_owner.edit_acctunit(zia_str, acct_cred_points=zia_acct_cred_points)
    assert str(excinfo.value) == f"AcctUnit '{zia_str}' does not exist."


def test_OwnerUnit_edit_acct_CorrectlyUpdatesObj():
    # ESTABLISH
    yao_owner = ownerunit_shop("Yao")
    zia_str = "Zia"
    old_zia_acct_cred_points = 55
    old_zia_acct_debt_points = 66
    yao_owner.set_acctunit(
        acctunit_shop(
            zia_str,
...
def test_OwnerUnit_get_acct_ReturnsObj():
    # ESTABLISH
    yao_owner = ownerunit_shop("Yao")
    zia_str = "Zia"
    sue_str = "Sue"
    ...
    # THEN
    assert zia_acct == yao_owner.accts.get(zia_str)
    assert sue_acct == yao_owner.accts.get(sue_str)

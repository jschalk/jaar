...
def test_OwnerUnit_edit_concept_label_FailsWhenConceptDoesNotExist():
    # ESTABLISH
    ...
    yao_owner.set_concept(conceptunit_shop(swim_str), parent_rope=casa_rope)

    # WHEN / THEN
    no_concept_rope = yao_owner.make_l1_rope("bees")
    with pytest_raises(Exception) as excinfo:
        yao_owner.edit_concept_label(
            old_rope=no_concept_rope, new_concept_label="birds"
        )
    assert str(excinfo.value) == f"Concept old_rope='{no_concept_rope}' does not exist"


def test_OwnerUnit_edit_concept_label_RaisesErrorForLevel0ConceptWhen_belief_label_isNone():
    # ESTABLISH
    yao_str = "Yao"
    ...
    with pytest_raises(Exception) as excinfo:
        moon_str = "moon"
        yao_owner.edit_concept_label(old_rope=root_rope, new_concept_label=moon_str)
    assert (
        str(excinfo.value)
        == f"Cannot set conceptroot to string different than '{yao_owner.belief_label}'"
    )

    assert yao_owner.conceptroot.concept_label != moon_str
    assert yao_owner.conceptroot.concept_label == yao_owner.belief_label


def test_OwnerUnit_edit_concept_label_RaisesErrorForLevel0When_belief_label_IsDifferent():
    # ESTABLISH
    yao_str = "Yao"
    yao_owner = ownerunit_shop(owner_name=yao_str)
    casa_str = "casa"
    ...
    yao_owner.conceptroot.belief_label = sun_str
    assert yao_owner.owner_name == yao_str
    assert yao_owner.belief_label == sun_str
    assert yao_owner.conceptroot.belief_label == sun_str
    assert yao_owner.conceptroot.concept_label == root_label()
    casa_concept = yao_owner.get_concept_obj(casa_rope)
...
def test_OwnerUnit_find_replace_rope_CorrectlyModifies_kids_Scenario1():
    ...
    yao_owner.set_l1_concept(conceptunit_shop(old_casa_str))
    yao_owner.set_concept(conceptunit_shop(bloomers_str), parent_rope=old_casa_rope)
    yao_owner.set_concept(conceptunit_shop(roses_str), parent_rope=old_bloomers_rope)
    yao_owner.set_concept(conceptunit_shop(red_str), parent_rope=old_roses_rope)
    r_concept_roses = yao_owner.get_concept_obj(old_roses_rope)
    r_concept_bloomers = yao_owner.get_concept_obj(old_bloomers_rope)
    ...
    yao_owner.edit_concept_label(old_rope=old_casa_rope, new_concept_label=new_casa_str)

    # THEN
    assert yao_owner.conceptroot._kids.get(new_casa_str) is not None
    assert yao_owner.conceptroot._kids.get(old_casa_str) is None
...
def test_owner_edit_concept_label_Modifies_factunits():
    ...
    yao_owner.set_l1_concept(conceptunit_shop(casa_str))
    yao_owner.set_concept(conceptunit_shop(roses_str), parent_rope=bloomers_rope)
    yao_owner.set_concept(conceptunit_shop(rain_str), parent_rope=old_water_rope)
    yao_owner.add_fact(fcontext=old_water_rope, fstate=old_rain_rope)

    concept_x = yao_owner.get_concept_obj(roses_rope)
    assert yao_owner.conceptroot.factunits[old_water_rope] is not None
    old_water_rain_factunit = yao_owner.conceptroot.factunits[old_water_rope]
    ...
    yao_owner.set_l1_concept(conceptunit_shop(new_water_str))
    assert yao_owner.conceptroot.factunits.get(new_water_rope) is None
    yao_owner.edit_concept_label(
        old_rope=old_water_rope, new_concept_label=new_water_str
    )

    # THEN
    assert yao_owner.conceptroot.factunits.get(old_water_rope) is None
    assert yao_owner.conceptroot.factunits.get(new_water_rope) is not None
    new_water_rain_factunit = yao_owner.conceptroot.factunits[new_water_rope]
    ...
    assert yao_owner.conceptroot.factunits.get(new_water_rope)
    x_factunit = yao_owner.conceptroot.factunits.get(new_water_rope)
    # for factunit_key, x_factunit in yao_owner.conceptroot.factunits.items():
    #     assert factunit_key == new_water_rope
    assert x_factunit.fcontext == new_water_rope
    assert x_factunit.fstate == new_rain_rope


def test_owner_edit_concept_label_ModifiesConceptReasonUnitsScenario1():
    # ESTABLISH
    ...
    casa_concept = sue_owner.get_concept_obj(sue_owner.make_l1_rope("casa"))
    ...
    wednesday_concept = sue_owner.get_concept_obj(old_wkday_rope)
...
def test_owner_set_owner_name_CorrectlyModifiesBoth():
    ...
    assert sue_owner.conceptroot.concept_label == sue_owner.belief_label
    # mid_concept_label1 = "Yao"
    # sue_owner.edit_concept_label(old_rope=old_concept_label, new_concept_label=mid_concept_label1)
    # assert sue_owner.owner_name == old_concept_label
    # assert sue_owner.conceptroot.concept_label == mid_concept_label1
...
def test_owner_edit_concept_label_RaisesErrorIfknotIsInLabel():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels_and_2reasons_2facts()
    old_wkday_str = "wkdays"
    old_wkday_rope = sue_owner.make_l1_rope(old_wkday_str)
    ...
    assert (
        str(excinfo.value)
        == f"Cannot modify '{old_wkday_rope}' because new_concept_label {new_wkday_str} contains knot {sue_owner.knot}"
    )

...
def test_OwnerUnit_get_relevant_ropes_EmptyRopeTermReturnsEmpty():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()

    # WHEN
    relevant_ropes = sue_owner._get_relevant_ropes({})
...
def test_OwnerUnit_get_relevant_ropes_RootRopeTermReturnsOnlyItself():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    ...
    relevant_ropes = sue_owner._get_relevant_ropes(root_dict)
...
def test_OwnerUnit_get_relevant_ropes_SimpleReturnsOnlyAncestors():
    # ESTABLISH
    sue_owner = get_ownerunit_with_4_levels()
    root_rope = to_rope(sue_owner.belief_label)
    ...
    sun_dict = {sun_rope}
    relevant_ropes = sue_owner._get_relevant_ropes(sun_dict)
...
def test_OwnerUnit_get_relevant_ropes_ReturnsSimpleReasonUnitRcontext():
    # ESTABLISH
    ...
    relevant_ropes = sue_owner._get_relevant_ropes(floor_dict)
...
def test_OwnerUnit_get_relevant_ropes_ReturnsReasonUnitRcontextAndDescendents():
    # ESTABLISH
    x_owner = get_mop_with_reason_ownerunit_example1()
    root_rope = to_rope(x_owner.belief_label)
    casa_str = "casa"
    casa_rope = x_owner.make_l1_rope(casa_str)
    floor_str = "mop floor"
    floor_rope = x_owner.make_rope(casa_rope, floor_str)

    unim_str = "unimportant"
    unim_rope = x_owner.make_l1_rope(unim_str)

    status_str = "cleaniness status"
    status_rope = x_owner.make_rope(casa_rope, status_str)

    clean_str = "clean"
    clean_rope = x_owner.make_rope(status_rope, clean_str)

    very_much_str = "very_much"
    very_much_rope = x_owner.make_rope(clean_rope, very_much_str)

    moderately_str = "moderately"
    moderately_rope = x_owner.make_rope(clean_rope, moderately_str)

    dirty_str = "dirty"
    dirty_rope = x_owner.make_rope(status_rope, dirty_str)

    # WHEN
    floor_dict = {floor_rope}
    relevant_ropes = x_owner._get_relevant_ropes(floor_dict)
...
def test_OwnerUnit_get_relevant_ropes_ReturnSimple():
    # ESTABLISH
    yao_str = "Yao"

...
def test_OwnerUnit_Exists():
    # ESTABLISH /  WHEN
    x_owner = OwnerUnit()

    # THEN
    assert x_owner
    assert x_owner.belief_label is None
    ...
    assert x_owner.knot is None
    assert x_owner.fund_pool is None
    assert x_owner.fund_iota is None
    assert x_owner.respect_bit is None
    ...
    assert x_owner._rational is None
    assert x_owner._keeps_justified is None
    assert x_owner._keeps_buildable is None
    assert x_owner._sum_healerlink_share is None
    assert x_owner._offtrack_kids_mass_set is None
...
def test_ownerunit_shop_ReturnsObjectWithFilledFields():
    # ESTABLISH
    sue_str = "Sue"
    ...
    # THEN
    assert x_owner
    assert x_owner.owner_name == sue_str
    ...
    assert x_owner.fund_iota == x_fund_iota
    assert x_owner.respect_bit == x_respect_bit
    assert x_owner.penny == x_penny
    assert x_owner.credor_respect == validate_respect_num()
    ...
    assert x_owner._rational is False
    assert x_owner._keeps_justified is False
    assert x_owner._keeps_buildable is False
    assert x_owner._sum_healerlink_share == 0
    assert x_owner._offtrack_kids_mass_set == set()
...
def test_ownerunit_shop_ReturnsObjectWithCorrectEmptyField():
    ...
    # THEN
    assert x_owner.owner_name == ""
    assert x_owner.belief_label == root_label()
    assert x_owner.knot == default_knot_if_None()
    assert x_owner.fund_pool == validate_fund_pool()
...
def test_OwnerUnit_set_max_tree_traverse_CorrectlySetsInt():
    ...
    assert zia_owner.max_tree_traverse == 3

    # WHEN
    zia_owner.set_max_tree_traverse(x_int=11)

    # THEN
    assert zia_owner.max_tree_traverse == 11


def test_OwnerUnit_set_max_tree_traverse_RaisesError_Scenario0():
    # ESTABLISH
    zia_str = "Zia"
    zia_owner = ownerunit_shop(owner_name=zia_str)
    assert zia_owner.max_tree_traverse == 3
    ...
    with pytest_raises(Exception) as excinfo:
        zia_owner.set_max_tree_traverse(x_int=zia_tree_traverse)
    assert (
        str(excinfo.value)
...
def test_OwnerUnit_set_max_tree_traverse_RaisesError_Scenario1():
    # ESTABLISH
    zia_str = "Zia"
    zia_owner = ownerunit_shop(owner_name=zia_str)
    assert zia_owner.max_tree_traverse == 3
...
def test_OwnerUnit_make_rope_ReturnsObj():
    # ESTABLISH
    ...
    casa_str = "casa"
    v1_casa_rope = sue_owner.make_l1_rope(casa_str)

    # WHEN
    v2_casa_rope = sue_owner.make_l1_rope(casa_str)

    # THEN
    assert v1_casa_rope == v2_casa_rope


def test_OwnerUnit_set_last_pack_id_SetsAttrCorrectly():
    # ESTABLISH
    sue_owner = ownerunit_shop("Sue", "Texas")
    assert sue_owner.last_pack_id is None

    # WHEN
    x_last_pack_id = 89
    sue_owner.set_last_pack_id(x_last_pack_id)

    # THEN
    assert sue_owner.last_pack_id == x_last_pack_id


def test_OwnerUnit_set_last_pack_id_RaisesError():
    # ESTABLISH
    sue_owner = ownerunit_shop("Sue", "Texas")
    old_last_pack_id = 89
    sue_owner.set_last_pack_id(old_last_pack_id)

    # WHEN / THEN
    new_last_pack_id = 72
    assert new_last_pack_id < old_last_pack_id
    with pytest_raises(Exception) as excinfo:
        sue_owner.set_last_pack_id(new_last_pack_id)
    assert (
        str(excinfo.value)
...
def test_OwnerUnit_del_last_pack_id_SetsAttrCorrectly():
    # ESTABLISH
    sue_owner = ownerunit_shop("Sue", "Texas")
    old_last_pack_id = 89
    sue_owner.set_last_pack_id(old_last_pack_id)
    assert sue_owner.last_pack_id is not None

    # WHEN
    sue_owner.del_last_pack_id()

    # WHEN
    assert sue_owner.last_pack_id is None


def test_OwnerUnit_set_fund_pool_CorrectlySetsAttr():
    # ESTABLISH
    sue_owner = ownerunit_shop("Sue", "Texas")
    sue_fund_pool = 99000
    assert sue_owner.fund_pool == validate_fund_pool()

    # WHEN
    sue_owner.set_fund_pool(sue_fund_pool)

    # THEN
    assert sue_owner.fund_pool == 99000


def test_OwnerUnit_set_fund_pool_RaisesErrorWhenArgIsNotMultiple():
    # ESTABLISH
    zia_str = "Zia"
    zia_owner = ownerunit_shop(zia_str)
    x_fund_pool = 23
    zia_owner.set_fund_pool(x_fund_pool)
    assert zia_owner.fund_iota == 1
    assert zia_owner.fund_pool == x_fund_pool

    # WHEN
    new_fund_pool = 13.5
    with pytest_raises(Exception) as excinfo:
        zia_owner.set_fund_pool(new_fund_pool)
    assert (
        str(excinfo.value)
        == f"Owner '{zia_str}' cannot set fund_pool='{new_fund_pool}'. It is not divisible by fund_iota '{zia_owner.fund_iota}'"
    )

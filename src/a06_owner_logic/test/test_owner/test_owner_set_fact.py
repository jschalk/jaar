...
def test_OwnerUnit_set_fact_CorrectlyModifiesAttr_1():
    ...
    x_planroot = sue_owner.planroot
    x_planroot.factunits = {sunday_owner_fact.fcontext: sunday_owner_fact}
    ...
    # THEN
    assert x_planroot.factunits == {sunday_owner_fact.fcontext: sunday_owner_fact}
...
def test_OwnerUnit_set_fact_FailsToCreateWhenRcontextAndFactAreDifferenctAndFactPlanIsNot_RangeRoot():
    # ESTABLISH
    bob_owner = ownerunit_shop("Bob")
    time_str = "time"
    time_plan = planunit_shop(time_str, begin=0, close=140)
    bob_owner.set_l1_plan(time_plan)
    ...
    bob_owner.set_plan(a1st_plan, parent_rope=time_rope)
    a1e1st_str = "a1_era1st"
    a1e1st_plan = planunit_shop(a1e1st_str, begin=20, close=30)
    bob_owner.set_plan(a1e1st_plan, parent_rope=a1st_rope)
    a1e1_rope = bob_owner.make_rope(a1st_rope, a1e1st_str)
    assert bob_owner.planroot.factunits in (None, {})
...
def test_OwnerUnit_get_fact_ReturnsFactUnit():
    ...
    sue_owner.add_fact(situations_rope, climate_rope, create_missing_plans=True)

    # WHEN
    generated_situations_rcontext = sue_owner.get_fact(situations_rope)

    # THEN
    static_situations_rcontext = sue_owner.planroot.factunits.get(situations_rope)
...
def test_OwnerUnit_get_rangeroot_factunits_ReturnsObjsScenario0():
    ...
    sue_owner.set_l1_plan(clean_plan)
    c_rope = sue_owner.make_l1_rope(clean_str)
    time_rope = sue_owner.make_l1_rope(time_str)
    # sue_owner.edit_plan_attr(c_rope, reason_rcontext=time_rope, reason_premise=time_rope, popen=5, reason_pnigh=10)

    sue_owner.add_fact(fcontext=time_rope, fstate=time_rope, fopen=5, fnigh=10)
    print(f"Establish a single ranged fact {sue_owner.planroot.factunits=}")
    assert len(sue_owner.planroot.factunits) == 1
    ...
    sue_owner.set_l1_plan(place_plan)
    place_rope = sue_owner.make_l1_rope(place_str)
    sue_owner.add_fact(fcontext=place_rope, fstate=place_rope, fopen=5, fnigh=10)
    print(f"When one ranged fact added {sue_owner.planroot.factunits=}")
    assert len(sue_owner.planroot.factunits) == 2
    ...
    mood = "mood_x"
    sue_owner.set_l1_plan(planunit_shop(mood))
    m_rope = sue_owner.make_l1_rope(mood)
    sue_owner.add_fact(fcontext=m_rope, fstate=m_rope)
    print(f"When one non-ranged_fact added {sue_owner.planroot.factunits=}")
    assert len(sue_owner.planroot.factunits) == 3
...
def test_OwnerUnit_get_rangeroot_factunits_ReturnsObjsScenario1():
    ...
    sue_owner = ownerunit_shop("Sue")
    time_str = "time"
    sue_owner.set_l1_plan(planunit_shop(time_str, begin=0, close=140))
    time_rope = sue_owner.make_l1_rope(time_str)
    mood_x = "mood_x"
    sue_owner.set_l1_plan(planunit_shop(mood_x))
    m_x_rope = sue_owner.make_l1_rope(mood_x)
    happy = "happy"
    sad = "Sad"
    sue_owner.set_plan(planunit_shop(happy), parent_rope=m_x_rope)
    sue_owner.set_plan(planunit_shop(sad), parent_rope=m_x_rope)
    sue_owner.add_fact(fcontext=time_rope, fstate=time_rope, fopen=5, fnigh=10)
    sue_owner.add_fact(fcontext=m_x_rope, fstate=sue_owner.make_rope(m_x_rope, happy))
    print(
        f"Establish a root ranged fact and non-range fact:\n{sue_owner.planroot.factunits=}"
    )
    assert len(sue_owner.planroot.factunits) == 2
...
def test_OwnerUnit_set_fact_create_missing_plans_CreatesRcontextAndFact():
    # ESTABLISH
    sue_owner = ownerunit_shop("Sue")
    situations_str = "situations"
    situations_rope = sue_owner.make_l1_rope(situations_str)
    climate_str = "climate"
    climate_rope = sue_owner.make_rope(situations_rope, climate_str)
    assert sue_owner.planroot.get_kid(situations_str) is None

    # WHEN
    sue_owner.add_fact(situations_rope, climate_rope, create_missing_plans=True)

    # THEN
    assert sue_owner.planroot.get_kid(situations_str) is not None
    assert sue_owner.get_plan_obj(situations_rope) is not None
    assert sue_owner.get_plan_obj(climate_rope) is not None

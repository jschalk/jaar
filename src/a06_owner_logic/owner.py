...
class OwnerUnit:
    ...
    # settle_owner Calculated field end
    ...
    def set_fund_pool(self, x_fund_pool):
        if valid_finance_ratio(x_fund_pool, self.fund_iota) is False:
            exception_str = f"Owner '{self.owner_name}' cannot set fund_pool='{x_fund_pool}'. It is not divisible by fund_iota '{self.fund_iota}'"
    ...
    def set_credor_respect(self, new_credor_respect: int):
        if valid_finance_ratio(new_credor_respect, self.respect_bit) is False:
            exception_str = f"Owner '{self.owner_name}' cannot set credor_respect='{new_credor_respect}'. It is not divisible by bit '{self.respect_bit}'"
            raise _bit_RatioException(exception_str)
    ...
    def set_debtor_respect(self, new_debtor_respect: int):
        if valid_finance_ratio(new_debtor_respect, self.respect_bit) is False:
            exception_str = f"Owner '{self.owner_name}' cannot set debtor_respect='{new_debtor_respect}'. It is not divisible by bit '{self.respect_bit}'"
            raise _bit_RatioException(exception_str)
    ...
    def add_acctunit(
        ...
        x_knot = self.knot
        acctunit = acctunit_shop(acct_name, acct_cred_points, acct_debt_points, x_knot)
        self.set_acctunit(acctunit)

    def set_acctunit(self, x_acctunit: AcctUnit, auto_set_membership: bool = True):
        if x_acctunit.knot != self.knot:
            x_acctunit.knot = self.knot
    ...
    def edit_acctunit(
        ...
        if acct_cred_points is not None:
            x_acctunit.set_acct_cred_points(acct_cred_points)
        if acct_debt_points is not None:
            x_acctunit.set_acct_debt_points(acct_debt_points)
    ...
    def create_symmetry_groupunit(self, x_group_title: GroupTitle) -> GroupUnit:
        ...
        for x_acctunit in self.accts.values():
            x_membership = membership_shop(
                ...
                group_cred_points=x_acctunit.acct_cred_points,
                group_debt_points=x_acctunit.acct_debt_points,
    ...
    def get_credit_ledger_debt_ledger(
        ...
        debt_ledger = {}
        for x_acctunit in self.accts.values():
            credit_ledger[x_acctunit.acct_name] = x_acctunit.acct_cred_points
            debt_ledger[x_acctunit.acct_name] = x_acctunit.acct_debt_points
        return credit_ledger, debt_ledger

    def _allot_offtrack_fund(self):
        self._add_to_acctunits_fund_give_take(self._offtrack_fund)

    def get_acctunits_acct_cred_points_sum(self) -> float:
        return sum(acctunit.get_acct_cred_points() for acctunit in self.accts.values())
    ...
    def _set_acctunits_fund_agenda_ratios(self):
        ...
        x_acctunits_acct_cred_points_sum = self.get_acctunits_acct_cred_points_sum()
        x_acctunits_acct_debt_points_sum = self.get_acctunits_acct_debt_points_sum()
        for x_acctunit in self.accts.values():
            x_acctunit.set_fund_agenda_ratio_give_take(
                ...
                acctunits_acct_cred_points_sum=x_acctunits_acct_cred_points_sum,
                acctunits_acct_debt_points_sum=x_acctunits_acct_debt_points_sum,
            )
...
def ownerunit_shop(
    ...
    x_owner.conceptroot = conceptunit_shop(
        ...
        knot=x_owner.knot,
        fund_iota=x_owner.fund_iota,
        parent_rope="",
    )
    x_owner.set_max_tree_traverse(3)
    x_owner._rational = False
    return x_owner


def get_from_json(x_owner_json: str) -> OwnerUnit:
    return get_from_dict(get_dict_from_json(x_owner_json))


def get_from_dict(owner_dict: dict) -> OwnerUnit:
    x_owner = ownerunit_shop()
    x_owner.set_owner_name(obj_from_owner_dict(owner_dict, "owner_name"))
    ...
    x_owner.fund_iota = default_fund_iota_if_None(
        obj_from_owner_dict(owner_dict, "fund_iota")
    )
    x_owner.respect_bit = default_RespectBit_if_None(
        obj_from_owner_dict(owner_dict, "respect_bit")
    )
    x_owner.penny = filter_penny(obj_from_owner_dict(owner_dict, "penny"))
    x_owner.credor_respect = obj_from_owner_dict(owner_dict, "credor_respect")
    x_owner.debtor_respect = obj_from_owner_dict(owner_dict, "debtor_respect")
    x_owner.last_pack_id = obj_from_owner_dict(owner_dict, "last_pack_id")
    x_knot = x_owner.knot
...
def obj_from_owner_dict(
    ...
    if dict_key == "accts":
        return acctunits_get_from_dict(x_dict[dict_key], _knot)
...
def get_dict_of_owner_from_dict(x_dict: dict[str, dict]) -> dict[str, OwnerUnit]:
    ownerunits = {}
    for ownerunit_dict in x_dict.values():
        x_owner = get_from_dict(owner_dict=ownerunit_dict)
        ownerunits[x_owner.owner_name] = x_owner
    return ownerunits

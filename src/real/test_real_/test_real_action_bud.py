...
def test_RealUnit_generate_action_bud_ReturnsRegeneratedObj(env_dir_setup_cleanup):
    ...
    assert sue_hubunit.get_action_bud().char_exists(bob_text)

    # WHEN
    after_sue_bud = music_real.generate_action_bud(sue_text)

    # THEN method should wipe over action bud
    assert after_sue_bud.char_exists(bob_text) is False
...
def test_RealUnit_generate_action_bud_SetsCorrectFileWithout_healerhold(
    ...
    sue_text = "Sue"
    assert before_bob_action_bud.char_exists(sue_text) is False

    # WHEN
    bob_voice_bud = bob_hubunit.get_voice_bud()
    bob_voice_bud.add_charunit(sue_text)
    ...
    # THEN
    assert after_bob_action_bud.char_exists(sue_text)
...
def test_RealUnit_generate_action_bud_SetsFileWith_healerhold(env_dir_setup_cleanup):
    ...
    bob_hubunit = hubunit_shop(music_real.reals_dir, music_real.real_id, bob_text, None)
    after_bob_action_bud = music_real.generate_action_bud(bob_text)
    assert after_bob_action_bud.char_exists(bob_text) is False

    # WHEN
    bob_voice_bud = bob_hubunit.get_voice_bud()
    bob_voice_bud.add_charunit(bob_text)
    bob_voice_bud.set_char_respect(100)
    texas_text = "Texas"
    ...
    bob_voice_bud.add_l1_idea(ideaunit_shop(texas_text, _problem_bool=True))
    bob_voice_bud.add_idea(elpaso_idea, texas_road)
    bob_hubunit.save_voice_bud(bob_voice_bud)
    after_bob_action_bud = music_real.generate_action_bud(bob_text)

    # THEN
    assert after_bob_action_bud.char_exists(bob_text)
...
def test_RealUnit_generate_all_action_buds_SetsCorrectFiles(
    ...
    assert before_bob_action_bud.char_exists(bob_text) is False
    assert before_sue_action_bud.char_exists(sue_text) is False
    ...
    assert after_bob_action_bud.char_exists(bob_text)
    assert after_sue_action_bud.char_exists(sue_text)

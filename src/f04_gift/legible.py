...
def add_budunit_legible_list(legible_list: list[str], x_atom: AtomUnit, x_bud: BudUnit):
    ...
    pact_time_int_value = jvalues.get(pact_time_int_str)
    ...
    if _tally_value is not None:
        ...
        legible_list.append(x_str)
    if pact_time_int_value is not None:
        x_str = (
            f"{x_bud.owner_name}'s bud {pact_time_int_str} set to {pact_time_int_value}"
        )
        legible_list.append(x_str)

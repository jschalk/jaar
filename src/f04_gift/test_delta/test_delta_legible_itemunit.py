...
def test_create_legible_list_ReturnsObj_itemunit_INSERT():
    ...
    clean_atomunit.set_arg(item_idee_str(), item_idee_value)
    ...
    # THEN
    x_str = f"Created Item '{item_idee_value}' with parent_road {parent_road_value}. addin={addin_value}.begin={begin_value}.close={close_value}.denom={denom_value}.numor={numor_value}.problem_bool={problem_bool_value}.morph={morph_value}.mass={mass_value}.pledge={pledge_value}."
    print(f"{x_str=}")
...
def test_create_legible_list_ReturnsObj_itemunit_UPDATE():
    ...
    # THEN
    x_str = f"Item '{item_idee_value}' with parent_road {parent_road_value} set these attributes: addin={addin_value}.begin={begin_value}.close={close_value}.denom={denom_value}.numor={numor_value}.problem_bool={problem_bool_value}.morph={morph_value}.mass={mass_value}.pledge={pledge_value}."
    print(f"{x_str=}")
    assert legible_list[0] == x_str
...
def test_create_legible_list_ReturnsObj_itemunit_DELETE():
    ...
    # THEN
    x_str = (
        f"Item '{item_idee_value}' with parent_road {parent_road_value} was deleted."
    )
    print(f"{x_str=}")
    assert legible_list[0] == x_str

...
def test_create_legible_list_ReturnsObjEstablishWithBudUpdate_bank_time_int():
    # ESTABLISH
    category = budunit_str()
    bank_time_int_str = "bank_time_int"
    bank_time_int_int = 55
    bank_time_int_atomunit = atomunit_shop(category, atom_update())
    bank_time_int_atomunit.set_arg(bank_time_int_str, bank_time_int_int)
    x_deltaunit = deltaunit_shop()
    x_deltaunit.set_atomunit(bank_time_int_atomunit)
    sue_bud = budunit_shop("Sue")
    ...
    # THEN
    x_str = f"{sue_bud.owner_name}'s bud bank_time_int set to {bank_time_int_int}"
    assert legible_list[0] == x_str

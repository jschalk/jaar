...
from src.f04_gift.atom_config import atom_insert, atom_delete, team_label_str, road_str
...
def test_create_legible_list_ReturnsObj_item_teamlink_INSERT():
    ...
    team_label_value = f"{sue_bud._bridge}Swimmers"
    swim_atomunit = atomunit_shop(category, atom_insert())
    swim_atomunit.set_arg(road_str(), road_value)
    swim_atomunit.set_arg(team_label_str(), team_label_value)

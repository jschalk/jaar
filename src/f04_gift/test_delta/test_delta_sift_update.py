...
def test_get_minimal_buddelta_ReturnsObjUPDATEAtomUnit_bud_acctunit():
    # ESTABLISH
    bob_str = "Bob"
    yao_str = "Yao"
    ...
    assert len(accts_buddelta.get_sorted_atomunits()) == 2

    # WHEN
    new_buddelta = get_minimal_buddelta(accts_buddelta, sue_bud)

    # THEN
    assert len(new_buddelta.get_sorted_atomunits()) == 1
    new_atomunit = new_buddelta.get_sorted_atomunits()[0]

...
def test_DeltaUnit_add_all_different_atomunits_Creates_AtomUnit_BudUnit_simple_attrs_update():
    ...
    x_purview_time_int = 990000
    after_sue_bud.tally = x_budunit_tally
    after_sue_bud.purview_time_int = x_purview_time_int
    ...
    assert xio_atomunit.get_value("purview_time_int") == x_purview_time_int
...
def test_DeltaUnit_add_all_different_atomunits_Creates_AtomUnit_item_insert():
    ...
    x_keylist = [
        atom_insert(),
        bud_itemunit_str(),
        after_sue_bud.deal_idea,
        accord45_str,
    ]
    ball_atomunit = get_from_nested_dict(sue_deltaunit.atomunits, x_keylist)
    assert ball_atomunit.get_value(idee_str()) == accord45_str
    assert ball_atomunit.get_value(parent_road_str()) == after_sue_bud.deal_idea

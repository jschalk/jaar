...
def _modify_bud_itemunit_insert(x_bud: BudUnit, x_atom: AtomUnit):
    x_bud.set_item(
        item_kid=itemunit_shop(
            _item_idee=x_atom.get_value("item_idee"),
            addin=x_atom.get_value("addin"),
            begin=x_atom.get_value("begin"),
            close=x_atom.get_value("close"),
            gogo_want=x_atom.get_value("gogo_want"),
...
class AtomRow:
    ...
    def _set_jaar_types(self):
        ...
        if self.item_idee != None and self.parent_road != None and self.road == None:
            self.road = create_road(self.parent_road, self.item_idee)
...
def sift_atomunit(x_bud: BudUnit, x_atom: AtomUnit) -> AtomUnit:
    ...
    x_item_idee = x_atom_reqs.get("item_idee")
    if x_parent_road != None and x_item_idee != None:
        x_atom_reqs["road"] = x_bud.make_road(x_parent_road, x_item_idee)

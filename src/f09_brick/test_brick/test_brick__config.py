...
def test_str_functions_ReturnObj():
    ...
    assert get_brick_types() == {budunit_str(), cmtyunit_str(), pidginunit_str()}
...
def test_get_brick_elements_sort_order_ReturnsObj():
    ...
    assert table_sorting_priority[2] == cmty_idea_str()
...
def test_get_brick_sqlite_type_ReturnsObj():
    ...
    assert sqlite_types.get(cmty_idea_str()) == "TEXT"
...
def test_get_brick_config_dict_ReturnsObj():
    ...
    assert cmtyunit_str() in brick_config_categorys
    assert cmty_deallog_str() not in brick_config_categorys
    assert cmty_deal_episode_str() in brick_config_categorys
    assert cmty_cashbook_str() in brick_config_categorys
    assert cmty_timeline_hour_str() in brick_config_categorys
    assert cmty_timeline_month_str() in brick_config_categorys
    assert cmty_timeline_weekday_str() in brick_config_categorys
    ...
    assert get_cmty_categorys().issubset(brick_config_categorys)
...
def _validate_brick_config(x_brick_config: dict):
    ...
    for brick_category, brick_dict in x_brick_config.items():
        ...
        elif brick_dict.get(brick_type_str()) == cmtyunit_str():
            sub_category = cmty_config_dict.get(brick_category)
        ...
        elif brick_category in {cmty_deal_episode_str(), cmty_cashbook_str()}:
        ...
        if brick_dict.get(brick_type_str()) != pidginunit_str():
            assert cmty_idea_str() in brick_jkeys_keys
        if brick_dict.get(brick_type_str()) == budunit_str():
            brick_jkeys_keys.remove(cmty_idea_str())
        ...
        if cmty_idea_str() in sub_jvalues_keys:
            sub_jvalues_keys.remove(cmty_idea_str())
        ...
        assert cmty_idea_str() not in brick_jvalues_keys
...
def _validate_brick_format_files(brick_filenames: set[str]):
    ...
    valid_brick_categorys.update(get_cmty_categorys())
...
def test_get_brick_config_dict_ReturnsObj_build_order():
    ...
    # set_brick_config_json(cmtyunit_str(), 5)
    # set_brick_config_json(cmty_timeline_hour_str(), 6)
    # set_brick_config_json(cmty_timeline_month_str(), 7)
    # set_brick_config_json(cmty_timeline_weekday_str(), 8)
    ...
    # set_brick_config_json(cmty_deal_episode_str(), 20)
    # set_brick_config_json(cmty_cashbook_str(), 21)
    ...
    assert x_brick_config.get(cmtyunit_str()).get(bo) == 5
    assert x_brick_config.get(cmty_timeline_hour_str()).get(bo) == 6
    assert x_brick_config.get(cmty_timeline_month_str()).get(bo) == 7
    assert x_brick_config.get(cmty_timeline_weekday_str()).get(bo) == 8
    ...
    assert x_brick_config.get(cmty_deal_episode_str()).get(bo) == 20
    assert x_brick_config.get(cmty_cashbook_str()).get(bo) == 21

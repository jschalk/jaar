...
def test_create_brick_df_Arg_brick_format_00020_bud_acct_membership_v0_0_0():
    ...
    assert array_headers == acct_brickref.get_headers_list()
    assert membership_dataframe.loc[0, cmty_idea_str()] == accord_cmty_idea
    ...
    assert membership_dataframe.loc[2, cmty_idea_str()] == accord_cmty_idea
    ...
    assert membership_dataframe.loc[4, cmty_idea_str()] == accord_cmty_idea
    ...
    assert membership_dataframe.loc[5, cmty_idea_str()] == accord_cmty_idea
...
def test_create_brick_df_Arg_brick_format_00013_itemunit_v0_0_0():
    ...
    assert itemunit_format.loc[0, cmty_idea_str()] == accord_cmty_idea
    assert itemunit_format.loc[0, item_idee_str()] == casa_str
    assert itemunit_format.loc[0, mass_str()] == casa_mass
    assert itemunit_format.loc[0, parent_road_str()] == accord_cmty_idea

    assert itemunit_format.loc[1, owner_name_str()] == sue_budunit.owner_name
    assert itemunit_format.loc[1, pledge_str()] == "Yes"
    assert itemunit_format.loc[1, cmty_idea_str()] == accord_cmty_idea
    assert itemunit_format.loc[1, parent_road_str()] == casa_road
    assert itemunit_format.loc[1, item_idee_str()] == clean_str
...
"""
    ...
    # THEN
    assert os_path_exists(csv_example_path)
    sue2_acct_example_csv = """face_name,event_int,cmty_idea,owner_name,acct_name,credit_belief,debtit_belief
,,accord56,Sue,Bob,13,29
,,accord56,Sue,Sue,11,23
,,accord56,Sue,Yao,41,37

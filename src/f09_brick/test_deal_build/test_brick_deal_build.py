...
def test_deal_build_from_df_ReturnsObj_Scenario0_OneDealIdea(brick_env_setup_cleanup):
    ...
    accord23_dealunit.add_pactepisode(
        x_owner_name="Sue",
        x_time_int=777,
    ...
    assert gen_dealunit.pactlogs == accord23_dealunit.pactlogs
    assert gen_dealunit.cashbook.tranunits == accord23_dealunit.cashbook.tranunits
    print(f"{gen_dealunit.pactlogs=}")
    assert len(gen_dealunit.pactlogs) == 1
...
def test_deal_build_from_df_ReturnsObj_Scenario1_TwoDealIdeas(
    ...
    assert len(creg_dealunit.pactlogs) == 3
    ...
    assert len(five_dealunit.pactlogs) == 2

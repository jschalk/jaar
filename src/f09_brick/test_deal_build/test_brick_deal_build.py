...
def test_deal_build_from_df_ReturnsObj_Scenario0_OnedealID(brick_env_setup_cleanup):
    ...
    # THEN
    assert x_dealunits
    assert x_dealunits.get(music23_str) != None
    creg_timelineunit = timelineunit_shop(get_default_timeline_config_dict())
    music23_dealunit = dealunit_shop(
        current_time=5500,
        deal_id=music23_str,
    ...
    music23_dealunit.add_cashpurchase(
        x_owner_id="Zia",
        x_acct_id="Bob",
        x_time_id=777,
        x_amount=888,
    )
    gen_dealunit = x_dealunits.get(music23_str)
    assert gen_dealunit.fund_coin == x_fund_coin
    assert gen_dealunit.respect_bit == x_respect_bit
    assert gen_dealunit.penny == x_penny
    assert gen_dealunit.deal_id == music23_str
    assert gen_dealunit.deals_dir == x_deals_dir
    assert gen_dealunit.timeline == music23_dealunit.timeline
    assert gen_dealunit.purviewlogs == music23_dealunit.purviewlogs
    assert gen_dealunit.cashbook.tranunits == music23_dealunit.cashbook.tranunits
    print(f"{gen_dealunit.purviewlogs=}")
    assert len(gen_dealunit.purviewlogs) == 1
    assert len(gen_dealunit.cashbook.tranunits) == 1
    assert gen_dealunit == music23_dealunit
...
def test_deal_build_from_df_ReturnsObj_Scenario1_TwodealIDs(
    ...
    assert x_dealunits
    assert x_dealunits.get(music23_str) != None
    creg_dealunit = x_dealunits.get(music23_str)
    ...
    assert creg_dealunit.deal_id == music23_str
    assert creg_dealunit.deals_dir == x_deals_dir
    assert creg_dealunit.timeline == music23_dealunit.timeline
    assert len(creg_dealunit.purviewlogs) == 3
    assert len(creg_dealunit.cashbook.tranunits) == 4
    # assert creg_dealunit == music23_dealunit

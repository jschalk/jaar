...
def test_barn_staging_str_ReturnsObj():
    # ESTABLISH / WHEN / THEN
    assert barn_staging_str() == "barn_staging"
    assert barn_agg_str() == "barn_agg"
    assert barn_valid_str() == "barn_valid"


def test_get_barn_staging_grouping_with_all_values_equal_df_ReturnsObj_Scenario0_EmptyDataframe():
    # ESTABLISH
    df1 = DataFrame([[]], columns=[])
    group_by_list = [group_id_str(), acct_id_str()]

    # WHEN
    group_by_dataframe = get_barn_staging_grouping_with_all_values_equal_df(
        df1, group_by_list
    )
...
def test_get_barn_staging_grouping_with_all_values_equal_df_ReturnsObj_Scenario1_GroupBySingleColumn():
    # ESTABLISH
...
def test_get_barn_staging_grouping_with_all_values_equal_df_ReturnsObj_Scenario2_GroupByExtraColumns():
    # ESTABLISH
    df_columns = [group_id_str(), credor_respect_str()]
    before_df_values = [["AA0", "BB0"], ["AA0", "BB0"]]
    df1 = DataFrame(before_df_values, columns=df_columns)
...
def test_get_barn_staging_grouping_with_all_values_equal_df_ReturnsObj_Scenario3_GroupByExtraColumns():
    # ESTABLISH
...
def test_get_barn_staging_grouping_with_all_values_equal_df_ReturnsObj_Scenario4_GroupByExtraColumns():
    # ESTABLISH
    df_columns = [group_id_str(), credor_respect_str(), "column3"]
    before_df_values = [
        ["AA0", "BB0", "CC0"],

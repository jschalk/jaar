...
def test_BudUnit_settle_bud_CorrectlySets_item_dict():
    ...
    assert casa_item._parent_road == sue_budunit.cmty_idea
    assert casa_item._kids == {}
    assert casa_item.mass == 30
    assert casa_item._item_idee == casa_str
    ...
    #     # print("")
    #     # print(f"{x_item._item_idee=}")
    #     # print(f"{len(x_item.reasonunits)=}")
    #     print(
    #         f"  {x_item._item_idee} iterate through every reasonheir... {len(x_item._reasonheirs)=} {x_item._item_idee=}"
    #     )
...
def test_BudUnit_settle_bud_CorrectlySetsData_budunit_v001():
    ...
    # day_hour = f"{yao_budunit.cmty_idea},day_hour"
    ...
    # if yao_budunit.itemroot._kids["Ultimate Frisbee"]._item_idee == "Ultimate Frisbee":
    ...
    #     )
    #     # print(f"{item._item_idee=}")
    #     if item._item_idee == laundry_str:
    #         for reason in item.reasonunits.values():
    #             print(f"{item._item_idee=} {reason.base=}")  # {reason.premises=}")
...
def test_BudUnit_settle_bud_OptionWeekdaysReturnsCorrectObj_budunit_v001():
    ...
    x_itemroot = yao_budunit.get_item_obj(yao_budunit.cmty_idea)
    ...
    # yao_budunit.set_fact(base=f"{yao_budunit.cmty_idea},weekdays", pick=f"{yao_budunit.cmty_idea},weekdays,Tuesday")
    # item_dict = yao_budunit.get_item_dict()
    # assert YR.get_active(road=bird_item, item_dict=item_dict) is True

    # yao_budunit.set_fact(base=f"{yao_budunit.cmty_idea},weekdays", pick=f"{yao_budunit.cmty_idea},weekdays,Wednesday")
    # item_dict = yao_budunit.get_item_dict()
...
def test_BudUnit_settle_bud_CorrectlySetsItemUnitsActiveWithEvery6WeeksReason_budunit_v001():
    ...
    print(
        f"{clean_sheet_item._item_idee=} {ced_week_reason.base=} {ced_week_premise.need=}"
    )
    # print(f"{clean_sheet_item._item_idee=} {ced_week_reason.base=} {premise_x=}")
    ...
    #     if item._item_idee == "clean sheets couch blankets":
    ...
    print(f"{clean_couch_item._item_idee=} {week_reason.base=} {week_premise=}")
...
def test_BudUnit_settle_bud_EveryItemHasActiveStatus_budunit_v001():
    ...
    #     item_kid_count += 1
    #     print(f"{item._item_idee=} {item_kid_count=}")
...
def test_BudUnit_settle_bud_EveryTwoMonthReturnsCorrectObj_budunit_v001():
    ...
    mat_item_idee = "deep clean play mat"
    mat_road = yao_budunit.make_road(clean_road, mat_item_idee)
...
def test_BudUnit_settle_bud_CorrectlySets_sum_healerlink_share(graphics_bool):
    ...
    print(f"{week_item._item_idee=} {week_item.problem_bool=} {week_item._fund_ratio=}")
    sue_budunit.settle_bud()
    # THEN

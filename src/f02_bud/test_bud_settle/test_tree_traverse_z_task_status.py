...
def test_BudUnit_settle_bud_CorrectlySets_item_dict():
    ...
    print(f"{sue_budunit._owner_id=} {len(casa_item.reasonunits)=}")
    # print(f"{casa_item.reasonunits=}")
    print(f"{sue_budunit._owner_id=} {len(sue_budunit._itemroot.factunits)=}")
    ...
    assert casa_item._parent_road == sue_budunit._deal_id
    assert casa_item._kids == {}
    assert casa_item.mass == 30
    assert casa_item._lx == casa_str
    ...
    #     print(
    #         f"  {x_item._lx} iterate through every reasonheir... {len(x_item._reasonheirs)=} {x_item._lx=}"
    #     )
...
def test_BudUnit_settle_bud_CorrectlySetsData_budunit_v001():
    ...
    # day_hour = f"{yao_budunit._deal_id},day_hour"
    ...
    # if yao_budunit._itemroot._kids["Ultimate Frisbee"]._lx == "Ultimate Frisbee":
    ...
    #     )
    #     # print(f"{item._lx=}")
    #     if item._lx == laundry_str:
    #         for reason in item.reasonunits.values():
    #             print(f"{item._lx=} {reason.base=}")  # {reason.premises=}")
...
def test_BudUnit_settle_bud_OptionWeekdaysReturnsCorrectObj_budunit_v001():
    ...
    x_itemroot = yao_budunit.get_item_obj(yao_budunit._deal_id)
    ...
    # yao_budunit.set_fact(base=f"{yao_budunit._deal_id},weekdays", pick=f"{yao_budunit._deal_id},weekdays,Tuesday")
    # item_dict = yao_budunit.get_item_dict()
    # assert YR.get_active(road=bird_item, item_dict=item_dict) is True

    # yao_budunit.set_fact(base=f"{yao_budunit._deal_id},weekdays", pick=f"{yao_budunit._deal_id},weekdays,Wednesday")
    # item_dict = yao_budunit.get_item_dict()
...
def test_BudUnit_settle_bud_CorrectlySetsItemUnitsActiveWithEvery6WeeksReason_budunit_v001():
    ...
    print(f"{clean_sheet_item._lx=} {ced_week_reason.base=} {ced_week_premise.need=}")
    # print(f"{clean_sheet_item._lx=} {ced_week_reason.base=} {premise_x=}")
    ...
    #     if item._lx == "clean sheets couch blankets":
    ...
    print(f"{clean_couch_item._lx=} {week_reason.base=} {week_premise=}")
...
def test_BudUnit_settle_bud_EveryItemHasActiveStatus_budunit_v001():
    ...
    #     item_kid_count += 1
    #     print(f"{item._lx=} {item_kid_count=}")
...
def test_BudUnit_settle_bud_EveryTwoMonthReturnsCorrectObj_budunit_v001():
    ...
    mat_lx = "deep clean play mat"
    mat_road = yao_budunit.make_road(clean_road, mat_lx)
...
def test_BudUnit_settle_bud_CorrectlySets_sum_healerlink_share(graphics_bool):
    ...
    print(f"{week_item._lx=} {week_item.problem_bool=} {week_item._fund_ratio=}")
    sue_budunit.settle_bud()
    # THEN

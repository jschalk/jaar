...
def test_BudUnit_settle_bud_TreeTraverseSetsAwardLine_fundFromRootCorrectly():
    ...
    sue_awardlink = awardlink_shop(awardee_tag=sue_str)
    ...
    assert sue_bud.itemroot._awardheirs.get(sue_str).awardee_tag == sue_str
    assert sue_bud.itemroot._awardlines != {}
    root_item = sue_bud.get_item_obj(road=sue_bud.itemroot._item_title)
    ...
    assert sue_bud.itemroot._awardlines == {x_awardline.awardee_tag: x_awardline}
...
def test_BudUnit_settle_bud_TreeTraverseSets_awardlines_ToRootItemUnitFromNonRootItemUnit():
    ...
    sue_bud.get_item_obj(casa_road).set_awardlink(awardlink_shop(awardee_tag=sue_str))
    assert sue_bud.itemroot._awardlines == {}
    ...
    assert casa_itemunit._awardlines == {x_awardline.awardee_tag: x_awardline}
...
def test_BudUnit_settle_bud_WithRootLevelAwardLinkSetsGroupUnit_fund_give_fund_take():
    ...
    x_itemroot = sue_bud.get_item_obj(sue_bud.cmty_title)
...
def test_BudUnit_settle_bud_WithLevel3AwardLinkAndEmptyAncestorsSetsGroupUnit_fund_give_fund_take():
    ...
    # THEN
    x_itemroot = x_bud.get_item_obj(x_bud.cmty_title)
...
def test_BudUnit_set_awardlink_CorrectlyCalculatesInheritedAwardLinkBudFund():
    ...
    item_bob = item_dict.get(sue_bud.cmty_title)
...
def test_BudUnit_settle_bud_CorrectlySetsGroupLinkBudCredAndDebt():
    ...
    yao_bud.edit_item_attr(yao_bud.cmty_title, awardlink=sue_awardlink)
    yao_bud.edit_item_attr(yao_bud.cmty_title, awardlink=bob_awardlink)
    yao_bud.edit_item_attr(yao_bud.cmty_title, awardlink=zia_awardlink)
...
def test_BudUnit_settle_bud_CreatesGroupUnitWith_budunit_v001():
    ...
    #     print(f"{item_key=}")
    #     if item._item_title == "D&B":
    #         print(f"{item._item_title=} {item.awardlinks=}")

...
def test_BudUnit_settle_bud_Sets_itemunit_fund_onset_fund_cease_Scenario1():
    ...
    shed_str = "shed"
    shed_road = yao_budunit.make_l1_road(shed_str)
    shed_item = itemunit_shop(shed_str, mass=60)
    yao_budunit.set_l1_item(shed_item)
    lamb_str = "lambs"
    lamb_road = yao_budunit.make_road(shed_road, lamb_str)
    lamb_item = itemunit_shop(lamb_str, mass=1)
    yao_budunit.set_item(lamb_item, parent_road=shed_road)
    duck_str = "ducks"
    duck_road = yao_budunit.make_road(shed_road, duck_str)
    duck_item = itemunit_shop(duck_str, mass=2)
    yao_budunit.set_item(duck_item, parent_road=shed_road)
    ...
    assert yao_budunit.get_item_obj(shed_road)._fund_onset is None
    assert yao_budunit.get_item_obj(shed_road)._fund_cease is None
    ...
    assert yao_budunit.get_item_obj(shed_road)._fund_onset == default_fund_pool() * 0.1
    assert yao_budunit.get_item_obj(shed_road)._fund_cease == default_fund_pool() * 0.7

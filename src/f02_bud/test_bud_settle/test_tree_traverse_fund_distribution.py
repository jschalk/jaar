...
def test_BudUnit_settle_bud_WithLevel3AwardLinkAndEmptyAncestorsSetsGroupUnit_fund_give_fund_take():
    ...
    # THEN
    x_itemroot = x_bud.get_item_obj(x_bud.fiscal_title)
...
def test_BudUnit_set_awardlink_CorrectlyCalculatesInheritedAwardLinkBudFund():
    ...
    item_bob = item_dict.get(sue_bud.fiscal_title)
...
def test_BudUnit_settle_bud_CorrectlySetsGroupLinkBudCredAndDebt():
    ...
    yao_bud.edit_item_attr(yao_bud.fiscal_title, awardlink=sue_awardlink)
    yao_bud.edit_item_attr(yao_bud.fiscal_title, awardlink=bob_awardlink)
    yao_bud.edit_item_attr(yao_bud.fiscal_title, awardlink=zia_awardlink)

...
def get_bud_bank_array(x_bud: BudUnit, settle_bud: bool = None) -> list[list]:
    if settle_bud:
        x_bud.settle_bud()
...
def get_bud_bank_csv(x_bud: BudUnit, settle_bud: bool = None) -> str:
    x_bank_array = get_bud_bank_array(x_bud, settle_bud)
    x_headers = ["acct_name", "fund_take", "fund_give"]
    return create_csv(x_headers, x_bank_array)

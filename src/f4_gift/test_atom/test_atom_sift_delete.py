...
from src.f2_bud.reason_item import reasonunit_shop, factunit_shop
...
from src.f4_gift.atom import atom_delete, atomunit_shop, sift_atomunit
...
def test_sift_atom_ReturnsObj_AtomUnit_DELETE_bud_itemunit():
    # ESTABLISH
    sue_bud = budunit_shop("Sue")
    casa_str = "casa"
...
def test_sift_atom_SetsDeltaUnitAtomUnit_bud_item_reasonunit():
    ...
    casa_week_atom = atomunit_shop(bud_item_reasonunit_str(), atom_delete())
    casa_week_atom.set_arg(road_str(), casa_road)
    casa_week_atom.set_arg(base_str(), week_road)
    clean_week_atom = atomunit_shop(bud_item_reasonunit_str(), atom_delete())
...
def test_sift_atom_SetsDeltaUnitAtomUnit_bud_item_reason_premiseunit_exists():
    ...
    casa_week_atom = atomunit_shop(bud_item_reason_premiseunit_str(), atom_delete())
    casa_week_atom.set_arg(road_str(), casa_road)
    casa_week_atom.set_arg(base_str(), week_road)
    casa_week_atom.set_arg("need", thur_road)
    clean_week_atom = atomunit_shop(bud_item_reason_premiseunit_str(), atom_delete())
    clean_week_atom.set_arg(road_str(), clean_road)
    clean_week_atom.set_arg(base_str(), week_road)
...
def test_sift_atom_SetsDeltaUnitAtomUnit_bud_item_healerlink():
    ...
    casa_swim_atom = atomunit_shop(bud_item_healerlink_str(), atom_delete())
    casa_swim_atom.set_arg(road_str(), casa_road)
    casa_swim_atom.set_arg(healer_id_str(), swim_str)
    clean_swim_atom = atomunit_shop(bud_item_healerlink_str(), atom_delete())
    clean_swim_atom.set_arg(road_str(), clean_road)
...
def test_sift_atom_SetsDeltaUnitAtomUnit_bud_item_factunit():
    ...
    casa_week_atom = atomunit_shop(bud_item_factunit_str(), atom_delete())
    casa_week_atom.set_arg(road_str(), casa_road)
    casa_week_atom.set_arg(base_str(), week_road)
    clean_week_atom = atomunit_shop(bud_item_factunit_str(), atom_delete())
    clean_week_atom.set_arg(road_str(), clean_road)
    clean_week_atom.set_arg(base_str(), week_road)
    sue_bud.add_item(casa_road)
    sue_bud.add_item(clean_road)

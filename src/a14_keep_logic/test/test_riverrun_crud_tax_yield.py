...
def test_RiverRun_set_person_tax_yield_SetsAttr():
    ...
    # WHEN
    yao_tax_yield = 7
    bob_riverrun.set_person_tax_yield(yao_str, yao_tax_yield)
...
def test_RiverRun_tax_yields_is_empty_ReturnsObj():
    ...
    yao_str = "Yao"
    yao_tax_yield = 500
    x_riverrun.set_person_tax_yield(yao_str, yao_tax_yield)
    # THEN
    ...
    bob_str = "Yao"
    bob_tax_yield = 300
    x_riverrun.set_person_tax_yield(bob_str, bob_tax_yield)
    x_riverrun.set_person_tax_yield(yao_str, yao_tax_yield)
    # THEN
    assert x_riverrun.tax_yields_is_empty() is False
...
def test_RiverRun_reset_tax_yields_CorrectlySetsAttr():
    ...
    sue_tax_yield = 56
    yao_tax_yield = 6
    bob_riverrun.set_person_tax_yield(bob_str, bob_tax_yield)
    bob_riverrun.set_person_tax_yield(sue_str, sue_tax_yield)
    bob_riverrun.set_person_tax_yield(yao_str, yao_tax_yield)
...
def test_RiverRun_person_has_tax_yield_ReturnsCorrectBool():
    ...
    bob_riverrun.set_person_tax_yield(sue_str, sue_tax_yield)
    bob_riverrun.set_person_tax_yield(yao_str, yao_tax_yield)
    assert bob_riverrun.person_has_tax_yield(bob_str)
    assert bob_riverrun.person_has_tax_yield(sue_str)
    assert bob_riverrun.person_has_tax_yield(yao_str)
    assert bob_riverrun.person_has_tax_yield(zia_str) is False

    # WHEN
    bob_riverrun.reset_tax_yields()

    # THEN
    assert bob_riverrun.person_has_tax_yield(bob_str) is False
    assert bob_riverrun.person_has_tax_yield(sue_str) is False
    assert bob_riverrun.person_has_tax_yield(yao_str) is False
    assert bob_riverrun.person_has_tax_yield(zia_str) is False
...
def test_RiverRun_delete_tax_yield_SetsAttr():
    ...
    bob_riverrun = riverrun_shop(bob_hubunit)
    yao_str = "Yao"
    bob_riverrun.set_person_tax_yield(yao_str, 5)
    assert bob_riverrun.person_has_tax_yield(yao_str)
...
def test_RiverRun_get_person_tax_yield_ReturnsObj():
    # ESTABLISH
    bob_str = "Bob"
    bob_money_amount = 1000
    bob_penny = 1
    ...
    bob_riverrun.set_person_tax_yield(yao_str, yao_tax_yield)
    assert bob_riverrun.person_has_tax_yield(bob_str)
    assert bob_riverrun.get_person_tax_yield(bob_str) == bob_tax_yield
    assert bob_riverrun.person_has_tax_yield(zia_str) is False
    assert bob_riverrun.get_person_tax_yield(zia_str) == 0
    ...
    # THEN
    assert bob_riverrun.person_has_tax_yield(bob_str) is False
    assert bob_riverrun.get_person_tax_yield(bob_str) == 0
    assert bob_riverrun.person_has_tax_yield(zia_str) is False
    assert bob_riverrun.get_person_tax_yield(zia_str) == 0


def test_RiverRun_add_person_tax_yield_ReturnsObj():
    # ESTABLISH
    bob_str = "Bob"
    bob_money_amount = 1000
    bob_penny = 1
    ...
    bob_riverrun.set_person_tax_yield(yao_str, yao_tax_yield)
    assert bob_riverrun.get_person_tax_yield(bob_str) == bob_tax_yield
    assert bob_riverrun.get_person_tax_yield(sue_str) == sue_tax_yield
    assert bob_riverrun.get_person_tax_yield(zia_str) == 0

    # WHEN
    bob_riverrun.add_person_tax_yield(sue_str, 5)
    bob_riverrun.add_person_tax_yield(zia_str, 10)

    # THEN
    assert bob_riverrun.get_person_tax_yield(bob_str) == bob_tax_yield
    assert bob_riverrun.get_person_tax_yield(sue_str) == sue_tax_yield + 5
    assert bob_riverrun.get_person_tax_yield(zia_str) == 10
...
def test_RiverRun_levy_tax_due_SetsAttr_ScenarioY():
    ...
    bob_believer.add_personunit(bob_str, 2, bob_tax_yield)
    bob_believer.add_personunit(sue_str, 2, sue_tax_yield)
    bob_believer.add_personunit(yao_str, 2, yao_tax_yield)
    ...
    assert bob_riverrun.get_person_tax_yield(bob_str) == 5
    ...
    assert bob_riverrun.get_person_tax_yield(bob_str) == 380

    # ESTABLISH
    assert bob_riverrun.get_person_tax_due(sue_str) == 560
    assert bob_riverrun.get_person_tax_yield(sue_str) == 0
    # WHEN
    ...
    assert bob_riverrun.get_person_tax_yield(sue_str) == 560
    ...
    assert bob_riverrun.get_person_tax_yield(yao_str) == 0
    # WHEN
    ...
    assert bob_riverrun.get_person_tax_yield(yao_str) == 60

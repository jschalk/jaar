...
def test_RiverRun_calc_metrics_SetsAttrsScenario01():
    ...
    yao_str = "Yao"
    yao_person_cred_points = 500
    x_riverrun = riverrun_shop(yao_hubunit)
    x_riverrun.set_keep_credorledger(yao_str, yao_str, yao_person_cred_points)
    assert x_riverrun.get_person_tax_due(yao_str) == 0
...
def test_RiverRun_calc_metrics_SetsAttrsScenario02():
    ...
    yao_person_cred_points = 500
    bob_str = "Bob"
    bob_person_debt_points = 350
    x_riverrun = riverrun_shop(yao_hubunit)
    x_riverrun.set_keep_credorledger(yao_str, yao_str, yao_person_cred_points)
    x_riverrun.set_tax_dues({bob_str: bob_person_debt_points})
    assert x_riverrun.get_person_tax_due(yao_str) == 0
    keep_money_amount = x_riverrun.hubunit.keep_point_magnitude
    assert x_riverrun.get_person_tax_due(bob_str) == keep_money_amount
...
def test_RiverRun_calc_metrics_SetsAttrsScenario03():
    ...
    yao_person_cred_points = 500
    bob_str = "Bob"
    bob_person_debt_points = 25
    sue_str = "Sue"
    sue_person_debt_points = 75
    x_riverrun = riverrun_shop(yao_hubunit)
    x_riverrun.set_keep_credorledger(yao_str, yao_str, yao_person_cred_points)
    debtorledger = {bob_str: bob_person_debt_points, sue_str: sue_person_debt_points}
    x_riverrun.set_tax_dues(debtorledger)
    assert x_riverrun.get_person_tax_due(yao_str) == 0
    keep_money_amount = x_riverrun.hubunit.keep_point_magnitude
    assert x_riverrun.get_person_tax_due(bob_str) == keep_money_amount * 0.25
    assert x_riverrun.get_person_tax_due(sue_str) == keep_money_amount * 0.75
...
def test_RiverRun_calc_metrics_SetsAttrsScenario04():
    ...
    yao_hubunit = example_yao_hubunit()
    yao_str = "Yao"
    yao_yao_person_cred_points = 500
    yao_sue_person_cred_points = 2000
    bob_str = "Bob"
    bob_person_debt_points = 25
    sue_str = "Sue"
    sue_person_debt_points = 75
    x_riverrun = riverrun_shop(yao_hubunit)
    x_riverrun.set_keep_credorledger(yao_str, yao_str, yao_yao_person_cred_points)
    x_riverrun.set_keep_credorledger(yao_str, sue_str, yao_sue_person_cred_points)
    debtorledger = {bob_str: bob_person_debt_points, sue_str: sue_person_debt_points}
    x_riverrun.set_tax_dues(debtorledger)
    assert x_riverrun.get_person_tax_due(yao_str) == 0
    keep_money_amount = x_riverrun.hubunit.keep_point_magnitude
    ...
    assert x_riverrun.get_person_tax_due(yao_str) == 0
    assert x_riverrun.get_person_tax_due(bob_str) == keep_money_amount * 0.25
    assert x_riverrun.get_person_tax_due(sue_str) == 0
    assert x_riverrun.get_person_tax_yield(sue_str) == keep_money_amount * 0.75
...
def test_RiverRun_calc_metrics_SetsAttrsScenario05():
    ...
    assert x_riverrun.get_person_tax_due(yao_str) == keep_money_amount
    assert x_riverrun.get_person_tax_yield(yao_str) == 0
    ...
    # THEN
    assert x_riverrun.get_person_tax_due(yao_str) == 0
    assert x_riverrun.get_person_tax_yield(yao_str) == keep_money_amount
    assert x_riverrun._cycle_count == 2
    assert x_riverrun._debtor_count == 1
    assert x_riverrun._credor_count == 1
    yao_rivergrade = x_riverrun.get_rivergrade(yao_str)

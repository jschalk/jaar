from src.a06_bud_logic.bud import BudUnit, VowLabel, budunit_shop, conceptunit_shop
from src.a14_keep_logic._test_util.a14_env import temp_vow_label


def get_1label_bud() -> BudUnit:
    x_bud = budunit_shop("A")
    x_bud.set_vow_label(temp_vow_label())
    x_bud.settle_bud()
    return x_bud
...
def get_2label_bud(vow_label: VowLabel = None) -> BudUnit:
    if vow_label is None:
        vow_label = temp_vow_label()
    a_str = "A"
    b_str = "B"
    x_bud = budunit_shop(owner_name=a_str)
    x_bud.set_vow_label(vow_label)
    concept_b = conceptunit_shop(b_str)
    x_bud.set_concept(concept_b, parent_way=temp_vow_label())
    x_bud.settle_bud()
    return x_bud

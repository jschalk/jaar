...
def test_get_road_from_doar_ReturnsObj_default_bridge():
    ...
    casa_doar = f"{casa_str}{x_s}{root_tag()}"
    bloomers_str = "bloomers"
    bloomers_road = f"{root_tag()}{x_s}{casa_str}{x_s}{bloomers_str}"
    bloomers_doar = f"{bloomers_str}{x_s}{casa_str}{x_s}{root_tag()}"
    roses_str = "roses"
    roses_road = f"{root_tag()}{x_s}{casa_str}{x_s}{bloomers_str}{x_s}{roses_str}"
    roses_doar = f"{roses_str}{x_s}{bloomers_str}{x_s}{casa_str}{x_s}{root_tag()}"

    # WHEN / THEN
    assert get_road_from_doar("") == ""
    assert get_road_from_doar(root_tag()) == root_tag()
...
def test_get_road_from_doar_ReturnsObj_Not_default_bridge():
    ...
    assert get_road_from_doar(root_tag(), x_s) == root_tag()
...
def test_get_doar_from_road_ReturnsObj_Not_default_bridge():
    ...
    assert get_doar_from_road(root_tag(), x_s) == root_tag()

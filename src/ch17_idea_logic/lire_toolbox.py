...
from src.ch17_idea_logic.idea_db_tool import get_ordered_csv, open_csv


def get_lire_name_dt_columns() -> list[str]:
    return [
        "event_int",
        "face_name",
        "otx_knot",
...
def get_lire_title_dt_columns() -> list[str]:
    return [
        "event_int",
        "face_name",
        "otx_knot",
...
def get_lire_label_dt_columns() -> list[str]:
    return [
        "event_int",
        "face_name",
        "otx_knot",
...
def get_lire_rope_dt_columns() -> list[str]:
    return [
        "event_int",
        "face_name",
        "otx_knot",
...
def create_lire_name_dt(x_map: NameMap) -> DataFrame:
    x_rows_list = [
        {
    ...
    return DataFrame(x_rows_list, columns=get_lire_name_dt_columns())


def create_lire_title_dt(x_map: TitleMap) -> DataFrame:
    x_rows_list = [
        {
            "event_int": x_map.event_int,
            "face_name": x_map.face_name,
    ...
    return DataFrame(x_rows_list, columns=get_lire_title_dt_columns())


def create_lire_label_dt(x_map: LabelMap) -> DataFrame:
    x_rows_list = [
        {
            "event_int": x_map.event_int,
            "face_name": x_map.face_name,
    ...
    return DataFrame(x_rows_list, columns=get_lire_label_dt_columns())


def create_lire_rope_dt(x_map: RopeMap) -> DataFrame:
    x_rows_list = [
        {
            "event_int": x_map.event_int,
            "face_name": x_map.face_name,
    ...
    return DataFrame(x_rows_list, columns=get_lire_rope_dt_columns())


def save_all_csvs_from_lireunit(x_dir: str, x_lireunit: LireUnit):
    _save_lire_name_csv(x_dir, x_lireunit.namemap)
    _save_lire_title_csv(x_dir, x_lireunit.titlemap)
    _save_lire_label_csv(x_dir, x_lireunit.labelmap)
    _save_lire_rope_csv(x_dir, x_lireunit.ropemap)


def _save_lire_name_csv(x_dir: str, namemap: NameMap):
    x_dt = create_lire_name_dt(namemap)
    save_file(x_dir, "name.csv", get_ordered_csv(x_dt))


def _save_lire_title_csv(x_dir: str, titlemap: TitleMap):
    x_dt = create_lire_title_dt(titlemap)
    save_file(x_dir, "title.csv", get_ordered_csv(x_dt))


def _save_lire_label_csv(x_dir: str, labelmap: LabelMap):
    x_dt = create_lire_label_dt(labelmap)
    save_file(x_dir, "label.csv", get_ordered_csv(x_dt))


def _save_lire_rope_csv(x_dir: str, ropemap: RopeMap):
    x_dt = create_lire_rope_dt(ropemap)
...
def create_dir_valid_empty_lireunit(x_dir: str) -> LireUnit:
    face_name_set = set()
    event_int_set = set()
    unknown_str_set = set()
    otx_knot_set = set()
...
def init_lireunit_from_dir(x_dir: str) -> LireUnit:
    x_lireunit = create_dir_valid_empty_lireunit(x_dir)
    _load_namemap_from_csv(x_dir, x_lireunit.namemap)
    _load_titlemap_from_csv(x_dir, x_lireunit.titlemap)
    _load_labelmap_from_csv(x_dir, x_lireunit.labelmap)
    _load_ropemap_from_csv(x_dir, x_lireunit.ropemap)
    x_lireunit.ropemap.labelmap = x_lireunit.labelmap
    return x_lireunit

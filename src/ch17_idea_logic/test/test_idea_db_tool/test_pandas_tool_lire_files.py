...
from src.ch16_lire_logic.lire_config import get_lire_filename
...
from src.ch17_idea_logic.idea_db_tool import (
    _get_lire_idea_format_filenames,
    move_otx_csvs_to_lire_inx,
    open_csv,
...
def test_move_otx_csvs_to_lire_inx_CreatesLireedFiles_Scenario0_SingleFile(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    bob_otx = "Bob"
    ...
    sue_inx = "Suita"
    xio_inx = "Xioita"
    sue_lireunit = lireunit_shop(sue_otx)
    sue_lireunit.set_namemap(get_suita_namemap())
    sue_dir = create_path(get_example_face_dir(), sue_otx)
    lireunit_file_path = create_path(sue_dir, get_lire_filename())
    print(f"{sue_dir=}")
    save_file(sue_dir, get_lire_filename(), sue_lireunit.get_json())
...
# save two dataframes to be lireed: two files in otx, two files in inx
def test_move_otx_csvs_to_lire_inx_CreatesLireedFiles_Scenario1_SingleFile_RopeTerm(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    otx_amy45_str = "amy45"
    ...
    sue_lireunit = get_casa_maison_lireunit_set_by_label()
    sue_dir = create_path(get_example_face_dir(), sue_lireunit.face_name)
    save_file(sue_dir, get_lire_filename(), sue_lireunit.get_json())
...
# save two dataframes to be lireed: two files in otx, two files in inx
def test_move_otx_csvs_to_lire_inx_CreatesLireedFiles_Scenario2_TwoFile(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_lireunit = get_casa_maison_lireunit_set_by_label()
...
def test_get_lire_idea_format_filenames_ReturnsObj():
    # ESTABLISH
    ...
    # WHEN
    x_lire_idea_filenames = _get_lire_idea_format_filenames()

    # THEN
    print(f"{x_lire_idea_filenames=}")
    assert br00003_filename not in x_lire_idea_filenames
    assert br00042_filename in x_lire_idea_filenames
    assert br00043_filename in x_lire_idea_filenames
    assert br00044_filename in x_lire_idea_filenames
    assert len(x_lire_idea_filenames) == 8

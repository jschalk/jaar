...
from src.ch17_idea_logic.idea_db_tool import (
    get_dataframe_lireable_columns,
    translate_all_columns_dataframe,
    translate_single_column_dataframe,
)


def test_get_dataframe_lireable_columns_ReturnsObj():
    # ESTABLISH / WHEN / THEN
    x_dt = DataFrame()
    assert get_dataframe_lireable_columns(x_dt) == set()
    x_dt = DataFrame(columns=[voice_name_str()])
    assert get_dataframe_lireable_columns(x_dt) == {voice_name_str()}
    ...
    assert get_dataframe_lireable_columns(x_dt) == {
        voice_name_str(),
        reason_context_str(),
    }
...
def test_translate_all_columns_dataframe_SetsParameterAttrs_Scenario1_VoiceName():
    ...
    sue_inx = "Suita"
    bob_inx = "Bobita"
    yao_lireunit = lireunit_shop(yao_str)
    yao_lireunit.set_otx2inx(NameTerm_str(), xio_otx, xio_inx)
    yao_lireunit.set_otx2inx(NameTerm_str(), sue_otx, sue_inx)
    yao_lireunit.set_otx2inx(NameTerm_str(), bob_otx, bob_inx)
    otx_dt = DataFrame(
        columns=[moment_label_str(), voice_name_str(), voice_cred_points_str()]
    )
    otx_dt.loc[0] = ["ZZ", zia_otx, 12]
    ...
    # WHEN
    translate_all_columns_dataframe(otx_dt, yao_lireunit)
...
def test_translate_all_columns_dataframe_SetsParameterAttrs_Scenario2_RodeUnit_get_casa_maison_lireunit_set_by_otx2inx():
    # ESTABLISH
    otx_amy45_str = "amy45"
    inx_amy87_str = "amy87"
    ...
    yao_lireunit = get_casa_maison_lireunit_set_by_otx2inx()
...
def test_translate_all_columns_dataframe_SetsParameterAttrs_Scenario3_RodeUnit_get_casa_maison_lireunit_set_by_label():
    # ESTABLISH
    otx_amy45_str = "amy45"
    inx_amy87_str = "amy87"
    otx_amy45_rope = to_rope(otx_amy45_str)
    ...
    sweep_otx_rope = create_rope(clean_otx_rope, sweep_str)
    sweep_inx_rope = create_rope(clean_inx_rope, sweep_str)
    yao_lireunit = get_casa_maison_lireunit_set_by_label()
    # print(f"{yao_lireunit=}")
    otx_dt = get_casa_maison_rope_otx_dt()
    old_otx_dt = copy_deepcopy(otx_dt)
    assert otx_dt.iloc[0][reason_context_str()] == otx_amy45_rope
    assert otx_dt.iloc[1][reason_context_str()] == casa_otx_rope

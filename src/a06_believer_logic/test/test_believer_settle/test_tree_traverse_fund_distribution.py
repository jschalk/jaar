...
def test_BelieverUnit_settle_believer_WhenPlanUnitHasFundsBut_kidsHaveNoMassDistributeFundsToAcctUnits_scenario0():
    ...
    assert sue_believerunit.get_acct(yao_str)._fund_give == 0
    assert sue_believerunit.get_acct(yao_str)._fund_take == 0
    ...
    assert sue_believerunit.get_acct(yao_str)._fund_give == default_fund_pool()
    assert sue_believerunit.get_acct(yao_str)._fund_take == default_fund_pool()
...
def test_BelieverUnit_settle_believer_WithRootLevelAwardLinkSetsGroupUnit_fund_give_fund_take():
    ...
    zia_str = "Zia"
    xio_str = "Xio"
    sue_believer.set_acctunit(acctunit_shop(yao_str))
    sue_believer.set_acctunit(acctunit_shop(zia_str))
    sue_believer.set_acctunit(acctunit_shop(xio_str))
    ...
    # ESTABLISH
    sue_believer.set_acctunit(acctunit_shop(sue_str))
    ...
    assert len(sue_believer.get_acctunit_group_titles_dict()) == 4
...
def test_BelieverUnit_settle_believer_WithLevel3AwardLinkSetsGroupUnit_fund_give_fund_take():
    ...
    zia_str = "Zia"
    xio_str = "Xio"
    x_believer.set_acctunit(acctunit_shop(yao_str))
    x_believer.set_acctunit(acctunit_shop(zia_str))
    x_believer.set_acctunit(acctunit_shop(xio_str))
    yao_awardlink = awardlink_shop(yao_str, give_force=20, take_force=6)
    zia_awardlink = awardlink_shop(zia_str, give_force=10, take_force=1)
    xio_awardlink = awardlink_shop(xio_str, give_force=10)
    ...
    assert len(x_believer.get_acctunit_group_titles_dict()) == 3
...
def test_BelieverUnit_settle_believer_CreatesNewGroupUnitAndSetsGroup_fund_give_fund_take():
    ...
    swim_plan.set_awardlink(zia_awardlink)
    swim_plan.set_awardlink(xio_awardlink)
    assert len(x_believer.get_acctunit_group_titles_dict()) == 2
    ...
    zia_groupunit = x_believer.get_groupunit(zia_str)
    xio_groupunit = x_believer.get_groupunit(xio_str)
    assert len(x_believer.get_acctunit_group_titles_dict()) != len(
...
def test_BelieverUnit_settle_believer_CorrectlySetsGroupLinkBelieverCredAndDebt():
    ...
    bob_str = "Bob"
    zia_str = "Zia"
    yao_believer.set_acctunit(acctunit_shop(sue_str))
    yao_believer.set_acctunit(acctunit_shop(bob_str))
    ...
    bob_acctunit = yao_believer.get_acct(bob_str)
    ...
    xio_str = "Xio"
    yao_believer.set_acctunit(acctunit_shop(xio_str))
...
def test_BelieverUnit_settle_believer_CorrectlySetsAcctUnitBeliever_fund():
    # ESTABLISH
    yao_believer = believerunit_shop("Yao")
    swim_str = "swim"
    ...
    # THEN
    xio_acctunit = yao_believer.get_acct(xio_str)
...
def test_BelieverUnit_settle_believer_CorrectlySetsPartGroupedLWAcctUnitBeliever_fund():
    # ESTABLISH
    yao_believer = believerunit_shop("Yao")
    swim_str = "swim"
    swim_rope = yao_believer.make_l1_rope(swim_str)
...
def test_BelieverUnit_settle_believer_CreatesNewGroupUnitAndSetsAcct_fund_give_fund_take():
    # ESTABLISH
    bob_str = "Bob"
    ...
    zia_str = "Zia"
    xio_str = "Xio"
    bob_believer.set_acctunit(acctunit_shop(yao_str))
    bob_believer.set_acctunit(acctunit_shop(zia_str))
    # bob_believer.set_acctunit(acctunit_shop(xio_str))
    yao_awardlink = awardlink_shop(yao_str, give_force=20, take_force=6)
    zia_awardlink = awardlink_shop(zia_str, give_force=10, take_force=1)
    xio_awardlink = awardlink_shop(xio_str, give_force=10)
    ...
    swim_plan.set_awardlink(zia_awardlink)
    swim_plan.set_awardlink(xio_awardlink)
    assert len(bob_believer.get_acctunit_group_titles_dict()) == 2

    # WHEN
    bob_believer.settle_believer()

    # THEN
    assert len(bob_believer.get_acctunit_group_titles_dict()) != len(
        bob_believer._groupunits
    )
    assert not bob_believer.acct_exists(xio_str)
    yao_acctunit = bob_believer.get_acct(yao_str)
...
def test_BelieverUnit_settle_believer_CorrectlySetsAcctUnit_fund_give_fund_take():
    # ESTABLISH
    yao_believer = believerunit_shop("Yao")
    ...
    bob_str = "Bob"
    zia_str = "Zia"
    yao_believer.set_acctunit(acctunit_shop(sue_str, 8))
    yao_believer.set_acctunit(acctunit_shop(bob_str))
    yao_believer.set_acctunit(acctunit_shop(zia_str))
    sue_acctunit = yao_believer.get_acct(sue_str)
    bob_acctunit = yao_believer.get_acct(bob_str)
...
def clear_all_acctunits_groupunits_fund_agenda_give_take(x_believer: BelieverUnit):
    ...
    # delete believer_agenda_debt and believer_agenda_cred
    for x_acctunit in x_believer.accts.values():
...
def test_BelieverUnit_agenda_cred_debt_IsCorrectlySet():
    ...
    clear_all_acctunits_groupunits_fund_agenda_give_take(yao_believer)
    ...
    #     print(f"{plan_rope=}")
    # for x_acct in yao_believer.accts.values():
    ...
    assert all_acctunits_have_legitimate_values(yao_believer)
...
def all_acctunits_have_legitimate_values(x_believer: BelieverUnit):
    return not any(
        (
...
def test_BelieverUnit_settle_believer_SetsAttrsWhenNoFactUnitsNoReasonUnitsEmpty_agenda_ratio_cred_debt():
    ...
    yao_believer.set_acctunit(sue_acctunit)
    yao_believer.set_acctunit(bob_acctunit)
    yao_believer.set_acctunit(zia_acctunit)
    sue_acct = yao_believer.get_acct(sue_str)
    bob_acct = yao_believer.get_acct(bob_str)

...
from src.a06_believer_logic.believer_main import believerunit_shop, get_sorted_plan_list
...
def test_BelieverUnit_set_plan_dict_Clears_gogo_calc_stop_calc():
    ...
    texas_str = "Texas"
    texas_rope = sue_believer.make_rope(usa_rope, texas_str)
...
def test_BelieverUnit_set_plan_dict_Sets_reason_contexts():
    # ESTABLISH
    sue_believer = believerunit_shop("Sue")
    nation_str = "nation"
    nation_rope = sue_believer.make_l1_rope(nation_str)
    polis_str = "polis"
    polis_rope = sue_believer.make_l1_rope(polis_str)
    ...
    # THEN
    assert sue_believer._reason_contexts == {polis_rope}
...
def test_BelieverUnit_set_plan_CreatesPlanUnitsUsedBy_reasonunits():
    ...
    # WHEN
    sue_believer.set_plan(clean_cookery_plan, cleaning_rope, create_missing_plans=True)

    # THEN
    assert sue_believer.plan_exists(buildings_rope)
...
def test_get_sorted_plan_list_ReturnsObj():
    ...
    # WHEN
    x_sorted_plan_list = get_sorted_plan_list(list(sue_believer._plan_dict.values()))

    # THEN
    assert x_sorted_plan_list is not None
    assert len(x_sorted_plan_list) == 17
    assert x_sorted_plan_list[0] == sue_believer.planroot
    assert x_sorted_plan_list[1] == sue_believer.get_plan_obj(casa_rope)
    assert x_sorted_plan_list[11] == sue_believer.get_plan_obj(mon_rope)

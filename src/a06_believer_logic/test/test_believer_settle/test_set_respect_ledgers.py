...
def test_create_groupunits_metrics_SetsAttrScenario1():
    ...
    yao_yao_membership = yao_personunit.get_membership(yao_str)
...
def test_BelieverUnit_set_personunit_groupunit_respect_ledgers_SetsAttr_scenario0():
    # ESTABLISH
    sue_str = "Sue"
    sue_believerunit = believerunit_shop(sue_str)
    assert sue_believerunit._groupunits == {}
    ...
    # WHEN
    sue_believerunit._set_personunit_groupunit_respect_ledgers()

    # THEN
    assert sue_believerunit._groupunits == {}


def test_BelieverUnit_set_personunit_groupunit_respect_ledgers_Clears_groups():
    # ESTABLISH
    sue_str = "Sue"
    sue_believerunit = believerunit_shop(sue_str)
...
def test_BelieverUnit_set_personunit_groupunit_respect_ledgers_SetsAttr_scenario1():
    # ESTABLISH
    sue_str = "Sue"
    sue_believerunit = believerunit_shop(sue_str)
    yao_str = "Yao"
    ...
    assert yao_personunit.get_membership(yao_str)._credor_pool == 0
    assert yao_personunit.get_membership(yao_str)._debtor_pool == 0
    # assert sue_believerunit._groupunits == {}
    ...
    assert yao_personunit._credor_pool != 0
    assert yao_personunit._debtor_pool != 0
    assert yao_personunit._credor_pool == sue_believerunit.credor_respect
    assert yao_personunit._debtor_pool == sue_believerunit.debtor_respect
    yao_membership = yao_personunit.get_membership(yao_str)
...
def test_BelieverUnit_set_personunit_groupunit_respect_ledgers_SetsAttr_scenario2():
    # ESTABLISH
    sue_str = "Sue"
    sue_believerunit = believerunit_shop(sue_str)
    yao_str = "Yao"
    sue_believerunit.add_personunit(yao_str)
    yao_personunit = sue_believerunit.get_person(yao_str)
    yao_personunit.add_membership(yao_str, 1, 4)
    ohio_str = ";Ohio"
    ...
    # THEN
    assert sue_believerunit.get_person(yao_str)._credor_pool != 0
    assert sue_believerunit.get_person(yao_str)._debtor_pool != 0
    assert yao_personunit.get_membership(yao_str)._credor_pool != 0
    assert yao_personunit.get_membership(yao_str)._debtor_pool != 0
    yao_yao_membership = yao_personunit.get_membership(yao_str)
...
def test_BelieverUnit_set_personunit_groupunit_respect_ledgers_ResetPersonUnitsAttrs():
    # ESTABLISH
    sue_str = "Sue"
    sue_believerunit = believerunit_shop(sue_str)
    yao_str = "Yao"
    zia_str = "Zia"
    sue_believerunit.add_personunit(yao_str, 55, 55)
    sue_believerunit.add_personunit(zia_str, 55, 55)
    yao_personunit = sue_believerunit.get_person(yao_str)
    zia_personunit = sue_believerunit.get_person(zia_str)
    yao_personunit.add_fund_give_take(0.5, 0.6, 0.1, 0.22)
    zia_personunit.add_fund_give_take(0.2, 0.1, 0.1, 0.22)
    zia_1 = 0.8
    zia_2 = 0.5
    zia_3 = 200
    zia_4 = 140
    zia_personunit.set_fund_agenda_ratio_give_take(
        ...
        personunits_person_cred_points_sum=zia_3,
        personunits_person_debt_points_sum=zia_4,
    )
    yao_1 = 0.2
    yao_2 = 0.5
    yao_3 = 204
    ...
    yao_personunit.set_fund_agenda_ratio_give_take(
        ...
        personunits_person_cred_points_sum=yao_3,
        personunits_person_debt_points_sum=yao_4,
    )
    assert zia_personunit._fund_agenda_ratio_give == 0.125
    assert zia_personunit._fund_agenda_ratio_take == 0.44
    assert yao_personunit._fund_agenda_ratio_give == 0.5
    assert yao_personunit._fund_agenda_ratio_take == 0.44

    # WHEN
    sue_believerunit._set_personunit_groupunit_respect_ledgers()

    # THEN
    assert zia_personunit._fund_agenda_ratio_give == 0
    assert zia_personunit._fund_agenda_ratio_take == 0

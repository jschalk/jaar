...
from src.a08_owner_atom_logic.atom_config import (
    ...
    get_normalized_owner_table_build,
    get_owner_dimens,
...
def test_get_owner_dimens_ReturnsObj():
    # ESTABLISH / WHEN / THEN
...
def test_get_all_owner_dimen_keys_ReturnsObj():
    ...
    # ESTABLISH / WHEN
    all_owner_dimen_keys = get_all_owner_dimen_keys()

    # THEN
    assert not all_owner_dimen_keys.isdisjoint({"acct_name"})
    expected_owner_keys = set()
    for owner_dimen in get_owner_dimens():
        expected_owner_keys.update(_get_atom_config_jkey_keys(owner_dimen))

    expected_owner_keys.add("owner_name")
    print(f"{expected_owner_keys=}")
    assert all_owner_dimen_keys == expected_owner_keys
...
def test_get_all_owner_dimen_delete_keys_ReturnsObj():
    # ESTABLISH / WHEN
    all_owner_dimen_delete_keys = get_all_owner_dimen_delete_keys()

    # THEN
    assert not all_owner_dimen_delete_keys.isdisjoint(
        {get_delete_key_name("acct_name")}
    )
    expected_owner_delete_keys = {
        get_delete_key_name(owner_dimen_key)
        for owner_dimen_key in get_all_owner_dimen_keys()
    }
    print(f"{expected_owner_delete_keys=}")
    assert all_owner_dimen_delete_keys == expected_owner_delete_keys
...
def test_get_atom_config_dict_EveryCrudOperationHasOwnerDeltaOrderGroup():
    # ESTABLISH
...
def test_get_sorted_jkey_keys_ReturnsObj_owner_acctunit():
    # ESTABLISH
...
def test_get_flattened_atom_table_build_ReturnsObj():
    ...
    assert atom_columns.get("ownerunit_UPDATE_credor_respect") == "REAL"
    # print(f"{atom_columns.keys()=}")


def test_get_normalized_owner_table_build_ReturnsObj():
    # ESTABLISH / WHEN
    normalized_owner_table_build = get_normalized_owner_table_build()
    nx = normalized_owner_table_build
    ...
    assert len(cat_ownerunit) == 2
    assert cat_ownerunit.get(columns_str) is not None

    ownerunit_columns = cat_ownerunit.get(columns_str)
    assert len(ownerunit_columns) == 9
    assert ownerunit_columns.get("uid") is not None
    assert ownerunit_columns.get("max_tree_traverse") is not None
    assert ownerunit_columns.get(credor_respect_str()) is not None
    assert ownerunit_columns.get(debtor_respect_str()) is not None
    assert ownerunit_columns.get("fund_pool") is not None
    assert ownerunit_columns.get(fund_iota_str()) is not None
    assert ownerunit_columns.get(respect_bit_str()) is not None
    assert ownerunit_columns.get(penny_str()) is not None
    assert ownerunit_columns.get("tally") is not None

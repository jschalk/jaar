...
def test_OwnerAtom_get_insert_sqlstr_RaisesErrorWhen_is_valid_False():
    # WHEN
    sports_str = "sports"
    ...
    with pytest_raises(Exception) as excinfo:
        update_disc_owneratom.get_insert_sqlstr()
    assert (
        str(excinfo.value)
...
def test_OwnerAtom_get_insert_sqlstr_ReturnsObj_OwnerUnitSimpleAttrs():
    # WHEN
    new2_value = 66
    dimen = ownerunit_str()
    opt_arg2 = "max_tree_traverse"
    x_owneratom = owneratom_shop(dimen, UPDATE_str())
    x_owneratom.set_jvalue(opt_arg2, new2_value)
    # THEN
    x_table = "atom_hx"
    example_sqlstr = f"""
INSERT INTO {x_table} (
...
def test_OwnerAtom_get_insert_sqlstr_ReturnsObj_plan_factunit():
    ...
    # WHEN
    generated_sqlstr = update_disc_owneratom.get_insert_sqlstr()
...
"""
    ...
    # WHEN
    x_owneratom = get_owneratom_from_rowdata(x_rowdata)
    ...
    assert update_disc_owneratom.crud_str == x_owneratom.crud_str
    assert update_disc_owneratom.jkeys == x_owneratom.jkeys
    assert update_disc_owneratom.jvalues == x_owneratom.jvalues

...
def test_atomrow_shop_ReturnsObj():
    # ESTABLISH
    x_atom_dimens = {owner_acctunit_str()}
...
def test_AtomRow_set_atom_dimen_SetsAttr():
    # ESTABLISH
    x_atomrow = atomrow_shop({owner_acctunit_str()}, INSERT_str())
    assert owner_acct_membership_str() not in x_atomrow._atom_dimens

    # WHEN
    x_atomrow.set_atom_dimen(owner_acct_membership_str())

    # THEN
    assert owner_acct_membership_str() in x_atomrow._atom_dimens


def test_AtomRow_atom_dimen_exists_ReturnsObj():
    # ESTABLISH
    x_atomrow = atomrow_shop(set(), INSERT_str())
    assert not x_atomrow.atom_dimen_exists(owner_acctunit_str())
    assert not x_atomrow.atom_dimen_exists(owner_acct_membership_str())
    ...
    # THEN
    assert not x_atomrow.atom_dimen_exists(owner_acctunit_str())
    assert x_atomrow.atom_dimen_exists(owner_acct_membership_str())


def test_AtomRow_delete_atom_dimen_SetsAttr():
    # ESTABLISH
    x_atomrow = atomrow_shop({owner_acctunit_str()}, INSERT_str())
    x_atomrow.set_atom_dimen(owner_acctunit_str())
    x_atomrow.set_atom_dimen(owner_acct_membership_str())
    assert x_atomrow.atom_dimen_exists(owner_acctunit_str())
    assert x_atomrow.atom_dimen_exists(owner_acct_membership_str())

    # WHEN
    x_atomrow.delete_atom_dimen(owner_acct_membership_str())
...
def test_AtomRow_get_owneratoms_ReturnsObj_owner_acctunit_str_INSERT_Scenario0():
    # ESTABLISH
    ...
    # WHEN
    x_owneratoms = x_atomrow.get_owneratoms()

    # THEN
    assert len(x_owneratoms) == 1
    static_atom = owneratom_shop(x_dimen, INSERT_str())
    static_atom.set_arg(acct_name_str(), "Bob")
    assert x_owneratoms[0] == static_atom


def test_AtomRow_get_owneratoms_ReturnsObj_owner_acctunit_str_INSERT_Scenario1():
    # ESTABLISH
    x_dimen = owner_acctunit_str()
    x_atomrow = atomrow_shop({x_dimen}, INSERT_str())
    x_atomrow.acct_name = "Bob"
...
def test_AtomRow_get_owneratoms_ReturnsObj_owner_acctunit_NSERT_Fails():
    # ESTABLISH
    x_dimen = owner_acctunit_str()
    x_atomrow = atomrow_shop({x_dimen}, INSERT_str())
    ...
    # THEN
    assert len(x_owneratoms) == 0


def test_AtomRow_get_owneratoms_ReturnsObj_owner_acctunit_INSERT_Scenario2():
    # ESTABLISH
    x_dimen = owner_acctunit_str()
    x_atomrow = atomrow_shop({x_dimen}, INSERT_str())
    x_atomrow.acct_name = "Bob"
...
def test_AtomRow_get_owneratoms_ReturnsObjIfDimenIsCorrect():
    # ESTABLISH
    x_atomrow = atomrow_shop(set(), INSERT_str())
    x_atomrow.acct_name = "Bob"
    four_str = "4"
    x_atomrow.acct_cred_points = four_str
    assert len(x_atomrow.get_owneratoms()) == 0

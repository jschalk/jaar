...
class OwnerAtomDescriptionException(Exception):
    pass
...
class OwnerAtom:
    ...
    def get_insert_sqlstr(self) -> str:
        if self.is_valid() is False:
            raise OwnerAtomDescriptionException(
    ...
    def get_nesting_order_args(self) -> list[str]:
        # When ChangUnit places an OwnerAtom in a nested dictionary ChangUnit.owneratoms
...
def get_from_dict(x_dict: dict) -> OwnerAtom:
    x_atom = owneratom_shop(x_dict["dimen"], x_dict["crud"])
...
def get_from_json(x_str: str) -> OwnerAtom:
    return get_from_dict(get_dict_from_json(x_str))


def _modify_owner_update_ownerunit(x_owner: OwnerUnit, x_atom: OwnerAtom):
    x_arg = "max_tree_traverse"
    if x_atom.get_value(x_arg) is not None:
        x_owner.set_max_tree_traverse(x_atom.get_value(x_arg))
    x_arg = "credor_respect"
    if x_atom.get_value(x_arg) is not None:
        x_owner.set_credor_respect(x_atom.get_value(x_arg))
    x_arg = "debtor_respect"
    if x_atom.get_value(x_arg) is not None:
        x_owner.set_debtor_respect(x_atom.get_value(x_arg))
    x_arg = "fund_pool"
    if x_atom.get_value(x_arg) is not None:
        x_owner.fund_pool = x_atom.get_value(x_arg)
    x_arg = "fund_iota"
    if x_atom.get_value(x_arg) is not None:
        x_owner.fund_iota = x_atom.get_value(x_arg)
    x_arg = "tally"
    if x_atom.get_value(x_arg) is not None:
        x_owner.tally = x_atom.get_value(x_arg)
    x_arg = "respect_bit"
    if x_atom.get_value(x_arg) is not None:
        x_owner.respect_bit = x_atom.get_value(x_arg)
    x_arg = "penny"
    if x_atom.get_value(x_arg) is not None:
        x_owner.penny = x_atom.get_value(x_arg)


def _modify_owner_acct_membership_delete(x_owner: OwnerUnit, x_atom: OwnerAtom):
    x_acct_name = x_atom.get_value("acct_name")
    x_group_title = x_atom.get_value("group_title")
    x_owner.get_acct(x_acct_name).delete_membership(x_group_title)


def _modify_owner_acct_membership_update(x_owner: OwnerUnit, x_atom: OwnerAtom):
    x_acct_name = x_atom.get_value("acct_name")
    x_group_title = x_atom.get_value("group_title")
    x_acctunit = x_owner.get_acct(x_acct_name)
...
def _modify_owner_acct_membership_insert(x_owner: OwnerUnit, x_atom: OwnerAtom):
    x_acct_name = x_atom.get_value("acct_name")
    x_group_title = x_atom.get_value("group_title")
    x_group_cred_points = x_atom.get_value("group_cred_points")
    x_group_debt_points = x_atom.get_value("group_debt_points")
...
def _modify_owner_acctunit_delete(x_owner: OwnerUnit, x_atom: OwnerAtom):
    x_owner.del_acctunit(x_atom.get_value("acct_name"))


def _modify_owner_acctunit_update(x_owner: OwnerUnit, x_atom: OwnerAtom):
    x_owner.edit_acctunit(
...
def _modify_owner_acctunit_insert(x_owner: OwnerUnit, x_atom: OwnerAtom):
    x_owner.set_acctunit(
        acctunit_shop(
            acct_name=x_atom.get_value("acct_name"),
            acct_cred_points=x_atom.get_value("acct_cred_points"),
            acct_debt_points=x_atom.get_value("acct_debt_points"),
...
def _modify_owner_ownerunit(x_owner: OwnerUnit, x_atom: OwnerAtom):
    if x_atom.crud_str == "UPDATE":
        _modify_owner_update_ownerunit(x_owner, x_atom)


def _modify_owner_acct_membership(x_owner: OwnerUnit, x_atom: OwnerAtom):
    if x_atom.crud_str == "DELETE":
        _modify_owner_acct_membership_delete(x_owner, x_atom)
    elif x_atom.crud_str == "UPDATE":
        _modify_owner_acct_membership_update(x_owner, x_atom)
...
def _modify_owner_acctunit(x_owner: OwnerUnit, x_atom: OwnerAtom):
    if x_atom.crud_str == "DELETE":
        _modify_owner_acctunit_delete(x_owner, x_atom)
    elif x_atom.crud_str == "UPDATE":
        _modify_owner_acctunit_update(x_owner, x_atom)
    elif x_atom.crud_str == "INSERT":
        _modify_owner_acctunit_insert(x_owner, x_atom)


def modify_owner_with_owneratom(x_owner: OwnerUnit, x_atom: OwnerAtom):
    if x_atom.dimen == "ownerunit":
...
def jvalues_different(dimen: str, x_obj: any, y_obj: any) -> bool:
    ...
    elif dimen in {"owner_acct_membership"}:
    ...
    elif dimen == "owner_acctunit":
        return (x_obj.acct_cred_points != y_obj.acct_cred_points) or (
            x_obj.acct_debt_points != y_obj.acct_debt_points
        )


class InvalidOwnerAtomException(Exception):
    pass


def get_owneratom_from_rowdata(x_rowdata: RowData) -> OwnerAtom:
    dimen_str, crud_str = get_dimen_from_dict(x_rowdata.row_dict)
    x_atom = owneratom_shop(dimen=dimen_str, crud_str=crud_str)
...
class AtomRow:
    ...
    def get_owneratoms(self) -> list[OwnerAtom]:
        self._set_class_types()
        x_list = []
        for x_dimen in self._atom_dimens:
            x_atom = owneratom_shop(x_dimen, self._crud_command)
...
def sift_owneratom(x_owner: OwnerUnit, x_atom: OwnerAtom) -> OwnerAtom:
    config_keys = get_atom_config_jkeys(x_atom.dimen)
    x_atom_reqs = {x_key: x_atom.get_value(x_key) for x_key in config_keys}

    x_exists = owner_attr_exists(x_atom.dimen, x_owner, x_atom_reqs)
    ...
    if x_atom.crud_str == "DELETE" and x_exists:
        ...
        x_owner_obj = owner_get_obj(x_atom.dimen, x_owner, x_atom_reqs)
        x_jvalues = x_atom.get_jvalues_dict()
        update_atom = owneratom_shop(x_atom.dimen, "UPDATE", x_atom.jkeys)
        for jvalue in x_jvalues:
            optional_jvalue = x_atom.get_value(jvalue)
            obj_jvalue = x_owner_obj.__dict__[jvalue]

...
def test_etl_fisc_raw_tables_to_fisc_csvs_CreateFiles(env_dir_setup_cleanup):
    ...
    with sqlite3_connect(":memory:") as fisc_db_conn:
        ...
        insert_raw_sqlstr = f"""
INSERT INTO {x_fisc.unit_raw_tablename} ({creed_number_str()}, {event_int_str()}, {face_name_str()}, {fisc_label_str()})
VALUES 
...
"""
        ...
        assert generated_fiscunit_csv == expected_fiscunit_csv_str
        # confirming file is non-zero length, has column headers
        assert len(open_file(x_fisc.deal_raw_csv_path)) == 95
        assert len(open_file(x_fisc.cash_raw_csv_path)) == 96
        assert len(open_file(x_fisc.hour_raw_csv_path)) == 86
        assert len(open_file(x_fisc.mont_raw_csv_path)) == 84
        assert len(open_file(x_fisc.week_raw_csv_path)) == 86

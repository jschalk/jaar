...
def test_etl_creed_raw_to_bud_prime_tables_CreatesBudRawTables(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    raw_srt = "_put_raw"
    ...
    with sqlite3_connect(":memory:") as conn:
        ...
        # WHEN
        etl_creed_raw_to_bud_prime_tables(cursor)
...
def test_etl_creed_raw_to_bud_prime_tables_Bud_dimen_creed_PopulatesFiscRawTables(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_inx = "Suzy"
    ...
    x_syntax_inz_dir = create_path(get_module_temp_dir(), "syntax_inz")
...
"""
    ...
    with sqlite3_connect(":memory:") as conn:
        cursor = conn.cursor()
        etl_inz_face_csv_files2creed_raw_tables(cursor, x_syntax_inz_dir)
...
def test_etl_creed_raw_to_bud_prime_tables_Sets_error_message(env_dir_setup_cleanup):
    # ESTABLISH
    sue_inx = "Suzy"
    bob_inx = "Bobby"
    yao_inx = "Bobby"
    ...
    with sqlite3_connect(":memory:") as fisc_db_conn:
        ...
        insert_raw_sqlstr = f"""
INSERT INTO {x_tablename} ({creed_number_str()},{event_int_str()},{face_name_str()},{fisc_tag_str()},{owner_name_str()},{acct_name_str()},{credit_belief_str()},{debtit_belief_str()})
VALUES

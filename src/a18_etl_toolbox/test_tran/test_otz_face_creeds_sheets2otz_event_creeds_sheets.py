...
from src.a18_etl_toolbox.transformers import etl_otz_face_creeds_to_otz_event_otx_creeds
...
def test_etl_otz_face_creeds_to_otz_event_otx_creeds_CreatesFaceCreedSheets_Scenario0_MultpleFaceNames(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"
    ...
    example_sue_df = DataFrame([sue0, sue1], columns=creed_columns)
    example_zia_df = DataFrame([zia0, zia1, zia2], columns=creed_columns)
    ...
    # WHEN
    etl_otz_face_creeds_to_otz_event_otx_creeds(x_syntax_otz_dir)
    ...
    example_event3_df = DataFrame([sue0, sue1], columns=creed_columns)
    example_event7_df = DataFrame([zia0], columns=creed_columns)
    example_event9_df = DataFrame([zia1, zia2], columns=creed_columns)

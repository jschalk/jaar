...
from src.a18_etl_toolbox.transformers import etl_brick_creeds_to_otz_face_creeds
...
def test_etl_brick_creeds_to_otz_face_creeds_CreatesFaceCreedSheets_Scenario0_SingleFaceName(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"
    ...
    # WHEN
    etl_brick_creeds_to_otz_face_creeds(x_brick_dir, x_syntax_otz_dir)
...
def test_etl_brick_creeds_to_otz_face_creeds_CreatesFaceCreedSheets_Scenario1_MultpleFaceNames(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"
    ...
    br00003_brick_agg_df = DataFrame([sue1, sue2, zia3], columns=creed_columns)
    x_etl_dir = get_module_temp_dir()
    x_brick_dir = create_path(x_etl_dir, "brick")
    x_syntax_otz_dir = create_path(x_etl_dir, "syntax_otz")
    br00003_filename = "br00003.xlsx"
    ...
    example_sue_df = DataFrame([sue1, sue2], columns=creed_columns)
    example_zia_df = DataFrame([zia3], columns=creed_columns)
    pandas_assert_frame_equal(sue_br3_agg_df, example_sue_df)
    pandas_assert_frame_equal(zia_br3_agg_df, example_zia_df)


def test_etl_brick_creeds_to_otz_face_creeds_Scenario2_PidginDimenCreedsAreNotLoaded(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"

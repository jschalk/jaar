...
def abbv(tablename: str) -> str:
    abbrevions = {
        ...
        f"{bud_idea_laborlink_str()}_put_agg": "BUDLABOR_PUT_AGG",
        f"{bud_idea_laborlink_str()}_put_raw": "BUDLABOR_PUT_RAW",
        ...
        f"{bud_idea_laborlink_str()}_del_agg": "BUDLABOR_DEL_AGG",
        f"{bud_idea_laborlink_str()}_del_raw": "BUDLABOR_DEL_RAW",
...
def test_create_prime_tablename_ReturnsObj():
    ...
    budlabor_s_agg_table = create_prime_tablename("budlabor", "s", agg_str, put_str)
...
def test_get_creed_stageble_put_dimens_HasAll_creed_numbersForAll_dimens():
    ...
    with sqlite3_connect(":memory:") as fisc_db_conn:
        ...
        expected_creed_stagable_dimens = {i_num: [] for i_num in sorted_creed_numbers}
        for x_dimen in sorted(creed_config):
            dimen_config = creed_config.get(x_dimen)
            ...
            for creed_number in sorted_creed_numbers:
                ...
                if required_columns_exist(cursor, src_tablename, dimen_key_columns):
                    expected_creed_stagable_dimens.get(creed_number).append(x_dimen)
    ...
    creed_stageble_dimen_list = sorted(list(expected_creed_stagable_dimens))
    print(f"{expected_creed_stagable_dimens=}")
    assert creed_dimen_combo_checked_count == 680
    assert creed_raw2dimen_count == 109
    assert get_creed_stageble_put_dimens() == expected_creed_stagable_dimens


def test_CREED_STAGEBLE_DEL_DIMENS_HasAll_creed_numbersForAll_dimens():
    # sourcery skip: extract-method, no-loop-in-tests, no-conditionals-in-tests
    # ESTABLISH / WHEN
    # THEN
    creed_config = get_creed_config_dict()
    ...
    with sqlite3_connect(":memory:") as fisc_db_conn:
        ...
        creed_dimen_combo_checked_count = 0
        sorted_creed_numbers = sorted(get_creed_numbers())
        x_creed_stagable_dimens = {i_num: [] for i_num in sorted_creed_numbers}
        for x_dimen in sorted(creed_config):
            dimen_config = creed_config.get(x_dimen)
            dimen_key_columns = set(dimen_config.get("jkeys").keys())
            ...
            for creed_number in sorted_creed_numbers:
                ...
                creed_dimen_combo_checked_count += 1
                if required_columns_exist(cursor, src_tablename, dimen_key_columns):
                    x_creed_stagable_dimens.get(creed_number).append(x_dimen)
                    creed_raw2dimen_count += 1
                    src_cols_set = set(src_columns)
                    # print(
                    #     f"{x_dimen} {creed_number} checking... {dimen_key_columns=} {dimen_value_columns=} {src_cols_set=}"
                    ...
                    # check sqlstr is correct?
                    assert generated_sqlstr != ""
    expected_creed_stagable_dimens = {
        x_creed_number: stagable_dimens
        for x_creed_number, stagable_dimens in x_creed_stagable_dimens.items()
        if stagable_dimens != []
    }
    creed_stageble_dimen_list = sorted(list(expected_creed_stagable_dimens))
    print(f"{expected_creed_stagable_dimens=}")
    ...
    assert CREED_STAGEBLE_DEL_DIMENS == expected_creed_stagable_dimens

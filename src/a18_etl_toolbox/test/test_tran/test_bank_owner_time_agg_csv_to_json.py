...
from src.a18_etl_toolbox.transformers import etl_bank_ote1_agg_csvs_to_jsons


def test_etl_bank_ote1_agg_csvs_to_jsons_CreatesFile_Scenaro0(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    bob_str = "Bob"
    ...
    a23_event_time_p = create_bank_ote1_csv_path(bank_mstr_dir, accord23_str)
    a45_event_time_p = create_bank_ote1_csv_path(bank_mstr_dir, accord45_str)
    a23_event_time_csv = f"""{bank_label_str()},{owner_name_str()},{event_int_str()},{bud_time_str()},error_message
{accord23_str},{bob_str},{event3},{timepoint55},
"""
    a45_event_time_csv = f"""{bank_label_str()},{owner_name_str()},{event_int_str()},{bud_time_str()},error_message
...
"""
    ...
    a23_ote1_json_path = create_bank_ote1_json_path(bank_mstr_dir, accord23_str)
    a45_ote1_json_path = create_bank_ote1_json_path(bank_mstr_dir, accord45_str)
    assert os_path_exists(a23_ote1_json_path) is False
    assert os_path_exists(a45_ote1_json_path) is False

    # WHEN
    etl_bank_ote1_agg_csvs_to_jsons(bank_mstr_dir)

...
from src.a18_etl_toolbox.tran_sqlstrs import CREATE_BELIEF_PARTNER_NETS_SQLSTR
...
def test_insert_tranunit_partners_net_PopulatesDatabase():
    ...
    with sqlite3_connect(":memory:") as db_conn:
        ...
        assert get_row_count(cursor, belief_partner_nets_tablename) == 0
        ...
        select_sqlstr = f"SELECT belief_label, believer_name, {believer_net_amount_str()} FROM {belief_partner_nets_tablename}"
...
def test_etl_belief_json_partner_nets_to_belief_partner_nets_table_PopulatesDatabase(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    a23_str = "amy23"
    ...
    with sqlite3_connect(":memory:") as db_conn:
        cursor = db_conn.cursor()
        belief_partner_nets_tablename = belief_partner_nets_str()
        assert not db_table_exists(cursor, belief_partner_nets_tablename)

        # WHEN
        etl_belief_json_partner_nets_to_belief_partner_nets_table(cursor, mstr_dir)

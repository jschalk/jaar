...
def test_etl_event_inherited_believerunits_to_coin_gut_SetsFiles_Scenario0(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_inx = "Suzy"
    ...
    a23_bob_e3_dir = create_believer_event_dir_path(
        coin_mstr_dir, a23_str, bob_inx, event3
    )
    a23_bob_e7_dir = create_believer_event_dir_path(
        coin_mstr_dir, a23_str, bob_inx, event7
    )
    ...
    e3_bob_believer.add_partnerunit(bob_inx, credit77)
    e3_bob_believer.add_partnerunit(yao_inx, credit44)
    e7_bob_believer.add_partnerunit(bob_inx, credit77)
    e7_bob_believer.add_partnerunit(sue_inx, credit88)
    e7_bob_believer.add_partnerunit(yao_inx, credit44)
    ...
    a23_bob_gut_path = create_gut_path(coin_mstr_dir, a23_str, bob_inx)
    assert os_path_exists(a23_bob_gut_path) is False

    # WHEN
    etl_event_inherited_believerunits_to_coin_gut(coin_mstr_dir)

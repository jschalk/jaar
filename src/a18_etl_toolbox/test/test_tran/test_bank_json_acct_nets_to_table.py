...
from src.a18_etl_toolbox.tran_sqlstrs import CREATE_BANK_ACCT_NETS_SQLSTR
...
def test_insert_tranunit_accts_net_PopulatesDatabase():
    ...
    with sqlite3_connect(":memory:") as db_conn:
        ...
        assert get_row_count(cursor, bank_acct_nets_tablename) == 0
        ...
        select_sqlstr = f"SELECT bank_label, owner_name, {owner_net_amount_str()} FROM {bank_acct_nets_tablename}"
...
def test_etl_bank_json_acct_nets_to_bank_acct_nets_table_PopulatesDatabase(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    a23_str = "accord23"
    mstr_dir = get_module_temp_dir()
    a23_bank = bankunit_shop(a23_str, mstr_dir)
    sue_str = "Sue"
    yao_str = "Yao"
    bob_str = "Bob"
    t55_tran_time = 5505
    ...
    t77_tran_time = 7707
    t77_yao_amount = -77
    a23_bank.add_paypurchase(sue_str, yao_str, t55_tran_time, t55_yao_amount)
    a23_bank.add_paypurchase(sue_str, yao_str, t66_tran_time, t66_yao_amount)
    a23_bank.add_paypurchase(sue_str, bob_str, t55_tran_time, t55_bob_amount)
    a23_bank.add_paypurchase(yao_str, yao_str, t77_tran_time, t77_yao_amount)
    a23_json_path = create_bank_json_path(mstr_dir, a23_str)
    save_json(a23_json_path, None, a23_bank.get_dict())

    with sqlite3_connect(":memory:") as db_conn:
        cursor = db_conn.cursor()
        bank_acct_nets_tablename = bank_acct_nets_str()
        assert not db_table_exists(cursor, bank_acct_nets_tablename)

        # WHEN
        etl_bank_json_acct_nets_to_bank_acct_nets_table(cursor, mstr_dir)

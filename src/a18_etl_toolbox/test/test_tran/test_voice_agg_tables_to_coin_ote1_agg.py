...
from src.a15_coin_logic.test._util.a15_str import coin_budunit_str
from src.a18_etl_toolbox.test._util.a18_str import coin_ote1_agg_str
...
def test_etl_voice_raw_tables_to_coin_ote1_agg_SetsTableAttr():
    # ESTABLISH
    bob_str = "Bob"
    sue_str = "Sue"
    event3 = 3
...
"""
        ...
        assert get_row_count(cursor, coinbud_v_raw_table) == 4
        assert db_table_exists(cursor, coin_ote1_agg_str()) is False

        # WHEN
        etl_voice_raw_tables_to_coin_ote1_agg(cursor)

        # THEN
        assert db_table_exists(cursor, coin_ote1_agg_str())
        assert get_row_count(cursor, coin_ote1_agg_str()) == 3
        cursor.execute(f"SELECT * FROM {coin_ote1_agg_str()};")
        coinunit_agg_rows = cursor.fetchall()
        ...
        print(f"{coinunit_agg_rows[0]=}")
        print(f"{coinunit_agg_rows[1]=}")
        print(f"{coinunit_agg_rows[2]=}")
        assert coinunit_agg_rows == [ex_row0, ex_row1, ex_row2]

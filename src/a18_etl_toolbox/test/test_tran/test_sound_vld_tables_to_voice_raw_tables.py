...
"""
        ...
        # WHEN
        sqlstr = get_insert_into_voice_raw_sqlstrs().get(onrawar_v_raw_put_tablename)
        cursor.execute(sqlstr)

        # THEN
        assert get_row_count(cursor, onrawar_v_raw_put_tablename) == 4
...
, {acct_cred_points_str()}
, {acct_debt_points_str()}
FROM {onrawar_v_raw_put_tablename}
"""
...
"""
        ...
        # THEN
        assert get_row_count(cursor, onracct_v_raw_put_tablename) == 4
        select_sqlstr = f"""SELECT {event_int_str()}
, {face_name_str()}_otx
, {belief_label_str()}_otx
, {owner_name_str()}_otx
...
, {acct_cred_points_str()}
, {acct_debt_points_str()}
FROM {onracct_v_raw_put_tablename}
"""
        cursor.execute(select_sqlstr)
        rows = cursor.fetchall()
        print(rows)

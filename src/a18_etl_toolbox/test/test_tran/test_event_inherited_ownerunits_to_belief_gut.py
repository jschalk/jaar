...
# test that budunit with depth 1 is able to create nested ownerunits directories and populate with event relevant


def test_etl_event_inherited_ownerunits_to_belief_gut_SetsFiles_Scenario0(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_inx = "Suzy"
    ...
    e3_bob_owner = ownerunit_shop(bob_inx, a23_str)
    e7_bob_owner = ownerunit_shop(bob_inx, a23_str)
    e3_bob_owner.add_acctunit(bob_inx, credit77)
    e3_bob_owner.add_acctunit(yao_inx, credit44)
    e7_bob_owner.add_acctunit(bob_inx, credit77)
    e7_bob_owner.add_acctunit(sue_inx, credit88)
    e7_bob_owner.add_acctunit(yao_inx, credit44)
    save_file(a23_bob_e3_dir, owner_filename, e3_bob_owner.get_json())
    save_file(a23_bob_e7_dir, owner_filename, e7_bob_owner.get_json())
    ...
    # WHEN
    etl_event_inherited_ownerunits_to_belief_gut(belief_mstr_dir)

    # THEN
    assert os_path_exists(a23_bob_gut_path)
    generated_gut_owner = ownerunit_get_from_json(open_file(a23_bob_gut_path))
    assert generated_gut_owner.accts == e7_bob_owner.accts
    assert generated_gut_owner == e7_bob_owner
    assert generated_gut_owner.get_dict() == e7_bob_owner.get_dict()

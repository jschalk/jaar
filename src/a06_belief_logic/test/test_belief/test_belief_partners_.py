...
def test_BeliefUnit_set_partnerunit_SetsAttr():
    # ESTABLISH
    yao_str = "Yao"
    ...
    # WHEN
    bob_belief.set_partnerunit(yao_partnerunit)

    # THEN
    assert bob_belief.partners.get(yao_str).knot == slash_str
    x_partners = {yao_partnerunit.partner_name: deepcopy_yao_partnerunit}
    assert bob_belief.partners != x_partners
    deepcopy_yao_partnerunit.knot = bob_belief.knot
    assert bob_belief.partners == x_partners


def test_BeliefUnit_set_partner_DoesNotSet_partner_name_membership():
    # ESTABLISH
    x_respect_bit = 5
    ...
    # WHEN
    yao_belief.set_partnerunit(partnerunit_shop(zia_str), auto_set_membership=False)

    # THEN
    assert yao_belief.get_partner(zia_str).get_membership(zia_str) is None


def test_BeliefUnit_set_partner_DoesSet_partner_name_membership():
    # ESTABLISH
    x_respect_bit = 5
    yao_belief = beliefunit_shop("Yao", respect_bit=x_respect_bit)
    zia_str = "Zia"
    ...
    # THEN
    zia_zia_membership = yao_belief.get_partner(zia_str).get_membership(zia_str)
...
def test_BeliefUnit_set_partner_DoesNotOverRide_partner_name_membership():
    # ESTABLISH
    x_respect_bit = 5
    yao_belief = beliefunit_shop("Yao", respect_bit=x_respect_bit)
    zia_str = "Zia"
    ...
    # THEN
    zia_ohio_membership = yao_belief.get_partner(zia_str).get_membership(ohio_str)
...
def test_BeliefUnit_add_partnerunit_Sets_partners():
    # ESTABLISH
    x_respect_bit = 6
    yao_belief = beliefunit_shop("Yao", respect_bit=x_respect_bit)
    zia_str = "Zia"
    ...
    # WHEN
    yao_belief.add_partnerunit(zia_str, partner_cred_points=13, partner_debt_points=8)
    yao_belief.add_partnerunit(sue_str, partner_debt_points=5)
    yao_belief.add_partnerunit(xio_str, partner_cred_points=17)

    # THEN
    assert len(yao_belief.partners) == 3
    assert len(yao_belief.get_partnerunit_group_titles_dict()) == 3
    assert yao_belief.partners.get(xio_str).partner_cred_points == 17
    assert yao_belief.partners.get(sue_str).partner_debt_points == 5
    assert yao_belief.partners.get(xio_str).respect_bit == x_respect_bit


def test_BeliefUnit_partner_exists_ReturnsObj():
    # ESTABLISH
    bob_belief = beliefunit_shop("Bob")
    yao_str = "Yao"

    # WHEN / THEN
    assert bob_belief.partner_exists(yao_str) is False

    # ESTABLISH
    bob_belief.add_partnerunit(yao_str)

    # WHEN / THEN
    assert bob_belief.partner_exists(yao_str)


def test_BeliefUnit_set_partner_Creates_membership():
    ...
    before_zia_credit = 7
    before_zia_debt = 17
    yao_belief.add_partnerunit(zia_str, before_zia_credit, before_zia_debt)
...
def test_BeliefUnit_edit_partner_RaiseExceptionWhenPartnerDoesNotExist():
    # ESTABLISH
    yao_belief = beliefunit_shop("Yao")
    zia_str = "Zia"
...
def test_BeliefUnit_edit_partner_UpdatesObj():
    # ESTABLISH
    yao_belief = beliefunit_shop("Yao")
    zia_str = "Zia"
...
def test_BeliefUnit_get_partner_ReturnsObj():
    # ESTABLISH
    yao_belief = beliefunit_shop("Yao")
    zia_str = "Zia"
    sue_str = "Sue"
    yao_belief.add_partnerunit(zia_str)
    yao_belief.add_partnerunit(sue_str)
    ...
    # THEN
    assert zia_partner == yao_belief.partners.get(zia_str)
    assert sue_partner == yao_belief.partners.get(sue_str)

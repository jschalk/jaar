...
from src.a05_plan_logic.plan import PlanUnit, get_default_coin_label, planunit_shop
...
def test_get_default_coin_label_ReturnsObj():
    # ESTABLISH / WHEN / THEN
    assert get_default_coin_label() == "ZZ"
...
def test_PlanUnit_Exists():
    ...
    assert x_planunit.coin_label is None
...
def test_planunit_shop_WithNoParametersReturnsObj():
    ...
    assert x_planunit.plan_label is None
    assert x_planunit.coin_label == get_default_coin_label()
    assert x_planunit._uid is None
    assert x_planunit.begin is None
    assert x_planunit.close is None
    assert x_planunit.addin is None
...
def test_PlanUnit_get_obj_key_ReturnsObj():
    ...
    round_rope = create_rope(get_default_coin_label(), round_str)
    ball_str = "ball"
...
def test_PlanUnit_get_rope_ReturnsObj():
    ...
    round_str = "round_stuff"
    slash_str = "/"
    round_rope = create_rope(get_default_coin_label(), round_str, knot=slash_str)
    ball_str = "ball"
...
def test_PlanUnit_set_parent_rope_SetsAttr():
    ...
    # WHEN
    sports_rope = create_rope(get_default_coin_label(), "sports", knot=slash_str)

...
from src._road.road import default_road_delimiter_if_none, OwnerID, RoadUnit, TribeID
...
from src.tribe.journal_sqlstr import get_create_table_if_not_exist_sqlstrs
...
class TribeUnit:
    ...
    # directory setup
    def _set_tribe_dirs(self, in_memory_journal: bool = None):
        self._tribe_dir = f"{self.tribes_dir}/{self.tribe_id}"
        self._owners_dir = f"{self._tribe_dir}/owners"
        self._gifts_dir = f"{self._tribe_dir}/{get_gifts_folder()}"
        set_dir(x_path=self._tribe_dir)
    ...
    def get_journal_db_path(self) -> str:
        return f"{self.tribes_dir}/{self.tribe_id}/journal.db"
...
def tribeunit_shop(
    ...
    tribe_x._set_tribe_dirs(in_memory_journal=in_memory_journal)
    return tribe_x

...
from src.tribe.examples.tribe_env import get_test_tribes_dir, env_dir_setup_cleanup
from os.path import exists as os_path_exists, isdir as os_path_isdir


def test_TribeUnit_exists(env_dir_setup_cleanup):
    music_str = "music"
    music_tribe = TribeUnit(tribe_id=music_str, tribes_dir=get_test_tribes_dir())
    assert music_tribe.tribe_id == music_str
    assert music_tribe.tribes_dir == get_test_tribes_dir()
    assert music_tribe._owners_dir is None
    assert music_tribe._journal_db is None
    assert music_tribe._gifts_dir is None
    assert music_tribe._road_delimiter is None
    assert music_tribe._fund_coin is None
    assert music_tribe._bit is None
    assert music_tribe._penny is None


def test_tribeunit_shop_ReturnsTribeUnit(env_dir_setup_cleanup):
    # ESTABLISH
    music_str = "music"
    ...
    assert music_tribe.tribe_id == music_str
    assert music_tribe.tribes_dir == get_test_tribes_dir()
    assert music_tribe._owners_dir is not None
    assert music_tribe._gifts_dir is not None
    assert music_tribe._road_delimiter == default_road_delimiter_if_none()
    assert music_tribe._fund_coin == default_fund_coin_if_none()
    assert music_tribe._bit == default_bit_if_none()
    assert music_tribe._penny == default_penny_if_none()


def test_tribeunit_shop_ReturnsTribeUnitWith_road_delimiter(env_dir_setup_cleanup):
    # ESTABLISH
    music_str = "music"
    slash_str = "/"
    x_fund_coin = 7.0
    ...
    music_tribe = tribeunit_shop(
        tribe_id=music_str,
        tribes_dir=get_test_tribes_dir(),
    ...
    # THEN
    assert music_tribe._road_delimiter == slash_str
    assert music_tribe._fund_coin == x_fund_coin
    assert music_tribe._bit == x_bit
    assert music_tribe._penny == x_penny


def test_TribeUnit_set_tribe_dirs_SetsCorrectDirsAndFiles(env_dir_setup_cleanup):
    # ESTABLISH
    music_str = "music"
    music_tribe = TribeUnit(tribe_id=music_str, tribes_dir=get_test_tribes_dir())
    x_tribe_dir = f"{get_test_tribes_dir()}/{music_str}"
    x_owners_dir = f"{x_tribe_dir}/owners"
    x_gifts_dir = f"{x_tribe_dir}/{get_gifts_folder()}"
    journal_file_name = "journal.db"
    journal_file_path = f"{x_tribe_dir}/{journal_file_name}"

    assert music_tribe._tribe_dir is None
    assert music_tribe._owners_dir is None
    assert music_tribe._gifts_dir is None
    assert os_path_exists(x_tribe_dir) is False
    assert os_path_isdir(x_tribe_dir) is False
    ...
    # WHEN
    music_tribe._set_tribe_dirs()

    # THEN
    assert music_tribe._tribe_dir == x_tribe_dir
    assert music_tribe._owners_dir == x_owners_dir
    assert music_tribe._gifts_dir == x_gifts_dir
    assert os_path_exists(x_tribe_dir)
    assert os_path_isdir(x_tribe_dir)
    assert os_path_exists(x_owners_dir)
    assert os_path_exists(x_gifts_dir)
    assert os_path_exists(journal_file_path)


def test_tribeunit_shop_SetsTribesDirs(env_dir_setup_cleanup):
    # ESTABLISH
    music_str = "music"
    ...
    # THEN
    assert music_tribe.tribe_id == music_str
    assert music_tribe._tribe_dir == f"{get_test_tribes_dir()}/{music_str}"
    assert music_tribe._owners_dir == f"{music_tribe._tribe_dir}/owners"


def test_TribeUnit_init_owner_econs_CorrectlySetsDirAndFiles(env_dir_setup_cleanup):
    # ESTABLISH
    music_str = "music"
    slash_str = "/"
    x_fund_coin = 4
    ...
    # WHEN
    music_tribe.init_owner_econs(sue_str)

    # THEN
    print(f"{get_test_tribes_dir()=}")
    assert os_path_exists(sue_hubunit.action_path())


def test_TribeUnit_get_owner_voice_from_file_ReturnsCorrectObj(env_dir_setup_cleanup):
    # ESTABLISH
    music_str = "music"
    music_tribe = tribeunit_shop(
        music_str, get_test_tribes_dir(), in_memory_journal=True
    ...
    # WHEN
    gen_sue_voice = music_tribe.get_owner_voice_from_file(sue_str)
...
def test_TribeUnit__set_all_healer_dutys_CorrectlySetsdutys(
    ...
    # WHEN
    music_tribe._set_all_healer_dutys(sue_str)
...
def test_TribeUnit_get_owner_hubunits_ReturnsCorrectObj(env_dir_setup_cleanup):
    # ESTABLISH
    music_tribe = tribeunit_shop("music", get_test_tribes_dir(), in_memory_journal=True)
    sue_str = "Sue"
    yao_str = "Yao"

    # WHEN / THEN
    assert len(music_tribe.get_owner_hubunits()) == 0

    # WHEN
    music_tribe.init_owner_econs(sue_str)
    music_tribe.init_owner_econs(yao_str)
    music_all_owners = music_tribe.get_owner_hubunits()

    # THEN
    sue_hubunit = hubunit_shop(
        tribes_dir=music_tribe.tribes_dir,
        tribe_id=music_tribe.tribe_id,
        owner_id=sue_str,
        econ_road=None,
        road_delimiter=music_tribe._road_delimiter,
        fund_coin=music_tribe._fund_coin,
        bit=music_tribe._bit,
    )
    ...
    assert len(music_tribe.get_owner_hubunits()) == 2

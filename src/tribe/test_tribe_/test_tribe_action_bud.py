...
def test_TribeUnit_generate_action_bud_Sets_action_BudFile(env_dir_setup_cleanup):
    # ESTABLISH
    music_str = "Music"
    music_tribe = tribeunit_shop(music_str, get_test_tribes_dir(), True)
    sue_str = "Sue"
    sue_hubunit = hubunit_shop(None, music_str, sue_str, None)
    x_sue_action_path = f"{music_tribe._owners_dir}/{sue_str}/action/{sue_str}.json"
    ...
    # WHEN
    sue_action = music_tribe.generate_action_bud(sue_str)

    # THEN
    example_bud = budunit_shop(sue_str, music_str)
    assert sue_action._tribe_id == example_bud._tribe_id
    assert sue_action._owner_id == example_bud._owner_id


def test_TribeUnit_generate_action_bud_ReturnsRegeneratedObj(env_dir_setup_cleanup):
    # ESTABLISH
    music_tribe = tribeunit_shop("music", get_test_tribes_dir(), True)
    sue_str = "Sue"
    music_tribe.init_owner_econs(sue_str)
    sue_hubunit = hubunit_shop(
        music_tribe.tribes_dir, music_tribe.tribe_id, sue_str, None
    )
    ...
    # WHEN
    after_sue_bud = music_tribe.generate_action_bud(sue_str)

    # THEN method should wipe over action bud
    assert after_sue_bud.acct_exists(bob_str) is False


def test_TribeUnit_generate_action_bud_SetsCorrectFileWithout_healerlink(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    music_tribe = tribeunit_shop("music", get_test_tribes_dir(), True)
    bob_str = "Bob"
    music_tribe.init_owner_econs(bob_str)
    bob_hubunit = hubunit_shop(
        music_tribe.tribes_dir, music_tribe.tribe_id, bob_str, None
    )
    before_bob_action_bud = music_tribe.generate_action_bud(bob_str)
    sue_str = "Sue"
    ...
    # WHEN
    after_bob_action_bud = music_tribe.generate_action_bud(bob_str)

    # THEN
    assert after_bob_action_bud.acct_exists(sue_str)


def test_TribeUnit_generate_action_bud_SetsFileWith_healerlink(env_dir_setup_cleanup):
    # ESTABLISH
    music_tribe = tribeunit_shop("music", get_test_tribes_dir(), True)
...
def test_TribeUnit_generate_all_action_buds_SetsCorrectFiles(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    music_tribe = tribeunit_shop("music", get_test_tribes_dir(), True)
    ...
    bob_hubunit = hubunit_shop(tribes_dir, music_tribe.tribe_id, bob_str, None)
    music_tribe.init_owner_econs(sue_str)
    sue_hubunit = hubunit_shop(tribes_dir, music_tribe.tribe_id, sue_str, None)
    bob_voice_bud = music_tribe.generate_action_bud(bob_str)
    sue_voice_bud = music_tribe.generate_action_bud(sue_str)
    ...
    before_bob_action_bud = music_tribe.get_action_file_bud(bob_str)
    before_sue_action_bud = music_tribe.get_action_file_bud(sue_str)
    ...
    # THEN
    after_bob_action_bud = music_tribe.get_action_file_bud(bob_str)
    after_sue_action_bud = music_tribe.get_action_file_bud(sue_str)
    assert after_bob_action_bud.acct_exists(bob_str)

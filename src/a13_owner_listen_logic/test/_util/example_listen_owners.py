...
def get_ownerunit_with_4_levels() -> OwnerUnit:
    ...
    casa = "casa"
    sue_owner.set_l1_plan(planunit_shop(casa, mass=30, task=True))

    cat = "cat have dinner"
    sue_owner.set_l1_plan(planunit_shop(cat, mass=30, task=True))

    week_str = "weekdays"
    week_rope = sue_owner.make_l1_rope(week_str)
    plan_kid_weekdays = planunit_shop(week_str, mass=40)
    sue_owner.set_l1_plan(plan_kid_weekdays)
    ...
    sue_owner.set_plan(plan_grandkidU, week_rope)
    sue_owner.set_plan(plan_grandkidM, week_rope)
    sue_owner.set_plan(plan_grandkidT, week_rope)
    sue_owner.set_plan(plan_grandkidW, week_rope)
    sue_owner.set_plan(plan_grandkidR, week_rope)
    sue_owner.set_plan(plan_grandkidF, week_rope)
    sue_owner.set_plan(plan_grandkidA, week_rope)
...
def get_fund_breakdown_owner() -> OwnerUnit:
    ...
    sue_owner.set_l1_plan(planunit_shop(casa_str, mass=30))
    sue_owner.set_plan(planunit_shop(cat_str, mass=30), casa_rope)
    sue_owner.set_plan(planunit_shop(hun_n_str, mass=30), cat_rope)
    sue_owner.set_plan(planunit_shop(hun_y_str, mass=30), cat_rope)
    sue_owner.set_plan(planunit_shop(clean_str, mass=30), casa_rope)
    sue_owner.set_plan(planunit_shop(sweep_str, mass=30, task=True), clean_rope)
    sue_owner.set_plan(planunit_shop(dish_str, mass=30, task=True), clean_rope)
...
def get_ownerunit_irrational_example() -> OwnerUnit:
    ...
    hatter_owner.add_plan(egg_rope)

    chicken_str = "chicken first"
    chicken_rope = hatter_owner.make_l1_rope(chicken_str)
    hatter_owner.add_plan(chicken_rope)

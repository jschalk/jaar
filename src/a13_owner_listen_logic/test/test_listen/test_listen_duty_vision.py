...
def get_example_yao_owner() -> OwnerUnit:
    yao_str = "Yao"
    zia_str = "Zia"
    bob_str = "Bob"
...
def get_yao_ohio_hubunit() -> HubUnit:
    yao_owner = get_example_yao_owner()
    return hubunit_shop(
        belief_mstr_dir=env_dir(),
        belief_label=yao_owner.belief_label,
        owner_name=yao_owner.owner_name,
...
def test_listen_to_owner_visions_Pipeline_Scenario1_yao_gut_CanOnlyReferenceItself(
    ...
    # yao_iowa_hubunit.save_vision_owner(yao_vision1)
    # yao_ohio_hubunit.save_vision_owner(yao_vision2)
    # zia_utah_hubunit.save_vision_owner(yao_vision3)
...
def test_create_vision_file_from_duty_file_CreatesEmptyvision(env_dir_setup_cleanup):
    ...
    yao_vision = sue_texas_hubunit.get_vision_owner(yao_str)

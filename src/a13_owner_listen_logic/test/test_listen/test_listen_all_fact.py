...
def test_get_ordered_debtors_roll_ReturnsObjsInOrder():
    ...
    yao_owner.add_acctunit(sue_str, sue_acct_cred_points, sue_acct_debt_points)
    yao_pool = 92
    yao_owner.set_acct_respect(yao_pool)

    # WHEN
    ordered_accts1 = get_ordered_debtors_roll(yao_owner)
    ...
    yao_owner.add_acctunit(bob_str, 0, bob_acct_debt_points)
    bob_acct = yao_owner.get_acct(bob_str)

    # WHEN
    ordered_accts2 = get_ordered_debtors_roll(yao_owner)
...
def test_get_ordered_debtors_roll_DoesNotReturnZero_acct_debt_points():
    ...
    bob_acct_debt_points = 75
    xio_str = "Xio"
    yao_owner.add_acctunit(zia_str, 0, zia_acct_debt_points)
    yao_owner.add_acctunit(sue_str, 0, sue_acct_debt_points)
    yao_owner.add_acctunit(bob_str, 0, bob_acct_debt_points)
    yao_owner.add_acctunit(yao_str, 0, 0)
    yao_owner.add_acctunit(xio_str, 0, 0)
...
def test_set_listen_to_speaker_fact_SetsFact():
    # ESTABLISH
    yao_str = "Yao"
    yao_listener = ownerunit_shop(yao_str)
    casa_str = "casa"

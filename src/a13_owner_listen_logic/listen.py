...
def _allocate_irrational_acct_debt_points(
    listener: OwnerUnit, speaker_owner_name: OwnerName
) -> OwnerUnit:
    speaker_acctunit = listener.get_acct(speaker_owner_name)
    speaker_acct_debt_points = speaker_acctunit.acct_debt_points
    speaker_acctunit.add_irrational_acct_debt_points(speaker_acct_debt_points)
    return listener


def _allocate_inallocable_acct_debt_points(
    listener: OwnerUnit, speaker_owner_name: OwnerName
) -> OwnerUnit:
    speaker_acctunit = listener.get_acct(speaker_owner_name)
    speaker_acctunit.add_inallocable_acct_debt_points(speaker_acctunit.acct_debt_points)
    return listener


def get_speaker_perspective(speaker: OwnerUnit, listener_owner_name: OwnerName):
    listener_hubunit = hubunit_shop("", "", listener_owner_name)
    return listener_hubunit.get_perspective_owner(speaker)
...
def _create_mass_data(listener: OwnerUnit, x_rope: RopeTerm) -> list:
...
def get_debtors_roll(x_duty: OwnerUnit) -> list[AcctUnit]:
    return [
        ...
        if x_acctunit.acct_debt_points != 0
    ]


def get_ordered_debtors_roll(x_owner: OwnerUnit) -> list[AcctUnit]:
    accts_ordered_list = get_debtors_roll(x_owner)
    accts_ordered_list.sort(
        key=lambda x: (x.acct_debt_points, x.acct_name), reverse=True
    )
...
def listen_to_speaker_agenda(listener: OwnerUnit, speaker: OwnerUnit) -> OwnerUnit:
    if listener.acct_exists(speaker.owner_name) is False:
        raise Missing_debtor_respectException(
            f"listener '{listener.owner_name}' owner is assumed to have {speaker.owner_name} acctunit."
        )
    perspective_owner = get_speaker_perspective(speaker, listener.owner_name)
    if perspective_owner._rational is False:
        return _allocate_irrational_acct_debt_points(listener, speaker.owner_name)
    if listener.debtor_respect is None:
        return _allocate_inallocable_acct_debt_points(listener, speaker.owner_name)
    if listener.owner_name != speaker.owner_name:
        agenda = generate_perspective_agenda(perspective_owner)
    else:
        agenda = list(perspective_owner.get_all_tasks().values())
    if len(agenda) == 0:
        return _allocate_inallocable_acct_debt_points(listener, speaker.owner_name)
...
def listen_to_agendas_create_init_job_from_guts(
    belief_mstr_dir: str, listener_job: OwnerUnit
):
    ...
    for x_acctunit in get_ordered_debtors_roll(listener_job):
        ...
        if speaker_gut is None:
            speaker_gut = create_empty_owner_from_owner(listener_job, speaker_id)
        if speaker_gut:
            listen_to_speaker_agenda(listener_job, speaker_gut)


def listen_to_agendas_jobs_into_job(belief_mstr_dir: str, listener_job: OwnerUnit):
    belief_label = listener_job.belief_label
    for x_acctunit in get_ordered_debtors_roll(listener_job):
        speaker_id = x_acctunit.acct_name
        speaker_job = open_job_file(belief_mstr_dir, belief_label, speaker_id)
        if speaker_job is None:
            speaker_job = create_empty_owner_from_owner(listener_job, speaker_id)
        listen_to_speaker_agenda(listener_job, speaker_job)


def listen_to_agendas_duty_vision(listener_vision: OwnerUnit, healer_hubunit: HubUnit):
    ...
    for x_acctunit in get_ordered_debtors_roll(listener_vision):
        if x_acctunit.acct_name == listener_id:
            listener_duty = healer_hubunit.get_duty_owner(listener_id)
            listen_to_speaker_agenda(listener_vision, listener_duty)
        else:
            speaker_id = x_acctunit.acct_name
            healer_name = healer_hubunit.owner_name
            speaker_vision = healer_hubunit.rj_speaker_owner(healer_name, speaker_id)
            if speaker_vision is None:
                speaker_vision = create_empty_owner_from_owner(
                    listener_vision, speaker_id
                )
            listen_to_speaker_agenda(listener_vision, speaker_vision)


def listen_to_facts_duty_vision(new_vision: OwnerUnit, healer_hubunit: HubUnit):
    duty = healer_hubunit.get_duty_owner(new_vision.owner_name)
    ...
    for x_acctunit in get_ordered_debtors_roll(new_vision):
        if x_acctunit.acct_name != new_vision.owner_name:
            speaker_vision = healer_hubunit.get_vision_owner(x_acctunit.acct_name)
            if speaker_vision is not None:
                listen_to_speaker_fact(new_vision, speaker_vision)


def listen_to_facts_gut_job(belief_mstr_dir: str, new_job: OwnerUnit):
    belief_label = new_job.belief_label
    old_job = open_job_file(belief_mstr_dir, belief_label, new_job.owner_name)
...
def listen_to_debtors_roll_jobs_into_job(
    ...
    old_job = open_job_file(belief_mstr_dir, belief_label, owner_name)
...
def listen_to_debtors_roll_duty_vision(
    ...
    duty = healer_hubunit.get_duty_owner(listener_id)
...
def fstate_keep_vision_and_listen(
    listener_owner_name: OwnerName, healer_hubunit: HubUnit, new_job: OwnerUnit
):
    listener_id = listener_owner_name
    if healer_hubunit.vision_file_exists(listener_id):
        keep_vision = healer_hubunit.get_vision_owner(listener_id)
    else:
        keep_vision = create_empty_owner_from_owner(new_job, new_job.owner_name)
...
def create_vision_file_from_duty_file(healer_hubunit: HubUnit, owner_name: OwnerName):
    ...
    healer_hubunit.save_vision_owner(x_vision)

from src.a01_way_logic.way import get_default_fisc_tag
...
def test_TimeConversion_Exists():
    ...
    # THEN
    assert not x_timeconversion.fisc_tag
    assert not x_timeconversion.addin


def test_timeconversion_shop_ReturnObj_WithParameters():
    # ESTABLISH
    accord_fisc_tag = "accord34"
    accord_addin = 91

    # WHEN
    x_timeconversion = timeconversion_shop(accord_fisc_tag, accord_addin)

    # THEN
    assert x_timeconversion.fisc_tag == accord_fisc_tag
...
def test_timeconversion_shop_ReturnObj_EmtpyParameters():
    ...
    # THEN
    assert x_timeconversion.fisc_tag == get_default_fisc_tag()

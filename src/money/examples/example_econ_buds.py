from src.bud.bud import BudUnit, budunit_shop, ideaunit_shop, PecunID
from src.money.examples.econ_env import temp_pecun_id


def get_1node_bud() -> BudUnit:
    x_bud = budunit_shop("A")
    x_bud.set_pecun_id(temp_pecun_id())
    x_bud.settle_bud()
    return x_bud
...
def get_2node_bud(pecun_id: PecunID = None) -> BudUnit:
    if pecun_id is None:
        pecun_id = temp_pecun_id()
    a_str = "A"
    b_str = "B"
    x_bud = budunit_shop(_owner_id=a_str)
    x_bud.set_pecun_id(pecun_id)
    idea_b = ideaunit_shop(b_str)
    x_bud.set_idea(idea_b, parent_road=temp_pecun_id())
    x_bud.settle_bud()
    return x_bud

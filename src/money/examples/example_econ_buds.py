from src.bud.bud import BudUnit, budunit_shop, ideaunit_shop, TribeID
from src.money.examples.econ_env import temp_tribe_id


def get_1node_bud() -> BudUnit:
    x_bud = budunit_shop("A")
    x_bud.set_tribe_id(temp_tribe_id())
    x_bud.settle_bud()
    return x_bud
...
def get_2node_bud(tribe_id: TribeID = None) -> BudUnit:
    if tribe_id is None:
        tribe_id = temp_tribe_id()
    a_str = "A"
    b_str = "B"
    x_bud = budunit_shop(_owner_id=a_str)
    x_bud.set_tribe_id(tribe_id)
    idea_b = ideaunit_shop(b_str)
    x_bud.set_idea(idea_b, parent_road=temp_tribe_id())
    x_bud.settle_bud()
    return x_bud

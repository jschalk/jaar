...
def test_GovUnit_generate_final_bud_Sets_final_BudFile(env_dir_setup_cleanup):
    # ESTABLISH
    accord45_str = "accord45"
    accord_gov = govunit_shop(accord45_str, get_test_govs_dir(), True)
    sue_str = "Sue"
    sue_hubunit = hubunit_shop(None, accord45_str, sue_str, None)

    x_sue_owner_dir = create_path(accord_gov._owners_dir, sue_str)
    ...
    # WHEN
    sue_final = accord_gov.generate_final_bud(sue_str)

    # THEN
    example_bud = budunit_shop(sue_str, accord45_str)
    assert sue_final.gov_idea == example_bud.gov_idea
    assert sue_final.owner_name == example_bud.owner_name


def test_GovUnit_generate_final_bud_ReturnsRegeneratedObj(env_dir_setup_cleanup):
    # ESTABLISH
    accord_gov = govunit_shop("accord45", get_test_govs_dir(), True)
    sue_str = "Sue"
    accord_gov.init_owner_keeps(sue_str)
    sue_hubunit = hubunit_shop(accord_gov.govs_dir, accord_gov.gov_idea, sue_str, None)
    ...
    # WHEN
    after_sue_bud = accord_gov.generate_final_bud(sue_str)

    # THEN method should wipe over final bud
    assert after_sue_bud.acct_exists(bob_str) is False


def test_GovUnit_generate_final_bud_SetsCorrectFileWithout_healerlink(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    accord_gov = govunit_shop("accord45", get_test_govs_dir(), True)
    bob_str = "Bob"
    accord_gov.init_owner_keeps(bob_str)
    bob_hubunit = hubunit_shop(accord_gov.govs_dir, accord_gov.gov_idea, bob_str, None)
    before_bob_final_bud = accord_gov.generate_final_bud(bob_str)
    sue_str = "Sue"
    ...
    # WHEN
    after_bob_final_bud = accord_gov.generate_final_bud(bob_str)

    # THEN
    assert after_bob_final_bud.acct_exists(sue_str)


def test_GovUnit_generate_final_bud_SetsFileWith_healerlink(env_dir_setup_cleanup):
    # ESTABLISH
    accord_gov = govunit_shop("accord45", get_test_govs_dir(), True)
...
def test_GovUnit_generate_all_final_buds_SetsCorrectFiles(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    accord_gov = govunit_shop("accord45", get_test_govs_dir(), True)
    ...
    bob_hubunit = hubunit_shop(govs_dir, accord_gov.gov_idea, bob_str, None)
    accord_gov.init_owner_keeps(sue_str)
    sue_hubunit = hubunit_shop(govs_dir, accord_gov.gov_idea, sue_str, None)
    bob_voice_bud = accord_gov.generate_final_bud(bob_str)
    sue_voice_bud = accord_gov.generate_final_bud(sue_str)
    ...
    before_bob_final_bud = accord_gov.get_final_file_bud(bob_str)
    before_sue_final_bud = accord_gov.get_final_file_bud(sue_str)
    ...
    # THEN
    after_bob_final_bud = accord_gov.get_final_file_bud(bob_str)
    after_sue_final_bud = accord_gov.get_final_file_bud(sue_str)
    assert after_bob_final_bud.acct_exists(bob_str)

...
def test_get_voices_view_dict_ReturnsObj_Scenario1_voices():
    ...
    voice_cred_shares_readable_key = add_readable(wx.voice_cred_shares)
    voice_debt_shares_readable_key = add_readable(wx.voice_debt_shares)
    ...
    irrational_voice_debt_shares_readable_key = add_readable(
        wx.irrational_voice_debt_shares
    )
    inallocable_voice_debt_shares_readable_key = add_readable(
    ...
    assert set(yao_voice_dict.keys()) == {
        ...
        voice_cred_shares_readable_key,
        voice_debt_shares_readable_key,
        memberships_readable_key,
        credor_pool_readable_key,
        debtor_pool_readable_key,
        irrational_voice_debt_shares_readable_key,
        inallocable_voice_debt_shares_readable_key,
    ...
    assert ypu.voice_cred_shares == yp_dict.get(wx.voice_cred_shares)
    assert ypu.voice_debt_shares == yp_dict.get(wx.voice_debt_shares)
    assert ypu.credor_pool == yp_dict.get(wx.credor_pool)
    assert ypu.debtor_pool == yp_dict.get(wx.debtor_pool)
    assert ypu.irrational_voice_debt_shares == yp_dict.get(
        wx.irrational_voice_debt_shares
    )
    assert ypu.inallocable_voice_debt_shares == yp_dict.get(
        wx.inallocable_voice_debt_shares
    )
    ...
    expected_voice_cred_shares_readable = f"voice_cred_shares: {ypu.voice_cred_shares}"
    expected_voice_debt_shares_readable = f"voice_debt_shares: {ypu.voice_debt_shares}"
    ...
    expected_irrational_voice_debt_shares_readable = (
        f"irrational_voice_debt_shares: {ypu.irrational_voice_debt_shares}"
    )
    expected_inallocable_voice_debt_shares_readable = (
        f"inallocable_voice_debt_shares: {ypu.inallocable_voice_debt_shares}"
    )
    ...
    assert (
        yp_dict.get(voice_cred_shares_readable_key)
        == expected_voice_cred_shares_readable
    )
    assert (
        yp_dict.get(voice_debt_shares_readable_key)
        == expected_voice_debt_shares_readable
    )
    ...
    assert (
        yp_dict.get(irrational_voice_debt_shares_readable_key)
        == expected_irrational_voice_debt_shares_readable
    )
    assert (
        yp_dict.get(inallocable_voice_debt_shares_readable_key)
        == expected_inallocable_voice_debt_shares_readable
    )
...
def test_get_voices_view_dict_ReturnsObj_Scenario2_memberships():
    ...
    expected_group_cred_shares_readable = add_small_dot(
        expected_group_cred_shares_readable
    )
    expected_group_debt_shares_readable = add_small_dot(
        expected_group_debt_shares_readable
    )

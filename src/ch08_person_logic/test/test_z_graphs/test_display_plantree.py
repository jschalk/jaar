...
from ch08_person_logic.person_graphic import (
    display_plantree,
    fund_graph0,
...
def test_PersonUnit_fund_graph0_VisualizesCirculation(graphics_bool):
    ...
    sue_person.add_plan(casa_rope, poynt=30)
    sue_person.add_plan(cat_rope, poynt=30)
    sue_person.add_plan(hun_n_rope, poynt=30)
    sue_person.add_plan(hun_y_rope, poynt=30)
    sue_person.add_plan(clean_rope, poynt=30)
    sue_person.add_plan(sweep_rope, poynt=30, pledge=True)
    sue_person.add_plan(dish_rope, poynt=30, pledge=True)
    dinner_str = "cat have dinner"
    dinner_rope = sue_person.make_l1_rope(dinner_str)
    sue_person.add_plan(dinner_rope, poynt=30, pledge=True)

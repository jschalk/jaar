...
def create_belief_owners_cell_trees(belief_mstr_dir, belief_label):
    ...
    for owner_name in get_level1_dirs(owners_dir):
        ...
        for bud_time in get_level1_dirs(buds_dir):
            create_cell_tree(belief_mstr_dir, belief_label, owner_name, bud_time)


def create_cell_tree(belief_mstr_dir, belief_label, bud_owner_name, bud_time):
    root_cell_json_path = create_cell_json_path(
        belief_mstr_dir, belief_label, bud_owner_name, bud_time
    )
...
def _exists_create_cell_tree(belief_mstr_dir, belief_label, bud_owner_name, bud_time):
    root_cell_dir = create_cell_dir_path(
        belief_mstr_dir, belief_label, bud_owner_name, bud_time, []
    )
    cells_to_evaluate = [cellunit_get_from_dir(root_cell_dir)]
    owner_events_sets = collect_owner_event_dir_sets(belief_mstr_dir, belief_label)
    ...
    for owner_name in get_level1_dirs(owners_dir):
        ...
        for bud_time in get_level1_dirs(buds_dir):
            ...
            set_cell_tree_decrees(
                belief_mstr_dir, belief_label, owner_name, bud_time, bud_time_dir
            )
...
def set_cell_tree_decrees(
    ...
    root_cell_dir = create_cell_dir_path(
        mstr_dir, belief_label, owner_name, bud_time, []
    )
    ...
    while to_evaluate_decreeunits != []:
        ...
        if x_cell := cellunit_get_from_dir(
            x_decree.cell_dir
        ) or generate_cell_from_decree(x_decree, mstr_dir, belief_label, owner_name):
...
def _add_child_decrees(
    ...
    for child_owner_name, child_mandate in x_cell._acct_mandate_ledger.items():
        ...
        child_dir = create_cell_dir_path(
            mstr_dir, belief_label, owner_name, bud_time, child_cell_ancestors
        )
...
def generate_cell_from_decree(
    x_decree: DecreeUnit, mstr_dir: str, belief_label: str, owner_name: OwnerName
) -> CellUnit:
    cell_owner_name = x_decree.cell_owner_name
    owners_downhill_events_ints = get_owners_downhill_event_ints(
        owner_events_sets=collect_owner_event_dir_sets(mstr_dir, belief_label),

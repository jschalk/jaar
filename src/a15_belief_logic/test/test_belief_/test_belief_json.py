...
from src.a15_belief_logic.belief import (
    beliefunit_shop,
    get_from_default_path as beliefunit_get_from_default_path,
...
def test_BeliefUnit_get_dict_ReturnsObjWith_paybook():
    ...
    accord_belief = beliefunit_shop(a45_str, belief_mstr_dir, offi_times=a45_offi_times)
    ...
    accord_belief.set_offi_time_max(accord_offi_time_max_int)
    accord_belief.add_budunit(bob_str, bob_x0_tran_time, bob_x0_quota)
    accord_belief.add_budunit(sue_str, sue_x4_tran_time, sue_x4_quota)
    accord_belief.add_budunit(sue_str, sue_x7_tran_time, sue_x7_quota)
    accord_belief.add_paypurchase(
        owner_name=bob_str,
        acct_name=sue_str,
    ...
    # WHEN
    x_dict = accord_belief.get_dict()

    # THEN
    offi_times_str = f"{offi_time_str()}s"
    print(f"{ accord_belief._get_brokerunits_dict()=}")
    print(f"{ accord_belief.paybook.get_dict()=}")
    ...
    assert x_dict.get(brokerunits_str()) == accord_belief._get_brokerunits_dict()
    assert x_dict.get(paybook_str()) == accord_belief.paybook.get_dict()
...
def test_BeliefUnit_get_dict_ReturnsObjWithOut_paybook():
    ...
    # WHEN
    x_dict = accord_belief.get_dict(include_paybook=False)
...
def test_get_from_dict_ReturnsBeliefUnit_Scenario0_WithParameters():
    ...
    belief_mstr_dir = create_path(get_module_temp_dir(), "temp1")
    a45_offi_times = {17, 37}
    accord_belief = beliefunit_shop(
        accord45_str, belief_mstr_dir, offi_times=a45_offi_times
    )
    sue_timeline_label = "sue casa"
    accord_belief.timeline.timeline_label = sue_timeline_label
    sue_knot = "/"
    sue_fund_iota = 0.3
    sue_respect_bit = 2
    sue_penny = 3
    ...
    accord_belief.add_budunit(sue_str, sue_x4_bud_time, sue_x4_quota)
    accord_belief.add_budunit(sue_str, sue_x7_bud_time, sue_x7_quota)
    accord_belief.knot = sue_knot
    accord_belief.fund_iota = sue_fund_iota
    accord_belief.respect_bit = sue_respect_bit
    accord_belief.penny = sue_penny
    accord_belief.add_paypurchase(
        owner_name=bob_str,
        acct_name=sue_str,
        tran_time=pay_tran_time,
    ...
    # THEN
    assert x_belief.belief_label == accord45_str
    ...
    assert x_belief.brokerunits == accord_belief.brokerunits
    assert x_belief.paybook == accord_belief.paybook
    assert x_belief.belief_mstr_dir == accord_belief.belief_mstr_dir
    assert x_belief != accord_belief
...
def test_get_from_dict_ReturnsBeliefUnit_Scenario1_WithOutParameters():
    ...
    # THEN
    assert generated_belief.belief_label == accord45_str
    print(f"{generated_belief.timeline=}")
    print(f"   {accord_belief.timeline=}")
    assert generated_belief.timeline == accord_belief.timeline
    ...
    assert generated_belief.brokerunits == accord_belief.brokerunits
    assert generated_belief.paybook == accord_belief.paybook
    assert generated_belief.belief_mstr_dir == accord_belief.belief_mstr_dir
    assert generated_belief == accord_belief
...
def test_get_from_json_ReturnsBeliefUnit():
    ...
    accord_belief = beliefunit_shop(accord45_str, temp_belief_mstr_dir)
    sue_timeline_label = "sue casa"
    accord_belief.timeline.timeline_label = sue_timeline_label
    ...
    accord_belief.respect_bit = sue_respect_bit
    accord_belief.penny = sue_penny
    accord_json = accord_belief.get_json()

    # WHEN
    x_belief = beliefunit_get_from_json(accord_json)
...
def test_get_from_file_ReturnsBeliefUnitWith_belief_mstr_dir(env_dir_setup_cleanup):
    # ESTABLISH
    accord45_str = "accord45"
    accord45_belief = beliefunit_shop(accord45_str, get_module_temp_dir())
    sue_timeline_label = "sue casa"
    accord45_belief.timeline.timeline_label = sue_timeline_label
    sue_respect_bit = 2
    accord45_belief.respect_bit = sue_respect_bit
    x_belief_mstr_dir = create_path(get_module_temp_dir(), "fizz_buzz")
    accord45_json_path = create_belief_json_path(x_belief_mstr_dir, accord45_str)
    save_file(accord45_json_path, None, accord45_belief.get_json())
    assert accord45_belief.belief_mstr_dir != x_belief_mstr_dir

    # WHEN
    generated_a45_belief = beliefunit_get_from_default_path(
        x_belief_mstr_dir, accord45_str
    )

    # THEN
    assert generated_a45_belief.belief_mstr_dir == x_belief_mstr_dir
    assert generated_a45_belief.belief_label == accord45_str
    ...
    expected_a45_belief_dir = create_path(x_beliefs_dir, accord45_str)

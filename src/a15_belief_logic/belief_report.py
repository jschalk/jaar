...
def get_belief_guts_partners_dataframe(x_belief: BeliefUnit) -> DataFrame:
    ...
    for believer_name in belief_believer_names:
        ...
        df = get_believer_partnerunits_dataframe(gut_believer)
        df.insert(0, "believer_name", gut_believer.believer_name)
        gut_dfs.append(df)
    return pandas_concat(gut_dfs, ignore_index=True)


def get_belief_guts_partners_plotly_fig(x_belief: BeliefUnit) -> plotly_Figure:
    column_header_list = [
        "believer_name",
        "partner_name",
        "partner_cred_points",
    ...
    df = get_belief_guts_partners_dataframe(x_belief)
    header_dict = dict(
    ...
    fig_label = f"belief '{x_belief.belief_label}', gut partners metrics"
...
def get_belief_jobs_partners_dataframe(x_belief: BeliefUnit) -> DataFrame:
    # get list of all believer paths
    belief_believer_names = x_belief._get_believer_folder_names()
    # for all believers get gut
    job_dfs = []
    for believer_name in belief_believer_names:
        ...
        job.settle_believer()
        job_df = get_believer_partnerunits_dataframe(job)
        job_df.insert(0, "believer_name", job.believer_name)
        job_dfs.append(job_df)
    return pandas_concat(job_dfs, ignore_index=True)


def get_belief_jobs_partners_plotly_fig(x_belief: BeliefUnit) -> plotly_Figure:
    column_header_list = [
        "believer_name",
        "partner_name",
        "partner_cred_points",
    ...
    df = get_belief_jobs_partners_dataframe(x_belief)
    header_dict = dict(
        values=column_header_list, fill_color="paleturquoise", align="left"
    )
    x_table = plotly_Table(
    ...
    fig = plotly_Figure(data=[x_table])
    fig_label = f"belief '{x_belief.belief_label}', job partners metrics"
    fig.update_xaxes(showgrid=False)
    fig.update_yaxes(showgrid=False, zeroline=True, showticklabels=False)
    fig.update_layout(plot_bgcolor="white", title=fig_label, title_font_size=20)
    return fig

...
def test_charlink_meld_ReturnsCorrectObj_BaseScenario():
    ...
    x2_beliefbox = beliefbox_shop(bikers_belief_id)

    # WHEN
    bikers_beliefbox.meld(exterior_belief=x2_beliefbox)
    print(f"{bikers_beliefbox.belief_id=} {x2_beliefbox.belief_id=}")

    # THEN
    assert len(bikers_beliefbox._chars) == 2
...
def test_charlink_meld_ReturnsCorrectObj_GainScenario():
    ...
    x2_beliefbox = beliefbox_shop(bikers_belief_id)
    x2_beliefbox.set_charlink(yao_charlink)
    x2_beliefbox.set_charlink(sue_charlink)
    ...
    # THEN
    assert len(bikers_beliefbox._chars) == 2
    assert bikers_beliefbox._chars.get(yao_text) != None

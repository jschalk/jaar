...
def test_WorldUnit_edit_idea_label_FailsWhenIdeaDoesNotExist():
    ...
    yao_world.add_idea(ideaunit_shop(swim_text), parent_road=casa_road)

    # WHEN / THEN
    no_idea_road = yao_world.make_l1_road("bees")
    with pytest_raises(Exception) as excinfo:
        yao_world.edit_idea_label(old_road=no_idea_road, new_label="pigeons")
...
def test_WorldUnit_edit_idea_label_RaisesErrorForLevel0IdeaWhen_real_id_isNone():
    ...
    casa_road = yao_world.make_l1_road(casa_text)
    swim_text = "swim"
    swim_road = yao_world.make_road(casa_road, swim_text)
    yao_world.add_l1_idea(ideaunit_shop(casa_text))
    yao_world.add_idea(ideaunit_shop(swim_text), parent_road=casa_road)
    assert yao_world._owner_id == yao_text
    assert yao_world._idearoot._label == yao_world._real_id
    casa_idea = yao_world.get_idea_obj(casa_road)
    assert casa_idea._parent_road == yao_world._real_id
    swim_idea = yao_world.get_idea_obj(swim_road)
    assert swim_idea._parent_road == casa_road

    # WHEN
    moon_text = "moon"
    yao_world.edit_idea_label(old_road=yao_world._real_id, new_label=moon_text)
    ...
    # assert (
    #     str(excinfo.value)
    #     == f"Cannot set idearoot to string othher than '{yao_world._real_id}'"
    # )

    assert yao_world._idearoot._label != moon_text
    assert yao_world._idearoot._label == yao_world._real_id
...
def test_WorldUnit_edit_idea_label_RaisesErrorForLevel0When_real_id_IsDifferent():
    ...
    yao_world.add_idea(ideaunit_shop(swim_text), parent_road=casa_road)
    sun_text = "sun"
    yao_world._real_id = sun_text
    yao_world._idearoot._world_real_id = sun_text
    assert yao_world._owner_id == yao_text
    assert yao_world._real_id == sun_text
    assert yao_world._idearoot._world_real_id == sun_text
    assert yao_world._idearoot._label == root_label()
    casa_idea = yao_world.get_idea_obj(casa_road)
    ...
    with pytest_raises(Exception) as excinfo:
        moon_text = "moon"
        yao_world.edit_idea_label(old_road=root_label(), new_label=moon_text)
    assert (
        str(excinfo.value)
...
def test_world_set_real_id_CorrectlySetsAttr():
    ...
    yao_world = worldunit_shop(_owner_id=yao_text)
    casa_text = "casa"
    old_casa_road = yao_world.make_l1_road(casa_text)
    swim_text = "swim"
    old_swim_road = yao_world.make_road(old_casa_road, swim_text)
    yao_world.add_l1_idea(ideaunit_shop(casa_text))
    yao_world.add_idea(ideaunit_shop(swim_text), parent_road=old_casa_road)
    assert yao_world._owner_id == yao_text
    assert yao_world._idearoot._label == yao_world._real_id
    casa_idea = yao_world.get_idea_obj(old_casa_road)
    assert casa_idea._parent_road == yao_world._real_id
    swim_idea = yao_world.get_idea_obj(old_swim_road)
    assert swim_idea._parent_road == old_casa_road
    assert yao_world._real_id == yao_world._real_id

    # WHEN
    real_id_text = "Sun"
    yao_world.set_real_id(real_id=real_id_text)

    # THEN
    new_casa_road = yao_world.make_l1_road(casa_text)
    swim_text = "swim"
    new_swim_road = yao_world.make_road(new_casa_road, swim_text)
    assert yao_world._real_id == real_id_text
    assert yao_world._idearoot._label == real_id_text
    casa_idea = yao_world.get_idea_obj(new_casa_road)
    assert casa_idea._parent_road == real_id_text
    swim_idea = yao_world.get_idea_obj(new_swim_road)
...
def test_WorldUnit_find_replace_road_CorrectlyModifies_kids_Scenario1():
    ...
    old_bloomers_road = yao_world.make_road(old_casa_road, bloomers_text)
    roses_text = "roses"
    old_roses_road = yao_world.make_road(old_bloomers_road, roses_text)
    red_text = "red"
    old_red_road = yao_world.make_road(old_roses_road, red_text)

    yao_world.add_l1_idea(ideaunit_shop(old_casa_text))
    yao_world.add_idea(ideaunit_shop(bloomers_text), parent_road=old_casa_road)
    yao_world.add_idea(ideaunit_shop(roses_text), parent_road=old_bloomers_road)
    yao_world.add_idea(ideaunit_shop(red_text), parent_road=old_roses_road)
    r_idea_roses = yao_world.get_idea_obj(old_roses_road)
    r_idea_bloomers = yao_world.get_idea_obj(old_bloomers_road)
    ...
    # THEN
    assert yao_world._idearoot._kids.get(new_casa_text) != None
    assert yao_world._idearoot._kids.get(old_casa_text) is None

    assert r_idea_bloomers._parent_road == new_casa_road
    assert r_idea_bloomers._kids.get(roses_text) != None

    r_idea_roses = r_idea_bloomers._kids.get(roses_text)
    new_bloomers_road = yao_world.make_road(new_casa_road, bloomers_text)
    assert r_idea_roses._parent_road == new_bloomers_road
    assert r_idea_roses._kids.get(red_text) != None
    r_idea_red = r_idea_roses._kids.get(red_text)
    new_roses_road = yao_world.make_road(new_bloomers_road, roses_text)
...
def test_world_edit_idea_label_Modifies_factunits():
    ...
    casa_road = yao_world.make_l1_road(casa_text)
    bloomers_text = "bloomers"
    bloomers_road = yao_world.make_road(casa_road, bloomers_text)
    roses_text = "roses"
    roses_road = yao_world.make_road(bloomers_road, roses_text)
    old_water_text = "water"
    old_water_road = yao_world.make_l1_road(old_water_text)
    rain_text = "rain"
    old_rain_road = yao_world.make_road(old_water_road, rain_text)

    yao_world.add_l1_idea(ideaunit_shop(casa_text))
    yao_world.add_idea(ideaunit_shop(roses_text), parent_road=bloomers_road)
    yao_world.add_idea(ideaunit_shop(rain_text), parent_road=old_water_road)
    yao_world.set_fact(base=old_water_road, pick=old_rain_road)

    idea_x = yao_world.get_idea_obj(roses_road)
    assert yao_world._idearoot._factunits[old_water_road] != None
    old_water_rain_factunit = yao_world._idearoot._factunits[old_water_road]
    assert old_water_rain_factunit.base == old_water_road
    assert old_water_rain_factunit.pick == old_rain_road

    # WHEN
    new_water_text = "h2o"
    new_water_road = yao_world.make_l1_road(new_water_text)
    yao_world.add_l1_idea(ideaunit_shop(new_water_text))
    assert yao_world._idearoot._factunits.get(new_water_road) is None
    yao_world.edit_idea_label(old_road=old_water_road, new_label=new_water_text)

    # THEN
    assert yao_world._idearoot._factunits.get(old_water_road) is None
    assert yao_world._idearoot._factunits.get(new_water_road) != None
    new_water_rain_factunit = yao_world._idearoot._factunits[new_water_road]
    assert new_water_rain_factunit.base == new_water_road
    new_rain_road = yao_world.make_road(new_water_road, rain_text)
    assert new_water_rain_factunit.pick == new_rain_road

    assert yao_world._idearoot._factunits.get(new_water_road)
    factunit_obj = yao_world._idearoot._factunits.get(new_water_road)
    # for factunit_key, factunit_obj in yao_world._idearoot._factunits.items():
...
def test_world_edit_idea_label_Modifies_idearoot_range_source_road():
    ...
    old_casa_road = yao_world.make_l1_road(old_casa_text)
    yao_world.add_l1_idea(ideaunit_shop(old_casa_text))
    yao_world.edit_idea_attr(yao_world._real_id, range_source_road=old_casa_road)
    ...
    # THEN
    new_casa_road = yao_world.make_l1_road(new_casa_text)
    assert yao_world._idearoot._range_source_road == new_casa_road
...
def test_world_set_road_delimiter_RaisesErrorIfNew_delimiter_IsAnIdea_label():
    ...
    zia_world.add_idea(ideaunit_shop(casa_text), parent_road=casa_road)

    # WHEN / THEN
    casa_road = zia_world.make_road(casa_road, casa_text)
    print(f"{casa_road=}")
    with pytest_raises(Exception) as excinfo:
        zia_world.set_road_delimiter(slash_text)
    assert (
        str(excinfo.value)
...
def test_world_set_road_delimiter_CorrectlyModifies_parent_road():
    ...
    casa_text = "casa"
    zia_world.add_l1_idea(ideaunit_shop(casa_text))
    comma_casa_road = zia_world.make_l1_road(casa_text)
    cook_text = "cook"
    zia_world.add_idea(ideaunit_shop(cook_text), parent_road=comma_casa_road)
    comma_cook_road = zia_world.make_road(comma_casa_road, cook_text)
    cook_idea = zia_world.get_idea_obj(comma_cook_road)
    comma_text = ","
    assert zia_world._road_delimiter == comma_text
    comma_cook_road = zia_world.make_road(comma_casa_road, cook_text)
    # print(f"{zia_world._real_id=} {zia_world._idearoot._label=} {casa_road=}")
    # print(f"{cook_idea._parent_road=} {cook_idea._label=}")
    # comma_casa_idea = zia_world.get_idea_obj(comma_casa_road)
    ...
    # THEN
    assert cook_idea.get_road() != comma_cook_road
    zia_real_id = zia_world._real_id
...
def test_world_set_road_delimiter_CorrectlyModifiesReasonUnit():
    ...
    zia_world.add_l1_idea(ideaunit_shop(casa_text))
    time_text = "time"
    comma_time_road = zia_world.make_l1_road(time_text)
    _8am_text = "8am"
    comma_8am_road = zia_world.make_road(comma_time_road, _8am_text)

    comma_time_reasonunit = reasonunit_shop(base=comma_time_road)
    comma_time_reasonunit.set_premise(comma_8am_road)

    comma_casa_road = zia_world.make_l1_road(casa_text)
    zia_world.edit_idea_attr(road=comma_casa_road, reason=comma_time_reasonunit)
    casa_idea = zia_world.get_idea_obj(comma_casa_road)
    ...
    # THEN
    slash_time_road = zia_world.make_l1_road(time_text)
    slash_8am_road = zia_world.make_road(slash_time_road, _8am_text)
    slash_casa_road = zia_world.make_l1_road(casa_text)
    casa_idea = zia_world.get_idea_obj(slash_casa_road)
    slash_time_road = zia_world.make_l1_road(time_text)
    slash_8am_road = zia_world.make_road(slash_time_road, _8am_text)
...
def test_world_set_road_delimiter_CorrectlyModifiesFactUnit():
    ...
    comma_casa_road = zia_world.make_l1_road(casa_text)
    zia_world.edit_idea_attr(comma_casa_road, factunit=comma_time_factunit)
    casa_idea = zia_world.get_idea_obj(comma_casa_road)
...
def test_world_set_road_delimiter_CorrectlyModifies_numeric_roadAND_range_source_road():
    ...
    zia_world.add_l1_idea(ideaunit_shop(taste_text, _begin=0, _close=6))
    comma_taste_road = zia_world.make_l1_road(taste_text)
    zia_world.edit_idea_attr(comma_cook_road, numeric_road=comma_taste_road)

    # range_source
    heat_text = "heat numbers"
    zia_world.add_l1_idea(ideaunit_shop(heat_text, _begin=0, _close=6))
    comma_heat_road = zia_world.make_l1_road(heat_text)
    zia_world.edit_idea_attr(comma_cook_road, range_source_road=comma_heat_road)
    ...
    # THEN
    slash_taste_road = zia_world.make_l1_road(taste_text)
    assert cook_idea._numeric_road != comma_taste_road
    assert cook_idea._numeric_road == slash_taste_road
    slash_heat_road = zia_world.make_l1_road(heat_text)
    assert cook_idea._range_source_road != comma_heat_road

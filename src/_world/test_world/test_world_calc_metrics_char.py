...
def test_WorldUnit_calc_world_metrics_CorrectlySetsCharLinkWorldCredAndDebt():
    ...
    yao_world._idearoot.set_awardlink(awardlink=bl_sue)
    yao_world._idearoot.set_awardlink(awardlink=bl_bob)
    yao_world._idearoot.set_awardlink(awardlink=bl_zia)
...
def test_WorldUnit_calc_world_metrics_CorrectlySetsCharUnitWorldImportance():
    ...
    bl_bob = awardlink_shop(belief_id=bob_text, credor_weight=10, debtor_weight=5)
    bl_zia = awardlink_shop(belief_id=zia_text, credor_weight=10, debtor_weight=5)
    yao_world._idearoot._kids.get(swim_text).set_awardlink(awardlink=bl_sue)
    yao_world._idearoot._kids.get(swim_text).set_awardlink(awardlink=bl_bob)
    yao_world._idearoot._kids.get(swim_text).set_awardlink(awardlink=bl_zia)
...
def test_WorldUnit_calc_world_metrics_CorrectlySetsPartBeliefedLWCharUnitWorldImportance():
    ...
    #     # print(
    #     #     f"{belief.belief_id=} {beliefbox._world_credor=} {belief._world_credor=}"
    #     # )
    #     print(f"{beliefbox.belief_id=} {beliefbox._world_cred=} {beliefbox._world_debt=} ")
...
def clear_all_charunits_beliefboxs_world_agenda_cred_debt(x_world: WorldUnit):
    ...
    for beliefbox_x in x_world._beliefs.values():
        ...
        #     print(f"{beliefbox_x.belief_id=} {charlink_x.credor_weight=}  {charlink_x._world_cred:.6f} {charlink_x.debtor_weight=} {charlink_x._world_debt:.6f} {charlink_x.} ")
...
def test_WorldUnit_agenda_ratio_cred_debt_IsCorrectlySetWhenWorldIsEmpty():
    ...
    bob_text = "Bob"
    zia_text = "Zia"
    sue_char = charunit_shop(sue_text, credor_weight=0.5, debtor_weight=2)
    bob_char = charunit_shop(bob_text, credor_weight=1.5, debtor_weight=3)
    zia_char = charunit_shop(zia_text, credor_weight=8, debtor_weight=5)
    noa_world.set_charunit(charunit=sue_char)
    noa_world.set_charunit(charunit=bob_char)
    noa_world.set_charunit(charunit=zia_char)
    noa_world_sue_char = noa_world._chars.get(sue_text)
    noa_world_bob_char = noa_world._chars.get(bob_text)
    noa_world_zia_char = noa_world._chars.get(zia_text)

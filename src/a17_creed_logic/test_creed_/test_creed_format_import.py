...
from src.a17_creed_logic.creed import get_creedref_obj, save_creed_csv, load_creed_csv
...
def test_open_csv_ReturnsObjWhenFileExists(env_dir_setup_cleanup):
    ...
    # WHEN
    acct_dataframe = open_csv(creed_examples_dir(), name_filename)
...
def test_load_creed_csv_Arg_creed_format_00021_bud_acctunit_v0_0_0_csvTo_job(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"
    ...
    csv_example_path = create_path(creed_examples_dir(), name_filename)
    print(f"{csv_example_path}")
    save_creed_csv(j1_creedname, sue_budunit, creed_examples_dir(), name_filename)
    sue_hubunit = hubunit_shop(
        creed_examples_dir(), accord_fisc_label, owner_name=sue_str
    )
    # Popen FiscUnit and confirm gut BudUnit does not exist
    assert not gut_file_exists(creed_examples_dir(), accord_fisc_label, sue_str)
    ...
    assert gut_file_exists(creed_examples_dir(), accord_fisc_label, sue_str)
    # assert gut Budunit acctunit now exists
    sue_gut = open_gut_file(creed_examples_dir(), accord_fisc_label, sue_str)
...
def test_load_creed_csv_csvTo_job(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"

...
def get_relevant_columns_dataframe(
    ...
    if relevant_columns is None:
        relevant_columns = get_creed_elements_sort_order()
...
def get_brick_raw_grouping_with_all_values_equal_df(
    x_df: DataFrame, groupby_list: list, creed_number: str
) -> DataFrame:
...
def _get_pidgen_creed_format_filenames() -> set[str]:
    ...
    creed_numbers.update(set(get_creed_dimen_ref().get("pidgin_tag")))
...
def append_df_to_excel(file_path: str, sheet_name: str, dataframe: DataFrame):
    try:
        ...
        for i, row in enumerate(rows, start_row):
            for j, value in enumerate(row, 1):  # 1-contextd index for Excel
...
def create_creed_table_from_csv(
...
def insert_creed_csv(
    csv_filepath: str, conn_or_cursor: sqlite3_Connection, tablename: str
):
    if db_table_exists(conn_or_cursor, tablename) is False:
        create_creed_table_from_csv(csv_filepath, conn_or_cursor, tablename)

    # Future feature? filtering csv file so only relevant creed columns are loaded
...
def get_pragma_table_fetchall(table_columns):
    ...
    for x_count, x_col in enumerate(table_columns):
        col_type = creed_sqlite_types.get(x_col)
...
def create_creed_sorted_table(
    conn: sqlite3_Connection, tablename: str, columns_list: list[str]
):
    columns_list = get_default_sorted_list(columns_list)
    create_table_from_columns(conn, tablename, columns_list, get_creed_sqlite_types())


def get_creed_into_dimen_raw_query(
    ...
    return f"""INSERT INTO {dst_table} (creed_number, {common_columns_header})
SELECT '{creed_number}' as creed_number, {common_columns_header}
FROM {src_table}

...
from src.a17_creed_logic.creed_db_tool import (
    get_ordered_csv,
    get_creed_elements_sort_order as sorting_columns,
)
...
def test_get_pidgin_word_dt_columns_ReturnsObj():
    # ESTABLISH / WHEN /THEN
    assert get_pidgin_word_dt_columns()
    assert len(get_pidgin_word_dt_columns()) == 7
    static_list = [
        event_int_str(),
        face_name_str(),
        otx_bridge_str(),
    ...
    assert get_pidgin_word_dt_columns() == static_list
    assert set(get_pidgin_word_dt_columns()).issubset(set(sorting_columns()))
...
def test_create_pidgin_word_dt_ReturnsObj():
    # ESTABLISH
    casa_pidginunit = get_casa_maison_pidginunit_set_by_word()
    casa_mapunit = casa_pidginunit.get_wordmap()

    # WHEN
    casa_dataframe = create_pidgin_word_dt(casa_mapunit)

    # THEN
    # print(f"{get_pidgin_word_dt_columns()=}")
    # print(f"    {list(casa_dataframe.columns)=}")
    # print("")
    # print(f"{casa_dataframe=}")
    assert list(casa_dataframe.columns) == get_pidgin_word_dt_columns()
    assert len(casa_dataframe) == 3
    casa_csv = get_ordered_csv(casa_dataframe)
    ex_word_csv = get_ordered_csv(get_casa_maison_word_dt())
    print(f"       {casa_csv=}")
    print(f"{ex_word_csv=}")
    assert casa_csv == ex_word_csv
...
def test_save_all_csvs_from_pidginunit_SavesFiles(env_dir_setup_cleanup):
    ...
    assert os_path_exists(word_csv_path) is False
    ...
    assert os_path_exists(word_csv_path)
...
def test_load_wordmap_from_csv_SetsAttrWhenFileExists(env_dir_setup_cleanup):
    ...
    assert os_path_exists(word_csv_path)
    empty_pidginunit = pidginunit_shop("Sue")
    sue_wordmap = empty_pidginunit.get_mapunit(type_WordStr_str())
    sue_wordmap.face_name = "Sue"
    print(f"{empty_pidginunit=} {sue_wordmap=}")
    assert len(sue_wordmap.otx2inx) == 0

    # WHEN
    sue_wordmap = _load_wordmap_from_csv(map_dir, sue_wordmap)

    # THEN
    assert len(sue_wordmap.otx2inx) == 2
    ex_wordmap = sue_pidginunit.get_mapunit(type_WordStr_str())
    assert ex_wordmap == sue_wordmap


def test_load_wordmap_from_csv_DoesNotChangeWhenFileDoesNotExist(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    map_dir = get_example_face_dir()
...
def test_load_waymap_from_csv_SetsAttrWhenFileExists(env_dir_setup_cleanup):
    ...
    assert ex_waymap.wordmap != sue_waymap.wordmap

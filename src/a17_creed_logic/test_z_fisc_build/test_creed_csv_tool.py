...
from src.a17_creed_logic.creed import fisc_build_from_df
...
from src.a17_creed_logic.creed_db_tool import get_ordered_csv
from src.a17_creed_logic._utils.env_a17 import (
    creed_fiscs_dir,
    env_dir_setup_cleanup,
)
from src.a17_creed_logic._utils.creed_df_examples import (
    get_ex2_br00000_df,
    get_ex2_br00001_df,
    get_ex2_br00002_df,
    get_ex2_br00003_df,
...
def test_create_init_stance_creed_csv_strs_ReturnsObj_Scenario0_EmptyFiscUnit(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    csv_delimiter = ","
    ...
    # WHEN
    x_creeds = create_init_stance_creed_csv_strs()
    ...
    expected_stance_csv_strs = {
        ...
        "br00026": "fisc_tag,owner_name,idea_way,rcontext,pbranch,pnigh,open,pdivisor\n",
    ...
    assert x_creeds.get("br00000") == f"{face_event_str}{expected_br00000_csv}"
    assert x_creeds.get("br00001") == f"{face_event_str}{expected_br00001_csv}"
    assert x_creeds.get("br00002") == f"{face_event_str}{expected_br00002_csv}"
    assert x_creeds.get("br00003") == f"{face_event_str}{expected_br00003_csv}"
    assert x_creeds.get("br00004") == f"{face_event_str}{expected_br00004_csv}"
    assert x_creeds.get("br00005") == f"{face_event_str}{expected_br00005_csv}"
    # assert x_creeds.get("br00006") == f"{face_event_str}{expected_br00006_csv}"
    print(f"{expected_br00020_csv=}")
    print(x_creeds.get("br00020"))
    assert x_creeds.get("br00020") == f"{face_event_str}{expected_br00020_csv}"
    assert x_creeds.get("br00021") == f"{face_event_str}{expected_br00021_csv}"
    assert x_creeds.get("br00022") == f"{face_event_str}{expected_br00022_csv}"
    assert x_creeds.get("br00023") == f"{face_event_str}{expected_br00023_csv}"
    assert x_creeds.get("br00024") == f"{face_event_str}{expected_br00024_csv}"
    assert x_creeds.get("br00025") == f"{face_event_str}{expected_br00025_csv}"
    assert x_creeds.get("br00026") == f"{face_event_str}{expected_br00026_csv}"
    assert x_creeds.get("br00027") == f"{face_event_str}{expected_br00027_csv}"
    assert x_creeds.get("br00028") == f"{face_event_str}{expected_br00028_csv}"
    assert x_creeds.get("br00029") == f"{face_event_str}{expected_br00029_csv}"
    assert x_creeds.get("br00042") == f"{face_event_str}{expected_br00042_csv}"
    assert x_creeds.get("br00043") == f"{face_event_str}{expected_br00043_csv}"
    assert x_creeds.get("br00044") == f"{face_event_str}{expected_br00044_csv}"
    assert x_creeds.get("br00045") == f"{face_event_str}{expected_br00045_csv}"
    assert len(x_creeds) == 20
...
def test_add_fiscunit_to_stance_csv_strs_ReturnsObj_Scenario0_OneFiscUnit(
    ...
    x_respect_bit = 1
    x_penny = 1
    x_fiscs_dir = create_path(creed_fiscs_dir(), "fizz")
    ...
    csv_delimiter = ","
    x_csvs = create_init_stance_creed_csv_strs()
...
def test_add_fiscunits_to_stance_csv_strs_ReturnsObj_Scenario1_TwoFiscUnits(
    ...
    # WHEN
    add_fiscunits_to_stance_csv_strs(x_fiscunits, x_creeds, csv_delimiter)
    ...
    assert len(x_creeds) == 20
    generated_br00000_csv = x_creeds.get("br00000")
    generated_br00001_csv = x_creeds.get("br00001")
    generated_br00002_csv = x_creeds.get("br00002")
    generated_br00003_csv = x_creeds.get("br00003")
    generated_br00004_csv = x_creeds.get("br00004")
    generated_br00005_csv = x_creeds.get("br00005")
    # print(f" {expected_br00000_csv=}")
...
def test_add_bud_to_br00020_csv_ReturnsObj():
    ...
    csv_header = x_creeds.get("br00020")
...
def test_add_bud_to_br00021_csv_ReturnsObj():
    ...
    csv_header = x_creeds.get("br00021")
...
def test_add_bud_to_br00026_csv_ReturnsObj():
    ...
    # THEN
    mop_row = f",,{a23_str},{bob_str},{mop_way},{casa_way},{clean_way},{clean_premise_open},{clean_pnigh},{clean_pdivisor}\n"
...
def test_add_bud_to_br00028_csv_ReturnsObj():
    ...
    csv_header = x_creeds.get("br00028")
    print(f"{csv_header=}")
...
def test_add_bud_to_br00029_csv_ReturnsObj():
    ...
    csv_header = x_creeds.get("br00029")
    print(f"{csv_header=}")
...
def test_add_budunit_to_stance_csv_strs_ReturnsObj():
    ...
    br00021_header = x_creeds.get("br00021")
    br00022_header = x_creeds.get("br00022")
    br00023_header = x_creeds.get("br00023")
    br00024_header = x_creeds.get("br00024")
    br00025_header = x_creeds.get("br00025")
    br00026_header = x_creeds.get("br00026")
    br00027_header = x_creeds.get("br00027")
    br00028_header = x_creeds.get("br00028")
    br00029_header = x_creeds.get("br00029")

    # WHEN
    bob_bud.settle_bud()
    add_budunit_to_stance_csv_strs(bob_bud, x_creeds, csv_delimiter)

    # THEN
    assert x_creeds.get("br00020") != br00020_header
    assert x_creeds.get("br00021") != br00021_header
    assert x_creeds.get("br00022") != br00022_header
    assert x_creeds.get("br00023") != br00023_header
    # assert x_creeds.get("br00024") != br00024_header
    # assert x_creeds.get("br00025") != br00025_header
    assert x_creeds.get("br00026") != br00026_header
    assert x_creeds.get("br00027") != br00027_header
    assert x_creeds.get("br00028") != br00028_header
    assert x_creeds.get("br00029") != br00029_header
...
def test_add_pidginunit_to_stance_csv_strs_ReturnsObj():
    ...
    br00045_header = x_creeds.get("br00045")

    # WHEN
    add_pidginunit_to_stance_csv_strs(bob7_pidginunit, x_creeds, csv_delimiter)

    # THEN
    assert x_creeds.get("br00042") != br00042_header
    assert x_creeds.get("br00043") != br00043_header
    assert x_creeds.get("br00044") != br00044_header
    assert x_creeds.get("br00045") != br00045_header
...
def test_add_pack_to_br00026_csv_ReturnsObj():
    ...
    # THEN
    mop_row = f"{sue_str},{event7},{a23_str},{bob_str},{mop_way},{casa_way},{clean_way},{clean_premise_open},{clean_pnigh},{clean_pdivisor}\n"
    assert x_csv == f"{csv_header}{mop_row}"
...
def test_add_packunit_to_stance_csv_strs_ReturnsObj():
    ...
    # WHEN
    add_packunit_to_stance_csv_strs(sue7_pack, x_creeds, csv_delimiter)
...
# TODO create function that saves excel file with all x_creeds
# def test_add_packunit_to_stance_csv_strs_ReturnsObj():
#     # ESTABLISH
#     csv_delimiter = ","
#     x_creeds = create_init_stance_creed_csv_strs()

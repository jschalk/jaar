...
from src.a08_owner_atom_logic.atom_config import get_atom_config_dict
...
from src.a10_owner_calc.owner_calc_config import (
    config_owner_calc_file_path,
    get_all_owner_calc_args,
    get_owner_calc_args_sqlite_datatype_dict,
    get_owner_calc_args_type_dict,
    get_owner_calc_config_dict,
    get_owner_calc_config_filename,
    get_owner_calc_dimen_args,
    get_owner_calc_dimens,
)
from src.a10_owner_calc.test._util.a10_str import jmetrics_str, owner_groupunit_str


def test_get_owner_calc_config_dict_Exists():
    # ESTABLISH
    src_dir = create_path(os_getcwd(), "src")
    expected_dir = create_path(src_dir, "a10_owner_calc")

    # WHEN / THEN
    assert get_owner_calc_config_filename() == "owner_calc_config.json"
    expected_path = create_path(expected_dir, get_owner_calc_config_filename())
    assert config_owner_calc_file_path() == expected_path
    assert os_path_exists(config_owner_calc_file_path())


def test_get_owner_calc_config_dict_ReturnsObj_CheckLevel0Keys():
    # ESTABLISH / WHEN
    owner_calc_config = get_owner_calc_config_dict()
    owner_calc_config_keys = set(owner_calc_config.keys())
    ...
    assert atom_config_dimens.issubset(owner_calc_config_keys)
    assert owner_calc_config_keys.difference(atom_config_dimens) == {
        owner_groupunit_str()
    }


def test_get_owner_calc_config_dict_ReturnsObj_CheckLevel1Keys():
    # ESTABLISH / WHEN
    owner_calc_config = get_owner_calc_config_dict()

    # THEN
    # sourcery skip: no-loop-in-tests
    for level1_key, aspect_dict in owner_calc_config.items():
...
def test_get_owner_calc_config_dict_ReturnsObj_CheckLevel2_And_Level3_Keys():
    # ESTABLISH / WHEN
    owner_calc_config = get_owner_calc_config_dict()
    ...
    onrunit_aspect = owner_calc_config.get(ownerunit_str())
    onracct_aspect = owner_calc_config.get(owner_acctunit_str())
    onrmemb_aspect = owner_calc_config.get(owner_acct_membership_str())
    onrplan_aspect = owner_calc_config.get(owner_planunit_str())
    onrawar_aspect = owner_calc_config.get(owner_plan_awardlink_str())
    onrreas_aspect = owner_calc_config.get(owner_plan_reasonunit_str())
    onrprem_aspect = owner_calc_config.get(owner_plan_reason_premiseunit_str())
    onrlabo_aspect = owner_calc_config.get(owner_plan_laborlink_str())
    onrheal_aspect = owner_calc_config.get(owner_plan_healerlink_str())
    onrfact_aspect = owner_calc_config.get(owner_plan_factunit_str())
    onrgrou_aspect = owner_calc_config.get(owner_groupunit_str())
    ...
    expected_onrlabo_jmetrics_keys = {"_owner_name_labor"}
...
def test_get_owner_calc_config_dict_ReturnsObj_CheckAbbreviations():
    # ESTABLISH / WHEN
    owner_calc_config = get_owner_calc_config_dict()
...
def test_get_all_owner_calc_args_ReturnsObj():
    # ESTABLISH / WHEN
    all_owner_calc_args = get_all_owner_calc_args()
    ...
    # onracct_jmetrics_dict = owner_acctunit_aspects.get("jmetrics")
    # rope_owner_calc_aspects = onracct_jmetrics_dict.get("_fund_give")
...
def test_get_owner_calc_config_dict_ReturnsObj_CheckArgDataTypesKeysExist():
    # ESTABLISH / WHEN
    owner_calc_config = get_owner_calc_config_dict()
...
def test_get_owner_calc_dimens_ReturnsObj():
    # ESTABLISH / WHEN
    owner_calc_dimens = get_owner_calc_dimens()

    # THEN
    expected_owner_calc_dimens = {
        ownerunit_str(),
        owner_acctunit_str(),
        owner_acct_membership_str(),
        owner_planunit_str(),
    ...
    assert owner_calc_dimens == set(get_owner_calc_config_dict().keys())
...
def test_get_owner_calc_dimen_args_ReturnsObj():
    ...
    #  THEN
    print(f"{owner_acctunit_args=}")
    print(f"{owner_groupunit_args=}")
    assert owner_acctunit_args == {
        belief_label_str(),
        owner_name_str(),
        "_fund_agenda_give",
        "_credor_pool",
...
def test_get_owner_calc_config_dict_ReturnsObj_CheckArgDataTypesCorrect():
    # ESTABLISH / WHEN
    cfig = get_owner_calc_config_dict()
    ...
    onracct = owner_acctunit_str()
    onrmemb = owner_acct_membership_str()
    onrplan = owner_planunit_str()
    onrawar = owner_plan_awardlink_str()
    onrreas = owner_plan_reasonunit_str()
    onrprem = owner_plan_reason_premiseunit_str()
    onrlabo = owner_plan_laborlink_str()
    onrheal = owner_plan_healerlink_str()
    onrfact = owner_plan_factunit_str()
    onrgrou = owner_groupunit_str()
    ...
    assert g_class_type(cfig, onrlabo, jm, "_owner_name_labor") == "int"
    assert g_sqlitetype(cfig, onrlabo, jm, "_owner_name_labor") == "INTEGER"
    ...
    assert g_class_type(cfig, ownerunit, jm, "_keeps_buildable") == "int"
    assert g_sqlitetype(cfig, ownerunit, jm, "_keeps_buildable") == "INTEGER"
    assert g_class_type(cfig, ownerunit, jm, "_keeps_justified") == "int"
    assert g_sqlitetype(cfig, ownerunit, jm, "_keeps_justified") == "INTEGER"
    assert g_class_type(cfig, ownerunit, jm, _offtrack_fund_str()) == "float"
    assert g_sqlitetype(cfig, ownerunit, jm, _offtrack_fund_str()) == "REAL"
    assert g_class_type(cfig, ownerunit, jm, _rational_str()) == "bool"
    assert g_sqlitetype(cfig, ownerunit, jm, _rational_str()) == "INTEGER"
    assert g_class_type(cfig, ownerunit, jm, _sum_healerlink_share_str()) == "float"
    assert g_sqlitetype(cfig, ownerunit, jm, _sum_healerlink_share_str()) == "REAL"
    assert g_class_type(cfig, ownerunit, jm, _tree_traverse_count_str()) == "int"
    assert g_sqlitetype(cfig, ownerunit, jm, _tree_traverse_count_str()) == "INTEGER"
    assert g_class_type(cfig, ownerunit, jv, credor_respect_str()) == "float"
    assert g_sqlitetype(cfig, ownerunit, jv, credor_respect_str()) == "REAL"
    assert g_class_type(cfig, ownerunit, jv, debtor_respect_str()) == "float"
    assert g_sqlitetype(cfig, ownerunit, jv, debtor_respect_str()) == "REAL"
    assert g_class_type(cfig, ownerunit, jv, fund_iota_str()) == "float"
    assert g_sqlitetype(cfig, ownerunit, jv, fund_iota_str()) == "REAL"
    assert g_class_type(cfig, ownerunit, jv, fund_pool_str()) == "float"
    assert g_sqlitetype(cfig, ownerunit, jv, fund_pool_str()) == "REAL"
    assert g_class_type(cfig, ownerunit, jv, max_tree_traverse_str()) == "int"
    assert g_sqlitetype(cfig, ownerunit, jv, max_tree_traverse_str()) == "INTEGER"
    assert g_class_type(cfig, ownerunit, jv, penny_str()) == "float"
    assert g_sqlitetype(cfig, ownerunit, jv, penny_str()) == "REAL"
    assert g_class_type(cfig, ownerunit, jv, respect_bit_str()) == "float"
    assert g_sqlitetype(cfig, ownerunit, jv, respect_bit_str()) == "REAL"
    assert g_class_type(cfig, ownerunit, jv, tally_str()) == "int"
    assert g_sqlitetype(cfig, ownerunit, jv, tally_str()) == "INTEGER"


def test_get_owner_calc_config_dict_ReturnsObj_EachArgHasOneClassType():
    # ESTABLISH
...
def test_get_owner_calc_config_dict_ReturnsObj_EachArgHasOne_sqlite_datatype():
    # ESTABLISH
    owner_calc_config_dict = get_owner_calc_config_dict()
    all_args = {}
    # sourcery skip: no-loop-in-tests, no-conditionals-in-tests
    ...
    # WHEN
    sqlite_datatype_dict = get_owner_calc_args_sqlite_datatype_dict()

    # THEN
    for x_arg, arg_types in all_args.items():
        # print(
        #     f"""assert owner_calc_args_type_dict.get("{x_arg}") == "{list(arg_types)[0]}" """
        # )
        print(f""""{x_arg}": "{list(arg_types)[0]}",""")
        assert list(arg_types)[0] == sqlite_datatype_dict.get(x_arg)


def test_get_owner_calc_args_type_dict_ReturnsObj():
    # ESTABLISH
    owner_calc_config_dict = get_owner_calc_config_dict()
    all_args = {}
    # sourcery skip: no-loop-in-tests, no-conditionals-in-tests
    ...
    # WHEN
    owner_calc_args_type_dict = get_owner_calc_args_type_dict()

    # THEN
    assert owner_calc_args_type_dict.get(acct_name_str()) == NameTerm_str()
    assert owner_calc_args_type_dict.get(group_title_str()) == TitleTerm_str()
    assert owner_calc_args_type_dict.get(_credor_pool_str()) == "float"
    assert owner_calc_args_type_dict.get(_debtor_pool_str()) == "float"
    assert owner_calc_args_type_dict.get(_fund_agenda_give_str()) == "float"
    assert owner_calc_args_type_dict.get(_fund_agenda_ratio_give_str()) == "float"
    assert owner_calc_args_type_dict.get(_fund_agenda_ratio_take_str()) == "float"
    assert owner_calc_args_type_dict.get(_fund_agenda_take_str()) == "float"
    assert owner_calc_args_type_dict.get(_fund_give_str()) == "float"
    assert owner_calc_args_type_dict.get(_fund_take_str()) == "float"
    assert owner_calc_args_type_dict.get(group_cred_points_str()) == "int"
    assert owner_calc_args_type_dict.get(group_debt_points_str()) == "int"
    assert owner_calc_args_type_dict.get(_inallocable_acct_debt_points_str()) == "float"
    assert owner_calc_args_type_dict.get(_irrational_acct_debt_points_str()) == "float"
    assert owner_calc_args_type_dict.get(acct_cred_points_str()) == "float"
    assert owner_calc_args_type_dict.get(acct_debt_points_str()) == "float"
    assert owner_calc_args_type_dict.get(addin_str()) == "float"
    assert owner_calc_args_type_dict.get(begin_str()) == "float"
    assert owner_calc_args_type_dict.get(close_str()) == "float"
    assert owner_calc_args_type_dict.get(denom_str()) == "int"
    assert owner_calc_args_type_dict.get(gogo_want_str()) == "float"
    assert owner_calc_args_type_dict.get(mass_str()) == "int"
    assert owner_calc_args_type_dict.get(morph_str()) == "bool"
    assert owner_calc_args_type_dict.get(numor_str()) == "int"
    assert owner_calc_args_type_dict.get(task_str()) == "bool"
    ...
    assert owner_calc_args_type_dict.get(fnigh_str()) == "float"
    assert owner_calc_args_type_dict.get(fopen_str()) == "float"
    assert owner_calc_args_type_dict.get(fstate_str()) == RopeTerm_str()
    assert owner_calc_args_type_dict.get(healer_name_str()) == NameTerm_str()
    assert owner_calc_args_type_dict.get(pstate_str()) == RopeTerm_str()
    assert owner_calc_args_type_dict.get("_status") == "int"
    assert owner_calc_args_type_dict.get("_chore") == "int"
    ...
    assert owner_calc_args_type_dict.get(_all_acct_cred_str()) == "int"
    assert owner_calc_args_type_dict.get(_all_acct_debt_str()) == "int"
    assert owner_calc_args_type_dict.get(_descendant_task_count_str()) == "int"
    assert owner_calc_args_type_dict.get(_fund_cease_str()) == "float"
    assert owner_calc_args_type_dict.get(_fund_onset_str()) == "float"
    assert owner_calc_args_type_dict.get(_fund_ratio_str()) == "float"
    assert owner_calc_args_type_dict.get(_gogo_calc_str()) == "float"
    assert owner_calc_args_type_dict.get("_healerlink_ratio") == "float"
    assert owner_calc_args_type_dict.get("_level") == "int"
    assert owner_calc_args_type_dict.get(_range_evaluated_str()) == "int"
    assert owner_calc_args_type_dict.get(_stop_calc_str()) == "float"
    assert owner_calc_args_type_dict.get("_keeps_buildable") == "int"
    assert owner_calc_args_type_dict.get("_keeps_justified") == "int"
    assert owner_calc_args_type_dict.get(_offtrack_fund_str()) == "int"
    assert owner_calc_args_type_dict.get(_rational_str()) == "bool"
    assert owner_calc_args_type_dict.get(_sum_healerlink_share_str()) == "float"
    assert owner_calc_args_type_dict.get(_tree_traverse_count_str()) == "int"
    assert owner_calc_args_type_dict.get(credor_respect_str()) == "float"
    assert owner_calc_args_type_dict.get(debtor_respect_str()) == "float"
    assert owner_calc_args_type_dict.get(fund_iota_str()) == "float"
    assert owner_calc_args_type_dict.get(fund_pool_str()) == "float"
    assert owner_calc_args_type_dict.get(max_tree_traverse_str()) == "int"
    assert owner_calc_args_type_dict.get(penny_str()) == "float"
    assert owner_calc_args_type_dict.get(respect_bit_str()) == "float"
    assert owner_calc_args_type_dict.get(tally_str()) == "int"
    assert len(owner_calc_args_type_dict) == 72

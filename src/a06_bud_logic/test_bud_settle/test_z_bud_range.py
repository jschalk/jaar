...
def test_BudUnit_get_item_ranged_kids_ReturnsAllChildren():
    ...
    yao_budunit.set_item(week_item, tech_way)
    mon_str = "Monday"
    tue_str = "Tuesday"
    wed_str = "Wednesday"
    thu_str = "Thursday"
    ...
    yao_budunit.set_item(mon_item, week_way)
    yao_budunit.set_item(tue_item, week_way)
    yao_budunit.set_item(wed_item, week_way)
    yao_budunit.set_item(thu_item, week_way)
    yao_budunit.set_item(fri_item, week_way)
    yao_budunit.set_item(sat_item, week_way)
    yao_budunit.set_item(sun_item, week_way)
    yao_budunit.settle_bud()

    # WHEN
    ranged_items = yao_budunit.get_item_ranged_kids(item_way=week_way)
...
def test_BudUnit_get_item_ranged_kids_ReturnsSomeChildrenScenario1():
    ...
    ranged_items = yao_budunit.get_item_ranged_kids(week_way, x_begin, x_close)

    # THEN
    # for item_x in week_item._kids.values():
    #     print(f"{item_x.item_tag=} {item_x._gogo_calc=} {item_x._stop_calc=} ")
    # print("")
...
def test_BudUnit_get_item_ranged_kids_ReturnsSomeChildrenScenario2():
    ...
    # WHEN / THEN
    assert len(yao_budunit.get_item_ranged_kids(week_way, 0, 1440)) == 1
    assert len(yao_budunit.get_item_ranged_kids(week_way, 0, 2000)) == 2
    assert len(yao_budunit.get_item_ranged_kids(week_way, 0, 3000)) == 3
...
def test_BudUnit_get_item_ranged_kids_ReturnsSomeChildrenScenario3():
    ...
    # WHEN / THEN
    assert len(yao_budunit.get_item_ranged_kids(week_way, 0)) == 1
    assert len(yao_budunit.get_item_ranged_kids(week_way, 1440)) == 1

    # ESTABLISH
    weekday_str = "weekday"
    weekdays_item = itemunit_shop(weekday_str, gogo_want=0, stop_want=1440 * 5)
    yao_budunit.set_item(weekdays_item, week_way)

    # WHEN
    yao_budunit.settle_bud()

    # THEN
    assert len(yao_budunit.get_item_ranged_kids(week_way, 1440)) == 2

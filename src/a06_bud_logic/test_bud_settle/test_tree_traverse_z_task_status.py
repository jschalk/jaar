...
def test_BudUnit_settle_bud_SetsStatus_active_WhenFactSaysNo():
    ...
    # WHEN
    sue_budunit.add_fact(base=week_road, fpick=sun_road)
...
def test_BudUnit_settle_bud_SetsStatus_active_WhenFactModifies():
    ...
    sue_budunit.add_fact(base=states_road, fpick=usa_road)
    ...
    sue_budunit.add_fact(base=states_road, fpick=france_road)
...
def test_BudUnit_settle_bud_CorrectlySets_item_dict():
    ...
    sue_budunit.add_fact(base=week_road, fpick=wed_road)
    sue_budunit.add_fact(base=state_road, fpick=france_road)
    ...
    # WHEN
    sue_budunit.add_fact(base=state_road, fpick=oregon_road)
    sue_budunit.settle_bud()
...
def test_BudUnit_settle_bud_CorrectlyCalculatesRangeAttributes():
    ...
    day24hr_fpick = day24hr_road
    day24hr_open = 0.0
    day24hr_nigh = 8.0

    # WHEN
    sue_budunit.add_fact(
        base=day24hr_base, fpick=day24hr_fpick, fopen=day24hr_open, fnigh=day24hr_nigh
    )
...
def test_BudUnit_settle_bud_CorrectlySetsData_budunit_v001():
    ...
    # yao_budunit.add_fact(base=day_hour, fpick=day_hour, open=0, nigh=23)
    day_min_str = "day_minute"
    day_min_road = yao_budunit.make_l1_road(day_min_str)
    yao_budunit.add_fact(base=day_min_road, fpick=day_min_road, fopen=0, fnigh=1439)

    mood_str = "Moods"
    mood_road = yao_budunit.make_l1_road(mood_str)
    yao_budunit.add_fact(base=mood_road, fpick=mood_road)
    print(f"{yao_budunit.get_reason_bases()=}")

    yr_mon_str = "year_month"
    yr_mon_road = yao_budunit.make_l1_road(yr_mon_str)
    yao_budunit.add_fact(base=yr_mon_road, fpick=yr_mon_road)
    inter_str = "Interweb"
    inter_road = yao_budunit.make_l1_road(inter_str)
    yao_budunit.add_fact(base=inter_road, fpick=inter_road)
    ...
    yao_budunit.add_fact(base=week_road, fpick=mon_road)
    yao_budunit.settle_bud()
...
def test_BudUnit_settle_bud_OptionWeekdaysReturnsObj_budunit_v001():
    ...
    yao_budunit.add_fact(base=day_hr_road, fpick=day_hr_road, fopen=0, fnigh=23)
    day_min_str = "day_minute"
    day_min_road = yao_budunit.make_l1_road(day_min_str)
    yao_budunit.add_fact(base=day_min_road, fpick=day_min_road, fopen=0, fnigh=59)
    mon_wk_str = "month_week"
    mon_wk_road = yao_budunit.make_l1_road(mon_wk_str)
    yao_budunit.add_fact(base=mon_wk_road, fpick=mon_wk_road)
    nation_str = "Nation-States"
    nation_road = yao_budunit.make_l1_road(nation_str)
    yao_budunit.add_fact(base=nation_road, fpick=nation_road)
    mood_str = "Moods"
    mood_road = yao_budunit.make_l1_road(mood_str)
    yao_budunit.add_fact(base=mood_road, fpick=mood_road)
    aaron_str = "Aaron Donald objects effected by him"
    aaron_road = yao_budunit.make_l1_road(aaron_str)
    yao_budunit.add_fact(base=aaron_road, fpick=aaron_road)
    inter_str = "Interweb"
    inter_road = yao_budunit.make_l1_road(inter_str)
    yao_budunit.add_fact(base=inter_road, fpick=inter_road)
    yr_mon_str = "year_month"
    yr_mon_road = yao_budunit.make_l1_road(yr_mon_str)
    yao_budunit.add_fact(base=yr_mon_road, fpick=yr_mon_road, fopen=0, fnigh=1000)
    ...
    # yao_budunit.add_fact(base=f"{yao_budunit.fisc_tag},weekdays", fpick=f"{yao_budunit.fisc_tag},weekdays,Tuesday")
    # item_dict = yao_budunit.get_item_dict()
    # assert YR.get_active(road=bird_item, item_dict=item_dict) is True

    # yao_budunit.add_fact(base=f"{yao_budunit.fisc_tag},weekdays", fpick=f"{yao_budunit.fisc_tag},weekdays,Wednesday")
    # item_dict = yao_budunit.get_item_dict()
...
def test_BudUnit_settle_bud_CorrectlySetsItemUnitsActiveWithEvery6WeeksReason_budunit_v001():
    ...
    # WHEN
    yao_budunit.add_fact(base=day_road, fpick=day_road, fopen=0, fnigh=23)
    yao_budunit.add_fact(base=min_road, fpick=min_road, fopen=0, fnigh=59)
    yao_budunit.settle_bud()
...
def test_BudUnit_settle_bud_EveryTwoMonthReturnsObj_budunit_v001():
    ...
    yao_budunit.add_fact(base=minute_road, fpick=minute_road, fopen=0, fnigh=1399)
    month_str = "month_week"
    month_road = yao_budunit.make_l1_road(month_str)
    yao_budunit.add_fact(base=month_road, fpick=month_road)
    nations_str = "Nation-States"
    nations_road = yao_budunit.make_l1_road(nations_str)
    yao_budunit.add_fact(base=nations_road, fpick=nations_road)
    mood_str = "Moods"
    mood_road = yao_budunit.make_l1_road(mood_str)
    yao_budunit.add_fact(base=mood_road, fpick=mood_road)
    aaron_str = "Aaron Donald objects effected by him"
    ...
    yao_budunit.add_fact(base=interweb_road, fpick=interweb_road)
    weekdays_str = "weekdays"
    weekdays_road = yao_budunit.make_l1_road(weekdays_str)
    yao_budunit.add_fact(base=weekdays_road, fpick=weekdays_road)
    item_dict = yao_budunit.get_item_dict()
    print(f"{len(item_dict)=}")
    ...
    # WHEN
    yao_budunit.add_fact(base=year_month_base, fpick=year_month_base, fopen=0, fnigh=8)
    ced_week = yao_budunit.make_l1_road("ced_week")
    yao_budunit.add_fact(base=ced_week, fpick=ced_week, fopen=0, fnigh=4)
    yao_budunit.settle_bud()

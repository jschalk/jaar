...
def test_BudUnit_settle_bud_CorrectlySets_idea_dict():
    ...
    assert casa_idea.parent_way == to_way(sue_budunit.fisc_label)
    assert casa_idea._kids == {}
    assert casa_idea.mass == 30
    assert casa_idea.idea_label == casa_str
    ...
    #     # print("")
    #     # print(f"{x_idea.idea_label=}")
    #     # print(f"{len(x_idea.reasonunits)=}")
    #     print(
    #         f"  {x_idea.idea_label} iterate through every reasonheir... {len(x_idea._reasonheirs)=} {x_idea.idea_label=}"
    #     )
...
def test_BudUnit_settle_bud_CorrectlySetsData_budunit_v001():
    ...
    # if yao_budunit.idearoot._kids["Ultimate Frisbee"].idea_label == "Ultimate Frisbee":
    ...
    #     )
    #     # print(f"{idea.idea_label=}")
    #     if idea.idea_label == laundry_str:
    #         for reason in idea.reasonunits.values():
    #             print(f"{idea.idea_label=} {reason.rcontext=}")  # {reason.premises=}")
...
def test_BudUnit_settle_bud_OptionWeekdaysReturnsObj_budunit_v001():
    ...
    x_idearoot = yao_budunit.get_idea_obj(to_way(yao_budunit.fisc_label))
...
def test_BudUnit_settle_bud_CorrectlySetsIdeaUnitsActiveWithEvery6WeeksReason_budunit_v001():
    ...
    print(
        f"{clean_sheet_idea.idea_label=} {ced_week_reason.rcontext=} {ced_week_premise.pbranch=}"
    )
    # print(f"{clean_sheet_idea.idea_label=} {ced_week_reason.rcontext=} {premise_x=}")
    ...
    #     if idea.idea_label == "clean sheets couch blankets":
    ...
    print(f"{clean_couch_idea.idea_label=} {week_reason.rcontext=} {week_premise=}")
...
def test_BudUnit_settle_bud_EveryIdeaHasActiveStatus_budunit_v001():
    ...
    #     idea_kid_count += 1
    #     print(f"{idea.idea_label=} {idea_kid_count=}")
...
def test_BudUnit_settle_bud_EveryTwoMonthReturnsObj_budunit_v001():
    ...
    mat_idea_label = "deep clean play mat"
    mat_way = yao_budunit.make_way(clean_way, mat_idea_label)
...
def test_BudUnit_settle_bud_CorrectlySets_sum_healerlink_share(graphics_bool):
    ...
    print(f"{week_idea.idea_label=} {week_idea.problem_bool=} {week_idea._fund_ratio=}")
    sue_budunit.settle_bud()
    # THEN

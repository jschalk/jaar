...
def test_BudUnit_set_idea_CorrectlySetsfisc_tag_AND_fund_coin():
    # ESTABLISH'
    x_fund_coin = 500
    ...
    assert cookery_idea.fisc_tag == bud_fisc_tag
...
def test_bud_set_fisc_tag_CorrectlySetsAttr():
    ...
    assert yao_bud.idearoot.idea_tag == x_fisc_tag
    casa_idea = yao_bud.get_idea_obj(new_casa_way)
    assert casa_idea.parent_way == to_way(x_fisc_tag)
    swim_idea = yao_bud.get_idea_obj(new_swim_way)
    assert swim_idea.parent_way == new_casa_way


def test_bud_set_bridge_RaisesErrorIfNew_bridge_IsAnIdea_tag():
    # ESTABLISH
    ...
    assert (
        str(excinfo.value)
        == f"Cannot modify bridge to '{slash_str}' because it exists an idea idea_tag '{casa_way}'"
    )
...
def test_bud_set_bridge_CorrectlyModifies_parent_way():
    ...
    semicolon_cook_way = zia_bud.make_way(semicolon_casa_way, cook_str)
    # print(f"{zia_bud.fisc_tag=} {zia_bud.idearoot.idea_tag=} {casa_way=}")
    # print(f"{cook_idea.parent_way=} {cook_idea.idea_tag=}")
    # semicolon_casa_idea = zia_bud.get_idea_obj(semicolon_casa_way)
    # print(f"{semicolon_casa_idea.parent_way=} {semicolon_casa_idea.idea_tag=}")

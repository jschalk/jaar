...
def test_BudUnit_set_fisc_label_CorrectlySetsAttr():
    # ESTABLISH
    x_fisc_label = "accord45"
    sue_str = "Sue"
    sue_bud = budunit_shop(sue_str)
    assert sue_bud.fisc_label == root_label()

    # WHEN
    sue_bud.set_fisc_label(fisc_label=x_fisc_label)

    # THEN
    assert sue_bud.fisc_label == x_fisc_label


def test_BudUnit_set_idea_CorrectlySetsfisc_label_AND_fund_coin():
    # ESTABLISH'
    x_fund_coin = 500
    ...
    assert cookery_idea.fisc_label == bud_fisc_label
...
def test_bud_set_fisc_label_CorrectlySetsAttr():
    ...
    assert yao_bud.fisc_label == yao_bud.fisc_label

    # WHEN
    x_fisc_label = "accord45"
    yao_bud.set_fisc_label(fisc_label=x_fisc_label)

    # THEN
    new_casa_way = yao_bud.make_l1_way(casa_str)
    swim_str = "swim"
    new_swim_way = yao_bud.make_way(new_casa_way, swim_str)
    assert yao_bud.fisc_label == x_fisc_label
    assert yao_bud.idearoot.idea_label == x_fisc_label
    casa_idea = yao_bud.get_idea_obj(new_casa_way)
    assert casa_idea.parent_way == to_way(x_fisc_label)
    swim_idea = yao_bud.get_idea_obj(new_swim_way)
    assert swim_idea.parent_way == new_casa_way


def test_bud_set_bridge_RaisesErrorIfNew_bridge_IsAnIdea_label():
    # ESTABLISH
    ...
    assert (
        str(excinfo.value)
        == f"Cannot modify bridge to '{slash_str}' because it exists an idea idea_label '{casa_way}'"
    )
...
def test_bud_set_bridge_CorrectlyModifies_parent_way():
    ...
    semicolon_cook_way = zia_bud.make_way(semicolon_casa_way, cook_str)
    # print(f"{zia_bud.fisc_label=} {zia_bud.idearoot.idea_label=} {casa_way=}")
    # print(f"{cook_idea.parent_way=} {cook_idea.idea_label=}")
    # semicolon_casa_idea = zia_bud.get_idea_obj(semicolon_casa_way)
    # print(f"{semicolon_casa_idea.parent_way=} {semicolon_casa_idea.idea_label=}")
    ...
    # THEN
    assert cook_idea.get_idea_way() != semicolon_cook_way
    zia_fisc_label = zia_bud.fisc_label
    slash_casa_way = create_way(zia_fisc_label, casa_str, bridge=slash_str)

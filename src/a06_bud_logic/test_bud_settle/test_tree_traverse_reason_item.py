...
def test_BudUnit_ReasonUnits_create():
    ...
    casa_wk_reason = reasonunit_shop(weekday_way, {wed_premise.need: wed_premise})
    print(f"{type(casa_wk_reason.base)=}")
    print(f"{casa_wk_reason.base=}")

    # WHEN
    sue_bud.edit_item_attr(casa_way, reason=casa_wk_reason)

    # THEN
    casa_item = sue_bud.get_item_obj(casa_way)
    assert casa_item.reasonunits is not None
    print(casa_item.reasonunits)
    assert casa_item.reasonunits[weekday_way] is not None
    assert casa_item.reasonunits[weekday_way] == casa_wk_reason
...
def test_BudUnit_edit_item_attr_reasonunit_CorrectlySets_bridge():
    ...
    # WHEN
    sue_bud.edit_item_attr(casa_way, reason=before_week_reason)

    # THEN
    casa_item = sue_bud.get_item_obj(casa_way)
    week_reasonunit = casa_item.reasonunits.get(week_way)
...
def test_BudUnit_set_reasonunits_status():
    ...
    casa_wk_reason = reasonunit_shop(
        base=weekday_way, premises={wed_premise.need: wed_premise}
    )
    print(f"{type(casa_wk_reason.base)=}")
    print(f"{casa_wk_reason.base=}")
...
def test_agenda_returned_WhenNoReasonsExist():
    ...
    # THEN
    casa_way = sue_bud.make_l1_way("casa")
    assert sue_bud.get_item_obj(casa_way)._task is True
    cat_way = sue_bud.make_l1_way("cat have dinner")
    assert sue_bud.get_item_obj(cat_way)._task is True
...
def test_BudUnit_reasonheirs_AreCorrectlyInherited_v1():
    ...
    sue_bud.edit_item_attr(casa_way, reason=casa_wk_build_reasonunit)
    casa_item = sue_bud.get_item_obj(casa_way)
    assert casa_item.reasonunits != {}
    assert casa_item.get_reasonunit(week_way)
    assert casa_item.get_reasonunit(week_way) == casa_wk_build_reasonunit
    assert not casa_item.get_reasonheir(week_way)

    # WHEN
    sue_bud.settle_bud()

    # THEN
    assert casa_item.get_reasonheir(week_way)
    assert len(casa_item.get_reasonheir(week_way).premises) == 1
    assert casa_item.get_reasonheir(week_way).get_premise(tue_way)
    premise_tue = casa_item.get_reasonheir(week_way).get_premise(tue_way)
    tue_premise = premiseunit_shop(need=tue_way)
    ...
    week_reasonheir = casa_item.get_reasonheir(week_way)
    assert week_reasonheir.premises == built_week_reasonheir.premises
    assert casa_item.get_reasonheir(week_way) == built_week_reasonheir
...
def test_BudUnit_reasonheirs_AreCorrectlyInheritedTo4LevelsFromRoot():
    ...
    casa_wk_build_reasonunit = reasonunit_shop(base=week_way, premises=premises_x)
    ...
    a4_bud.edit_item_attr(casa_way, reason=casa_wk_build_reasonunit)

    # WHEN
    rla_str = "hp"
    rla_way = a4_bud.make_way(casa_way, rla_str)
    a4_bud.set_item(itemunit_shop(rla_str), parent_way=rla_way)
    cost_str = "cost_quantification"
    cost_way = a4_bud.make_way(rla_way, cost_str)
    a4_bud.set_item(itemunit_shop(cost_str), parent_way=cost_way)
    a4_bud.settle_bud()
    ...
    # 1
    casa_wk_calc_reasonheir = casa_item._reasonheirs[week_way]
    assert casa_wk_calc_reasonheir == casa_wk_built_reasonheir

    # 2
    rla_week_reasonheir = rla_item._reasonheirs[week_way]
    ...
    # 3
    cost_week_reasonheir = cost_item._reasonheirs[week_way]
...
def test_BudUnit_reasonheirs_AreCorrectlyInheritedTo4LevelsFromLevel2():
    ...
    week_way = a4_bud.make_l1_way(week_item_tag)
    wed_str = "Wednesday"
    wed_way = a4_bud.make_way(week_way, wed_str)
    ...
    # 1
    assert casa_item._reasonheirs[week_way] == casa_wk_built_reasonheir
...
def test_BudUnit_ReasonUnits_set_UnCoupledMethod():
    ...
    assert casa_item1.reasonunits[week_way] is not None
    assert casa_item1.reasonunits[week_way].premises[wed_way].open is None
    assert casa_item1.reasonunits[week_way].premises[wed_way].nigh is None
    ...
    assert casa_item1.reasonunits[week_way] == casa_wk_reason1
    ...
    # THEN
    assert casa_item1.reasonunits[week_way].premises[wed_way].open == 12
    assert casa_item1.reasonunits[week_way].premises[wed_way].nigh == 12

    wed_premise2 = premiseunit_shop(
        need=wed_way, divisor=divisor_x, open=open_x, nigh=nigh_x
    )
    casa_wk_reason2 = reasonunit_shop(
        base=week_way, premises={wed_premise2.need: wed_premise2}
    )
    print(f"{type(casa_wk_reason2.base)=}")
    print(f"{casa_wk_reason2.base=}")
    assert casa_item1.reasonunits[week_way] == casa_wk_reason2
    ...
    # THEN
    assert len(casa_item1.reasonunits[week_way].premises) == 2
...
def test_BudUnit_ReasonUnits_set_premiseItemWithDenomSetsPremiseDivision():
    ...
    sue_bud.set_item(itemunit_shop(week_str, denom=7), parent_way=time_way)
    ...
    # THEN
    casa_item1 = sue_bud.get_item_obj(casa_way)
    assert casa_item1.reasonunits[time_way] is not None
    assert casa_item1.reasonunits[time_way].premises[week_way].divisor == 7
    assert casa_item1.reasonunits[time_way].premises[week_way].open == 2
    assert casa_item1.reasonunits[time_way].premises[week_way].nigh == 5
...
def test_BudUnit_ReasonUnits_set_premiseItemWithBeginCloseSetsPremiseOpen_Nigh():
    ...
    sue_bud.set_item(itemunit_shop(rus_war, begin=22, close=34), time_way)
    ...
    casa_item1 = sue_bud.get_item_obj(casa_way)
    assert casa_item1.reasonunits[time_way] is not None
    assert casa_item1.reasonunits[time_way].premises[rus_war_way].divisor is None
    assert casa_item1.reasonunits[time_way].premises[rus_war_way].open == 22
    assert casa_item1.reasonunits[time_way].premises[rus_war_way].nigh == 34
...
def test_BudUnit_ReasonUnits_edit_item_attr_CorrectlyDeletes_ReasonUnits_And_PremiseUnits():
    ...
    casa_item1 = sue_bud.get_item_obj(casa_way)
    assert len(casa_item1.reasonunits[weekday_way].premises) == 2
    ...
    # THEN
    assert len(casa_item1.reasonunits[weekday_way].premises) == 1
    ...
    with pytest_raises(KeyError) as excinfo:
        casa_item1.reasonunits[weekday_way]
...
def test_BudUnit_ReasonUnits_del_reason_premise_UncoupledMethod2():
    ...
    with pytest_raises(Exception) as excinfo:
        casa_item1.del_reasonunit_base(weekdays_way)
...
def test_BudUnit_edit_item_attr_budIsAbleToEdit_base_item_active_requisite_AnyItemIfInvaildThrowsError():
    ...
    run_item = sue_bud.get_item_obj(run_way)
    ...
    reasonunit_casa = run_item.reasonunits.get(casa_way)
    assert reasonunit_casa.base == casa_way
...
def test_BudUnit_ReasonUnits_ItemUnit_active_InfluencesReasonUnitStatus():
    ...
    sue_bud.add_fact(fbase=weekdays_way, fneed=wed_way)
    sue_bud.settle_bud()
    ...
    sue_bud.add_fact(fbase=weekdays_way, fneed=thu_way)

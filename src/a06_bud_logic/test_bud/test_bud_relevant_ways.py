...
def test_BudUnit_get_relevant_ways_ReturnSimple():
    ...
    hour_length_concept = conceptunit_shop(hour_length_str)
    yao_bud.set_l1_concept(hour_length_concept)
...
def test_BudUnit_get_inheritor_concept_list_ReturnsObj_Scenario0():
    ...
    yao_budunit.set_concept(conceptunit_shop(week_str, begin=0, close=10800), tech_way)
    mon_str = "Monday"
    mon_way = yao_budunit.make_way(week_way, mon_str)
    yao_budunit.set_concept(conceptunit_shop(mon_str), week_way)
    yao_budunit.settle_bud()

    # WHEN
    x_inheritor_concept_list = yao_budunit.get_inheritor_concept_list(week_way, mon_way)

    # # THEN
    assert len(x_inheritor_concept_list) == 2
    week_concept = yao_budunit.get_concept_obj(week_way)
    mon_concept = yao_budunit.get_concept_obj(mon_way)
    assert x_inheritor_concept_list == [week_concept, mon_concept]

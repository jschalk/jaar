...
def test_BudUnit_get_relevant_ways_EmptyWayUnitReturnsEmpty():
    # ESTABLISH
...
def test_BudUnit_get_relevant_ways_RootWayUnitReturnsOnlyItself():
    # ESTABLISH
    sue_bud = get_budunit_with_4_levels()
...
def test_BudUnit_get_relevant_ways_ReturnsSimpleReasonUnitBase():
    ...
    sue_bud.set_idea(floor_idea, parent_way=casa_way)

    unim_str = "unimportant"
    unim_way = sue_bud.make_l1_way(unim_str)
    unim_idea = ideaunit_shop(unim_str)
    sue_bud.set_idea(unim_idea, parent_way=sue_bud.fisc_tag)
...
def test_BudUnit_get_relevant_ways_ReturnSimple():
    ...
    day_length_idea = ideaunit_shop(day_length_str, begin=0, close=1440)
    yao_bud.set_l1_idea(day_length_idea)

    hour_length_str = "hour_length"
    hour_length_way = yao_bud.make_l1_way(hour_length_str)
    hour_length_idea = ideaunit_shop(hour_length_str)
    yao_bud.set_l1_idea(hour_length_idea)
    ...
    yao_bud.set_idea(min_days_idea, parent_way=min_range_x_way)
    ...
    assert hour_length_way not in relevant_ways
    assert min_days_way in relevant_ways
    assert root_way in relevant_ways
    # min_days_idea = yao_bud.get_idea_obj(min_days_way)
...
def test_BudUnit_get_inheritor_idea_list_ReturnsObj_Scenario0():
    ...
    yao_budunit.set_idea(ideaunit_shop(week_str, begin=0, close=10800), tech_way)
    mon_str = "Monday"
    mon_way = yao_budunit.make_way(week_way, mon_str)
    yao_budunit.set_idea(ideaunit_shop(mon_str), week_way)
    yao_budunit.settle_bud()

    # WHEN
    x_inheritor_idea_list = yao_budunit.get_inheritor_idea_list(week_way, mon_way)

    # # THEN
    assert len(x_inheritor_idea_list) == 2
    week_idea = yao_budunit.get_idea_obj(week_way)
    mon_idea = yao_budunit.get_idea_obj(mon_way)

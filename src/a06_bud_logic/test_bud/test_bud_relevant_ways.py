...
def test_BudUnit_get_relevant_ways_ReturnsSimpleReasonUnitBase():
    ...
    sue_bud.set_item(floor_item, parent_way=casa_way)

    unim_str = "unimportant"
    unim_way = sue_bud.make_l1_way(unim_str)
    unim_item = itemunit_shop(unim_str)
    sue_bud.set_item(unim_item, parent_way=sue_bud.fisc_tag)
...
def test_BudUnit_get_relevant_ways_ReturnSimple():
    ...
    yao_bud.set_item(min_days_item, parent_way=min_range_x_way)
    ...
    # min_days_item = yao_bud.get_item_obj(min_days_way)
...
def test_BudUnit_get_inheritor_item_list_ReturnsObj_Scenario0():
    ...
    yao_budunit.set_item(itemunit_shop(week_str, begin=0, close=10800), tech_way)
    mon_str = "Monday"
    mon_way = yao_budunit.make_way(week_way, mon_str)
    yao_budunit.set_item(itemunit_shop(mon_str), week_way)
    yao_budunit.settle_bud()

    # WHEN
    x_inheritor_item_list = yao_budunit.get_inheritor_item_list(week_way, mon_way)

    # # THEN
    assert len(x_inheritor_item_list) == 2
    week_item = yao_budunit.get_item_obj(week_way)
    mon_item = yao_budunit.get_item_obj(mon_way)

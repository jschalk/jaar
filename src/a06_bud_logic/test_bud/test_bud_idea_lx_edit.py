...
def test_BudUnit_edit_idea_label_FailsWhenIdeaDoesNotExist():
    ...
    with pytest_raises(Exception) as excinfo:
        yao_bud.edit_idea_label(old_way=no_idea_way, new_idea_label="birds")
    assert str(excinfo.value) == f"Idea old_way='{no_idea_way}' does not exist"


def test_BudUnit_edit_idea_label_RaisesErrorForLevel0IdeaWhen_fisc_label_isNone():
    # ESTABLISH
    yao_str = "Yao"
    ...
    assert yao_bud.idearoot.idea_label == yao_bud.fisc_label
    casa_idea = yao_bud.get_idea_obj(casa_way)
    assert casa_idea.parent_way == to_way(yao_bud.fisc_label)
    ...
    with pytest_raises(Exception) as excinfo:
        moon_str = "moon"
        yao_bud.edit_idea_label(old_way=root_way, new_idea_label=moon_str)
    assert (
        str(excinfo.value)
        == f"Cannot set idearoot to string different than '{yao_bud.fisc_label}'"
    )

    assert yao_bud.idearoot.idea_label != moon_str
    assert yao_bud.idearoot.idea_label == yao_bud.fisc_label


def test_BudUnit_edit_idea_label_RaisesErrorForLevel0When_fisc_label_IsDifferent():
    # ESTABLISH
    yao_str = "Yao"
    yao_bud = budunit_shop(owner_name=yao_str)
    casa_str = "casa"
    ...
    yao_bud.idearoot.fisc_label = sun_str
    assert yao_bud.owner_name == yao_str
    assert yao_bud.fisc_label == sun_str
    assert yao_bud.idearoot.fisc_label == sun_str
    assert yao_bud.idearoot.idea_label == root_label()
    casa_idea = yao_bud.get_idea_obj(casa_way)
    ...
    with pytest_raises(Exception) as excinfo:
        moon_str = "moon"
        yao_bud.edit_idea_label(old_way=get_default_fisc_way(), new_idea_label=moon_str)
    assert (
        str(excinfo.value)
...
def test_BudUnit_find_replace_way_CorrectlyModifies_kids_Scenario1():
    ...
    yao_bud.edit_idea_label(old_way=old_casa_way, new_idea_label=new_casa_str)
...
def test_bud_edit_idea_label_Modifies_factunits():
    ...
    yao_bud.edit_idea_label(old_way=old_water_way, new_idea_label=new_water_str)
...
def test_bud_edit_idea_label_ModifiesIdeaReasonUnitsScenario1():
    ...
    print(f"before {wednesday_idea.idea_label=}")
    print(f"before {wednesday_idea.parent_way=}")
    sue_bud.edit_idea_label(old_way=old_weekday_way, new_idea_label=new_weekday_str)
    # for key_x, x_reason in casa_idea.reasonunits.items():
    #     print(f"after {key_x=} {x_reason.rcontext=}")
    print(f"after  {wednesday_idea.idea_label=}")
...
def test_bud_set_owner_name_CorrectlyModifiesBoth():
    ...
    assert sue_bud.idearoot.idea_label == sue_bud.fisc_label
    # mid_idea_label1 = "Yao"
    # sue_bud.edit_idea_label(old_way=old_idea_label, new_idea_label=mid_idea_label1)
    # assert sue_bud.owner_name == old_idea_label
    # assert sue_bud.idearoot.idea_label == mid_idea_label1
...
def test_bud_edit_idea_label_RaisesErrorIfbridgeIsInLabel():
    # ESTABLISH
    sue_bud = get_budunit_with_4_levels_and_2reasons_2facts()
    old_weekday_str = "weekdays"
    old_weekday_way = sue_bud.make_l1_way(old_weekday_str)
    ...
    assert (
        str(excinfo.value)
        == f"Cannot modify '{old_weekday_way}' because new_idea_label {new_weekday_str} contains bridge {sue_bud.bridge}"
    )

...
def test_BudUnit_set_fact_CorrectlyModifiesAttrWhen_open_IsNone():
    ...
    assert x_itemroot.factunits.get(weekday_way) == x7_factunit
    ...
    assert x_itemroot.factunits.get(weekday_way) == x10_factunit
...
def test_BudUnit_set_fact_FailsToCreateWhenBaseAndFactAreDifferenctAndFactItemIsNot_RangeRoot():
    ...
    bob_bud.set_item(a1st_item, parent_way=time_way)
    a1e1st_str = "a1_era1st"
    a1e1st_item = itemunit_shop(a1e1st_str, begin=20, close=30)
    bob_bud.set_item(a1e1st_item, parent_way=a1st_way)
...
def test_BudUnit_get_fact_ReturnsFactUnit():
    ...
    sue_bud.add_fact(situations_way, climate_way, create_missing_items=True)

    # WHEN
    generated_situations_base = sue_bud.get_fact(situations_way)

    # THEN
    static_situations_base = sue_bud.itemroot.factunits.get(situations_way)
...
def test_BudUnit_get_rangeroot_factunits_ReturnsObjsScenario1():
    ...
    happy = "happy"
    sad = "Sad"
    sue_bud.set_item(itemunit_shop(happy), parent_way=m_x_way)
    sue_bud.set_item(itemunit_shop(sad), parent_way=m_x_way)
    sue_bud.add_fact(fbase=time_way, fneed=time_way, fopen=5, fnigh=10)
...
def test_BudUnit_set_fact_create_missing_items_CreatesBaseAndFact():
    ...
    assert sue_bud.get_item_obj(situations_way) is not None
    assert sue_bud.get_item_obj(climate_way) is not None

...
def test_BudUnit_set_idea_RaisesErrorWhen_parent_way_IsInvalid():
    ...
    with pytest_raises(Exception) as excinfo:
        zia_bud.set_idea(ideaunit_shop(casa_str), parent_way=invalid_roottag_swim_way)
    exception_str = f"set_idea failed because parent_way '{invalid_roottag_swim_way}' has an invalid root tag. Should be {zia_bud.fisc_tag}."
    assert str(excinfo.value) == exception_str


def test_BudUnit_set_idea_RaisesErrorWhen_parent_way_IdeaDoesNotExist():
    # ESTABLISH
    zia_bud = budunit_shop("Zia")
    ...
    with pytest_raises(Exception) as excinfo:
        zia_bud.set_idea(
            ideaunit_shop(casa_str),
            parent_way=swim_way,
            create_missing_ancestors=False,
        )
    exception_str = f"set_idea failed because '{swim_way}' idea does not exist."
    assert str(excinfo.value) == exception_str


def test_BudUnit_set_idea_RaisesErrorWhen_idea_tag_IsNotTag():
    # ESTABLISH
    zia_bud = budunit_shop("Zia")
    swim_way = zia_bud.make_l1_way("swimming")
    casa_str = "casa"
...
def test_BudUnit_set_idea_CorrectlySetsAttr():
    ...
    # WHEN
    zia_bud.set_idea(ideaunit_shop(casa_str), parent_way=to_way(zia_bud.fisc_tag))

    # THEN
    print(f"{zia_bud.idearoot._kids.keys()=}")
    assert zia_bud.idearoot._kids.get(casa_str)
...
def test_BudUnit_set_l1_idea_CorrectlySetsAttr():
    ...
    casa_str = "casa"
    casa_way = zia_bud.make_l1_way(casa_str)
    assert not zia_bud.idearoot._kids.get(casa_way)
...
def test_BudUnit_add_idea_SetsAttr_Scenario0():
    ...
    assert not bob_budunit.idea_exists(casa_way)

    # WHEN
    bob_budunit.add_idea(casa_way)

    # THEN
    assert bob_budunit.idea_exists(casa_way)
    casa_ideaunit = bob_budunit.get_idea_obj(casa_way)
    assert casa_ideaunit.bridge == bob_budunit.bridge
...
def test_BudUnit_add_idea_SetsAttr_Scenario1():
    ...
    # WHEN
    bob_budunit.add_idea(casa_way, mass=casa_mass, pledge=casa_pledge)
...
def test_BudUnit_add_idea_ReturnsObj():
    ...
    # THEN
    assert casa_ideaunit.idea_tag == "casa"
    assert casa_ideaunit.mass == casa_mass


def test_BudUnit_set_idea_CorrectlyAddsIdeaObjWithNonDefault_bridge():
    # ESTABLISH
    slash_str = "/"
    ...
    assert len(bob_bud.idearoot._kids) == 2
    ...
    assert casa_idea.reasonunits.get(week_way) is not None
...
def test_BudUnit_set_idea_CanCreateMissingIdeaUnits():
    ...
    assert sue_bud.idea_exists(battles_way) is False
    assert sue_bud.idea_exists(coralsea_way) is False

    # WHEN
    sue_bud.set_idea(saratoga_idea, parent_way=coralsea_way)

    # THEN
    assert sue_bud.idea_exists(battles_way)
    assert sue_bud.idea_exists(coralsea_way)
...
def test_BudUnit_del_idea_obj_Level0CannotBeDeleted():
    ...
    # WHEN / THEN
    with pytest_raises(Exception) as excinfo:
        sue_bud.del_idea_obj(way=root_way)
...
def test_BudUnit_del_idea_obj_Level1CanBeDeleted_ChildrenDeleted():
    ...
    assert sue_bud.get_idea_obj(week_way)
    assert sue_bud.get_idea_obj(sun_way)

    # WHEN
    sue_bud.del_idea_obj(way=week_way)

    # THEN
    with pytest_raises(Exception) as excinfo:
        sue_bud.get_idea_obj(week_way)
    assert str(excinfo.value) == f"get_idea_obj failed. no idea at '{week_way}'"
    new_sunday_way = sue_bud.make_l1_way("Sunday")
    with pytest_raises(Exception) as excinfo:
        sue_bud.get_idea_obj(new_sunday_way)
    assert str(excinfo.value) == f"get_idea_obj failed. no idea at '{new_sunday_way}'"
...
def test_BudUnit_del_idea_obj_Level1CanBeDeleted_ChildrenInherited():
    ...
    assert sue_bud.get_idea_obj(old_sunday_way)

    # WHEN
    sue_bud.del_idea_obj(way=week_way, del_children=False)

    # THEN
    with pytest_raises(Exception) as excinfo:
        sue_bud.get_idea_obj(old_sunday_way)
    assert str(excinfo.value) == f"get_idea_obj failed. no idea at '{old_sunday_way}'"
    ...
    assert new_sunday_idea.parent_way == to_way(sue_bud.fisc_tag)
...
def test_BudUnit_del_idea_obj_LevelNCanBeDeleted_ChildrenInherited():
    ...
    assert sue_bud.idea_exists(usa_way)
    assert sue_bud.idea_exists(usa_texas_way)
    assert sue_bud.idea_exists(usa_oregon_way)
    assert sue_bud.idea_exists(states_texas_way) is False
    assert sue_bud.idea_exists(states_oregon_way) is False

    # WHEN
    sue_bud.del_idea_obj(way=usa_way, del_children=False)

    # THEN
    assert sue_bud.idea_exists(states_texas_way)
    assert sue_bud.idea_exists(states_oregon_way)
    assert sue_bud.idea_exists(usa_texas_way) is False
    assert sue_bud.idea_exists(usa_oregon_way) is False
    assert sue_bud.idea_exists(usa_way) is False
...
def test_BudUnit_del_idea_obj_Level2CanBeDeleted_ChildrenDeleted():
    ...
    assert sue_bud.get_idea_obj(monday_way)

    # WHEN
    sue_bud.del_idea_obj(way=monday_way)

    # THEN
    with pytest_raises(Exception) as excinfo:
        sue_bud.get_idea_obj(monday_way)
    assert str(excinfo.value) == f"get_idea_obj failed. no idea at '{monday_way}'"
...
def test_BudUnit_del_idea_obj_LevelNCanBeDeleted_ChildrenDeleted():
    ...
    texas_str = "Texas"
    usa_texas_way = sue_bud.make_way(usa_way, texas_str)
    assert sue_bud.get_idea_obj(usa_texas_way)

    # WHEN
    sue_bud.del_idea_obj(way=usa_texas_way)

    # THEN
    with pytest_raises(Exception) as excinfo:
        sue_bud.get_idea_obj(usa_texas_way)
    assert str(excinfo.value) == f"get_idea_obj failed. no idea at '{usa_texas_way}'"
...
def test_BudUnit_edit_idea_attr_IsAbleToEditAnyAncestor_Idea():
    ...
    print(f"{casa_way=}")
    old_mass = sue_bud.idearoot._kids[casa_str].mass
    assert old_mass == 30
    sue_bud.edit_idea_attr(casa_way, mass=23)
    new_mass = sue_bud.idearoot._kids[casa_str].mass
    assert new_mass == 23

    # uid: int = None,
    sue_bud.idearoot._kids[casa_str]._uid = 34
    x_uid = sue_bud.idearoot._kids[casa_str]._uid
    assert x_uid == 34
    sue_bud.edit_idea_attr(casa_way, uid=23)
    uid_new = sue_bud.idearoot._kids[casa_str]._uid
    assert uid_new == 23

    # begin: float = None,
    # close: float = None,
    sue_bud.idearoot._kids[casa_str].begin = 39
    x_begin = sue_bud.idearoot._kids[casa_str].begin
    assert x_begin == 39
    sue_bud.idearoot._kids[casa_str].close = 43
    x_close = sue_bud.idearoot._kids[casa_str].close
    assert x_close == 43
    sue_bud.edit_idea_attr(casa_way, begin=25, close=29)
    assert sue_bud.idearoot._kids[casa_str].begin == 25
    assert sue_bud.idearoot._kids[casa_str].close == 29

    # gogo_want: float = None,
    # stop_want: float = None,
    sue_bud.idearoot._kids[casa_str].gogo_want = 439
    x_gogo_want = sue_bud.idearoot._kids[casa_str].gogo_want
    assert x_gogo_want == 439
    sue_bud.idearoot._kids[casa_str].stop_want = 443
    x_stop_want = sue_bud.idearoot._kids[casa_str].stop_want
    assert x_stop_want == 443
    sue_bud.edit_idea_attr(casa_way, gogo_want=425, stop_want=429)
    assert sue_bud.idearoot._kids[casa_str].gogo_want == 425
    assert sue_bud.idearoot._kids[casa_str].stop_want == 429

    # factunit: factunit_shop = None,
    # sue_bud.idearoot._kids[casa_str].factunits = None
    ...
    x_descendant_pledge_count = sue_bud.idearoot._kids[
        casa_str
    ]._descendant_pledge_count
    assert x_descendant_pledge_count == 81
    sue_bud.edit_idea_attr(casa_way, descendant_pledge_count=67)
    _descendant_pledge_count_new = sue_bud.idearoot._kids[
        casa_str
    ]._descendant_pledge_count
    assert _descendant_pledge_count_new == 67

    # _all_acct_cred: bool = None,
    sue_bud.idearoot._kids[casa_str]._all_acct_cred = 74
    x_all_acct_cred = sue_bud.idearoot._kids[casa_str]._all_acct_cred
    assert x_all_acct_cred == 74
    sue_bud.edit_idea_attr(casa_way, all_acct_cred=59)
    _all_acct_cred_new = sue_bud.idearoot._kids[casa_str]._all_acct_cred
    assert _all_acct_cred_new == 59

    # _all_acct_debt: bool = None,
    sue_bud.idearoot._kids[casa_str]._all_acct_debt = 74
    x_all_acct_debt = sue_bud.idearoot._kids[casa_str]._all_acct_debt
    assert x_all_acct_debt == 74
    sue_bud.edit_idea_attr(casa_way, all_acct_debt=59)
    _all_acct_debt_new = sue_bud.idearoot._kids[casa_str]._all_acct_debt
    ...
    sue_bud.idearoot._kids[casa_str]._is_expanded = "what"
    _is_expanded = sue_bud.idearoot._kids[casa_str]._is_expanded
    assert _is_expanded == "what"
    sue_bud.edit_idea_attr(casa_way, is_expanded=True)
    assert sue_bud.idearoot._kids[casa_str]._is_expanded is True

    # pledge: dict = None,
    sue_bud.idearoot._kids[casa_str].pledge = "funfun3"
    pledge = sue_bud.idearoot._kids[casa_str].pledge
    assert pledge == "funfun3"
    sue_bud.edit_idea_attr(casa_way, pledge=True)
    assert sue_bud.idearoot._kids[casa_str].pledge is True

    # _healerlink:
    sue_bud.idearoot._kids[casa_str].healerlink = "fun3rol"
    src_healerlink = sue_bud.idearoot._kids[casa_str].healerlink
    ...
    sue_bud.edit_idea_attr(casa_way, healerlink=x_healerlink)
    assert sue_bud.idearoot._kids[casa_str].healerlink == x_healerlink

    # _problem_bool: bool
    sue_bud.idearoot._kids[casa_str].problem_bool = "fun3rol"
    src_problem_bool = sue_bud.idearoot._kids[casa_str].problem_bool
    assert src_problem_bool == "fun3rol"
    x_problem_bool = True
    sue_bud.edit_idea_attr(casa_way, problem_bool=x_problem_bool)
    assert sue_bud.idearoot._kids[casa_str].problem_bool == x_problem_bool


def test_BudUnit_edit_idea_attr_RaisesErrorWhen_healerlink_healer_names_DoNotExist():
    # ESTABLISH
    ...
    day_str = "day_range"
    day_idea = ideaunit_shop(day_str, begin=44, close=110)
...
def test_BudUnit_set_idea_MustReorderKidsDictToBeAlphabetical():
    ...
    # THEN
    assert idea_list[0].idea_tag == casa_str
...
def test_BudUnit_set_idea_adoptee_RaisesErrorIfAdopteeIdeaDoesNotHaveCorrectParent():
    ...
    bob_bud.set_l1_idea(ideaunit_shop(sports_str))
    swim_str = "swim"
    bob_bud.set_idea(ideaunit_shop(swim_str), parent_way=sports_way)
    ...
    with pytest_raises(Exception) as excinfo:
        bob_bud.set_idea(
            idea_kid=ideaunit_shop(summer_str),
            parent_way=sports_way,
            adoptees=[swim_str, hike_str],
        )
    assert str(excinfo.value) == f"get_idea_obj failed. no idea at '{hike_way}'"
...
def test_BudUnit_set_idea_adoptee_CorrectlyAddsAdoptee():
    ...
    bob_bud.set_idea(ideaunit_shop(swim_str), parent_way=sports_way)
    hike_str = "hike"
    bob_bud.set_idea(ideaunit_shop(hike_str), parent_way=sports_way)

    sports_swim_way = bob_bud.make_way(sports_way, swim_str)
    sports_hike_way = bob_bud.make_way(sports_way, hike_str)
    assert bob_bud.idea_exists(sports_swim_way)
    assert bob_bud.idea_exists(sports_hike_way)
    summer_str = "summer"
    ...
    assert bob_bud.idea_exists(summer_swim_way) is False
    assert bob_bud.idea_exists(summer_hike_way) is False
    ...
    # THEN
    summer_idea = bob_bud.get_idea_obj(summer_way)
    print(f"{summer_idea._kids.keys()=}")
    assert bob_bud.idea_exists(summer_swim_way)
    assert bob_bud.idea_exists(summer_hike_way)
    assert bob_bud.idea_exists(sports_swim_way) is False
    assert bob_bud.idea_exists(sports_hike_way) is False
...
def test_BudUnit_set_idea_bundling_SetsNewParentWithMassEqualToSumOfAdoptedIdeas():
    ...
    sports_str = "sports"
    sports_way = bob_bud.make_l1_way(sports_str)
    bob_bud.set_l1_idea(ideaunit_shop(sports_str, mass=2))
    swim_str = "swim"
    swim_mass = 3
    bob_bud.set_idea(ideaunit_shop(swim_str, mass=swim_mass), sports_way)
    hike_str = "hike"
    hike_mass = 5
    bob_bud.set_idea(ideaunit_shop(hike_str, mass=hike_mass), sports_way)
    bball_str = "bball"
    bball_mass = 7
    bob_bud.set_idea(ideaunit_shop(bball_str, mass=bball_mass), sports_way)

    sports_swim_way = bob_bud.make_way(sports_way, swim_str)
    sports_hike_way = bob_bud.make_way(sports_way, hike_str)
    sports_bball_way = bob_bud.make_way(sports_way, bball_str)
    assert bob_bud.get_idea_obj(sports_swim_way).mass == swim_mass
    assert bob_bud.get_idea_obj(sports_hike_way).mass == hike_mass
    assert bob_bud.get_idea_obj(sports_bball_way).mass == bball_mass
    summer_str = "summer"
    summer_way = bob_bud.make_way(sports_way, summer_str)
    summer_swim_way = bob_bud.make_way(summer_way, swim_str)
    summer_hike_way = bob_bud.make_way(summer_way, hike_str)
    ...
    assert bob_bud.idea_exists(summer_swim_way) is False
    assert bob_bud.idea_exists(summer_hike_way) is False
    assert bob_bud.idea_exists(summer_bball_way) is False
    ...
    # THEN
    assert bob_bud.get_idea_obj(summer_way).mass == swim_mass + hike_mass
    assert bob_bud.get_idea_obj(summer_swim_way).mass == swim_mass
    assert bob_bud.get_idea_obj(summer_hike_way).mass == hike_mass
    assert bob_bud.idea_exists(summer_bball_way) is False
    assert bob_bud.idea_exists(sports_swim_way) is False
    assert bob_bud.idea_exists(sports_hike_way) is False
    assert bob_bud.idea_exists(sports_bball_way)
...
def test_BudUnit_del_idea_obj_DeletingBundledIdeaReturnsIdeasToOriginalState():
    ...
    # WHEN
    bob_bud.del_idea_obj(way=summer_way, del_children=False)

    # THEN
    sports_swim_idea = bob_bud.get_idea_obj(sports_swim_way)
    sports_hike_idea = bob_bud.get_idea_obj(sports_hike_way)
    sports_bball_idea = bob_bud.get_idea_obj(sports_bball_way)
...
def test_BudUnit_edit_idea_attr_DeletesIdeaUnit_awardlinks():
    ...
    assert len(yao_bud.idearoot._kids[swim_str].awardlinks) == 3
    ...
    # THEN
    swim_idea = yao_bud.get_idea_obj(swim_way)
    print(f"{swim_idea.idea_tag=}")
    print(f"{swim_idea.awardlinks=}")
    print(f"{swim_idea._awardheirs=}")

    assert len(yao_bud.idearoot._kids[swim_str].awardlinks) == 2


def test_BudUnit__get_filtered_awardlinks_idea_CorrectlyRemovesAcct_awardlinks():
    # ESTABLISH
    bob_str = "Bob"
    ...
    example_bud.set_l1_idea(ideaunit_shop(sports_str))
    example_bud.edit_idea_attr(sports_way, awardlink=awardlink_shop(run_str))
    example_bud.edit_idea_attr(sports_way, awardlink=awardlink_shop(hike_str))
    example_bud_sports_idea = example_bud.get_idea_obj(sports_way)
    assert len(example_bud_sports_idea.awardlinks) == 2
    bob_bud = budunit_shop(bob_str)
    bob_bud.add_acctunit(xia_str)
    bob_bud.get_acct(xia_str).add_membership(run_str)
    print(f"{example_bud_sports_idea.awardlinks=}")

    # WHEN
    cleaned_idea = bob_bud._get_filtered_awardlinks_idea(example_bud_sports_idea)

    # THEN
    assert len(cleaned_idea.awardlinks) == 1
    assert list(cleaned_idea.awardlinks.keys()) == [run_str]


def test_BudUnit__get_filtered_awardlinks_idea_CorrectlyRemovesGroup_awardlink():
    # ESTABLISH
    bob_str = "Bob"
    example_bud = budunit_shop(bob_str)
    xia_str = "Xia"
    ...
    example_bud.set_l1_idea(ideaunit_shop(swim_str))
    example_bud.edit_idea_attr(swim_way, awardlink=awardlink_shop(xia_str))
    example_bud.edit_idea_attr(swim_way, awardlink=awardlink_shop(zoa_str))
    example_bud_swim_idea = example_bud.get_idea_obj(swim_way)
    assert len(example_bud_swim_idea.awardlinks) == 2
    bob_bud = budunit_shop(bob_str)
    bob_bud.add_acctunit(xia_str)

    # WHEN
    cleaned_idea = bob_bud._get_filtered_awardlinks_idea(example_bud_swim_idea)

    # THEN
    assert len(cleaned_idea.awardlinks) == 1
    assert list(cleaned_idea.awardlinks.keys()) == [xia_str]
...
def test_BudUnit_set_idea_CorrectlyCleansIdea_awardlinks():
    ...
    swim_str = "swim"
    swim_way = example_bud.make_l1_way(swim_str)
    example_bud.set_l1_idea(ideaunit_shop(casa_str))
    example_bud.set_l1_idea(ideaunit_shop(swim_str))
    example_bud.edit_idea_attr(swim_way, awardlink=awardlink_shop(xia_str))
    example_bud.edit_idea_attr(swim_way, awardlink=awardlink_shop(zoa_str))
    example_bud_swim_idea = example_bud.get_idea_obj(swim_way)
    ...
    bob_bud = budunit_shop(bob_str)
    bob_bud.add_acctunit(xia_str)
    bob_bud.set_l1_idea(example_bud_swim_idea, create_missing_ideas=False)

    # THEN
    bob_bud_swim_idea = bob_bud.get_idea_obj(swim_way)
    assert len(bob_bud_swim_idea.awardlinks) == 1
    assert list(bob_bud_swim_idea.awardlinks.keys()) == [xia_str]
...
def test_BudUnit_get_idea_obj_ReturnsIdea():
    ...
    # WHEN
    brazil_idea = sue_bud.get_idea_obj(way=brazil_way)

    # THEN
    assert brazil_idea is not None
    assert brazil_idea.idea_tag == brazil_str

    # WHEN
    week_str = "weekdays"
    week_way = sue_bud.make_l1_way(week_str)
    week_idea = sue_bud.get_idea_obj(way=week_way)

    # THEN
    assert week_idea is not None
    assert week_idea.idea_tag == week_str

    # WHEN
    root_idea = sue_bud.get_idea_obj(to_way(sue_bud.fisc_tag))

    # THEN
    assert root_idea is not None
    assert root_idea.idea_tag == sue_bud.fisc_tag

    # WHEN / THEN
    bobdylan_str = "bobdylan"
    wrong_way = sue_bud.make_l1_way(bobdylan_str)
    with pytest_raises(Exception) as excinfo:
        sue_bud.get_idea_obj(way=wrong_way)
    assert str(excinfo.value) == f"get_idea_obj failed. no idea at '{wrong_way}'"
...
def test_BudUnit_idea_exists_ReturnsCorrectBool():
    ...
    assert sue_bud.idea_exists(to_way(sue_bud.fisc_tag))
    assert sue_bud.idea_exists(cat_way)
    assert sue_bud.idea_exists(week_way)
    assert sue_bud.idea_exists(casa_way)
    assert sue_bud.idea_exists(nation_way)
    assert sue_bud.idea_exists(sun_way)
    assert sue_bud.idea_exists(mon_way)
    assert sue_bud.idea_exists(tue_way)
    assert sue_bud.idea_exists(wed_way)
    assert sue_bud.idea_exists(thu_way)
    assert sue_bud.idea_exists(fri_way)
    assert sue_bud.idea_exists(sat_way)
    assert sue_bud.idea_exists(usa_way)
    assert sue_bud.idea_exists(france_way)
    assert sue_bud.idea_exists(brazil_way)
    assert sue_bud.idea_exists(texas_way)
    assert sue_bud.idea_exists(oregon_way)
    assert sue_bud.idea_exists(to_way("B")) is False
    assert sue_bud.idea_exists(sports_way) is False
    assert sue_bud.idea_exists(swim_way) is False
    assert sue_bud.idea_exists(idaho_way) is False
    assert sue_bud.idea_exists(japan_way) is False
...
def test_BudUnit_set_offtrack_fund_ReturnsObj():
    ...
    casa_idea.parent_way = bob_budunit.fisc_tag
    week_idea.parent_way = bob_budunit.fisc_tag
    wed_idea.parent_way = week_way
    bob_budunit.set_l1_idea(casa_idea)
    bob_budunit.set_l1_idea(week_idea)
    bob_budunit.set_idea(wed_idea, week_way)

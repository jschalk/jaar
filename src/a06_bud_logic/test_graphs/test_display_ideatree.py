...
def test_display_ideatree_Scenario0(graphics_bool):
    ...
    # a_bud = budunit_v001()
    a_bud.settle_bud()
    print(f"Bud {a_bud.fisc_label}: Labels ({len(a_bud._idea_dict)})")
...
def test_BudUnit_fund_flow(graphics_bool):
    ...
    sue_bud.add_idea(casa_way, mass=30)
    sue_bud.add_idea(cat_way, mass=30)
    sue_bud.add_idea(hun_n_way, mass=30)
    sue_bud.add_idea(hun_y_way, mass=30)
    sue_bud.add_idea(clean_way, mass=30)
    sue_bud.add_idea(sweep_way, mass=30, pledge=True)
    sue_bud.add_idea(dish_way, mass=30, pledge=True)
    dinner_str = "cat have dinner"
    dinner_way = sue_bud.make_l1_way(dinner_str)
    sue_bud.add_idea(dinner_way, mass=30, pledge=True)

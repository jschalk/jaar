...
def test_display_itemtree_Scenario0(graphics_bool):
    ...
    # a_bud = budunit_v001()
    a_bud.settle_bud()
    print(f"Bud {a_bud.fisc_tag}: Tags ({len(a_bud._item_dict)})")
...
def test_BudUnit_fund_flow(graphics_bool):
    ...
    sue_bud.add_item(casa_way, mass=30)
    sue_bud.add_item(cat_way, mass=30)
    sue_bud.add_item(hun_n_way, mass=30)
    sue_bud.add_item(hun_y_way, mass=30)
    sue_bud.add_item(clean_way, mass=30)
    sue_bud.add_item(sweep_way, mass=30, pledge=True)
    sue_bud.add_item(dish_way, mass=30, pledge=True)
    dinner_str = "cat have dinner"
    dinner_way = sue_bud.make_l1_way(dinner_str)
    sue_bud.add_item(dinner_way, mass=30, pledge=True)

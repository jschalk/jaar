...
class BudUnit:
    ...
    def edit_concept_label(self, old_way: WayTerm, new_concept_label: LabelTerm):
        ...
        if self.concept_exists(old_way) is False:
            raise InvalidBudException(f"Concept {old_way=} does not exist")
        ...
        if healerlink is not None:
            for x_healer_name in healerlink._healer_names:
                if self.get_acctunit_group_titles_dict().get(x_healer_name) is None:
                    exception_str = f"Concept cannot edit healerlink because group_title '{x_healer_name}' does not exist as group in Bud"
    ...
    def _clear_concept_dict_and_bud_obj_settle_attrs(self):
    ...
    def settle_bud(self, keep_exceptions: bool = False):
        self._clear_concept_dict_and_bud_obj_settle_attrs()
        self._set_concept_dict()
...
def create_conceptroot_from_bud_dict(x_bud: BudUnit, bud_dict: dict):
...
def create_conceptroot_kids_from_dict(x_bud: BudUnit, conceptroot_dict: dict):
    ...
    while to_evaluate_concept_dicts != []:
        ...
        for kid_dict in get_obj_from_concept_dict(concept_dict, "_kids").values():
            ...
            kid_dict[parent_way_str] = x_bud.make_way(parent_way, kid_concept_label)
        ...
        x_bud.set_concept(x_conceptkid, parent_way=concept_dict[parent_way_str])

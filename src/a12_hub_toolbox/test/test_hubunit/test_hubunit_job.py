...
from src.a12_hub_toolbox.hub_path import create_bank_dir_path, create_job_path
...
def test_HubUnit_initialize_job_file_CorrectlySavesFile(env_dir_setup_cleanup):
    ...
    sue_str = "Sue"
    sue_hubunit = hubunit_shop(bank_mstr_dir, root_label(), sue_str, None)
    sue_plan = planunit_shop(sue_str, root_label())
    assert job_file_exists(bank_mstr_dir, root_label(), sue_str) is False

    # WHEN
    sue_hubunit.initialize_job_file(sue_plan)

    # THEN
    job = open_job_file(bank_mstr_dir, root_label(), sue_str)
    assert job.bank_label == root_label()
...
def test_HubUnit_initialize_job_file_CorrectlyDoesNotOverwrite(
    ...
    sue_str = "Sue"
    bank_mstr_dir = env_dir()
    sue_bank_dir = create_path(bank_mstr_dir, root_label())
    sue_fund_pool = 50000
    sue_fund_iota = 5
    sue_bit = 25
    ...
    sue_hubunit.initialize_job_file(sue_plan)
    assert job_file_exists(bank_mstr_dir, root_label(), sue_str)
    sue_job_path = create_job_path(bank_mstr_dir, root_label(), sue_str)
...
def test_HubUnit_initialize_job_file_CreatesDirsAndFiles(env_dir_setup_cleanup):
    ...
    bank_mstr_dir = env_dir()
    sue_hubunit = hubunit_shop(bank_mstr_dir, root_label(), sue_str, None)
    bank_dir = create_bank_dir_path(bank_mstr_dir, root_label())
    delete_dir(bank_dir)
    assert job_file_exists(bank_mstr_dir, root_label(), sue_str) is False

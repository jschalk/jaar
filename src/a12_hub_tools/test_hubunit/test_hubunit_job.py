...
def test_HubUnit_initialize_job_file_CorrectlySavesFile(env_dir_setup_cleanup):
    ...
    sue_str = "Sue"
    sue_hubunit = hubunit_shop(fisc_mstr_dir, root_tag(), sue_str, None)
    sue_bud = budunit_shop(sue_str, root_tag())
    assert job_file_exists(fisc_mstr_dir, root_tag(), sue_str) is False

    # WHEN
    sue_hubunit.initialize_job_file(sue_bud)

    # THEN
    job = open_job_file(fisc_mstr_dir, root_tag(), sue_str)
    assert job.fisc_tag == root_tag()
...
def test_HubUnit_initialize_job_file_CorrectlyDoesNotOverwrite(
    ...
    sue_str = "Sue"
    fisc_mstr_dir = env_dir()
    sue_fisc_dir = create_path(fisc_mstr_dir, root_tag())
    sue_fund_pool = 50000
    sue_fund_coin = 5
    sue_bit = 25
    ...
    sue_hubunit.initialize_job_file(sue_bud)
    assert job_file_exists(fisc_mstr_dir, root_tag(), sue_str)
    sue_job_path = create_job_path(fisc_mstr_dir, root_tag(), sue_str)
...
def test_HubUnit_initialize_job_file_CreatesDirsAndFiles(env_dir_setup_cleanup):
    ...
    fisc_mstr_dir = env_dir()
    sue_hubunit = hubunit_shop(fisc_mstr_dir, root_tag(), sue_str, None)
    fisc_dir = create_fisc_dir_path(fisc_mstr_dir, root_tag())
    delete_dir(fisc_dir)
    assert job_file_exists(fisc_mstr_dir, root_tag(), sue_str) is False

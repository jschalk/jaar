...
def get_nodes_with_weighted_facts(
    nodes_facts_dict: dict[tuple[OwnerName], dict[WayUnit, FactUnit]],
    nodes_quota_ledger_dict: dict[tuple[OwnerName], dict[OwnerName, float]],
) -> dict[tuple[OwnerName], dict[WayUnit, FactUnit]]:
...
def _add_to_tuple_quota_sum(
    to_eval_temp: dict[WayUnit, dict[tuple[WayUnit, WayUnit, float, float], float]],
    child_fact: FactUnit,
    child_quota: float,
):

...
def test_create_empty_bud_from_bud_ReturnsObj():
    ...
    yao_gut.set_l1_idea(ideaunit_shop("Iowa"))
    zia_str = "Zia"
    ...
    assert yao_empty_plan.fisc_tag == yao_gut.fisc_tag
...
def test_create_listen_basis_ReturnsObj():
    ...
    assert yao_basis_plan.fisc_tag == yao_duty.fisc_tag
    ...
    assert len(yao_basis_plan._idea_dict) != len(yao_duty._idea_dict)
    assert len(yao_basis_plan._idea_dict) == 1
...
def test_get_default_job_ReturnsObj():
    ...
    assert default_job.fisc_tag == sue_budunit.fisc_tag
    assert default_job.fisc_tag == blue_str
    ...
    assert len(default_job._idea_dict) == 1

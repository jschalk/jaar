...
def test_get_believer_dimens_ReturnsObj():
    ...
    assert believer_acctunit_str() in get_believer_dimens()
...
def test_get_all_believer_dimen_keys_ReturnsObj():
    ...
    # THEN
    assert not all_believer_dimen_keys.isdisjoint({"acct_name"})
...
def test_get_atom_config_dict_EveryCrudOperationHasBelieverDeltaOrderGroup():
    ...
    # set_mog(INSERT_str(), believer_acctunit_str(), 0)
    # set_mog(INSERT_str(), believer_acct_membership_str(), 1)
    ...
    # set_mog(UPDATE_str(), believer_acctunit_str(), 9)
    # set_mog(UPDATE_str(), believer_acct_membership_str(), 10)
    ...
    # set_mog(DELETE_str(), believer_acct_membership_str(), 23)
    # set_mog(DELETE_str(), believer_acctunit_str(), 24)
    # set_mog(UPDATE_str(), believerunit_str(), 25)

    assert 0 == q_order(INSERT_str(), believer_acctunit_str())
    assert 1 == q_order(INSERT_str(), believer_acct_membership_str())
    ...
    assert 9 == q_order(UPDATE_str(), believer_acctunit_str())
    assert 10 == q_order(UPDATE_str(), believer_acct_membership_str())
    ...
    assert 23 == q_order(DELETE_str(), believer_acct_membership_str())
    assert 24 == q_order(DELETE_str(), believer_acctunit_str())
...
def test_get_atom_config_dict_CheckEachDimenHasCorrectArgCount():
    ...
    assert _get_atom_config_jkeys_len(believer_acctunit_str()) == 1
    assert _get_atom_config_jkeys_len(believer_acct_membership_str()) == 2
    ...
    assert _get_atom_config_jvalues_len(believer_acctunit_str()) == 2
    assert _get_atom_config_jvalues_len(believer_acct_membership_str()) == 2
...
def test_get_sorted_jkey_keys_ReturnsObj_believer_acctunit():
    # ESTABLISH
    x_dimen = believer_acctunit_str()
...
def test_get_normalized_believer_table_build_ReturnsObj():
    ...
    cat_acctunit = nx.get(believer_acctunit_str())
    cat_membership = nx.get(believer_acct_membership_str())
...
def test_get_class_type_ReturnsObj():
    # ESTABLISH / WHEN / THEN
    assert get_class_type(believer_acctunit_str(), acct_name_str()) == NameTerm_str()

...
def test_sift_atom_ReturnsObj_BelieverAtom_INSERT_believer_partnerunit():
    # ESTABLISH
    bob_str = "Bob"
    zia_str = "Zia"
    ...
    # THEN
    assert new_bob_believeratom
    assert new_bob_believeratom == bob_atom
    assert not new_zia_believeratom


def test_sift_atom_ReturnsObj_BelieverAtom_INSERT_believer_partner_membership():
    # ESTABLISH
    bob_str = "Bob"
    yao_str = "Yao"
    run_str = ";run"
    ...
    bob_run_atom = believeratom_shop(believer_partner_membership_str(), INSERT_str())
    bob_run_atom.set_arg(partner_name_str(), bob_str)
    bob_run_atom.set_arg(group_title_str(), run_str)
    yao_run_atom = believeratom_shop(believer_partner_membership_str(), INSERT_str())
    ...
    # THEN
    assert new_bob_run_believeratom
    assert new_bob_run_believeratom == bob_run_atom
    assert not new_yao_run_believeratom
...
def test_sift_atom_ReturnsObj_BelieverAtom_INSERT_believer_planunit():
    ...
    root_atom = believeratom_shop(believer_planunit_str(), INSERT_str())
    root_atom.set_arg(plan_rope_str(), root_rope)
    casa_atom = believeratom_shop(believer_planunit_str(), INSERT_str())
    casa_atom.set_arg(plan_rope_str(), casa_rope)
    clean_atom = believeratom_shop(believer_planunit_str(), INSERT_str())
    clean_atom.set_arg(plan_rope_str(), clean_rope)
    sweep_atom = believeratom_shop(believer_planunit_str(), INSERT_str())
    sweep_atom.set_arg(plan_rope_str(), sweep_rope)
    assert not sift_believeratom(sue_believer, root_atom)
    assert sift_believeratom(sue_believer, casa_atom)
    assert sift_believeratom(sue_believer, clean_atom)
    assert sift_believeratom(sue_believer, sweep_atom)
...
def test_sift_atom_ReturnsObj_BelieverAtom_INSERT_believer_plan_awardlink():
    # ESTABLISH
    sue_believer = believerunit_shop("Sue")
    casa_str = "casa"
    casa_rope = sue_believer.make_l1_rope(casa_str)
    ...
    casa_swim_atom = believeratom_shop(believer_plan_awardlink_str(), INSERT_str())
    casa_swim_atom.set_arg(plan_rope_str(), casa_rope)
    casa_swim_atom.set_arg(awardee_title_str(), swim_str)
    clean_swim_atom = believeratom_shop(believer_plan_awardlink_str(), INSERT_str())
...
def test_sift_atom_ReturnsObj_BelieverAtom_INSERT_believer_plan_reasonunit():
    # ESTABLISH
    sue_believer = believerunit_shop("Sue")
    casa_str = "casa"
    casa_rope = sue_believer.make_l1_rope(casa_str)
    ...
    casa_week_atom = believeratom_shop(believer_plan_reasonunit_str(), INSERT_str())
    casa_week_atom.set_arg(plan_rope_str(), casa_rope)
    casa_week_atom.set_arg(reason_context_str(), week_rope)
    clean_week_atom = believeratom_shop(believer_plan_reasonunit_str(), INSERT_str())
...
def test_sift_atom_ReturnsObj_BelieverAtom_INSERT_believer_plan_partyunit():
    # ESTABLISH
    sue_believer = believerunit_shop("Sue")
    casa_str = "casa"
    casa_rope = sue_believer.make_l1_rope(casa_str)
    ...
    casa_swim_atom = believeratom_shop(believer_plan_partyunit_str(), INSERT_str())
    casa_swim_atom.set_arg(plan_rope_str(), casa_rope)
    casa_swim_atom.set_arg(party_title_str(), swim_str)
    clean_swim_atom = believeratom_shop(believer_plan_partyunit_str(), INSERT_str())
    clean_swim_atom.set_arg(plan_rope_str(), clean_rope)
    ...
    # WHEN
    sue_believer.get_plan_obj(casa_rope).laborunit.set_partyunit(swim_str)
    ...
    # WHEN
    sue_believer.get_plan_obj(clean_rope).laborunit.set_partyunit(swim_str)
    # THEN
    assert not sift_believeratom(sue_believer, casa_swim_atom)
    assert not sift_believeratom(sue_believer, clean_swim_atom)
...
def test_sift_atom_ReturnsObj_BelieverAtom_INSERT_believer_plan_factunit():
    # ESTABLISH
    sue_believer = believerunit_shop("Sue")
    casa_str = "casa"
    casa_rope = sue_believer.make_l1_rope(casa_str)
    ...
    casa_week_atom = believeratom_shop(believer_plan_factunit_str(), INSERT_str())
    casa_week_atom.set_arg(plan_rope_str(), casa_rope)
    casa_week_atom.set_arg(fact_context_str(), week_rope)
    clean_week_atom = believeratom_shop(believer_plan_factunit_str(), INSERT_str())
    clean_week_atom.set_arg(plan_rope_str(), clean_rope)

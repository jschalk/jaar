from src.f04_gift.atom_config import gov_idea_str
...
def test_RiverGrade_Exists():
    ...
    # THEN
    #: Healer voice get_acct._debtit_belief (SELECT tax_due_amount FROM acct WHERE acct_name = bob_str)
    ...
    #: Healer voice get_acct._credit_belief (SELECT grant_amount FROM acct WHERE acct_name = bob_str)
...
def test_RiverGrade_get_dict_ReturnsCorrectObj():
    ...
    # THEN
    assert rivergrade_dict.get(gov_idea_str()) == yao_hubunit.gov_idea

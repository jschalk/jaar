from src.f02_bud.bud import BudUnit, budunit_shop, itemunit_shop, GovIdea
from src.f06_keep.examples.keep_env import temp_gov_idea


def get_1idea_bud() -> BudUnit:
    x_bud = budunit_shop("A")
    x_bud.set_gov_idea(temp_gov_idea())
    x_bud.settle_bud()
    return x_bud
...
def get_2idea_bud(gov_idea: GovIdea = None) -> BudUnit:
    if gov_idea is None:
        gov_idea = temp_gov_idea()
    a_str = "A"
    b_str = "B"
    x_bud = budunit_shop(owner_name=a_str)
    x_bud.set_gov_idea(gov_idea)
    item_b = itemunit_shop(b_str)
    x_bud.set_item(item_b, parent_road=temp_gov_idea())
    x_bud.settle_bud()
    return x_bud

...
class FiscUnit:
    ...
    def _set_fisc_dirs(self, in_memory_journal: bool = None):
        ...
        self._fisc_dir = create_path(fiscs_dir, self.fisc_tag)
    ...
    def get_journal_db_path(self) -> str:
        fiscs_dir = create_path(self.fisc_mstr_dir, "fiscs")
        fisc_dir = create_path(fiscs_dir, self.fisc_tag)
    ...
    def _set_all_healer_dutys(self, owner_name: OwnerName):
        x_gut = open_gut_file(self.fisc_mstr_dir, self.fisc_tag, owner_name)
        x_gut.settle_bud()
    ...
    def create_gut_file_if_none(self, owner_name: OwnerName):
        if not gut_file_exists(self.fisc_mstr_dir, self.fisc_tag, owner_name):
    ...
    def rotate_job(self, owner_name: OwnerName) -> BudUnit:
        x_job = open_job_file(self.fisc_mstr_dir, self.fisc_tag, owner_name)
        x_job.settle_bud()
        ...
        return listen_to_debtors_roll_jobs_into_job(
            self.fisc_mstr_dir, self.fisc_tag, owner_name
        )
    ...
    def get_job_file_bud(self, owner_name: OwnerName) -> BudUnit:
        return open_job_file(self.fisc_mstr_dir, self.fisc_tag, owner_name)
    ...
    def set_all_tranbook(self):
        ...
        x_tranbook = tranbook_shop(self.fisc_tag, x_tranunits)
    ...
    def _create_deal_root_cell(
        ...
        root_cell_dir = create_cell_dir_path(
            self.fisc_mstr_dir, self.fisc_tag, owner_name, deal_time, []
        )
...
def fiscunit_shop(
    ...
    x_fiscunit = FiscUnit(
        ...
        timeline=timeline,
        brokerunits={},
        cashbook=tranbook_shop(fisc_tag),
        ...
        _all_tranbook=tranbook_shop(fisc_tag),
...
def get_from_dict(fisc_dict: dict) -> FiscUnit:
    x_fisc_tag = fisc_dict.get("fisc_tag")
...
def get_from_default_path(fisc_mstr_dir: str, fisc_tag: FiscTag) -> FiscUnit:

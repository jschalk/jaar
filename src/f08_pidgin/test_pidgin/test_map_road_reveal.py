...
def test_RoadMap_reveal_inx_ReturnsObjAndSetsAttr_road_Scenario0():
    ...
    # WHEN
    gen_inx_road = road_roadmap.reveal_inx(otx_music45_str)

    # THEN
    assert gen_inx_road == otx_music45_str
    assert road_roadmap.otx_exists(otx_music45_str)
    assert road_roadmap.otx2inx_exists(otx_music45_str, otx_music45_str)
...
def test_RoadMap_reveal_inx_ReturnsObjAndSetsAttr_road_Scenario1():
    ...
    assert road_roadmap.otx_exists(otx_music45_str) is False
    assert road_roadmap.otx_exists(clean_otx_road) is False
    assert road_roadmap.otx2inx_exists(otx_music45_str, inx_music87_str) is False

    # ESTABLISH
    road_roadmap.set_otx2inx(otx_music45_str, inx_music87_str)
...
def test_RoadMap_reveal_inx_AddsMissingObjsTo_otx2inx_RoadUnit():
    ...
    x_roadmap.set_idea(otx_music45_str, inx_music87_str)
    ...
    assert x_roadmap.otx_exists(otx_music45_str) is False
    ...
    assert x_roadmap.otx2inx_exists(otx_music45_str, inx_music87_str) is False
    ...
    # WHEN
    assert x_roadmap.reveal_inx(otx_music45_str) == inx_music87_str
    print(f"{x_roadmap.ideamap.otx2inx=}")
    print(f"{x_roadmap.otx2inx=}")
    # THEN
    assert x_roadmap.otx_exists(otx_music45_str)
    assert x_roadmap.otx_exists(casa_otx_road) is False
    assert x_roadmap.otx_exists(clean_otx_road) is False
    assert x_roadmap.otx_exists(sweep_otx_road) is False

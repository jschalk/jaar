...
def test_BudUnit_set_real_id_CorrectlySetsAttr():
    ...
    # WHEN
    sue_bud.set_real_id(real_id=real_id_str)

    # THEN
    assert sue_bud._real_id == real_id_str
...
def test_bud_set_real_id_CorrectlySetsAttr():
    ...
    # WHEN
    real_id_str = "Sun"
    yao_bud.set_real_id(real_id=real_id_str)

    # THEN
    new_casa_road = yao_bud.make_l1_road(casa_str)
    swim_str = "swim"
    new_swim_road = yao_bud.make_road(new_casa_road, swim_str)
    assert yao_bud._real_id == real_id_str
    assert yao_bud._idearoot._label == real_id_str
    casa_idea = yao_bud.get_idea_obj(new_casa_road)
    assert casa_idea._parent_road == real_id_str
...
def test_bud_set_road_delimiter_CorrectlyModifies_parent_road():
    ...
    slash_casa_road = create_road(zia_real_id, casa_str, delimiter=slash_str)

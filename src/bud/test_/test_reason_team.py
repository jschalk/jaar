...
def test_TeamUnit_teamlink_exists_ReturnsCorrectObj():
    ...
    x_teamunit = teamunit_shop()
    yao_text = "Yao"
    assert x_teamunit.teamlink_exists(yao_text) is False

    # WHEN
    x_teamunit.set_teamlink(group_id=yao_text)

    # THEN
    assert x_teamunit.teamlink_exists(yao_text)
...
def test_TeamUnit_del_teamlink_CorrectlyDeletes_teamlinks_v1():
    ...
    sue_text = "Sue"
    x_teamunit.set_teamlink(group_id=yao_text)
    x_teamunit.set_teamlink(group_id=sue_text)
    assert len(x_teamunit._teamlinks) == 2

    # WHEN
    x_teamunit.del_teamlink(group_id=sue_text)
...
def test_TeamHeir_set_owner_id_team_CorrectlySetsAttribute_NonEmptyx_teamlinks_v2():
    ...
    sue_groupbox.set_membership(membership_shop(sue_text, _acct_id=sue_text))
    x_groupboxs = {yao_text: yao_groupbox, sue_text: sue_groupbox}
    x_teamlinks = {sue_text}
    x_teamheir = teamheir_shop(_teamlinks=x_teamlinks)
    assert yao_groupbox.get_membership(yao_text) is not None
    assert x_teamheir._owner_id_team is False

    # WHEN
    x_teamheir.set_owner_id_team(x_groupboxs, yao_text)
...
def test_TeamHeir_set_owner_id_team_CorrectlySetsAttribute_NonEmptyx_teamlinks_v3():
    ...
    x_teamlinks = {swim_text}
    x_teamheir = teamheir_shop(_teamlinks=x_teamlinks)
    assert x_teamheir._owner_id_team is False
    x_teamheir.set_owner_id_team(x_groupboxs, bud_owner_id=yao_text)
    assert x_teamheir._owner_id_team
...
def test_TeamHeir_set_teamlink_TeamUnitNotEmpty_ParentTeamHeirIsNone():
    ...
    swim_text = ",swim"
    x_teamunit = teamunit_shop()
    x_teamunit.set_teamlink(group_id=kent_text)
    x_teamunit.set_teamlink(group_id=swim_text)
...
def test_TeamHeir_set_teamlink_TeamUnitEmpty_ParentTeamHeirNotEmpty():
    ...
    teamunit_swim.set_teamlink(group_id=kent_text)
    teamunit_swim.set_teamlink(group_id=swim_text)
...
def test_TeamHeir_set_teamlink_TeamUnit_NotEqual_ParentTeamHeir_NonEmpty():
    ...
    parent_teamunit.set_teamlink(group_id=swim3_text)
    parent_teamheir = teamheir_shop()
    ...
    teamunit_swim2.set_teamlink(group_id=swim2_text)
    ...
    assert list(x_teamheir._teamlinks) == [swim2_text]
...
#     parent_teamunit = teamunit_shop()
#     parent_teamunit.set_teamlink(swim2_text)
#     parent_teamheir = teamheir_shop()
#     parent_teamheir.set_teamlinks(None, parent_teamunit, x_groupboxs)

#     teamunit_swim3 = teamunit_shop()
#     teamunit_swim3.set_teamlink(group_id=swim3_text)

#     # WHEN / THEN
#     x_teamheir = teamheir_shop()
#     all_parent_teamheir_accts = {yao_text, sue_text}
#     all_teamunit_accts = {yao_text, sue_text, zia_text}
...
def test_TeamUnit_get_teamlink_ReturnsCorrectObj():
    ...
    x_teamunit = teamunit_shop()
    x_teamunit.set_teamlink(climb_text)
    x_teamunit.set_teamlink(hike_text)
    x_teamunit.set_teamlink(swim_text)

    # WHEN / THEN
    assert x_teamunit.get_teamlink(hike_text) is not None
    assert x_teamunit.get_teamlink(swim_text) is not None
    assert x_teamunit.get_teamlink(run_text) is None
...
def test_TeamHeir_group_id_in_ReturnsCorrectBoolWhen_teamlinksNotEmpty():
    ...
    x_teamunit = teamunit_shop()
    x_teamunit.set_teamlink(group_id=swim_text)
    x_teamunit.set_teamlink(group_id=hike_text)
    x_teamheir = teamheir_shop()
    x_teamheir.set_teamlinks(
    ...
    play_text = ",play"
    play_dict = {play_text}
    assert swim_text in x_teamheir._teamlinks
    assert hike_text in x_teamheir._teamlinks
    print(f"{hunt_text in x_teamheir._teamlinks=}")
    assert hunt_text not in x_teamheir._teamlinks
    assert play_text not in x_teamheir._teamlinks

...
class IdeaUnit:
    ...
    def set_label(self, _label: str):
        if (
            ...
            and _label != self._bud_tribe_id
            and self._bud_tribe_id is not None
        ):
            raise Idea_root_LabelNotEmptyException(
                f"Cannot set idearoot to string different than '{self._bud_tribe_id}'"
            )
        elif self._root and self._bud_tribe_id is None:
...
def ideaunit_shop(
    ...
    _bud_tribe_id = root_label() if _bud_tribe_id is None else _bud_tribe_id
    ...
    if x_ideakid._root:
        x_ideakid.set_label(_label=_bud_tribe_id)
    else:

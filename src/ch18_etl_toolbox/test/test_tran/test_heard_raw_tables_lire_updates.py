...
def test_create_update_heard_raw_existing_inx_col_sqlstr_UpdatesTable_Scenario0_FullLireTables():
    # ESTABLISH
    bob_otx = "Bob"
    bob_inx = "Bobby"
    sue_otx = "Sue"
    ...
    with sqlite3_connect(":memory:") as db_conn:
        ...
        face_name_inx_count_sql = f"SELECT COUNT(*) FROM {blrawar_h_raw_put_tablename} WHERE {face_name_str()}_inx IS NOT NULL"
        assert cursor.execute(face_name_inx_count_sql).fetchone()[0] == 0

        # WHEN
        update_sqlstr = create_update_heard_raw_existing_inx_col_sqlstr(
            "name", blrawar_h_raw_put_tablename, face_name_str()
        )
        print(update_sqlstr)
        ...
        select_face_name_only_sqlstr = f"""SELECT {event_int_str()}, {face_name_str()}_otx, {face_name_str()}_inx FROM {blrawar_h_raw_put_tablename}"""
...
def test_create_update_heard_raw_existing_inx_col_sqlstr_UpdatesTable_Scenario1_PartialLireTables():
    # ESTABLISH
    bob_otx = "Bob"
    bob_inx = "Bobby"
    sue_otx = "Sue"
    ...
    with sqlite3_connect(":memory:") as db_conn:
        ...
        rows = cursor.fetchall()
        print(rows)
        # event5 does not link to event7 lire record's
        assert rows == [
...
def test_create_update_heard_raw_existing_inx_col_sqlstr_UpdatesTable_Scenario2_Different_event_int_LireMappings():
    # ESTABLISH
    sue_otx = "Sue"
    sue_inx = "Suzy"
...
def test_create_update_heard_raw_empty_inx_col_sqlstr_UpdatesTable_Scenario0_EmptyLireTables():
    # ESTABLISH
    bob_otx = "Bob"
    bob_inx = "Bobby"
    sue_otx = "Sue"
    ...
    with sqlite3_connect(":memory:") as db_conn:
        cursor = db_conn.cursor()
        create_sound_and_heard_tables(cursor)
        lirname_s_vld_tablename = prime_tbl(lire_name_str(), "s", "vld")
        print(f"{lirname_s_vld_tablename=}")
        print(f"{get_table_columns(cursor, lirname_s_vld_tablename)=}")

        blrawar_dimen = belief_plan_awardunit_str()
        blrawar_h_raw_put_tablename = prime_tbl(blrawar_dimen, "h", "raw", "put")
        print(f"{get_table_columns(cursor, blrawar_h_raw_put_tablename)=}")
        insert_face_name_only_sqlstr = f"""INSERT INTO {blrawar_h_raw_put_tablename} ({event_int_str()}, {face_name_str()}_otx, {face_name_str()}_inx)
VALUES
  ({event1}, '{sue_otx}', '{sue_inx}')
, ({event2}, '{yao_otx}', NULL)
, ({event5}, '{sue_otx}', NULL)
...
"""
        ...
        update_sqlstr = create_update_heard_raw_empty_inx_col_sqlstr(
            blrawar_h_raw_put_tablename, face_name_str()
        )
        print(update_sqlstr)
        cursor.execute(update_sqlstr)

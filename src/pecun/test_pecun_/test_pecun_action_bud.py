...
def test_PecunUnit_generate_action_bud_Sets_action_BudFile(env_dir_setup_cleanup):
    # ESTABLISH
    music_str = "Music"
    music_pecun = pecununit_shop(music_str, get_test_pecuns_dir(), True)
    sue_str = "Sue"
    sue_hubunit = hubunit_shop(None, music_str, sue_str, None)
    x_sue_action_path = f"{music_pecun._owners_dir}/{sue_str}/action/{sue_str}.json"
    ...
    # WHEN
    sue_action = music_pecun.generate_action_bud(sue_str)

    # THEN
    example_bud = budunit_shop(sue_str, music_str)
    assert sue_action._pecun_id == example_bud._pecun_id
    assert sue_action._owner_id == example_bud._owner_id


def test_PecunUnit_generate_action_bud_ReturnsRegeneratedObj(env_dir_setup_cleanup):
    # ESTABLISH
    music_pecun = pecununit_shop("music", get_test_pecuns_dir(), True)
    sue_str = "Sue"
    music_pecun.init_owner_econs(sue_str)
    sue_hubunit = hubunit_shop(
        music_pecun.pecuns_dir, music_pecun.pecun_id, sue_str, None
    )
    ...
    # WHEN
    after_sue_bud = music_pecun.generate_action_bud(sue_str)

    # THEN method should wipe over action bud
    assert after_sue_bud.acct_exists(bob_str) is False


def test_PecunUnit_generate_action_bud_SetsCorrectFileWithout_healerlink(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    music_pecun = pecununit_shop("music", get_test_pecuns_dir(), True)
    bob_str = "Bob"
    music_pecun.init_owner_econs(bob_str)
    bob_hubunit = hubunit_shop(
        music_pecun.pecuns_dir, music_pecun.pecun_id, bob_str, None
    )
    before_bob_action_bud = music_pecun.generate_action_bud(bob_str)
    sue_str = "Sue"
    ...
    # WHEN
    after_bob_action_bud = music_pecun.generate_action_bud(bob_str)

    # THEN
    assert after_bob_action_bud.acct_exists(sue_str)


def test_PecunUnit_generate_action_bud_SetsFileWith_healerlink(env_dir_setup_cleanup):
    # ESTABLISH
    music_pecun = pecununit_shop("music", get_test_pecuns_dir(), True)
...
def test_PecunUnit_generate_all_action_buds_SetsCorrectFiles(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    music_pecun = pecununit_shop("music", get_test_pecuns_dir(), True)
    ...
    bob_hubunit = hubunit_shop(pecuns_dir, music_pecun.pecun_id, bob_str, None)
    music_pecun.init_owner_econs(sue_str)
    sue_hubunit = hubunit_shop(pecuns_dir, music_pecun.pecun_id, sue_str, None)
    bob_voice_bud = music_pecun.generate_action_bud(bob_str)
    sue_voice_bud = music_pecun.generate_action_bud(sue_str)
    ...
    before_bob_action_bud = music_pecun.get_action_file_bud(bob_str)
    before_sue_action_bud = music_pecun.get_action_file_bud(sue_str)
    ...
    # THEN
    after_bob_action_bud = music_pecun.get_action_file_bud(bob_str)
    after_sue_action_bud = music_pecun.get_action_file_bud(sue_str)
    assert after_bob_action_bud.acct_exists(bob_str)

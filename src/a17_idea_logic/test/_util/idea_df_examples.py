...
def get_ex1_br00000_df() -> DataFrame:
    ...
    c400_number,coin_label,fund_iota,monthday_distortion,penny,respect_bit,knot,timeline_label,yr1_jan1_offset
    """
    x_df = DataFrame(
        columns=[
            c400_number_str(),
...
def get_ex1_br00003_df() -> DataFrame:
    ...
    cumulative_minute,coin_label,hour_label"""
    x_df = DataFrame(columns=["coin_label", "hour_label", "cumulative_minute"])
...
def get_ex1_br00004_df() -> DataFrame:
    ...
    cumulative_day,coin_label,month_label"""
    x_df = DataFrame(columns=["coin_label", "month_label", "cumulative_day"])
...
def get_ex1_br00005_df() -> DataFrame:
    ...
    coin_label,weekday_label,weekday_order"""
    x_df = DataFrame(
        columns=[coin_label_str(), weekday_label_str(), weekday_order_str()]
    )
...
def get_ex2_br00000_df() -> DataFrame:
    """idea_format_00000_coinunit_v0_0_0
    c400_number,coin_label,fund_iota,monthday_distortion,penny,respect_bit,knot,timeline_label,yr1_jan1_offset,job_listen_rotations
    """
    x_df = DataFrame(
        columns=[
            c400_number_str(),
...
def get_ex2_br00005_df() -> DataFrame:
    """idea_format_00005_coin_timeline_weekday_v0_0_0
    coin_label,weekday_label,weekday_order"""
    x_df = DataFrame(columns=["coin_label", "weekday_label", "weekday_order"])
    x_df.loc[0] = [AMY23_STR, "Wednesday", 0]
    x_df.loc[1] = [AMY23_STR, "Thursday", 1]
    x_df.loc[2] = [AMY23_STR, "Friday", 2]
    x_df.loc[3] = [AMY23_STR, "Saturday", 3]
...
#     coin_label,offi_time,_offi_time_maxt"""
#     x_df = DataFrame(columns=["coin_label", "offi_time", "_offi_time_max"])

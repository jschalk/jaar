...
def test_get_idea_into_dimen_raw_query_ReturnsObj_Scenario0_believer_plan_partyunit():
    ...
    with sqlite3_connect(":memory:") as conn:
        ...
        # THEN
        columns_str = (
            "event_int, face_name, coin_label, believer_name, plan_rope, party_title"
        )
...
WHERE event_int IS NOT NULL AND face_name IS NOT NULL AND coin_label IS NOT NULL AND believer_name IS NOT NULL AND plan_rope IS NOT NULL AND party_title IS NOT NULL
...
def test_get_idea_into_dimen_raw_query_ReturnsObj_Scenario1_believer_partnerunit():
    ...
    with sqlite3_connect(":memory:") as conn:
        ...
        # THEN
        columns_str = "event_int, face_name, coin_label, believer_name, partner_name, partner_cred_points, partner_debt_points"
        expected_sqlstr = f"""INSERT INTO {blrpern_cat}_raw (idea_number, {columns_str})
SELECT '{idea_number}' as idea_number, {columns_str}
FROM {idea_number}_raw
WHERE event_int IS NOT NULL AND face_name IS NOT NULL AND coin_label IS NOT NULL AND believer_name IS NOT NULL AND partner_name IS NOT NULL
GROUP BY {columns_str}
;
"""
        print("")
...
def test_get_idea_into_dimen_raw_query_ReturnsObj_Scenario2_believer_partnerunit():
    ...
    with sqlite3_connect(":memory:") as conn:
        ...
        # THEN
        columns_str = "event_int, face_name, coin_label, believer_name, partner_name, partner_cred_points"
        expected_sqlstr = f"""INSERT INTO {blrpern_cat}_raw (idea_number, {columns_str})
SELECT '{idea_number}' as idea_number, {columns_str}
FROM {idea_number}_raw
WHERE event_int IS NOT NULL AND face_name IS NOT NULL AND coin_label IS NOT NULL AND believer_name IS NOT NULL AND partner_name IS NOT NULL

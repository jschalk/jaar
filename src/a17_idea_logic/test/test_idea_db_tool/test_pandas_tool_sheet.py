...
def test_append_df_to_excel_CreatesSheet(env_dir_setup_cleanup):
    # ESTABLISH
    test_file = create_path(idea_bank_mstr_dir(), "test.xlsx")
    append_data = {
        "Name": ["Alice", "Bob"],
        "Age": [25, 30],
...
def test_if_nan_return_None_ReturnsObj(env_dir_setup_cleanup):
    ...
    ex1_path = create_path(idea_bank_mstr_dir(), ex1_filename)
...
def test_update_all_face_name_event_int_columns_Scenario0_UpdatesValidSheet(
    ...
    # ESTABLISH
    excel_path = create_path(idea_bank_mstr_dir(), "test_excel.xlsx")
    set_dir(idea_bank_mstr_dir())
    yao_str = "Yao"
    event3 = 3

...
def test_load_idea_csv_Arg_idea_format_00021_plan_acctunit_v0_0_0_csvTo_job(
    ...
    csv_example_path = create_path(bank_mstr_dir, name_filename)
    print(f"{csv_example_path}")
    save_idea_csv(j1_ideaname, sue_planunit, bank_mstr_dir, name_filename)
    # Popen BankUnit and confirm gut PlanUnit does not exist
    assert not gut_file_exists(bank_mstr_dir, accord_bank_label, sue_str)

    # WHEN
    load_idea_csv(bank_mstr_dir, idea_examples_dir(), name_filename)

    # THEN
    # assert gut Planunit now exists
    assert gut_file_exists(idea_examples_dir(), accord_bank_label, sue_str)
    # assert gut Planunit acctunit now exists
    sue_gut = open_gut_file(idea_examples_dir(), accord_bank_label, sue_str)

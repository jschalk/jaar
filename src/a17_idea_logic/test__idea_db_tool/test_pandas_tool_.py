from src.a00_data_toolboxs.file_toolbox import open_file, create_path, get_dir_file_strs
...
def test_cochlea_raw_str_ReturnsObj():
    # ESTABLISH / WHEN / THEN
    assert cochlea_raw_str() == "cochlea_raw"
    assert cochlea_agg_str() == "cochlea_agg"
    assert cochlea_valid_str() == "cochlea_valid"


def test_get_cochlea_raw_grouping_with_all_values_equal_df_ReturnsObj_Scenario0_EmptyDataframe():
    # ESTABLISH
    df1 = DataFrame([[]], columns=[])
    groupby_list = [group_label_str(), acct_name_str()]

    # WHEN
    groupby_dataframe = get_cochlea_raw_grouping_with_all_values_equal_df(
...
def test_get_cochlea_raw_grouping_with_all_values_equal_df_ReturnsObj_Scenario1_GroupBySingleColumn():
    # ESTABLISH
...
def test_get_cochlea_raw_grouping_with_all_values_equal_df_ReturnsObj_Scenario2_GroupByExtraColumns():
    # ESTABLISH
    df_columns = [group_label_str(), credor_respect_str()]
    before_df_values = [["AA0", "BB0"], ["AA0", "BB0"]]
    df1 = DataFrame(before_df_values, columns=df_columns)
...
def test_get_cochlea_raw_grouping_with_all_values_equal_df_ReturnsObj_Scenario3_GroupByExtraColumns():
    # ESTABLISH
...
def test_get_cochlea_raw_grouping_with_all_values_equal_df_ReturnsObj_Scenario4_GroupByExtraColumns():
    # ESTABLISH
    df_columns = [group_label_str(), credor_respect_str(), "column3"]
    before_df_values = [
        ["AA0", "BB0", "CC0"],

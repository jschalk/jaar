...
def test_get_idea_elements_sort_order_ReturnsObj():
    ...
    assert table_sorting_priority[2] == "pidgin_event_int"
    assert table_sorting_priority[3] == event_int_str()
    assert table_sorting_priority[4] == face_name_str()
    assert table_sorting_priority[5] == fisc_tag_str()
    assert table_sorting_priority[6] == timeline_tag_str()
    assert table_sorting_priority[7] == offi_time_str()
    assert table_sorting_priority[8] == c400_number_str()
    assert table_sorting_priority[9] == yr1_jan1_offset_str()
    assert table_sorting_priority[10] == monthday_distortion_str()
    assert table_sorting_priority[11] == cumlative_day_str()
    assert table_sorting_priority[12] == month_tag_str()
    assert table_sorting_priority[13] == cumlative_minute_str()
    assert table_sorting_priority[14] == hour_tag_str()
    assert table_sorting_priority[15] == weekday_order_str()
    assert table_sorting_priority[16] == weekday_tag_str()
    assert table_sorting_priority[17] == owner_name_str()
    assert table_sorting_priority[18] == get_delete_key_name(owner_name_str())
    assert table_sorting_priority[19] == acct_name_str()
    assert table_sorting_priority[20] == get_delete_key_name(acct_name_str())
    assert table_sorting_priority[21] == group_label_str()
    assert table_sorting_priority[22] == get_delete_key_name(group_label_str())
    assert table_sorting_priority[23] == parent_road_str()
    assert table_sorting_priority[24] == get_delete_key_name(parent_road_str())
    assert table_sorting_priority[25] == item_tag_str()
    assert table_sorting_priority[26] == get_delete_key_name(item_tag_str())
    assert table_sorting_priority[27] == road_str()
    assert table_sorting_priority[28] == get_delete_key_name(road_str())
    assert table_sorting_priority[29] == base_str()
    assert table_sorting_priority[30] == get_delete_key_name(base_str())
    assert table_sorting_priority[31] == f"{base_str()}_EXCISE"
    assert table_sorting_priority[32] == "need"
    assert table_sorting_priority[33] == get_delete_key_name("need")
    assert table_sorting_priority[34] == "pick"
    assert table_sorting_priority[35] == team_title_str()
    assert table_sorting_priority[36] == get_delete_key_name(team_title_str())
    assert table_sorting_priority[37] == awardee_title_str()
    assert table_sorting_priority[38] == get_delete_key_name(awardee_title_str())
    assert table_sorting_priority[39] == healer_name_str()
    assert table_sorting_priority[40] == get_delete_key_name(healer_name_str())
    assert table_sorting_priority[41] == deal_time_str()
    assert table_sorting_priority[42] == tran_time_str()
    assert table_sorting_priority[43] == begin_str()
    assert table_sorting_priority[44] == close_str()
    assert table_sorting_priority[45] == addin_str()
    assert table_sorting_priority[46] == numor_str()
    assert table_sorting_priority[47] == denom_str()
    assert table_sorting_priority[48] == morph_str()
    assert table_sorting_priority[49] == gogo_want_str()
    assert table_sorting_priority[50] == stop_want_str()
    assert table_sorting_priority[51] == base_item_active_requisite_str()
    assert table_sorting_priority[52] == credit_belief_str()
    assert table_sorting_priority[53] == debtit_belief_str()
    assert table_sorting_priority[54] == credit_vote_str()
    assert table_sorting_priority[55] == debtit_vote_str()
    assert table_sorting_priority[56] == credor_respect_str()
    assert table_sorting_priority[57] == debtor_respect_str()
    assert table_sorting_priority[58] == fopen_str()
    assert table_sorting_priority[59] == fnigh_str()
    assert table_sorting_priority[60] == "fund_pool"
    assert table_sorting_priority[61] == give_force_str()
    assert table_sorting_priority[62] == mass_str()
    assert table_sorting_priority[63] == "max_tree_traverse"
    assert table_sorting_priority[64] == "nigh"
    assert table_sorting_priority[65] == "open"
    assert table_sorting_priority[66] == "divisor"
    assert table_sorting_priority[67] == pledge_str()
    assert table_sorting_priority[68] == "problem_bool"
    assert table_sorting_priority[69] == take_force_str()
    assert table_sorting_priority[70] == "tally"
    assert table_sorting_priority[71] == fund_coin_str()
    assert table_sorting_priority[72] == penny_str()
    assert table_sorting_priority[73] == respect_bit_str()
    assert table_sorting_priority[74] == amount_str()
    assert table_sorting_priority[75] == otx_tag_str()
    assert table_sorting_priority[76] == inx_tag_str()
    assert table_sorting_priority[77] == otx_road_str()
    assert table_sorting_priority[78] == inx_road_str()
    assert table_sorting_priority[79] == otx_name_str()
    assert table_sorting_priority[80] == inx_name_str()
    assert table_sorting_priority[81] == otx_label_str()
    assert table_sorting_priority[82] == inx_label_str()
    assert table_sorting_priority[83] == otx_bridge_str()
    assert table_sorting_priority[84] == inx_bridge_str()
    assert table_sorting_priority[85] == bridge_str()
    assert table_sorting_priority[86] == unknown_word_str()
    assert table_sorting_priority[87] == quota_str()
    assert table_sorting_priority[88] == celldepth_str()
    assert table_sorting_priority[89] == "job_listen_rotations"
    assert table_sorting_priority[90] == "error_message"
    assert table_sorting_priority[91] == "_owner_name_team"
    assert table_sorting_priority[92] == "_active"
    assert table_sorting_priority[93] == "_task"
    assert table_sorting_priority[94] == "_status"
    assert table_sorting_priority[95] == "_credor_pool"
    assert table_sorting_priority[96] == "_debtor_pool"
    assert table_sorting_priority[97] == "_rational"
    assert table_sorting_priority[98] == "_fund_give"
    assert table_sorting_priority[99] == "_fund_take"
    assert table_sorting_priority[100] == "_fund_onset"
    assert table_sorting_priority[101] == "_fund_cease"
    assert table_sorting_priority[102] == "_fund_ratio"
    assert table_sorting_priority[103] == "_fund_agenda_give"
    assert table_sorting_priority[104] == "_fund_agenda_take"
    assert table_sorting_priority[105] == "_fund_agenda_ratio_give"
    assert table_sorting_priority[106] == "_fund_agenda_ratio_take"
    assert table_sorting_priority[107] == "_inallocable_debtit_belief"
    assert table_sorting_priority[108] == "_gogo_calc"
    assert table_sorting_priority[109] == "_stop_calc"
    assert table_sorting_priority[110] == "_level"
    assert table_sorting_priority[111] == "_range_evaluated"
    assert table_sorting_priority[112] == "_descendant_pledge_count"
    assert table_sorting_priority[113] == "_healerlink_ratio"
    assert table_sorting_priority[114] == "_all_acct_cred"
    assert table_sorting_priority[115] == "_keeps_justified"
    assert table_sorting_priority[116] == "_offtrack_fund"
    assert table_sorting_priority[117] == "_base_item_active_value"
    assert table_sorting_priority[118] == "_irrational_debtit_belief"
    assert table_sorting_priority[119] == "_sum_healerlink_share"
    assert table_sorting_priority[120] == "_keeps_buildable"
    assert table_sorting_priority[121] == "_all_acct_debt"
    assert table_sorting_priority[122] == "_tree_traverse_count"
    assert len(table_sorting_priority) == 123

...
from src.f06_listen.hub_tool import save_voice_file, save_plan_file
from src.f06_listen.hubunit import hubunit_shop
from src.f06_listen.listen import create_listen_basis, listen_to_agendas_voice_plan
...
def test_listen_to_agendas_voice_plan_AddsTasksToBudWhenNo_teamlinkIsSet(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    ...
    save_voice_file(fisc_mstr_dir, yao_voice)
    ...
    new_yao_plan = create_listen_basis(yao_voice)
    assert len(new_yao_plan.get_agenda_dict()) == 0

    # WHEN
    print(f"{len(new_yao_plan.get_item_dict())=}")
    listen_to_agendas_voice_plan(fisc_mstr_dir, new_yao_plan)

    # THEN
    assert len(new_yao_plan.get_agenda_dict()) == 2


def test_listen_to_agendas_voice_plan_AddsTasksToBud(env_dir_setup_cleanup):
    # ESTABLISH
    fisc_mstr_dir = env_dir()
    a23_str = "accord23"
    yao_str = "Yao"
    ...
    yao_voice.set_acct_respect(zia_pool)
    a23_str = "accord23"
    save_plan_file(fisc_mstr_dir, yao_voice)
...
def test_listen_to_agendas_voice_plan_AddsTasksToBudWithDetailsDecidedBy_debtit_belief(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    fisc_mstr_dir = env_dir()
    ...
    new_yao_plan2 = create_listen_basis(yao_voice)
    assert new_yao_plan2.item_exists(cook_road()) is False

    # WHEN
    listen_to_agendas_voice_plan(fisc_mstr_dir, new_yao_plan2)
...
def test_listen_to_agendas_voice_plan_ProcessesIrrationalBud(env_dir_setup_cleanup):
    # ESTABLISH
    fisc_mstr_dir = env_dir()
    a23_str = "accord23"
    yao_str = "Yao"
...
def test_listen_to_agendas_voice_plan_ProcessesMissingDebtorBud(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    fisc_mstr_dir = env_dir()
    yao_str = "Yao"
    a23_str = "accord23"
    yao_voice_path = create_voice_path(fisc_mstr_dir, a23_str, yao_str)
...
def test_listen_to_agendas_voice_plan_ListensToOwner_voice_AndNotOwner_plan(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    fisc_mstr_dir = env_dir()

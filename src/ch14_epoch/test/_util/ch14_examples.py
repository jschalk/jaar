...
def display_current_creg_five_time_attrs(graphics_bool: bool):
    if graphics_bool:
        ...
        creg_blurb = f"<b>{creg_EpochInstant.get_blurb()}</b>"
        five_blurb = f"<b>{five_EpochInstant.get_blurb()}</b>"
...
def display_creg_five_squirt_time_attrs(graphics_bool: bool):
    if graphics_bool:
        ...
        creg_EpochInstant = beliefEpochInstant_shop(sue_belief, kw.creg, creg_min)
        five_EpochInstant = beliefEpochInstant_shop(sue_belief, kw.five, five_min)
        squirt_EpochInstant = beliefEpochInstant_shop(
            sue_belief, squirt_rope, squirt_min
        )
        creg_EpochInstant.calc_epoch()
        five_EpochInstant.calc_epoch()
        squirt_EpochInstant.calc_epoch()
        creg_blurb = f"<b>{creg_EpochInstant.get_blurb()}</b>"
        five_blurb = f"<b>{five_EpochInstant.get_blurb()}</b>"
        squirt_blurb = f"<b>{squirt_EpochInstant.get_blurb()}</b>"

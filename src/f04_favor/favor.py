...
from src.f01_road.jaar_config import get_init_favor_id_if_None, get_json_filename
...
from src.f04_favor.atom import BudAtom, get_from_json as budatom_get_from_json
from src.f04_favor.delta import (
    BudDelta,
    buddelta_shop,
...
class favor_bud_conflict_Exception(Exception):
    pass
...
class FavorUnit:
    ...
    def set_delta_start(self, x_delta_start: int):
        self._delta_start = get_init_favor_id_if_None(x_delta_start)
    ...
    def get_delta_atom_numbers(self, favorunit_dict: list[str]) -> int:
        delta_dict = favorunit_dict.get("delta")
    ...
    def _save_favor_file(self):
        x_filename = self._get_num_filename(self._favor_id)
        save_file(self._favors_dir, x_filename, self.get_deltametric_json())

    def favor_file_exists(self) -> bool:
        x_filename = self._get_num_filename(self._favor_id)
        return os_path_exists(create_path(self._favors_dir, x_filename))
    ...
    def get_edited_bud(self, before_bud: BudUnit) -> BudUnit:
        if (
            ...
            raise favor_bud_conflict_Exception(
                f"favor bud conflict {self.fisc_title} != {before_bud.fisc_title} or {self.owner_name} != {before_bud.owner_name}"
            )
...
def favorunit_shop(
    ...
    x_favorunit = FavorUnit(
        ...
        _favor_id=get_init_favor_id_if_None(_favor_id),
        _buddelta=_buddelta,
        _favors_dir=_favors_dir,
        _atoms_dir=_atoms_dir,
        event_int=event_int,
    )
    x_favorunit.set_delta_start(_delta_start)
    return x_favorunit


def create_favorunit_from_files(
    favors_dir: str,
    favor_id: str,
    atoms_dir: str,
) -> FavorUnit:
    favor_filename = get_json_filename(favor_id)
    favor_dict = open_json(favors_dir, favor_filename)
    x_owner_name = favor_dict.get("owner_name")
    x_fisc_title = favor_dict.get("fisc_title")
    x_face_name = favor_dict.get("face_name")
    delta_atom_numbers_list = favor_dict.get("delta_atom_numbers")
    ...
    x_favorunit._create_buddelta_from_atom_files(delta_atom_numbers_list)
    return x_favorunit


def get_favorunit_from_json(x_json: str) -> FavorUnit:
    favor_dict = get_dict_from_json(x_json)
    if favor_dict.get("event_int") is None:
        x_event_int = None
    else:
        x_event_int = int(favor_dict.get("event_int"))
    x_favorunit = favorunit_shop(
        face_name=favor_dict.get("face_name"),
        owner_name=favor_dict.get("owner_name"),
        fisc_title=favor_dict.get("fisc_title"),
        _favor_id=favor_dict.get("favor_id"),
        _atoms_dir=favor_dict.get("atoms_dir"),
        event_int=x_event_int,
    )
    x_buddelta = get_buddelta_from_ordered_dict(favor_dict.get("delta"))

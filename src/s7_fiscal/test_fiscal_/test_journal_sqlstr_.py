...
from src.s2_bud.bud_tool import bud_idea_factunit_str
from src.s4_gift.atom_config import fopen_str, atom_insert, atom_hx_table_name
from src.s4_gift.atom import atomunit_shop
from src.s7_fiscal.journal_sqlstr import (
...
;"""
    assert generated_sqlstr.find(begin_sqlstr) == 0
    assert generated_sqlstr.find(end_sqlstr) > 0
    assert generated_sqlstr.find(end_sqlstr) == 5435
    example_idea_reasonunit_str = (
        "idea_reasonunit_UPDATE_base_idea_active_requisite INTEGER NULL"
    )
    assert generated_sqlstr.find(example_idea_reasonunit_str) > 0

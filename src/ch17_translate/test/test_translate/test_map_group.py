...
from ch17_translate.translate_config import default_unknown_str_if_None
...
def test_TitleMap_reveal_inx_ReturnsObjAndSetsAttr_group_title():
    ...
    x_titlemap.otx_exists(boulder_otx) is False
    ...
    assert x_titlemap.otx_exists(boulder_otx) is False
    assert x_titlemap.get_inx_value(bowl_otx) == bowl_inx

    # WHEN
    assert x_titlemap.reveal_inx(boulder_otx) is None
    # THEN
    assert x_titlemap.otx_exists(bowl_otx)
    assert x_titlemap.otx_exists(boulder_otx) is False

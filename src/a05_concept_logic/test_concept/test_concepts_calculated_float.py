...
def test_concepts_calculated_range_ReturnsObj_EmptyConceptUnit():
    # ESTABLISH
    wk_str = "wk"
    wk_concept = conceptunit_shop(wk_str)
    x_rangeunit = RangeUnit(3, 8)

    # WHEN / THEN
    assert (
        concepts_calculated_range([wk_concept], x_rangeunit.gogo, x_rangeunit.stop)
        == x_rangeunit
    )
...
def test_concepts_calculated_range_ReturnsObj_1ConceptUnit_addin():
    ...
    wk_str = "wk"
    wk_addin = 5
    wk_concept = conceptunit_shop(wk_str, addin=wk_addin)
    old_gogo = 3
    old_stop = 8
    ...
    new_rangeunit = concepts_calculated_range(
        [wk_concept], old_rangeunit.gogo, old_rangeunit.stop
    )
...
def test_concepts_calculated_range_ReturnsObj_2ConceptUnit_addin():
    ...
    # WHEN
    new_rangeunit = concepts_calculated_range(
        [wk_concept, tue_concept], old_rangeunit.gogo, old_rangeunit.stop
    )
...
def test_concepts_calculated_range_ReturnsObj_2ConceptUnit_numor():
    ...
    wk_str = "wk"
    wk_numor = 5
    wk_concept = conceptunit_shop(wk_str, numor=wk_numor)
    tue_numor = 10
    tue_concept = conceptunit_shop("Tue", numor=tue_numor)
    old_gogo = 3
    old_stop = 8
    old_rangeunit = RangeUnit(old_gogo, old_stop)
    concept_list = [wk_concept, tue_concept]
...
def test_concepts_calculated_range_ReturnsObj_2ConceptUnit_denom():
    ...
    wk_str = "wk"
    wk_denom = 5
    wk_concept = conceptunit_shop(wk_str, denom=wk_denom)
    tue_denom = 2
...
def test_concepts_calculated_range_ReturnsObj_2ConceptUnit_denom_morph():
    ...
    wk_str = "wk"
    wk_denom = 50
    wk_concept = conceptunit_shop(wk_str, denom=wk_denom, morph=True)
    tue_denom = 20

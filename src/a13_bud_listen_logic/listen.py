...
def listen_to_agendas_create_init_job_from_guts(
    vow_mstr_dir: str, listener_job: BudUnit
):
...
def listen_to_agendas_jobs_into_job(vow_mstr_dir: str, listener_job: BudUnit):
    vow_label = listener_job.vow_label
    for x_acctunit in get_ordered_debtors_roll(listener_job):
        speaker_id = x_acctunit.acct_name
...
def listen_to_agendas_duty_vision(listener_vision: BudUnit, healer_hubunit: HubUnit):
    ...
    for x_acctunit in get_ordered_debtors_roll(listener_vision):
        if x_acctunit.acct_name == listener_id:
            ...
            speaker_vision = healer_hubunit.rj_speaker_bud(healer_name, speaker_id)
            if speaker_vision is None:
                speaker_vision = create_empty_bud_from_bud(listener_vision, speaker_id)
            listen_to_speaker_agenda(listener_vision, speaker_vision)


def listen_to_facts_duty_vision(new_vision: BudUnit, healer_hubunit: HubUnit):
    duty = healer_hubunit.get_duty_bud(new_vision.owner_name)
    ...
    for x_acctunit in get_ordered_debtors_roll(new_vision):
        if x_acctunit.acct_name != new_vision.owner_name:
            speaker_vision = healer_hubunit.get_vision_bud(x_acctunit.acct_name)
            if speaker_vision is not None:
                listen_to_speaker_fact(new_vision, speaker_vision)


def listen_to_facts_gut_job(vow_mstr_dir: str, new_job: BudUnit):
...
def fstate_keep_vision_and_listen(
    ...
    if healer_hubunit.vision_file_exists(listener_id):
        keep_vision = healer_hubunit.get_vision_bud(listener_id)
    else:
        keep_vision = create_empty_bud_from_bud(new_job, new_job.owner_name)
    listen_to_vision_agenda(new_job, keep_vision)


def listen_to_vision_agenda(listener: BudUnit, vision: BudUnit):
...
def create_vision_file_from_duty_file(healer_hubunit: HubUnit, owner_name: OwnerName):
    ...
    healer_hubunit.save_vision_bud(x_vision)

...
def migrate_all_facts(src_listener: BudUnit, dst_listener: BudUnit):
    for x_factunit in src_listener.itemroot.factunits.values():
        ...
        if dst_listener.item_exists(fpick_road) is False:
            fpick_item = src_listener.get_item_obj(fpick_road)
            dst_listener.set_item(fpick_item, fpick_item.parent_road)
        dst_listener.add_fact(base_road, fpick_road)
...
def listen_to_owner_plans(listener_hubunit: HubUnit) -> None:
    ...
    for x_healer_name, keep_dict in gut._healers_dict.items():
        ...
        _fpick_keep_plans_and_listen(listener_id, keep_dict, healer_hubunit, new_job)
...
def _fpick_keep_plans_and_listen(
    ...
    for keep_path in keep_dict:
        ...
        fpick_keep_plan_and_listen(listener_id, healer_hubunit, new_job)


def fpick_keep_plan_and_listen(

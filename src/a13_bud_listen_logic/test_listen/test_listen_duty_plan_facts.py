...
def test_listen_to_facts_duty_plan_GetsFactsFromSrcBudSelfNotSpeakerSelf(
    ...
    # new_yao_plan fstates yao_duty fact eat_way = full
    ...
    assert old_yao_plan.get_fact(eat_way()).fstate == hungry_way()
    ...
    # THEN
    assert new_yao_plan.get_fact(eat_way()) is not None
    assert new_yao_plan.get_fact(eat_way()).fstate == full_way()


def test_listen_to_facts_duty_plan_ConfirmNoFactfstateedFromOwnersSpeakerDirBud_v1(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    yao_duty = get_example_yao_speaker()
    ...
    assert zia_plan.get_fact(eat_way()).fstate == eat_way()
    sue_texas_hubunit.save_plan_bud(zia_plan)
    ...
    assert zia_plan.get_fact(eat_way()).fstate == eat_way()
    assert old_yao_plan.get_fact(eat_way()).fstate == hungry_way()
    assert new_yao_plan.get_fact(eat_way()).fstate == eat_way()
...
def test_listen_to_facts_duty_plan_SetsPrioritizesSelfFactsOverSpeakers(
    ...
    assert zia_plan.get_fact(eat_way()).fstate == hungry_way()
    sue_texas_hubunit.save_plan_bud(zia_plan)
...
def test_listen_to_facts_duty_plan_ConfirmNoFactfstateedFromOwnersSpeakerDirBud_v2(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    zia_plan = get_example_zia_speaker()
    ...
    assert bob_plan.get_fact(eat_way()).fstate == hungry_way()
    ...
    assert bob_plan.get_fact(eat_way()).fstate == hungry_way()
    assert zia_plan.get_fact(eat_way()).fstate == eat_way()
    assert new_yao_plan1.get_fact(eat_way()).fstate == hungry_way()
    ...
    assert bob_plan.get_fact(eat_way()).fstate == hungry_way()
    assert zia_plan.get_fact(eat_way()).fstate == eat_way()
    assert new_yao_plan2.get_fact(eat_way()).fstate == eat_way()

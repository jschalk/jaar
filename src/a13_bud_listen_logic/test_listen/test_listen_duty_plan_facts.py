...
def test_listen_to_facts_duty_plan_GetsFactsFromSrcBudSelfNotSpeakerSelf(
    ...
    # new_yao_plan fpicks yao_duty fact eat_road = full
    yao_duty = get_example_yao_speaker()
    yao_duty.add_fact(eat_road(), full_road())
    sue_texas_hubunit = get_texas_hubunit()
    sue_texas_hubunit.save_duty_bud(yao_duty)
    print(f"{sue_texas_hubunit.duty_path(yao_duty)=}")
    assert yao_duty.get_fact(eat_road()).fpick == full_road()

    old_yao_plan = get_example_yao_speaker()
    assert old_yao_plan.get_fact(eat_road()).fpick == hungry_road()
    ...
    # THEN
    assert new_yao_plan.get_fact(eat_road()) is not None
    assert new_yao_plan.get_fact(eat_road()).fpick == full_road()


def test_listen_to_facts_duty_plan_ConfirmNoFactfpickedFromOwnersSpeakerDirBud_v1(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    yao_duty = get_example_yao_speaker()
    ...
    assert zia_plan.get_fact(eat_road()).fpick == eat_road()
    sue_texas_hubunit.save_plan_bud(zia_plan)
    ...
    assert zia_plan.get_fact(eat_road()).fpick == eat_road()
    assert old_yao_plan.get_fact(eat_road()).fpick == hungry_road()
    assert new_yao_plan.get_fact(eat_road()).fpick == eat_road()
...
def test_listen_to_facts_duty_plan_SetsPrioritizesSelfFactsOverSpeakers(
    ...
    assert zia_plan.get_fact(eat_road()).fpick == hungry_road()
    sue_texas_hubunit.save_plan_bud(zia_plan)
...
def test_listen_to_facts_duty_plan_ConfirmNoFactfpickedFromOwnersSpeakerDirBud_v2(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    zia_plan = get_example_zia_speaker()
    ...
    assert bob_plan.get_fact(eat_road()).fpick == hungry_road()
    ...
    assert bob_plan.get_fact(eat_road()).fpick == hungry_road()
    assert zia_plan.get_fact(eat_road()).fpick == eat_road()
    assert new_yao_plan1.get_fact(eat_road()).fpick == hungry_road()
    ...
    assert bob_plan.get_fact(eat_road()).fpick == hungry_road()
    assert zia_plan.get_fact(eat_road()).fpick == eat_road()
    assert new_yao_plan2.get_fact(eat_road()).fpick == eat_road()
...
#     assert yao_duty.get_fact(status_road).fpick == dirty_road
...
#     assert yao_duty.get_fact(fridge_road).fpick == running_road

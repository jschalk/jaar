...
from src.a13_bud_listen_logic._utils.example_listen_hub import (
    get_dakota_hubunit,
    get_dakota_way,
)
...
def test_listen_to_agenda_duty_plan_agenda_AddsTasksToplan_BudWhenNo_teamlinkIsSet(
    ...
    zia_plan.set_item(itemunit_shop(cook_str(), pledge=True), casa_way())
...
def test_listen_to_agenda_duty_plan_agenda_AddsTasksToplan_Bud(env_dir_setup_cleanup):
    ...
    cook_itemunit = zia_plan.get_item_obj(cook_way())
...
def test_listen_to_agenda_duty_plan_agenda_AddsTasksToplanBudWithDetailsDecidedBy_debtit_belief(
    ...
    bob_cook_itemunit = bob_plan.get_item_obj(cook_way())
    zia_cook_itemunit = zia_plan.get_item_obj(cook_way())
    ...
    assert new_yao_job1.item_exists(cook_way()) is False

    # WHEN
    listen_to_agendas_duty_plan(new_yao_job1, sue_dakota_hubunit)

    # THEN
    assert new_yao_job1.item_exists(cook_way())
    new_cook_item = new_yao_job1.get_item_obj(cook_way())
    ...
    assert new_yao_job2.item_exists(cook_way()) is False

    # WHEN
    listen_to_agendas_duty_plan(new_yao_job2, sue_dakota_hubunit)

    # THEN
    assert new_yao_job2.item_exists(cook_way())
    new_cook_item = new_yao_job2.get_item_obj(cook_way())
...
def test_listen_to_agenda_duty_plan_agenda_ProcessesIrrationalBud(
    ...
    vacuum_itemunit = sue_plan.get_item_obj(vacuum_way)
...
def test_listen_to_agenda_duty_plan_agenda_ListensToOwner_duty_AndNotOwner_plan(
    ...
    vacuum_itemunit = yao_old_plan.get_item_obj(vacuum_way)
    vacuum_itemunit.teamunit.set_teamlink(yao_str)
...
def test_listen_to_agenda_duty_plan_agenda_GetsAgendaFromSrcBudNotSpeakerSelf(
    ...
    # yao_new_plan fneeds yao_duty task run_way and not clean_way
    yao_duty = get_example_yao_speaker()
    assert yao_duty.item_exists(run_way()) is False
    assert yao_duty.item_exists(clean_way()) is False
    yao_duty.set_item(itemunit_shop(run_str(), pledge=True), casa_way())
    sue_dakota_hubunit = get_dakota_hubunit()
    sue_dakota_hubunit.save_duty_bud(yao_duty)

    yao_old_plan = get_example_yao_speaker()
    assert yao_old_plan.item_exists(run_way()) is False
    assert yao_old_plan.item_exists(clean_way()) is False
    yao_old_plan.set_item(itemunit_shop(clean_str(), pledge=True), casa_way())
    sue_dakota_hubunit.save_plan_bud(yao_old_plan)

    yao_new_plan = create_listen_basis(yao_duty)
    assert yao_new_plan.item_exists(run_way()) is False
    assert yao_new_plan.item_exists(clean_way()) is False
    ...
    # THEN
    assert yao_new_plan.item_exists(clean_way()) is False
    assert yao_new_plan.item_exists(run_way())

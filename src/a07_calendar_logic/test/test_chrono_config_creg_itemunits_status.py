...
def test_BudUnit_get_agenda_dict_ReturnsDictWith_days_item_Scenario0():
    ...
    # THEN
    clean_item = sue_budunit.get_item_obj(clean_way)
    print(f"{clean_item._factheirs.keys()=}")
    print(f"{clean_item._factheirs.get(days_way)=}")
    assert sue_agenda.get(clean_way)
...
def test_BudUnit_add_time_creg_itemunit_SyncsWeekDayAndYear_Wednesday_March1_2000():
    ...
    print(f"{clean_item._factheirs.get(year_way).fopen=}")
    print(f"{clean_item._factheirs.get(year_way).fnigh=}")
    print(f"{clean_item.get_reasonheir(year_way)._status=} \n")

    # WHEN / THEN
    sue_budunit.add_fact(creg_way, creg_way, yr2000_mar6day, yr2000_mar7day)
    sue_budunit.settle_bud()
    print(f"{clean_item._factheirs.get(wed_way).fopen=}")
    print(f"{clean_item._factheirs.get(wed_way).fnigh=}")
    print(f"{clean_item.get_reasonheir(wed_way)._status=}")

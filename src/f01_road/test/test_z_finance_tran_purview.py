...
def test_PurviewEpisode_Exists():
    ...
    assert not x_purviewepisode.time_int
...
def test_purviewepisode_shop_ReturnsObj():
    ...
    # WHEN
    t4_purviewepisode = purviewepisode_shop(t4_time_int)

    # THEN
    assert t4_purviewepisode
    assert t4_purviewepisode.time_int == t4_time_int
...
def test_purviewepisode_shop_ReturnsObjWith_net_purviews():
    ...
    # THEN
    assert x_purviewepisode
    assert x_purviewepisode.time_int == t4_time_int
...
def test_PurviewEpisode_get_dict_ReturnsObj():
    ...
    t4_time_int = 4
    t4_quota = 55
    t4_purviewepisode = purviewepisode_shop(t4_time_int, t4_quota)
...
def test_PurviewEpisode_calc_magnitude_SetsAttr_Scenario1():
    ...
    t4_purviewepisode = purviewepisode_shop(t4_time_int, net_purviews=t4_net_purviews)
    assert t4_purviewepisode._magnitude == 0
...
def test_PurviewEpisode_get_dict_ReturnsObjWith_net_purviews():
    ...
    t4_net_purviews = {"Sue": -4}
    t4_magnitude = 67
    t4_purviewepisode = purviewepisode_shop(t4_time_int, t4_quota, t4_net_purviews)
...
def test_PurviewLog_Exists():
    ...
    assert not x_purviewlog._time_int_min
    assert not x_purviewlog._time_int_max
...
def test_PurviewLog_get_2d_array_ReturnsObj_Scenario1():
    ...
    x7_time_int = 7
    x7_quota = 66
    sue_purviewlog.add_episode(x4_time_int, x4_quota)
...
def test_PurviewLog_get_time_ints_ReturnsObj():
    # ESTABLISH
    sue_str = "Sue"
    sue_purviewlog = purviewlog_shop(sue_str)
    x4_time_int = 4
    ...
    x7_time_int = 7
    x7_quota = 66
    assert sue_purviewlog.get_time_ints() == set()
    ...
    # THEN
    assert sue_purviewlog.get_time_ints() == {x4_time_int, x7_time_int}
...
def test_get_purviewlog_from_dict_ReturnsObj_Scenario2():
    ...
    zia_net_purview = 887
    sue_net_purview = 445
    sue_purviewlog.get_episode(x7_time_int).set_net_purview(sue_str, sue_net_purview)
    ...
    assert len(x_purviewlog.get_episode(x7_time_int)._net_purviews) == 2
    assert x_purviewlog.episodes == sue_purviewlog.episodes
    assert x_purviewlog == sue_purviewlog
...
def test_PurviewLog_get_tranbook_ReturnsObj():
    ...
    zia_net_purview = 887
    bob_net_purview = 445
    sue_purviewlog.get_episode(x4_time_int).set_net_purview(bob_str, bob_net_purview)
    sue_purviewlog.get_episode(x7_time_int).set_net_purview(zia_str, zia_net_purview)
    sue_episodes_dict = sue_purviewlog.get_dict()
    assert sue_episodes_dict == {
        "owner_name": sue_str,
    ...
    sue_tranbook = sue_purviewlog.get_tranbook(x_deal_idea)
    ...
    assert sue_tranbook.get_amount(sue_str, zia_str, x7_time_int) == zia_net_purview
    assert sue_tranbook.get_amount(sue_str, bob_str, x4_time_int) == bob_net_purview

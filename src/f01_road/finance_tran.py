...
class time_fid_Exception(Exception):
    pass


def time_fid_str() -> str:
    return "time_fid"
...
class TranUnit:
    src: AcctID = None
    dst: AcctID = None
    time_fid: TimeLinePoint = None
    amount: FundNum = None


def tranunit_shop(
    src: AcctID, dst: AcctID, time_fid: TimeLinePoint, amount: FundNum
) -> TranUnit:
    return TranUnit(src=src, dst=dst, time_fid=time_fid, amount=amount)
...
class TranBook:
    ...
    def set_tranunit(
        self,
        x_tranunit: TranUnit,
        x_blocked_time_fids: set[TimeLinePoint] = None,
        x_current_time: TimeLinePoint = None,
    ):
        self.add_tranunit(
            x_owner_id=x_tranunit.src,
            x_acct_id=x_tranunit.dst,
            x_time_fid=x_tranunit.time_fid,
            x_amount=x_tranunit.amount,
            x_blocked_time_fids=x_blocked_time_fids,
    ...
    def add_tranunit(
        ...
        x_owner_id: OwnerID,
        x_acct_id: AcctID,
        x_time_fid: TimeLinePoint,
        x_amount: FundNum,
        x_blocked_time_fids: set[TimeLinePoint] = None,
        x_current_time: TimeLinePoint = None,
    ):
        if x_time_fid in get_empty_set_if_none(x_blocked_time_fids):
            exception_str = f"Cannot set tranunit for time_fid={x_time_fid}, timelinepoint is blocked"
            raise time_fid_Exception(exception_str)
        if x_current_time != None and x_time_fid >= x_current_time:
            exception_str = f"Cannot set tranunit for time_fid={x_time_fid}, timelinepoint is greater than current time={x_current_time}"
            raise time_fid_Exception(exception_str)
        x_keylist = [x_owner_id, x_acct_id, x_time_fid]
        set_in_nested_dict(self.tranunits, x_keylist, x_amount)

    def tranunit_exists(
        self, src: AcctID, dst: AcctID, time_fid: TimeLinePoint
    ) -> bool:
        return get_from_nested_dict(self.tranunits, [src, dst, time_fid], True) != None

    def get_tranunit(
        self, src: AcctID, dst: AcctID, time_fid: TimeLinePoint
    ) -> TranUnit:
        x_amount = get_from_nested_dict(self.tranunits, [src, dst, time_fid], True)
        if x_amount != None:
            return tranunit_shop(src, dst, time_fid, x_amount)

    def get_amount(self, src: AcctID, dst: AcctID, time_fid: TimeLinePoint) -> TranUnit:
        return get_from_nested_dict(self.tranunits, [src, dst, time_fid], True)

    def del_tranunit(
        self, src: AcctID, dst: AcctID, time_fid: TimeLinePoint
    ) -> TranUnit:
        x_keylist = [src, dst, time_fid]
        if exists_in_nested_dict(self.tranunits, x_keylist):
            del_in_nested_dict(self.tranunits, x_keylist)

    def get_time_fids(self) -> set[TimeLinePoint]:
        x_set = set()
        for dst_dict in self.tranunits.values():
            for time_fid_dict in dst_dict.values():
                x_set.update(set(time_fid_dict.keys()))
        return x_set
    ...
    #         for dst_acct_id, time_fid_dict in dst_dict.items():
    #             for x_time_fid, x_amount in time_fid_dict.items():
    #                 self.add_tranunit(src_acct_id, dst_acct_id, x_time_fid, x_amount)
...
class PurviewEpisode:
    ...
    def get_dict(self) -> dict[str,]:
        x_dict = {"time_fid": self.time_fid, "quota": self.quota}
...
class PurviewLog:
    ...
    _time_fid_min: TimeLinePoint = None
    _time_fid_max: TimeLinePoint = None

    def set_episode(self, x_episode: PurviewEpisode):
        self.episodes[x_episode.time_fid] = x_episode

    def add_episode(self, x_time_fid: TimeLinePoint, x_quota: FundNum):
        self.set_episode(purviewepisode_shop(x_time_fid, x_quota))

    def episode_exists(self, x_time_fid: TimeLinePoint) -> bool:
        return self.episodes.get(x_time_fid) != None

    def get_episode(self, x_time_fid: TimeLinePoint) -> PurviewEpisode:
        return self.episodes.get(x_time_fid)

    def del_episode(self, x_time_fid: TimeLinePoint):
        self.episodes.pop(x_time_fid)

    def get_2d_array(self) -> list[list]:
        return [
            [self.owner_id, x_episode.time_fid, x_episode.quota]
            for x_episode in self.episodes.values()
        ]

    def get_headers(self) -> list:
        return ["owner_id", "time_fid", "quota"]
    ...
    def _get_episodes_dict(self) -> dict:
        return {
            x_episode.time_fid: x_episode.get_dict()
            for x_episode in self.episodes.values()
        }
    ...
    def get_tranbook(self, fiscal_id: FiscalID) -> TranBook:
        ...
        for x_time_fid, x_episode in self.episodes.items():
...
def get_purviewepisode_from_dict(x_dict: dict) -> PurviewEpisode:
    x_time_fid = x_dict.get("time_fid")
    x_quota = x_dict.get("quota")
    x_net_purviews = x_dict.get("net_purviews")
    x_magnitude = x_dict.get("magnitude")
    return purviewepisode_shop(x_time_fid, x_quota, x_net_purviews, x_magnitude)
...
def get_episodes_from_dict(episodes_dict: dict) -> dict[TimeLinePoint, PurviewEpisode]:
    ...
    for x_episode_dict in episodes_dict.values():
        ...
        x_dict[x_purview_episode.time_fid] = x_purview_episode
    return x_dict

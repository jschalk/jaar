...
from src._road.jaar_config import init_gift_id, get_test_pecun_id as pecun_id
from src.hear.hubunit import hubunit_shop
from src.hear.examples.example_hear_gifts import sue_2atomunits_giftunit
...
def test_HubUnit_default_voice_bud_ReturnsCorrectObj():
    ...
    # THEN
    assert sue_default_voice._pecun_id == sue_hubunit.pecun_id
...
def test_HubUnit_initialize_gift_voice_files_CorrectlySavesvoiceFileAndGiftFile(
    ...
    sue_str = "Sue"
    seven_int = 25
    sue_hubunit = hubunit_shop(env_dir(), pecun_id(), sue_str, bit=seven_int)
    assert sue_hubunit.voice_file_exists() is False
    init_gift_file_path = f"{sue_hubunit.gifts_dir()}/{init_gift_id()}.json"
    ...
    assert voice_bud._pecun_id == pecun_id()
...
def test_HubUnit_initialize_gift_voice_files_CorrectlySavesOnlygiftFile(
    ...
    # THEN
    assert sue_voice_bud._pecun_id == pecun_id()

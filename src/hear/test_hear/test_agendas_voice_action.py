...
from src.hear.hear import create_hear_basis, hear_to_agendas_voice_action
...
def test_hear_to_agendas_voice_action_AddsTasksToBudWhenNo_teamlinkIsSet(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    yao_str = "Yao"
    ...
    new_yao_action = create_hear_basis(yao_voice)
    assert len(new_yao_action.get_agenda_dict()) == 0

    # WHEN
    print(f"{len(new_yao_action.get_idea_dict())=}")
    hear_to_agendas_voice_action(new_yao_action, yao_hubunit)

    # THEN
    assert len(new_yao_action.get_agenda_dict()) == 2


def test_hear_to_agendas_voice_action_AddsTasksToBud(env_dir_setup_cleanup):
    # ESTABLISH
    yao_str = "Yao"
    yao_voice = budunit_shop(yao_str)
    zia_str = "Zia"
...
def test_hear_to_agendas_voice_action_AddsTasksToBudWithDetailsDecidedBy_debtit_belief(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    ...
    new_yao_action1 = create_hear_basis(yao_voice)
    assert new_yao_action1.idea_exists(cook_road()) is False

    # WHEN
    hear_to_agendas_voice_action(new_yao_action1, yao_hubunit)
    ...
    new_yao_action2 = create_hear_basis(yao_voice)
    assert new_yao_action2.idea_exists(cook_road()) is False

    # WHEN
    hear_to_agendas_voice_action(new_yao_action2, yao_hubunit)
...
def test_hear_to_agendas_voice_action_ProcessesIrrationalBud(env_dir_setup_cleanup):
    # ESTABLISH
    yao_str = "Yao"
    yao_voice = budunit_shop(yao_str)
    zia_str = "Zia"
...
def test_hear_to_agendas_voice_action_ProcessesMissingDebtorBud(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    yao_str = "Yao"
...
def test_hear_to_agendas_voice_action_ListensToOwner_voice_AndNotOwner_action(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    yao_str = "Yao"

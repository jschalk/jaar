...
def test_set_hear_to_speaker_fact_SetsFact():
    # ESTABLISH
    yao_str = "Yao"
    yao_hearer = budunit_shop(yao_str)
    casa_str = "casa"
    casa_road = yao_hearer.make_l1_road(casa_str)
    status_str = "status"
    status_road = yao_hearer.make_road(casa_road, status_str)
    clean_str = "clean"
    clean_road = yao_hearer.make_road(status_road, clean_str)
    dirty_str = "dirty"
    dirty_road = yao_hearer.make_road(status_road, dirty_str)
    sweep_str = "sweep"
    sweep_road = yao_hearer.make_road(casa_road, sweep_str)

    yao_hearer.add_acctunit(yao_str)
    yao_hearer.set_acct_respect(20)
    yao_hearer.set_idea(ideaunit_shop(clean_str), status_road)
    yao_hearer.set_idea(ideaunit_shop(dirty_str), status_road)
    yao_hearer.set_idea(ideaunit_shop(sweep_str, pledge=True), casa_road)
    yao_hearer.edit_idea_attr(
        sweep_road, reason_base=status_road, reason_premise=dirty_road
    )
    missing_fact_bases = list(yao_hearer.get_missing_fact_bases().keys())

    yao_speaker = budunit_shop(yao_str)
    yao_speaker.set_fact(status_road, clean_road, create_missing_ideas=True)
    assert yao_hearer.get_missing_fact_bases().keys() == {status_road}

    # WHEN
    hear_to_speaker_fact(yao_hearer, yao_speaker, missing_fact_bases)

    # THEN
    assert len(yao_hearer.get_missing_fact_bases().keys()) == 0


def test_set_hear_to_speaker_fact_DoesNotOverrideFact():
    # ESTABLISH
    yao_str = "Yao"
    yao_hearer = budunit_shop(yao_str)
    yao_hearer.add_acctunit(yao_str)
    ...
    sweep_road = yao_hearer.make_road(casa_road, sweep_str)
    fridge_str = "fridge"
    fridge_road = yao_hearer.make_road(casa_road, fridge_str)
    running_str = "running"
    running_road = yao_hearer.make_road(fridge_road, running_str)

    yao_hearer.set_idea(ideaunit_shop(running_str), fridge_road)
    yao_hearer.set_idea(ideaunit_shop(clean_str), status_road)
    yao_hearer.set_idea(ideaunit_shop(dirty_str), status_road)
    yao_hearer.set_idea(ideaunit_shop(sweep_str, pledge=True), casa_road)
    yao_hearer.edit_idea_attr(
    ...
    assert len(yao_hearer.get_missing_fact_bases()) == 2
    yao_hearer.set_fact(status_road, dirty_road)
    assert len(yao_hearer.get_missing_fact_bases()) == 1
    assert yao_hearer.get_fact(status_road).pick == dirty_road
    ...
    # THEN
    assert len(yao_hearer.get_missing_fact_bases()) == 0
    # did not grab speaker's factunit
    assert yao_hearer.get_fact(status_road).pick == dirty_road
    # grabed speaker's factunit
    assert yao_hearer.get_fact(fridge_road).pick == running_road
...
def test_migrate_all_facts_CorrectlyAddsIdeaUnitsAndSetsFactUnits():
    ...
    yao_src.set_idea(ideaunit_shop(clean_str), status_road)
    yao_src.set_idea(ideaunit_shop(dirty_str), status_road)
    yao_src.set_idea(ideaunit_shop(sweep_str, pledge=True), casa_road)
    yao_src.edit_reason(sweep_road, status_road, dirty_road)
    # missing_fact_bases = list(yao_src.get_missing_fact_bases().keys())
    yao_src.set_idea(ideaunit_shop(rain_str), weather_road)
    yao_src.set_idea(ideaunit_shop(snow_str), weather_road)

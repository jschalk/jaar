...
from src.hear.hear import hear_to_speaker_agenda, create_empty_bud
from copy import deepcopy as copy_deepcopy
from pytest import raises as pytest_raises


def test_hear_to_speaker_agenda_RaisesErrorIfPoolIsNotSet():
    # ESTABLISH
    yao_str = "Yao"
    ...
    with pytest_raises(Exception) as excinfo:
        hear_to_speaker_agenda(yao_budunit, zia_budunit)
    assert (
        str(excinfo.value)
        == f"hearer '{yao_str}' bud is assumed to have {zia_budunit._owner_id} acctunit."
    )


def test_hear_to_speaker_agenda_ReturnsEqualBud():
    # ESTABLISH
    yao_str = "Yao"
    yao_budunit = budunit_shop(yao_str)
    zia_str = "Zia"
    ...
    # WHEN
    after_yao_budunit = hear_to_speaker_agenda(yao_budunit, zia_budunit)

    # THEN
    assert after_yao_budunit == yao_budunit


def test_hear_to_speaker_agenda_ReturnsSingleTaskBud():
    # ESTABLISH
    yao_str = "Yao"
    ...
    # WHEN
    after_yao_budunit = hear_to_speaker_agenda(before_yao_budunit, zia_budunit)
...
def test_hear_to_speaker_agenda_ReturnsLevel2TaskBud():
    # ESTABLISH
    yao_str = "Yao"
    before_yao_budunit = budunit_shop(yao_str)
    zia_str = "Zia"
...
def test_hear_to_speaker_agenda_Returns2AgendaIdeasLevel2TaskBud():
    # ESTABLISH
    yao_str = "Yao"
    before_yao_budunit = budunit_shop(yao_str)
    zia_str = "Zia"
...
def test_hear_to_speaker_agenda_Returns2AgendaIdeasLevel2TaskBudWhereAnIdeaUnitAlreadyExists():
    # ESTABLISH
    yao_str = "Yao"
    before_yao_budunit = budunit_shop(yao_str)
    zia_str = "Zia"
...
def test_hear_to_speaker_agenda_ProcessesIrrationalBud():
    # ESTABLISH
    yao_str = "Yao"
    ...
    yao_job = hear_to_speaker_agenda(yao_job, sue_budunit)
    yao_job.settle_bud()
...
def test_hear_to_speaker_agenda_ProcessesBarrenBud():
    # ESTABLISH
    yao_str = "Yao"
    yao_duty = budunit_shop(yao_str)
    zia_str = "Zia"
    ...
    yao_job.add_acctunit(sue_str, sue_credit_belief, sue_debtit_belief)
    yao_job.set_acct_respect(yao_pool)
    yao_job = hear_to_speaker_agenda(yao_job, speaker=sue_job)

...
from src.hear.hear import create_hear_basis, hear_to_agendas_duty_job
...
def test_hear_to_agenda_duty_job_agenda_AddsTasksToJob_BudWhenNo_teamlinkIsSet(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    yao_str = "Yao"
    ...
    hear_to_agendas_duty_job(new_yao_job, yao_dakota_hubunit)

    # THEN
    assert len(new_yao_job.get_agenda_dict()) == 2


def test_hear_to_agenda_duty_job_agenda_AddsTasksToJob_Bud(env_dir_setup_cleanup):
    # ESTABLISH
    yao_str = "Yao"
    yao_duty = budunit_shop(yao_str)
    zia_str = "Zia"
...
def test_hear_to_agenda_duty_job_agenda_AddsTasksToJobBudWithDetailsDecidedBy_debtit_belief(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    ...
    new_yao_action1 = create_hear_basis(yao_duty)
    assert new_yao_action1.idea_exists(cook_road()) is False

    # WHEN
    hear_to_agendas_duty_job(new_yao_action1, sue_dakota_hubunit)
    ...
    new_yao_action2 = create_hear_basis(yao_duty)
    assert new_yao_action2.idea_exists(cook_road()) is False

    # WHEN
    hear_to_agendas_duty_job(new_yao_action2, sue_dakota_hubunit)
...
def test_hear_to_agenda_duty_job_agenda_ProcessesIrrationalBud(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    yao_str = "Yao"
...
def test_hear_to_agenda_duty_job_agenda_ProcessesMissingDebtorJobBud(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    yao_str = "Yao"
...
def test_hear_to_agenda_duty_job_agenda_ListensToOwner_duty_AndNotOwner_job(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    yao_str = "Yao"
...
def test_hear_to_agenda_duty_job_agenda_GetsAgendaFromSrcBudNotSpeakerSelf(
    env_dir_setup_cleanup,
):
    # ESTABLISH

...
from src.hear.hear import (
    create_hear_basis,
    hear_to_facts_duty_job,
    hear_to_agendas_duty_job,
)
from src.hear.examples.hear_env import get_texas_hubunit, env_dir_setup_cleanup
from src.hear.examples.example_hear import (
    casa_str,
    cook_str,
    eat_str,
    hungry_str,
...
def test_hear_to_facts_duty_job_SetsSingleFactUnit_v1(env_dir_setup_cleanup):
    # ESTABLISH
    yao_str = "Yao"
    ...
    new_yao_job = create_hear_basis(yao_duty)
    assert new_yao_job.get_missing_fact_bases().get(eat_road()) is None
    hear_to_agendas_duty_job(new_yao_job, sue_texas_hubunit)
    assert new_yao_job.get_missing_fact_bases().get(eat_road()) is not None

    # WHEN
    hear_to_facts_duty_job(new_yao_job, sue_texas_hubunit)

    # THEN
    assert new_yao_job.get_missing_fact_bases().get(eat_road()) is None


def test_hear_to_facts_duty_job_SetsSingleFactUnitWithDifferentTask(
...
def test_hear_to_facts_duty_job_GetsFactsFromSrcBudSelfNotSpeakerSelf(
    env_dir_setup_cleanup,
):
    # ESTABLISH
...
def test_hear_to_facts_duty_job_ConfirmNoFactPickedFromOwnersSpeakerDirBud_v1(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    yao_duty = get_example_yao_speaker()
...
def test_hear_to_facts_duty_job_SetsPrioritizesSelfFactsOverSpeakers(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    yao_duty = get_example_yao_speaker()
...
def test_hear_to_facts_duty_job_ConfirmNoFactPickedFromOwnersSpeakerDirBud_v2(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    zia_job = get_example_zia_speaker()
    ...
    # WHEN
    hear_to_facts_duty_job(new_yao_job1, sue_texas_hubunit)
    ...
    new_yao_job2 = create_hear_basis(yao_duty)
    hear_to_agendas_duty_job(new_yao_job2, sue_texas_hubunit)
    hear_to_facts_duty_job(new_yao_job2, sue_texas_hubunit)
...
# def test_hear_to_facts_duty_job_SetsFact(env_dir_setup_cleanup):
#     # ESTABLISH
#     yao_str = "Yao"
#     sue_str = "Sue"
...
#     hear_to_agendas_duty_job(new_yao_job, texas_hubunit)
#     print(f"{new_yao_job.get_idea_dict().keys()=}")
...
#     # # WHEN
#     # hear_to_facts_duty_job(yao_duty, yao_job, missing_fact_bases)
...
# def test_hear_to_facts_duty_job_DoesNotOverrideFact():
#     # ESTABLISH
#     yao_str = "Yao"
#     yao_duty = budunit_shop(yao_str)
#     yao_duty.add_acctunit(yao_str)

...
def get_hear_structures0_fig(graphics_bool: bool = False) -> plotly_Figure:
    if graphics_bool:
        sue_str = "Sue"
        bob_str = "Bob"
        yao_str = "Yao"
        ...
        fig.add_trace(
            plotly_Scatter(
                ...
                y=[9.0, 8.75],
                text=[
                    "Pecunity Bud Listening Structures",
                    "The voice bud hears to other's action buds and builds a new bud from itself and others",
                ],
                mode="text",
            )
        )
...
def get_hear_structures1_fig(graphics_bool: bool = False) -> plotly_Figure:
    if graphics_bool:
        fig = get_hubunit_base_fig()
        sue_str = "Sue"
        bob_str = "Bob"
...
def get_hear_structures2_fig(graphics_bool: bool = False) -> plotly_Figure:
    if graphics_bool:
        fig = get_hubunit_base_fig()
...
def get_hear_structures3_fig(graphics_bool: bool = False) -> plotly_Figure:
    if graphics_bool:
        fig = get_hubunit_base_fig()
        fig.update_yaxes(range=[-4, 10])
        sue_str = "Sue"

...
from src.a01_term_logic.term import BeliefName, CoinLabel, FaceName
...
class PackUnit:
    ...
    _atoms_dir: str = None
    event_int: int = None
    """Represents a per coin_label/event_int BeliefDelta for a belief_name"""
    ...
    def get_edited_belief(self, before_belief: BeliefUnit) -> BeliefUnit:
        if (
            self.coin_label != before_belief.coin_label
            or self.belief_name != before_belief.belief_name
        ):
            raise pack_belief_conflict_Exception(
                f"pack belief conflict {self.coin_label} != {before_belief.coin_label} or {self.belief_name} != {before_belief.belief_name}"
            )

...
def test_BelieverDelta_get_dimens_cruds_believerdelta_ReturnsObjWithCorrectDimensAndCRUDsBy_acctunit_insert():
    # ESTABLISH
    sue_str = "Sue"
    yao_str = "Yao"
    before_sue_believer = believerunit_shop(sue_str)
    before_sue_believer.add_acctunit(yao_str)
    after_sue_believer = believerunit_shop(sue_str)
    bob_str = "Bob"
    bob_acct_cred_points = 33
    bob_acct_debt_points = 44
    after_sue_believer.add_acctunit(bob_str, bob_acct_cred_points, bob_acct_debt_points)
    ...
    dimen_set = [believer_acctunit_str()]
    curd_set = {INSERT_str()}
    ...
    bob_believeratom = sue_acctunit_dict.get(bob_str)
    assert bob_believeratom.get_value(acct_name_str()) == bob_str
    assert bob_believeratom.get_value("acct_cred_points") == bob_acct_cred_points
    assert bob_believeratom.get_value("acct_debt_points") == bob_acct_debt_points

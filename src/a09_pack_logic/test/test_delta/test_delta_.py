...
def test_ChangUnit_believeratom_exists_ReturnsObj_believer_acctunit_str():
    # ESTABLISH
    bob_str = "Bob"
...
def test_ChangUnit_believeratom_exists_ReturnsObj_believer_acct_membership_str():
    # ESTABLISH
    bob_str = "Bob"
    iowa_str = ";Iowa"
    x_believerdelta = believerdelta_shop()
...
def test_BelieverDelta_add_believeratom_CorrectlySets_BelieverUnit_acctunits():
    # ESTABLISH
    ex1_believerdelta = believerdelta_shop()
    assert ex1_believerdelta.believeratoms == {}
    ...
    assert len(ex1_believerdelta.believeratoms) == 1
    assert (
        ex1_believerdelta.believeratoms.get(INSERT_str()).get(acctunit_str).get(bob_str)
        is not None
    )
...
def test_BelieverDelta_get_dimen_sorted_believeratoms_list_ReturnsObj_Scenario0_rope():
    ...
    print(f"{z_atom=}")
    print(delete_dict.get(believer_acctunit_str()).keys())
    zia_acctunit_delete = delete_dict.get(believer_acctunit_str()).get("Zia")
...
def test_BelieverDelta_get_sorted_believeratoms_ReturnsObj():
    ...
    delete_dict = ex1_believerdelta.believeratoms.get(DELETE_str())
    assert len(delete_dict.keys()) == 1
    assert delete_dict.get(believer_acctunit_str()) is not None
...
def test_believer_built_from_delta_is_valid_ReturnsObjEstablishWithNoBeliever_scenario1():
    ...
    x_believeratom.set_arg(acct_name_str(), zia_str)
    x_believeratom.set_arg(acct_cred_points_str(), "70 is the number")
    sue_believerdelta.set_believeratom(x_believeratom)
...
def test_believer_built_from_delta_is_valid_ReturnsObjEstablishWithNoBeliever_scenario2():
    ...
    yao_str = "Yao"
    x_believeratom = believeratom_shop(dimen, INSERT_str())
    x_believeratom.set_arg(acct_name_str(), yao_str)
    x_believeratom.set_arg(acct_cred_points_str(), 30)
    sue_believerdelta.set_believeratom(x_believeratom)
    ...
    bob_str = "Bob"
    x_believeratom = believeratom_shop(dimen, INSERT_str())
    x_believeratom.set_arg(acct_name_str(), bob_str)
    x_believeratom.set_arg(acct_cred_points_str(), "70 is the number")
    sue_believerdelta.set_believeratom(x_believeratom)
...
def test_BelieverDelta_get_ordered_believeratoms_ReturnsObj_EstablishWithNoStartingNumber():
    ...
    zia_believeratom.set_arg(acct_name_str(), zia_str)
    zia_believeratom.set_arg(acct_cred_points_str(), 70)
    ...
    yao_believeratom.set_arg(acct_cred_points_str(), 30)

...
from src.s4_gift.atom import atom_insert, atom_update, atom_delete
from src.s4_gift.delta import DeltaUnit, deltaunit_shop
from src.s5_listen.examples.example_listen_buds import get_budunit_with_4_levels
from src.s0_instrument.python_tool import get_nested_value, get_empty_list_if_None
...
def test_DeltaUnit_add_all_different_atomunits_Creates_AtomUnit_acct_membership_insert():
    ...
    yao_atomunit = get_nested_value(sue_deltaunit.atomunits, x_keylist)
    assert yao_atomunit.get_value(acct_id_str()) == yao_str

    x_keylist = [atom_insert(), bud_acctunit_str(), zia_str]
    zia_atomunit = get_nested_value(sue_deltaunit.atomunits, x_keylist)
...
def test_DeltaUnit_add_all_different_atomunits_Creates_AtomUnit_idea_healerlink_insert_IdeaUnitInsert():
    ...
    ball_atomunit = get_nested_value(sue_deltaunit.atomunits, x_keylist, True)

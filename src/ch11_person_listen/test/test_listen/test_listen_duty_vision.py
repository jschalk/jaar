from ch05_rope.rope import LabelTerm, RopeTerm, create_rope
...
def casa_rope() -> RopeTerm:
    return create_rope(ch11_example_moment_rope(), exx.casa)
...
def get_example_yao_person() -> PersonUnit:
    yao_speaker = personunit_shop(exx.yao, ch11_example_moment_rope())
    yao_speaker.set_plan_obj(planunit_shop(run_str()), casa_rope())
    yao_speaker.add_contactunit(exx.yao, contact_debt_mass=10)
    yao_speaker.add_contactunit(exx.zia, contact_debt_mass=30)
    yao_speaker.add_contactunit(exx.bob, contact_debt_mass=40)
...
def get_usa_rope() -> RopeTerm:
    return create_rope(ch11_example_moment_rope(), "USA")
...
def get_bowl_rope() -> RopeTerm:
    return create_rope(ch11_example_moment_rope(), exx.bowl)


def get_location_rope() -> RopeTerm:
    return create_rope(ch11_example_moment_rope(), get_location_str())
...
def test_listen_to_person_visions_Pipeline_Scenario1_yao_gut_CanOnlyReferenceItself(
    ...
    # ESTABLISH
    # yao0_gut with 3 debotors of different contact_cred_masss
    ...
    assert yao_job.get_contact(exx.yao).irrational_contact_debt_mass == 0

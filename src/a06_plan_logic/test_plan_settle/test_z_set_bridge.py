...
def test_PlanUnit_set_concept_CorrectlySetsvow_label_AND_fund_iota():
    ...
    clean_way = sue_plan.make_way(casa_way, "cleaning")
    cookery_str = "cookery to use"
    cookery_way = sue_plan.make_way(clean_way, cookery_str)

    # WHEN
    sue_plan.set_concept(conceptunit_shop(cookery_str), clean_way)

    # THEN
    cookery_concept = sue_plan.get_concept_obj(cookery_way)
...
def test_plan_set_vow_label_CorrectlySetsAttr():
    ...
    casa_str = "casa"
    old_casa_way = yao_plan.make_l1_way(casa_str)
    swim_str = "swim"
    old_swim_way = yao_plan.make_way(old_casa_way, swim_str)
    yao_plan.set_l1_concept(conceptunit_shop(casa_str))
    yao_plan.set_concept(conceptunit_shop(swim_str), parent_way=old_casa_way)
    assert yao_plan.owner_name == yao_str
    assert yao_plan.conceptroot.concept_label == yao_plan.vow_label
    casa_concept = yao_plan.get_concept_obj(old_casa_way)
    assert casa_concept.parent_way == to_way(yao_plan.vow_label)
    swim_concept = yao_plan.get_concept_obj(old_swim_way)
    ...
    # THEN
    new_casa_way = yao_plan.make_l1_way(casa_str)
    swim_str = "swim"
    new_swim_way = yao_plan.make_way(new_casa_way, swim_str)
    assert yao_plan.vow_label == x_vow_label
    assert yao_plan.conceptroot.concept_label == x_vow_label
    casa_concept = yao_plan.get_concept_obj(new_casa_way)
    assert casa_concept.parent_way == to_way(x_vow_label)
    swim_concept = yao_plan.get_concept_obj(new_swim_way)
    assert swim_concept.parent_way == new_casa_way


def test_plan_set_bridge_RaisesErrorIfNew_bridge_IsAnConcept_label():
    # ESTABLISH
    ...
    zia_plan.set_concept(conceptunit_shop(casa_str), parent_way=casa_way)

    # WHEN / THEN
    casa_way = zia_plan.make_way(casa_way, casa_str)
    print(f"{casa_way=}")
    with pytest_raises(Exception) as excinfo:
        zia_plan.set_bridge(slash_str)
    assert (
        str(excinfo.value)
...
def test_plan_set_bridge_CorrectlyModifies_parent_way():
    # ESTABLISH
    zia_plan = planunit_shop("Zia", "Texas")
    casa_str = "casa"
    zia_plan.set_l1_concept(conceptunit_shop(casa_str))
    semicolon_casa_way = zia_plan.make_l1_way(casa_str)
    cook_str = "cook"
    zia_plan.set_concept(conceptunit_shop(cook_str), semicolon_casa_way)
    semicolon_cook_way = zia_plan.make_way(semicolon_casa_way, cook_str)
    cook_concept = zia_plan.get_concept_obj(semicolon_cook_way)
    semicolon_str = ";"
    assert zia_plan.bridge == semicolon_str
    semicolon_cook_way = zia_plan.make_way(semicolon_casa_way, cook_str)
    # print(f"{zia_plan.vow_label=} {zia_plan.conceptroot.concept_label=} {casa_way=}")
    # print(f"{cook_concept.parent_way=} {cook_concept.concept_label=}")
    # semicolon_casa_concept = zia_plan.get_concept_obj(semicolon_casa_way)
...
def test_plan_set_bridge_CorrectlyModifiesReasonUnit():
    # ESTABLISH
    zia_plan = planunit_shop("Zia", "Texas")
    casa_str = "casa"
    zia_plan.set_l1_concept(conceptunit_shop(casa_str))
    time_str = "time"
    semicolon_time_way = zia_plan.make_l1_way(time_str)
    _8am_str = "8am"
    semicolon_8am_way = zia_plan.make_way(semicolon_time_way, _8am_str)
    ...
    semicolon_casa_way = zia_plan.make_l1_way(casa_str)
    zia_plan.edit_concept_attr(semicolon_casa_way, reason=semicolon_time_reasonunit)
    casa_concept = zia_plan.get_concept_obj(semicolon_casa_way)
    ...
    # THEN
    slash_time_way = zia_plan.make_l1_way(time_str)
    slash_8am_way = zia_plan.make_way(slash_time_way, _8am_str)
    slash_casa_way = zia_plan.make_l1_way(casa_str)
    casa_concept = zia_plan.get_concept_obj(slash_casa_way)
    slash_time_way = zia_plan.make_l1_way(time_str)
    slash_8am_way = zia_plan.make_way(slash_time_way, _8am_str)
...
def test_plan_set_bridge_CorrectlyModifiesFactUnit():
    # ESTABLISH
    zia_plan = planunit_shop("Zia", "Texas")
    casa_str = "casa"
    zia_plan.set_l1_concept(conceptunit_shop(casa_str))
    ...
    semicolon_casa_way = zia_plan.make_l1_way(casa_str)
    zia_plan.edit_concept_attr(semicolon_casa_way, factunit=semicolon_time_factunit)
    casa_concept = zia_plan.get_concept_obj(semicolon_casa_way)
...
def test_PlanUnit_set_bridge_CorrectlySetsAttr():
    # ESTABLISH
    x_vow_label = "accord45"
    slash_bridge = "/"
    sue_str = "Sue"
    sue_plan = planunit_shop(sue_str, x_vow_label, bridge=slash_bridge)
    assert sue_plan.bridge == slash_bridge

    # WHEN
    at_label_bridge = "@"
    sue_plan.set_bridge(new_bridge=at_label_bridge)

    # THEN
    assert sue_plan.bridge == at_label_bridge

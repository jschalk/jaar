from pytest import raises as pytest_raises
from src.a04_reason_logic.reason_concept import factheir_shop, factunit_shop
from src.a05_concept_logic.concept import conceptunit_shop
from src.a06_plan_logic._test_util.example_plans import (
    get_planunit_1Chore_1CE0MinutesReason_1Fact,
)
from src.a06_plan_logic.plan import planunit_shop


def test_PlanUnit_settle_plan_ChangesConceptUnit_task_chore():
    # ESTABLISH
    yao_plan = get_planunit_1Chore_1CE0MinutesReason_1Fact()
    hr_str = "hr"
    hr_way = yao_plan.make_l1_way(hr_str)

    # WHEN
    yao_plan.add_fact(fcontext=hr_way, fstate=hr_way, fopen=82, fnigh=85)

    # THEN
    mail_way = yao_plan.make_l1_way("obtain mail")
    concept_dict = yao_plan.get_concept_dict()
    mail_concept = concept_dict.get(mail_way)
    yao_plan.add_fact(fcontext=hr_way, fstate=hr_way, fopen=82, fnigh=95)
    assert mail_concept.task is True
    assert mail_concept._chore is False

    # WHEN
    yao_plan.settle_plan()

    # THEN
    mail_concept = yao_plan.get_concept_obj(mail_way)
    assert mail_concept.task
    assert mail_concept._chore


def test_PlanUnit_settle_plan_ExecutesWithRangeRootFacts():
    # ESTABLISH
    zia_plan = planunit_shop("Zia")
    casa_str = "casa"
    casa_way = zia_plan.make_l1_way(casa_str)
    zia_plan.set_l1_concept(conceptunit_shop(casa_str))
    clean_str = "clean"
    clean_way = zia_plan.make_way(casa_way, clean_str)
    clean_begin = -3
    clean_close = 7
    clean_concept = conceptunit_shop(clean_str, begin=clean_begin, close=clean_close)
    sweep_str = "sweep"
    sweep_gogo_want = -2
    sweep_stop_want = 1
    sweep_concept = conceptunit_shop(sweep_str, gogo_want=sweep_gogo_want)
    sweep_concept.stop_want = sweep_stop_want
    zia_plan.set_concept(clean_concept, parent_way=casa_way)
    zia_plan.add_fact(fcontext=clean_way, fstate=clean_way, fopen=1, fnigh=5)
    assert zia_plan.conceptroot._factheirs == {}

    # WHEN
    zia_plan.settle_plan()

    # THEN
    assert zia_plan.conceptroot._factheirs != {}
    clean_factheir = factheir_shop(clean_way, clean_way, 1.0, 5.0)
    assert zia_plan.conceptroot._factheirs == {clean_factheir.fcontext: clean_factheir}


def test_PlanUnit_settle_plan_RaisesErrorIfNonRangeRootHasFactUnit():
    # ESTABLISH
    zia_plan = planunit_shop("Zia")
    casa_str = "casa"
    casa_way = zia_plan.make_l1_way(casa_str)
    zia_plan.set_l1_concept(conceptunit_shop(casa_str))
    clean_str = "clean"
    clean_way = zia_plan.make_way(casa_way, clean_str)
    clean_begin = -3
    clean_close = 7
    clean_concept = conceptunit_shop(clean_str, begin=clean_begin, close=clean_close)
    sweep_str = "sweep"
    sweep_way = zia_plan.make_way(clean_way, sweep_str)
    sweep_concept = conceptunit_shop(sweep_str, addin=2)
    zia_plan.set_concept(clean_concept, parent_way=casa_way)
    zia_plan.set_concept(sweep_concept, parent_way=clean_way)
    zia_plan.add_fact(sweep_way, sweep_way, fopen=1, fnigh=5)

    # WHEN
    with pytest_raises(Exception) as excinfo:
        zia_plan.settle_plan()
    assert (
        str(excinfo.value)
        == f"Cannot have fact for range inheritor '{sweep_way}'. A ranged fact concept must have _begin, _close"
    )


def test_PlanUnit_settle_plan_FactHeirsCorrectlyInherited():
    # ESTABLISH
    zia_plan = planunit_shop("Zia")
    swim_str = "swim"
    swim_way = zia_plan.make_l1_way(swim_str)
    zia_plan.set_l1_concept(conceptunit_shop(swim_str))
    fast_str = "fast"
    slow_str = "slow"
    fast_way = zia_plan.make_way(swim_way, fast_str)
    slow_way = zia_plan.make_way(swim_way, slow_str)
    zia_plan.set_concept(conceptunit_shop(fast_str), parent_way=swim_way)
    zia_plan.set_concept(conceptunit_shop(slow_str), parent_way=swim_way)

    earth_str = "earth"
    earth_way = zia_plan.make_l1_way(earth_str)
    zia_plan.set_l1_concept(conceptunit_shop(earth_str))

    swim_concept = zia_plan.get_concept_obj(swim_way)
    fast_concept = zia_plan.get_concept_obj(fast_way)
    slow_concept = zia_plan.get_concept_obj(slow_way)
    zia_plan.add_fact(fcontext=earth_way, fstate=earth_way, fopen=1.0, fnigh=5.0)
    assert swim_concept._factheirs == {}
    assert fast_concept._factheirs == {}
    assert slow_concept._factheirs == {}

    # WHEN
    zia_plan.settle_plan()

    # THEN
    assert swim_concept._factheirs != {}
    assert fast_concept._factheirs != {}
    assert slow_concept._factheirs != {}
    factheir_set_range = factheir_shop(earth_way, earth_way, 1.0, 5.0)
    factheirs_set_range = {factheir_set_range.fcontext: factheir_set_range}
    assert swim_concept._factheirs == factheirs_set_range
    assert fast_concept._factheirs == factheirs_set_range
    assert slow_concept._factheirs == factheirs_set_range
    print(f"{swim_concept._factheirs=}")
    assert len(swim_concept._factheirs) == 1

    # WHEN
    swim_earth_factheir = swim_concept._factheirs.get(earth_way)
    swim_earth_factheir.set_range_null()

    # THEN
    fact_none_range = factheir_shop(earth_way, earth_way, None, None)
    facts_none_range = {fact_none_range.fcontext: fact_none_range}
    assert swim_concept._factheirs == facts_none_range
    assert fast_concept._factheirs == factheirs_set_range
    assert slow_concept._factheirs == factheirs_set_range

    fact_x1 = swim_concept._factheirs.get(earth_way)
    fact_x1.set_range_null()
    print(type(fact_x1))
    assert str(type(fact_x1)).find(".reason.FactHeir'>")


def test_PlanUnit_settle_plan_FactUnitMoldsFactHeir():
    # ESTABLISH
    zia_plan = planunit_shop("Zia")
    swim_str = "swim"
    swim_way = zia_plan.make_l1_way(swim_str)
    zia_plan.set_l1_concept(conceptunit_shop(swim_str))
    swim_concept = zia_plan.get_concept_obj(swim_way)

    fast_str = "fast"
    slow_str = "slow"
    zia_plan.set_concept(conceptunit_shop(fast_str), parent_way=swim_way)
    zia_plan.set_concept(conceptunit_shop(slow_str), parent_way=swim_way)

    earth_str = "earth"
    earth_way = zia_plan.make_l1_way(earth_str)
    zia_plan.set_l1_concept(conceptunit_shop(earth_str))

    assert swim_concept._factheirs == {}

    # WHEN
    zia_plan.add_fact(fcontext=earth_way, fstate=earth_way, fopen=1.0, fnigh=5.0)
    zia_plan.settle_plan()

    # THEN
    first_earthheir = factheir_shop(earth_way, earth_way, fopen=1.0, fnigh=5.0)
    first_earthdict = {first_earthheir.fcontext: first_earthheir}
    assert swim_concept._factheirs == first_earthdict

    # WHEN
    # earth_curb = factunit_shop(fcontext=earth_way, fstate=earth_way, popen=3.0, pnigh=4.0)
    # swim_y.set_factunit(factunit=earth_curb) Not sure what this is for. Testing what "set_factunit" does with the parameters, but what?
    zia_plan.add_fact(fcontext=earth_way, fstate=earth_way, fopen=3.0, fnigh=5.0)
    zia_plan.settle_plan()

    # THEN
    after_earthheir = factheir_shop(earth_way, earth_way, fopen=3.0, fnigh=5.0)
    after_earthdict = {after_earthheir.fcontext: after_earthheir}
    assert swim_concept._factheirs == after_earthdict


def test_PlanUnit_settle_plan_FactHeirCorrectlyDeletesFactUnit():
    # ESTABLISH
    sue_plan = planunit_shop("Sue")
    swim_str = "swim"
    swim_way = sue_plan.make_l1_way(swim_str)
    sue_plan.set_l1_concept(conceptunit_shop(swim_str))
    fast_str = "fast"
    slow_str = "slow"
    sue_plan.set_concept(conceptunit_shop(fast_str), parent_way=swim_way)
    sue_plan.set_concept(conceptunit_shop(slow_str), parent_way=swim_way)
    earth_str = "earth"
    earth_way = sue_plan.make_l1_way(earth_str)
    sue_plan.set_l1_concept(conceptunit_shop(earth_str))
    swim_concept = sue_plan.get_concept_obj(swim_way)
    first_earthheir = factheir_shop(earth_way, earth_way, fopen=200.0, fnigh=500.0)
    first_earthdict = {first_earthheir.fcontext: first_earthheir}
    sue_plan.add_fact(earth_way, earth_way, fopen=200.0, fnigh=500.0)
    assert swim_concept._factheirs == {}

    # WHEN
    sue_plan.settle_plan()

    # THEN
    assert swim_concept._factheirs == first_earthdict

    # WHEN
    earth_curb = factunit_shop(earth_way, earth_way, fopen=3.0, fnigh=4.0)
    swim_concept.set_factunit(factunit=earth_curb)
    sue_plan.settle_plan()

    # THEN
    assert swim_concept._factheirs == first_earthdict
    assert swim_concept.factunits == {}


def test_PlanUnit_settle_plan_SetsChoreAsComplete():
    # ESTABLISH
    yao_plan = get_planunit_1Chore_1CE0MinutesReason_1Fact()
    mail_str = "obtain mail"
    assert yao_plan is not None
    assert len(yao_plan.conceptroot._kids[mail_str].reasonunits) == 1
    concept_dict = yao_plan.get_concept_dict()
    mail_concept = concept_dict.get(yao_plan.make_l1_way(mail_str))
    hr_str = "hr"
    hr_way = yao_plan.make_l1_way(hr_str)
    yao_plan.add_fact(hr_way, hr_way, fopen=82, fnigh=85)
    assert mail_concept.task
    assert mail_concept._chore

    # WHEN
    yao_plan.settle_plan()

    # THEN
    assert mail_concept.task
    assert not mail_concept._chore

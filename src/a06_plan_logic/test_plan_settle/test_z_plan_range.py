...
def test_PlanUnit_get_concept_ranged_kids_ReturnsAllChildren():
    ...
    time_way = yao_planunit.make_l1_way("time")
    tech_way = yao_planunit.make_way(time_way, "tech")
    wk_str = "wk"
    wk_way = yao_planunit.make_way(tech_way, wk_str)
    wk_concept = conceptunit_shop(wk_str, begin=0, close=10800)
    yao_planunit.set_concept(wk_concept, tech_way)
    mon_str = "Monday"
    tue_str = "Tuesday"
    wed_str = "Wednesday"
    thu_str = "Thursday"
    ...
    yao_planunit.set_concept(mon_concept, wk_way)
    yao_planunit.set_concept(tue_concept, wk_way)
    yao_planunit.set_concept(wed_concept, wk_way)
    yao_planunit.set_concept(thu_concept, wk_way)
    yao_planunit.set_concept(fri_concept, wk_way)
    yao_planunit.set_concept(sat_concept, wk_way)
    yao_planunit.set_concept(sun_concept, wk_way)
    yao_planunit.settle_plan()

    # WHEN
    ranged_concepts = yao_planunit.get_concept_ranged_kids(concept_way=wk_way)
...
def test_PlanUnit_get_concept_ranged_kids_ReturnsSomeChildrenScenario1():
    ...
    ranged_concepts = yao_planunit.get_concept_ranged_kids(wk_way, x_begin, x_close)
...
def test_PlanUnit_get_concept_ranged_kids_ReturnsSomeChildrenScenario2():
    ...
    # WHEN / THEN
    assert len(yao_planunit.get_concept_ranged_kids(wk_way, 0, 1440)) == 1
    assert len(yao_planunit.get_concept_ranged_kids(wk_way, 0, 2000)) == 2
    assert len(yao_planunit.get_concept_ranged_kids(wk_way, 0, 3000)) == 3
...
def test_PlanUnit_get_concept_ranged_kids_ReturnsSomeChildrenScenario3():
    ...
    # WHEN / THEN
    assert len(yao_planunit.get_concept_ranged_kids(wk_way, 0)) == 1
    assert len(yao_planunit.get_concept_ranged_kids(wk_way, 1440)) == 1

    # ESTABLISH
    wkday_str = "wkday"
    wkdays_concept = conceptunit_shop(wkday_str, gogo_want=0, stop_want=1440 * 5)
    yao_planunit.set_concept(wkdays_concept, wk_way)

    # WHEN
    yao_planunit.settle_plan()

    # THEN
    assert len(yao_planunit.get_concept_ranged_kids(wk_way, 1440)) == 2

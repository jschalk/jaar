...
def test_PlanUnit_get_agenda_dict_ReturnsObj():
    ...
    assert sue_plan.make_l1_way("casa") in agenda_dict.keys()
    assert sue_plan.make_l1_way("cat have dinner") in agenda_dict.keys()
...
def test_PlanUnit_get_agenda_dict_ReturnsAgendaWithOnlyCorrectConcepts():
    ...
    wk_str = "wkdays"
    wk_way = x_plan.make_l1_way(wk_str)
    sun_str = "Sunday"
    sun_way = x_plan.make_way(wk_way, sun_str)
    x_plan.add_fact(fcontext=wk_way, fstate=sun_way)
    ...
    print(f"{agenda_dict=}")
    assert x_plan.make_l1_way("cat have dinner") in agenda_dict.keys()
...
def test_PlanUnit_get_agenda_dict_WithLargePlan_fund():
    ...
    assert agenda_dict
    assert len(agenda_dict) == 2
    assert agenda_dict.get(x_plan.make_l1_way("cat have dinner"))._fund_ratio

    casa_str = "casa"
    print(f"{agenda_dict.keys()=} {x_plan.make_l1_way(casa_str)=}")
    print(f"{agenda_dict.get(x_plan.make_l1_way(casa_str)).concept_label=}")
    assert agenda_dict.get(x_plan.make_l1_way(casa_str))._fund_ratio
...
def test_PlanUnit_get_agenda_dict_WithNo7amConceptExample():
    ...
    print(f"{agenda_dict.keys()=} {x_plan.make_l1_way(clean_str)=}")
    # print(f"{agenda_dict[0].concept_label=}")
    assert len(agenda_dict) == 1

    cat_str = "cat have dinner"
    cat_agenda_concept = agenda_dict.get(x_plan.make_l1_way(cat_str))
...
def test_PlanUnit_get_agenda_dict_With7amConceptExample():
    ...
    timetech_way = x_plan.make_l1_way("timetech")
    day24hr_way = x_plan.make_way(timetech_way, "24hr day")
    day24hr_popen = 0.0
    day24hr_pnigh = 8.0
    housemanagement_str = "housemanagement"
    housemanagement_way = x_plan.make_l1_way(housemanagement_str)
    clean_str = "clean table"
    clean_way = x_plan.make_way(housemanagement_way, clean_str)

    # WHEN
    x_plan.add_fact(day24hr_way, day24hr_way, day24hr_popen, day24hr_pnigh, True)

    # THEN
    print(x_plan.conceptroot.factunits[day24hr_way])
    print(x_plan.get_concept_obj(clean_way).reasonunits)
    print(x_plan.get_concept_obj(clean_way)._active)
    agenda_dict = x_plan.get_agenda_dict()
...
def test_planunit_v001_AgendaExists():
    ...
    min_str = "day_minute"
    min_way = yao_plan.make_l1_way(min_str)
    yao_plan.add_fact(fcontext=min_way, fstate=min_way, fopen=0, fnigh=1399)
    assert yao_plan
...
def test_PlanUnit_get_agenda_dict_PlanUnitHasCorrectAttributes_planunit_v001():
    ...
    yao_plan.add_fact(fcontext=day_min_way, fstate=day_min_way, fopen=0, fnigh=1399)
    month_wk_str = "month_wk"
    month_wk_way = yao_plan.make_l1_way(month_wk_str)
    nations_str = "Nation-States"
    nations_way = yao_plan.make_l1_way(nations_str)
    mood_str = "Moods"
    mood_way = yao_plan.make_l1_way(mood_str)
    aaron_str = "Aaron Donald objects effected by him"
    aaron_way = yao_plan.make_l1_way(aaron_str)
    # interweb_str = "Interweb"
    # interweb_way = yao_plan.make_l1_way(interweb_str)
    yr_month_str = "yr_month"
    yr_month_way = yao_plan.make_l1_way(yr_month_str)
    yao_plan.add_fact(fcontext=month_wk_way, fstate=month_wk_way)
    yao_plan.add_fact(fcontext=nations_way, fstate=nations_way)
    yao_plan.add_fact(fcontext=mood_way, fstate=mood_way)
    yao_plan.add_fact(fcontext=aaron_way, fstate=aaron_way)
    # yao_plan.add_fact(fcontext=interweb_way, fstate=interweb_way)
    yao_plan.add_fact(fcontext=yr_month_way, fstate=yr_month_way)
    # season_str = "Seasons"
    # season_way = yao_plan.make_l1_way(season_str)
    # yao_plan.add_fact(fcontext=season_way, fstate=season_way)
    ced_wk_str = "ced_wk"
    ced_wk_way = yao_plan.make_l1_way(ced_wk_str)
    yao_plan.add_fact(fcontext=ced_wk_way, fstate=ced_wk_way)
    # water_str = "WaterExistence"
    # water_way = yao_plan.make_l1_way(water_str)
    # yao_plan.add_fact(fcontext=water_way, fstate=water_way)
    # movie_str = "No Movie playing"
    # movie_way = yao_plan.make_l1_way(movie_str)
    # yao_plan.add_fact(fcontext=movie_way, fstate=movie_str)
    ...
    wk1_way = yao_plan.make_way(month_wk_way, "1st wk")
    yao_plan.add_fact(month_wk_way, wk1_way)
    concept_task_list = yao_plan.get_agenda_dict()
    assert len(concept_task_list) == 27

    wkday_str = "wkdays"
    wkday_way = yao_plan.make_l1_way(wkday_str)
    monday_str = "Monday"
    monday_way = yao_plan.make_way(wkday_way, monday_str)

    yao_plan.add_fact(fcontext=wkday_way, fstate=monday_way)
    concept_task_list = yao_plan.get_agenda_dict()
    assert len(concept_task_list) == 39

    yao_plan.add_fact(fcontext=wkday_way, fstate=wkday_way)
    concept_task_list = yao_plan.get_agenda_dict()
...
def test_PlanUnit_get_agenda_dict_PlanUnitCanCleanOnRcontext_planunit_v001_with_large_agenda():
    ...
    # WHEN
    task_list = yao_plan.get_agenda_dict(necessary_rcontext=wk_way)
...
def test_PlanUnit_set_agenda_chore_as_complete_SetsAttrCorrectly_Range():
    ...
    run_str = "run"
    run_way = zia_plan.make_l1_way(run_str)
    time_way = zia_plan.make_l1_way("time")
    day_str = "day"
    day_way = zia_plan.make_way(time_way, day_str)

    zia_plan.set_l1_concept(conceptunit_shop(run_str, task=True))
    zia_plan.set_concept(conceptunit_shop(day_str, begin=0, close=500), time_way)
    ...
    zia_plan.add_fact(fcontext=day_way, fstate=day_way, fopen=30, fnigh=87)
    zia_plan.get_agenda_dict()
    run_reasonunits = zia_plan.conceptroot._kids[run_str].reasonunits[day_way]
    ...
    assert zia_plan.get_agenda_dict().get(run_way)._chore is True

    # WHEN
    zia_plan.set_agenda_chore_complete(chore_way=run_way, rcontext=day_way)
...
def test_PlanUnit_set_agenda_chore_as_complete_SetsAttrCorrectly_Division():
    ...
    run_way = zia_plan.make_l1_way(run_str)
    time_str = "time"
    time_way = zia_plan.make_l1_way(time_str)
    day_str = "day"
    day_way = zia_plan.make_way(time_way, day_str)
    ...
    run_concept = zia_plan.get_concept_obj(run_way)
    # print(f"{run_concept._factheirs=}")
    zia_plan.add_fact(fcontext=day_way, fstate=day_way, fopen=1, fnigh=2)
    assert len(zia_plan.get_agenda_dict()) == 1
    zia_plan.add_fact(fcontext=day_way, fstate=day_way, fopen=2, fnigh=2)
    assert len(zia_plan.get_agenda_dict()) == 0
    zia_plan.add_fact(fcontext=day_way, fstate=day_way, fopen=400, fnigh=400)
    assert len(zia_plan.get_agenda_dict()) == 0
    zia_plan.add_fact(fcontext=day_way, fstate=day_way, fopen=401, fnigh=402)
    assert len(zia_plan.get_agenda_dict()) == 1
    # print(f"{run_concept._factheirs=}")
...
def test_planunit_get_from_json_CorrectlyLoadsTaskFromJSON():
    ...
    body_str = "exercise"
    body_way = yao_plan.make_way(casa_way, body_str)
    veg_str = "cook veggies every morning"
    veg_way = yao_plan.make_way(body_way, veg_str)
    veg_concept = yao_plan.get_concept_obj(veg_way)
...
def test_PlanUnit_set_fact_Isue116Resolved_correctlySetsChoreAsTrue():
    ...
    time_way = yao_plan.make_l1_way("time")
    gregtime_way = yao_plan.make_way(time_way, "gregtime")

    # WHEN
    yao_plan.add_fact(gregtime_way, gregtime_way, fopen=1063998720, fnigh=1064130373)
    task_concept_list = yao_plan.get_agenda_dict()

    # THEN
    assert len(task_concept_list) == 66
    db_way = yao_plan.make_l1_way("D&B")
    evening_str = "late_evening_go_to_sleep"
    evening_way = yao_plan.make_way(db_way, evening_str)
    evening_concept = yao_plan._concept_dict.get(evening_way)
...
def test_PlanUnit_agenda_IsSetByLaborUnit_1AcctGroup():
    ...
    # WHEN
    yao_plan.edit_concept_attr(casa_way, laborunit=laborunit_sue)
    ...
    # WHEN
    yao_plan.edit_concept_attr(casa_way, laborunit=laborunit_yao)
...
def test_PlanUnit_get_agenda_dict_IsSetByLaborUnit_2AcctGroup():
    ...
    # WHEN
    yao_plan.edit_concept_attr(casa_way, laborunit=run_laborunit)
...
def test_PlanUnit_get_all_tasks_ReturnsObj():
    ...
    clean_str = "clean"
    clean_way = zia_plan.make_way(casa_way, clean_str)
    sweep_str = "sweep"
    sweep_way = zia_plan.make_way(clean_way, sweep_str)
    couch_str = "couch"
    couch_way = zia_plan.make_way(casa_way, couch_str)
    zia_plan.set_concept(conceptunit_shop(couch_str), casa_way)
    zia_plan.set_concept(conceptunit_shop(clean_str, task=True), casa_way)
    zia_plan.set_concept(conceptunit_shop(sweep_str, task=True), clean_way)
    sweep_concept = zia_plan.get_concept_obj(sweep_way)
    yao_str = "Yao"
    ...
    # THEN
    assert all_tasks_dict.get(sweep_way) == zia_plan.get_concept_obj(sweep_way)
    assert all_tasks_dict.get(clean_way) == zia_plan.get_concept_obj(clean_way)

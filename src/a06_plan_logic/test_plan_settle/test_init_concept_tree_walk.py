...
def test_PlanUnit_set_concept_dict_Scenario0():
    ...
    assert not root_concept._gogo_calc
    assert not root_concept._stop_calc
    assert yao_plan._concept_dict == {root_concept.get_concept_way(): root_concept}
    assert yao_plan._reason_rcontexts == set()
...
def test_PlanUnit_set_concept_dict_Scenario1():
    ...
    time0_close = 31
    root_way = to_way(yao_plan.vow_label)
    yao_plan.edit_concept_attr(root_way, begin=time0_begin, close=time0_close)
    root_way = to_way(yao_plan.vow_label)
    root_concept = yao_plan.get_concept_obj(root_way)
...
def test_PlanUnit_set_concept_dict_Clears_gogo_calc_stop_calc():
    ...
    texas_str = "Texas"
    texas_way = sue_plan.make_way(usa_way, texas_str)
...
def test_PlanUnit_set_concept_dict_Sets_reason_rcontexts():
    ...
    nation_way = sue_plan.make_l1_way(nation_str)
    polis_str = "polis"
    polis_way = sue_plan.make_l1_way(polis_str)
    sue_plan.add_concept(polis_way)
    sue_plan.add_concept(nation_way)
    ...
    # THEN
    assert sue_plan._reason_rcontexts == {polis_way}
...
def test_PlanUnit_set_concept_CreatesConceptUnitsUsedBy_reasonunits():
    ...
    cleaning_way = sue_plan.make_way(casa_way, "cleaning")
    ...
    buildings_way = sue_plan.make_l1_way(buildings_str)
    cookery_room_str = "cookery"
    cookery_room_way = sue_plan.make_way(buildings_way, cookery_room_str)
    ...
    assert sue_plan.concept_exists(buildings_way) is False
    ...
    # THEN
    assert sue_plan.concept_exists(buildings_way)
...
def test_get_sorted_concept_list_ReturnsObj():
    ...
    assert x_sorted_concept_list[1] == sue_plan.get_concept_obj(casa_way)
    assert x_sorted_concept_list[11] == sue_plan.get_concept_obj(mon_way)

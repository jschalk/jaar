...
def clear_all_acctunits_groupunits_fund_agenda_give_take(x_plan: PlanUnit):
    ...
    for groupunit_x in x_plan._groupunits.values():
        ...
        #     print(f"{groupunit_x.} {membership_x.}  {membership_x._fund_give:.6f} {membership_x.debtit_score=} {membership__fund_take:t:.6f} {membership_x.} ")
...
def test_PlanUnit_settle_plan_SetsAttrsWhenNoFactUnitsNoReasonUnitsEmpty_agenda_ratio_cred_debt():
    ...
    bob_str = "Bob"
    zia_str = "Zia"
    sue_acctunit = acctunit_shop(sue_str, 0.5, debtit_score=2)
    bob_acctunit = acctunit_shop(bob_str, 1.5, debtit_score=3)
    zia_acctunit = acctunit_shop(zia_str, 8, debtit_score=5)

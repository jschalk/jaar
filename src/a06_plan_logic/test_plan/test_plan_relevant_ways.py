...
def test_PlanUnit_get_relevant_ways_EmptyWayTermReturnsEmpty():
    # ESTABLISH
    sue_plan = get_planunit_with_4_levels()

    # WHEN
    relevant_ways = sue_plan._get_relevant_ways({})
...
def test_PlanUnit_get_relevant_ways_RootWayTermReturnsOnlyItself():
    # ESTABLISH
    sue_plan = get_planunit_with_4_levels()
    ...
    relevant_ways = sue_plan._get_relevant_ways(root_dict)
...
def test_PlanUnit_get_relevant_ways_SimpleReturnsOnlyAncestors():
    # ESTABLISH
    sue_plan = get_planunit_with_4_levels()
    root_way = to_way(sue_plan.vow_label)
    ...
    sun_dict = {sun_way}
    relevant_ways = sue_plan._get_relevant_ways(sun_dict)
...
def test_PlanUnit_get_relevant_ways_ReturnsSimpleReasonUnitRcontext():
    # ESTABLISH
    ...
    sue_plan.set_concept(floor_concept, parent_way=casa_way)

    unim_str = "unimportant"
    unim_way = sue_plan.make_l1_way(unim_str)
    unim_concept = conceptunit_shop(unim_str)
    sue_plan.set_concept(unim_concept, parent_way=sue_plan.vow_label)

    status_str = "cleaniness status"
    status_way = sue_plan.make_way(casa_way, status_str)
    status_concept = conceptunit_shop(status_str)
    sue_plan.set_concept(status_concept, parent_way=casa_way)
    ...
    relevant_ways = sue_plan._get_relevant_ways(floor_dict)
...
def test_PlanUnit_get_relevant_ways_ReturnsReasonUnitRcontextAndDescendents():
    # ESTABLISH
    x_plan = get_mop_with_reason_planunit_example1()
    root_way = to_way(x_plan.vow_label)
    casa_str = "casa"
    casa_way = x_plan.make_l1_way(casa_str)
    floor_str = "mop floor"
    floor_way = x_plan.make_way(casa_way, floor_str)

    unim_str = "unimportant"
    unim_way = x_plan.make_l1_way(unim_str)

    status_str = "cleaniness status"
    status_way = x_plan.make_way(casa_way, status_str)

    clean_str = "clean"
    clean_way = x_plan.make_way(status_way, clean_str)

    very_much_str = "very_much"
    very_much_way = x_plan.make_way(clean_way, very_much_str)

    moderately_str = "moderately"
    moderately_way = x_plan.make_way(clean_way, moderately_str)

    dirty_str = "dirty"
    dirty_way = x_plan.make_way(status_way, dirty_str)

    # WHEN
    floor_dict = {floor_way}
    relevant_ways = x_plan._get_relevant_ways(floor_dict)
...
def test_PlanUnit_get_relevant_ways_ReturnSimple():
    ...
    day_length_way = yao_plan.make_l1_way(day_length_str)
    ...
    hr_length_way = yao_plan.make_l1_way(hr_length_str)
    ...
    min_days_way = yao_plan.make_way(min_range_x_way, min_days_str)
    min_days_concept = conceptunit_shop(min_days_str)
    yao_plan.set_concept(min_days_concept, parent_way=min_range_x_way)

    # WHEN
    print(f"{yao_plan._concept_dict.keys()}")
    ways_dict = {min_days_way}
    relevant_ways = yao_plan._get_relevant_ways(ways_dict)
    ...
    # min_days_concept = yao_plan.get_concept_obj(min_days_way)


def test_PlanUnit_get_inheritor_concept_list_ReturnsObj_Scenario0():
    # ESTABLISH
    yao_planunit = planunit_shop("Yao")
    tech_way = yao_planunit.make_l1_way("tech")
    wk_str = "wk"
    wk_way = yao_planunit.make_way(tech_way, wk_str)
    yao_planunit.set_concept(conceptunit_shop(wk_str, begin=0, close=10800), tech_way)
    mon_str = "Monday"
    mon_way = yao_planunit.make_way(wk_way, mon_str)
    yao_planunit.set_concept(conceptunit_shop(mon_str), wk_way)
    yao_planunit.settle_plan()

    # WHEN
    x_inheritor_concept_list = yao_planunit.get_inheritor_concept_list(wk_way, mon_way)

    # # THEN
    assert len(x_inheritor_concept_list) == 2
    wk_concept = yao_planunit.get_concept_obj(wk_way)
    mon_concept = yao_planunit.get_concept_obj(mon_way)

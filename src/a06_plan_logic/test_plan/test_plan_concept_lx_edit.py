...
def test_PlanUnit_edit_concept_label_FailsWhenConceptDoesNotExist():
    ...
    yao_plan.set_concept(conceptunit_shop(swim_str), parent_way=casa_way)

    # WHEN / THEN
    no_concept_way = yao_plan.make_l1_way("bees")
    with pytest_raises(Exception) as excinfo:
        yao_plan.edit_concept_label(old_way=no_concept_way, new_concept_label="birds")
...
def test_PlanUnit_edit_concept_label_RaisesErrorForLevel0ConceptWhen_vow_label_isNone():
    ...
    casa_way = yao_plan.make_l1_way(casa_str)
    swim_str = "swim"
    swim_way = yao_plan.make_way(casa_way, swim_str)
    yao_plan.set_l1_concept(conceptunit_shop(casa_str))
    yao_plan.set_concept(conceptunit_shop(swim_str), parent_way=casa_way)
    ...
    assert casa_concept.parent_way == to_way(yao_plan.vow_label)
    ...
    with pytest_raises(Exception) as excinfo:
        moon_str = "moon"
        yao_plan.edit_concept_label(old_way=root_way, new_concept_label=moon_str)
    assert (
        str(excinfo.value)
...
def test_PlanUnit_find_replace_way_CorrectlyModifies_kids_Scenario1():
    # ESTABLISH Concept with kids that will be different
    yao_str = "Yao"
    yao_plan = planunit_shop(yao_str)

    old_casa_str = "casa"
    old_casa_way = yao_plan.make_l1_way(old_casa_str)
    bloomers_str = "bloomers"
    old_bloomers_way = yao_plan.make_way(old_casa_way, bloomers_str)
    roses_str = "roses"
    old_roses_way = yao_plan.make_way(old_bloomers_way, roses_str)
    red_str = "red"
    old_red_way = yao_plan.make_way(old_roses_way, red_str)

    yao_plan.set_l1_concept(conceptunit_shop(old_casa_str))
    yao_plan.set_concept(conceptunit_shop(bloomers_str), parent_way=old_casa_way)
    yao_plan.set_concept(conceptunit_shop(roses_str), parent_way=old_bloomers_way)
    yao_plan.set_concept(conceptunit_shop(red_str), parent_way=old_roses_way)
    r_concept_roses = yao_plan.get_concept_obj(old_roses_way)
    r_concept_bloomers = yao_plan.get_concept_obj(old_bloomers_way)
    ...
    # WHEN
    new_casa_str = "casita"
    new_casa_way = yao_plan.make_l1_way(new_casa_str)
    yao_plan.edit_concept_label(old_way=old_casa_way, new_concept_label=new_casa_str)
    ...
    new_bloomers_way = yao_plan.make_way(new_casa_way, bloomers_str)
    assert r_concept_roses.parent_way == new_bloomers_way
    assert r_concept_roses._kids.get(red_str) is not None
    r_concept_red = r_concept_roses._kids.get(red_str)
    new_roses_way = yao_plan.make_way(new_bloomers_way, roses_str)
...
def test_plan_edit_concept_label_Modifies_factunits():
    ...
    casa_way = yao_plan.make_l1_way(casa_str)
    bloomers_str = "bloomers"
    bloomers_way = yao_plan.make_way(casa_way, bloomers_str)
    roses_str = "roses"
    roses_way = yao_plan.make_way(bloomers_way, roses_str)
    old_water_str = "water"
    old_water_way = yao_plan.make_l1_way(old_water_str)
    rain_str = "rain"
    old_rain_way = yao_plan.make_way(old_water_way, rain_str)

    yao_plan.set_l1_concept(conceptunit_shop(casa_str))
    yao_plan.set_concept(conceptunit_shop(roses_str), parent_way=bloomers_way)
    yao_plan.set_concept(conceptunit_shop(rain_str), parent_way=old_water_way)
    yao_plan.add_fact(fcontext=old_water_way, fstate=old_rain_way)

    concept_x = yao_plan.get_concept_obj(roses_way)
    assert yao_plan.conceptroot.factunits[old_water_way] is not None
    old_water_rain_factunit = yao_plan.conceptroot.factunits[old_water_way]
    assert old_water_rain_factunit.fcontext == old_water_way
    assert old_water_rain_factunit.fstate == old_rain_way

    # WHEN
    new_water_str = "h2o"
    new_water_way = yao_plan.make_l1_way(new_water_str)
    yao_plan.set_l1_concept(conceptunit_shop(new_water_str))
    assert yao_plan.conceptroot.factunits.get(new_water_way) is None
    yao_plan.edit_concept_label(old_way=old_water_way, new_concept_label=new_water_str)

    # THEN
    assert yao_plan.conceptroot.factunits.get(old_water_way) is None
    assert yao_plan.conceptroot.factunits.get(new_water_way) is not None
    new_water_rain_factunit = yao_plan.conceptroot.factunits[new_water_way]
    assert new_water_rain_factunit.fcontext == new_water_way
    new_rain_way = yao_plan.make_way(new_water_way, rain_str)
    assert new_water_rain_factunit.fstate == new_rain_way

    assert yao_plan.conceptroot.factunits.get(new_water_way)
    x_factunit = yao_plan.conceptroot.factunits.get(new_water_way)
...
def test_plan_edit_concept_label_ModifiesConceptReasonUnitsScenario1():
    ...
    old_wkday_way = sue_plan.make_l1_way(old_wkday_str)
    wednesday_str = "Wednesday"
    old_wednesday_way = sue_plan.make_way(old_wkday_way, wednesday_str)
    casa_concept = sue_plan.get_concept_obj(sue_plan.make_l1_way("casa"))
    ...
    wednesday_concept = sue_plan.get_concept_obj(old_wkday_way)
    ...
    new_wkday_way = sue_plan.make_l1_way(new_wkday_str)
    new_wednesday_way = sue_plan.make_way(new_wkday_way, wednesday_str)
    ...
    sue_plan.edit_concept_label(old_way=old_wkday_way, new_concept_label=new_wkday_str)
    # for key_x, x_reason in casa_concept.reasonunits.items():
...
def test_plan_set_owner_name_CorrectlyModifiesBoth():
    ...
    # sue_plan.edit_concept_label(old_way=old_concept_label, new_concept_label=mid_concept_label1)
...
def test_plan_edit_concept_label_RaisesErrorIfbridgeIsInLabel():
    # ESTABLISH
    sue_plan = get_planunit_with_4_levels_and_2reasons_2facts()
    old_wkday_str = "wkdays"
    old_wkday_way = sue_plan.make_l1_way(old_wkday_str)
    ...
    assert (
        str(excinfo.value)
        == f"Cannot modify '{old_wkday_way}' because new_concept_label {new_wkday_str} contains bridge {sue_plan.bridge}"
    )

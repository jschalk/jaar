from src.a01_term_logic.test._util.a01_str import (
    LabelTerm_str,
    NameTerm_str,
    RopeTerm_str,
    TitleTerm_str,
    parent_rope_str,
)
from src.a02_finance_logic.test._util.a02_str import (
    acct_name_str,
    amount_str,
    fund_iota_str,
    fund_pool_str,
    penny_str,
)
from src.a03_group_logic.test._util.a03_str import (
    acct_cred_points_str,
    acct_debt_points_str,
    awardee_title_str,
    fund_give_str,
    fund_take_str,
    give_force_str,
    group_cred_points_str,
    group_debt_points_str,
    group_title_str,
    respect_bit_str,
    take_force_str,
)
from src.a04_reason_logic.test._util.a04_str import (
    fcontext_str,
    fnigh_str,
    fopen_str,
    fstate_str,
    labor_title_str,
    pnigh_str,
    popen_str,
    pstate_str,
    rconcept_active_requisite_str,
    rcontext_str,
)
from src.a05_concept_logic.test._util.a05_str import (
    addin_str,
    begin_str,
    close_str,
    concept_label_str,
    concept_rope_str,
    denom_str,
    gogo_want_str,
    healer_name_str,
    mass_str,
    morph_str,
    numor_str,
    stop_want_str,
    task_str,
)


def _keeps_buildable_str() -> str:
    return "_keeps_buildable"


def _keeps_justified_str() -> str:
    return "_keeps_justified"


def _offtrack_fund_str() -> str:
    return "_offtrack_fund"


def _offtrack_kids_mass_set_str() -> str:
    return "_offtrack_kids_mass_set"


def _rational_str() -> str:
    return "_rational"


def _reason_rcontexts_str() -> str:
    return "_reason_rcontexts"


def _sum_healerlink_share_str() -> str:
    return "_sum_healerlink_share"


def _tree_traverse_count_str() -> str:
    return "_tree_traverse_count"


def acct_pool_str() -> str:
    return "acct_pool"


def ancestors_str() -> str:
    return "ancestors"


def attributes_str() -> str:
    return "attributes"


def credor_respect_str() -> str:
    return "credor_respect"


def debtor_respect_str() -> str:
    return "debtor_respect"


def dimen_str() -> str:
    return "dimen"


def dimens_str() -> str:
    return "dimens"


def jkeys_str() -> str:
    return "jkeys"


def last_pack_id_str() -> str:
    return "last_pack_id"


def mandate_str() -> str:
    return "mandate"


def max_tree_traverse_str() -> str:
    return "max_tree_traverse"


def plan_acct_membership_str() -> str:
    return "plan_acct_membership"


def plan_acctunit_str() -> str:
    return "plan_acctunit"


def plan_concept_awardlink_str() -> str:
    return "plan_concept_awardlink"


def plan_concept_factunit_str() -> str:
    return "plan_concept_factunit"


def plan_concept_healerlink_str() -> str:
    return "plan_concept_healerlink"


def plan_concept_laborlink_str() -> str:
    return "plan_concept_laborlink"


def plan_concept_reason_premiseunit_str() -> str:
    return "plan_concept_reason_premiseunit"


def plan_concept_reasonunit_str() -> str:
    return "plan_concept_reasonunit"


def plan_conceptunit_str() -> str:
    return "plan_conceptunit"


def planunit_str() -> str:
    return "planunit"


def tally_str() -> str:
    return "tally"


def timeline_str() -> str:
    return "timeline"

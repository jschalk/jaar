...
def test_PlanUnit_set_belief_label_CorrectlySetsAttr():
    ...
    # WHEN
    sue_plan.set_belief_label(belief_label=x_belief_label)

    # THEN
    assert sue_plan.belief_label == x_belief_label


def test_PlanUnit_set_concept_CorrectlySetsbelief_label_AND_fund_iota():
    # ESTABLISH'
    x_fund_iota = 500
    sue_plan = get_planunit_with_4_levels()
    sue_plan.fund_iota = x_fund_iota
    plan_belief_label = "Texas"
    sue_plan.set_belief_label(plan_belief_label)
    assert sue_plan.belief_label == plan_belief_label
    ...
    assert cookery_concept.belief_label == plan_belief_label
    assert cookery_concept.fund_iota == x_fund_iota


def test_plan_set_belief_label_CorrectlySetsAttr():
    # ESTABLISH
    yao_str = "Yao"
    ...
    assert yao_plan.conceptroot.concept_label == yao_plan.belief_label
    casa_concept = yao_plan.get_concept_obj(old_casa_rope)
    assert casa_concept.parent_rope == to_rope(yao_plan.belief_label)
    ...
    assert yao_plan.belief_label == x_belief_label
    assert yao_plan.conceptroot.concept_label == x_belief_label
...
def test_plan_set_knot_CorrectlyModifies_parent_rope():
    ...
    semicolon_cook_rope = zia_plan.make_rope(semicolon_casa_rope, cook_str)
    # print(f"{zia_plan.belief_label=} {zia_plan.conceptroot.concept_label=} {casa_rope=}")
    ...
    # THEN
    assert cook_concept.get_concept_rope() != semicolon_cook_rope
    zia_belief_label = zia_plan.belief_label

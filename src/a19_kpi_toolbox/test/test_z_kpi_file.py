...
def test_create_kpi_csvs_Scenario0_NotCreateFileWhenNoKPITables(env_dir_setup_cleanup):
    ...
    with sqlite3_connect(db_path) as db_conn:
        ...
        create_populate_table = (
            "CREATE TABLE test_table AS SELECT 'fizz' as belief_label;"
        )
...
def test_create_kpi_csvs_Scenario1_CreateFile(env_dir_setup_cleanup):
    ...
    with sqlite3_connect(db_path) as db_conn:
        cursor = db_conn.cursor()
        cursor.execute("CREATE TABLE test_table AS SELECT 'fizz' as belief_label;")
        cursor.execute(
            f"CREATE TABLE {kpi_tablename} AS SELECT 'fizz' as belief_label;"
        )
    ...
    expected_df = DataFrame(["fizz"], columns=["belief_label"])

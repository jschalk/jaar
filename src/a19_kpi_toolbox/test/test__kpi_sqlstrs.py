...
, {coin_partner_nets_str()}.{believer_name_str()}
...
LEFT JOIN {blrplan_job} ON
  ...
  AND {blrplan_job}.{believer_name_str()} = {coin_partner_nets_str()}.{believer_name_str()}
GROUP BY {coin_partner_nets_str()}.{coin_label_str()}, {coin_partner_nets_str()}.{believer_name_str()}
;
"""
    assert kpi001_sqlstr == expected_kpi001_sqlstr


def test_get_coin_kpi002_believer_tasks_sqlstr_ReturnsObj():
    # ESTABLISH
    blrplan_str = believer_planunit_str()
    blrplan_job = create_prime_tablename(blrplan_str, "job", None)

    # WHEN
    kpi002_sqlstr = get_coin_kpi002_believer_tasks_sqlstr()

    # THEN
    expected_kpi002_sqlstr = f"""
CREATE TABLE {coin_kpi002_believer_tasks_str()} AS
SELECT

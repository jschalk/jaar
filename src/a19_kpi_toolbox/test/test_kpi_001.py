...
"""
        ...
        insert_sqlstr = f"""
INSERT INTO {job_onrplan_tablename} ({belief_label_str()}, {believer_name_str()}, {plan_rope_str()}, {task_str()})
...
"""
        ...
        # THEN
        assert get_row_count(cursor, belief_kpi001_acct_nets_tablename)
        select_sqlstr = f"""SELECT {belief_label_str()}, {believer_name_str()}, funds, fund_rank, tasks_count FROM {belief_kpi001_acct_nets_tablename}"""
        cursor.execute(select_sqlstr)
        rows = cursor.fetchall()
        print(rows)
        assert rows == [

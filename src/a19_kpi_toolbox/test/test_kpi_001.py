...
"""
        ...
        # THEN
        assert get_table_columns(cursor, belief_kpi001_partner_nets_tablename) == [
            belief_label_str(),
            believer_name_str(),
            "funds",
            "fund_rank",
        ...
        assert get_row_count(cursor, belief_kpi001_partner_nets_tablename)
        select_sqlstr = f"""
        SELECT 
  {belief_label_str()}
, {believer_name_str()}
...
, fund_rank
, tasks_count
FROM {belief_kpi001_partner_nets_tablename}
"""
...
"""
        ...
        # THEN
        assert get_row_count(cursor, belief_kpi001_partner_nets_tablename)
        select_sqlstr = f"""SELECT {belief_label_str()}, {believer_name_str()}, funds, fund_rank, tasks_count FROM {belief_kpi001_partner_nets_tablename}"""
        cursor.execute(select_sqlstr)
        rows = cursor.fetchall()
        print(rows)
        assert rows == [

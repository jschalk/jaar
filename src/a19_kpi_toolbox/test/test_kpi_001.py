...
"""
        ...
        insert_sqlstr = f"""
INSERT INTO {job_blrplan_tablename} ({coin_label_str()}, {believer_name_str()}, {plan_rope_str()}, {task_str()})
...
"""
        ...
        # THEN
        assert get_row_count(cursor, coin_kpi001_partner_nets_tablename)
        select_sqlstr = f"""SELECT {coin_label_str()}, {believer_name_str()}, funds, fund_rank, tasks_count FROM {coin_kpi001_partner_nets_tablename}"""
        cursor.execute(select_sqlstr)
        rows = cursor.fetchall()
        print(rows)
        assert rows == [

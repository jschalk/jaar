...
"""
        ...
        # THEN
        assert get_table_columns(cursor, belief_kpi001_person_nets_tablename) == [
            belief_label_str(),
            believer_name_str(),
            "funds",
            "fund_rank",
        ...
        assert get_row_count(cursor, belief_kpi001_person_nets_tablename)
        select_sqlstr = f"""
        SELECT 
  {belief_label_str()}
, {believer_name_str()}
...
, fund_rank
, tasks_count
FROM {belief_kpi001_person_nets_tablename}
"""
...
"""
        ...
        insert_sqlstr = f"""
INSERT INTO {job_blrplan_tablename} ({belief_label_str()}, {believer_name_str()}, {plan_rope_str()}, {task_str()})
...
"""
        ...
        # THEN
        assert get_row_count(cursor, belief_kpi001_person_nets_tablename)
        select_sqlstr = f"""SELECT {belief_label_str()}, {believer_name_str()}, funds, fund_rank, tasks_count FROM {belief_kpi001_person_nets_tablename}"""
        cursor.execute(select_sqlstr)
        rows = cursor.fetchall()
        print(rows)
        assert rows == [
            (a23_str, bob_str, bob_person_net, 1, 1),
            (a23_str, yao_str, yao_person_net, 2, 0),
        ]

...
def test_create_calendar_markdown_files_Senario0_NoFileIfFisIsEmpty(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    temp_dir = get_module_temp_dir()
    bank_mstr_dir = create_path(temp_dir, "bank_mstr")
    output_dir = create_path(temp_dir, "output")
    assert not os_path_exists(output_dir)

    # WHEN
    create_calendar_markdown_files(bank_mstr_dir, output_dir)
...
def test_create_calendar_markdown_files_Senario1_Add_CreatesFile(
    ...
    output_dir = create_path(temp_dir, "output")
    a23_str = "accord23"
    a23_bank_path = create_bank_json_path(bank_mstr_dir, a23_str)
    a23_bankunit = bankunit_shop(a23_str, bank_mstr_dir)
...
#     fizz_fis = shop(fizz_str, fiss_dir(), output_dir)
#     sue_str = "Sue"
#     event2 = 2

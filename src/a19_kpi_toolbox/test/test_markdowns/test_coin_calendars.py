...
def test_create_calendar_markdown_files_Senario0_NoFileIfWorldIsEmpty(
    ...
    coin_mstr_dir = create_path(temp_dir, "coin_mstr")
    output_dir = create_path(temp_dir, "output")
    assert not os_path_exists(output_dir)

    # WHEN
    create_calendar_markdown_files(coin_mstr_dir, output_dir)
...
def test_create_calendar_markdown_files_Senario1_CreatesFileFromCoinUnitJSON(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    fay_str = "Fay"
    ...
    output_dir = create_path(temp_dir, "output")
    a23_str = "amy23"
    a23_coin_path = create_coin_json_path(coin_mstr_dir, a23_str)
    a23_coinunit = coinunit_shop(a23_str, coin_mstr_dir)
    assert a23_coinunit.timeline == timelineunit_shop(get_creg_config())
    save_json(a23_coin_path, None, a23_coinunit.to_dict())

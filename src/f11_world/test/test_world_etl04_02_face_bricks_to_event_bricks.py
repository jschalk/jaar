...
def test_WorldUnit_face_bricks_to_event_bricks_CreatesFaceBrickSheets_Scenario0_MultpleFaceIDs(
    ...
    upsert_sheet(sue_br00003_filepath, zoo_agg_str(), example_sue_df)
    upsert_sheet(zia_br00003_filepath, zoo_agg_str(), example_zia_df)
    ...
    assert sheet_exists(event3_br00003_filepath, zoo_agg_str()) is False
    assert sheet_exists(event7_br00003_filepath, zoo_agg_str()) is False
    ...
    assert sheet_exists(event7_br00003_filepath, zoo_agg_str())
    assert sheet_exists(event9_br00003_filepath, zoo_agg_str())
    gen_event3_df = pandas_read_excel(event3_br00003_filepath, zoo_agg_str())
    gen_event7_df = pandas_read_excel(event7_br00003_filepath, zoo_agg_str())
    gen_event9_df = pandas_read_excel(event9_br00003_filepath, zoo_agg_str())

...
#     user_str = "user"
#     hair_str = "hair"
#     key_columns = [user_str, "hair"]
#     x_column_types = {hair_str: "INTEGER", user_str: "TEXT", "y_int": "INTEGER"}

#     with sqlite3_connect(":memory:") as conn:
#         cursor = conn.cursor()
#         cursor.execute(
#             f"""CREATE TABLE {x_tablename} ({hair_str} INTEGER,{user_str} TEXT,y_int INTEGER)"""
#         )
#         cursor.execute(
#             f"""INSERT INTO {x_tablename} ({hair_str}, {user_str}, y_int)
# VALUES
# (1, "A", 100),
# (2, "A", 200),
# (3, "B", 300),
...
#         )
#         x_dir = get_instrument_temp_env_dir()
#         users_dir = create_path(x_dir, f"{user_str}s")
#         A_dir = create_path(users_dir, "A")
#         B_dir = create_path(users_dir, "B")
#         C_dir = create_path(users_dir, "C")
#         A_hair_dir = create_path(A_dir, f"{hair_str}s")
#         B_hair_dir = create_path(B_dir, f"{hair_str}s")
#         C_hair_dir = create_path(C_dir, f"{hair_str}s")
#         A1_dir = create_path(A_hair_dir, 1)
#         A2_dir = create_path(A_hair_dir, 2)
#         B3_dir = create_path(B_hair_dir, 3)
#         C4_dir = create_path(C_hair_dir, 4)
#         A1_path = create_path(A1_dir, f"{x_tablename}.csv")
#         A2_path = create_path(A2_dir, f"{x_tablename}.csv")
#         B3_path = create_path(B3_dir, f"{x_tablename}.csv")
#         C4_path = create_path(C4_dir, f"{x_tablename}.csv")
...
#         assert A1_csv[0] == (hair_str, user_str, "y_int")
#         assert A2_csv[0] == (hair_str, user_str, "y_int")
#         assert B3_csv[0] == (hair_str, user_str, "y_int")
#         assert C4_csv[0] == (hair_str, user_str, "y_int")
#         assert A1_csv[1] == expected_A1_row
#         assert A2_csv[1] == expected_A2_row
#         assert B3_csv[1] == expected_B3_row
#         assert C4_csv[1] == expected_C4_0_row

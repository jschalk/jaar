...
def test_ReasonHeir_clear_CorrectlyClearsField():
    ...
    email_premises = {email_premise.branch: email_premise}
...
def test_ReasonHeir_set_status_CorrectlySetsStatus():
    ...
    wed_premise = premiseunit_shop(branch=wed_way)
    wed_premises = {wed_premise.branch: wed_premise}
    ...
    # ESTABLISH
    thu_premise = premiseunit_shop(branch=thu_way)
    two_premises = {wed_premise.branch: wed_premise, thu_premise.branch: thu_premise}
...
def test_ReasonUnit_get_dict_ReturnsCorrectDictWithSinglethu_premiseequireds():
    ...
    static_wkday_reason_dict = {
        "context": wkday_way,
        "premises": {wed_way: {"branch": wed_way}},
    }
...
def test_ReasonUnit_get_dict_ReturnsCorrectDictWithTwoPremisesReasons():
    ...
    static_wkday_reason_dict = {
        "context": wkday_way,
        "premises": {wed_way: {"branch": wed_way}, thu_way: {"branch": thu_way}},
    }
    print(wkday_reason_dict)
    assert wkday_reason_dict == static_wkday_reason_dict
...
def test_ReasonHeir_correctSetsPledgeState():
    ...
    day_str = "ced_day"
    day_way = create_way(root_tag(), day_str)
    range_3_to_6_premise = premiseunit_shop(branch=day_way, open=3, nigh=6)
    range_3_to_6_premises = {range_3_to_6_premise.branch: range_3_to_6_premise}
...
def test_ReasonCore_set_premise_CorrectlySetsPremise():
    ...
    assert day_reason.get_premises_count() == 1
    range_3_to_6_premise = premiseunit_shop(branch=day_way, open=3, nigh=6)
    premises = {range_3_to_6_premise.branch: range_3_to_6_premise}
...
def test_ReasonCore_find_replace_way_casas():
    ...
    assert x_reason.premises.get(old_sunday_way).branch == old_sunday_way
    ...
    print(f"{x_reason.premises=}")
    assert x_reason.premises.get(new_sunday_way).branch == new_sunday_way
...
def test_ReasonCore_set_bridge_SetsAttrsCorrectly():
    ...
    assert week_reasonunit.premises.get(slash_sun_way).branch == slash_sun_way
    ...
    assert week_reasonunit.premises.get(star_sun_way).branch == star_sun_way
...
def test_ReasonCore_get_obj_key():
    ...
    email_way = create_way(casa_way, email_str)
    email_premise = premiseunit_shop(branch=email_way)
    premises_x = {email_premise.branch: email_premise}

...
def test_PremiseUnit_Exists():
    ...
    # WHEN
    email_premise = PremiseUnit(branch=email_way)

    # THEN
    assert email_premise.branch == email_way
...
def test_premiseunit_shop_ReturnsObj():
    ...
    # WHEN
    email_premise = premiseunit_shop(branch=email_way)
...
def test_PremiseUnit_clear_status_CorrectlySetsAttrs():
    ...
    casa_str = "casa"
    casa_way = create_way(root_tag(), casa_str)
    casa_premise = premiseunit_shop(branch=casa_way)
    # THEN
...
def test_PremiseUnit_is_range_IdentifiesStatus():
    ...
    # WHEN
    casa_premise = premiseunit_shop(branch=casa_way, open=1, nigh=3)
    # THEN
    ...
    # WHEN
    casa_premise = premiseunit_shop(branch=casa_way, divisor=5, open=3, nigh=3)
    # THEN
    assert casa_premise._is_range() is False
...
def test_PremiseUnit_is_in_lineage_CorrectlyIdentifiesLineageWithNonDefaultBridge():
    ...
    # THEN
    texas_premise = premiseunit_shop(branch=texas_way, bridge=slash_str)
    assert texas_premise.is_in_lineage(fact_fbranch=texas_fact.fbranch)

    idaho_premise = premiseunit_shop(branch=idaho_way, bridge=slash_str)
    assert idaho_premise.is_in_lineage(fact_fbranch=texas_fact.fbranch) is False

    usa_premise = premiseunit_shop(branch=usa_way, bridge=slash_str)
    print(f"  {usa_premise.branch=}")
    ...
    # WHEN
    sea_premise = premiseunit_shop(branch=sea_way, bridge=slash_str)
...
def test_PremiseUnit_get_range_segregate_status_ReturnsCorrectStatusBoolFor_is_rangePremise():
    ...
    yr_premise = premiseunit_shop(branch=yr_way, open=3, nigh=13)
...
def test_PremiseUnit_get_range_segregate_status_ReturnsCorrectStatusBoolForSegregatePremise():
    ...
    yr_str = "ced_year"
    yr_way = create_way(root_tag(), yr_str)
    yr_premise = premiseunit_shop(branch=yr_way, divisor=5, open=0, nigh=0)
    ...
    # ESTABLISH
    yr_premise = premiseunit_shop(branch=yr_way, divisor=5, open=0, nigh=2)
...
def test_PremiseUnitUnit_is_range_or_segregate_ReturnsCorrectBool():
    ...
    # WHEN / THEN
    wkday_premise = premiseunit_shop(branch=wkday_way)
    assert wkday_premise._is_range_or_segregate() is False

    wkday_premise = premiseunit_shop(branch=wkday_way, open=5, nigh=13)
    assert wkday_premise._is_range_or_segregate()

    wkday_premise = premiseunit_shop(branch=wkday_way, divisor=17, open=7, nigh=7)
    assert wkday_premise._is_range_or_segregate()
...
def test_PremiseUnitUnit_get_active_Returns_is_range_active_Boolean():
    ...
    wkday_str = "weekday"
    wkday_way = create_way(root_tag(), wkday_str)
    wkday_premise = premiseunit_shop(branch=wkday_way, open=3, nigh=7)
...
def test_PremiseUnitUnit_set_status_SetsAttr_status_WhenFactUnitIsNull():
    ...
    premise_2 = premiseunit_shop(branch=after_way)
    bud_fact_2 = None
...
def test_PremiseUnit_set_status_SetsAttr_status_Scenario2():
    ...
    wed_after_premise = premiseunit_shop(branch=wed_after_way)
...
def test_PremiseUnit_set_status_SetsAttr_status_Scenario5():
    ...
    wed_sun_premise = premiseunit_shop(branch=wed_cloudy_way)
...
def test_PremiseUnit_set_status_SetsStatus_status_ScenarioTime():
    ...
    hr24_premise = premiseunit_shop(branch=hr24_way, open=7, nigh=7)
...
def test_PremiseUnit_get_task_status_ReturnsObjWhen_status_IsFalse():
    ...
    no_range_premise = premiseunit_shop(branch=hr24_way)
...
def test_PremiseUnit_get_task_status_ReturnsObjWhenBool_is_range_True():
    ...
    hr24_str = "24hr"
    hr24_way = create_way(root_tag(), hr24_str)
    range_5_to_31_premise = premiseunit_shop(branch=hr24_way, open=5, nigh=31)
...
def test_PremiseUnit_get_task_status_ReturnsObjWhenBoolSegregateFalse_01():
    ...
    hr24_str = "24hr"
    hr24_way = create_way(root_tag(), hr24_str)
    o0_n0_d5_premise = premiseunit_shop(branch=hr24_way, divisor=5, open=0, nigh=0)
...
def test_PremiseUnit_set_status_SetsAttrs_Scenario01():
    ...
    hr24_str = "24hr"
    hr24_way = create_way(root_tag(), hr24_str)
    range_2_to_7_premise = premiseunit_shop(branch=hr24_way, open=2, nigh=7)
...
def test_PremiseUnit_set_status_CorrectlySetCEDWeekStatusFalse():
    ...
    o1_n1_d6_premise = premiseunit_shop(branch=week_way, divisor=6, open=1, nigh=1)
...
def test_PremiseUnit_set_status_CorrectlySetCEDWeekStatusTrue():
    ...
    week_str = "ced_week"
    week_way = create_way(timetech_way, week_str)
    week_premise = premiseunit_shop(branch=week_way, divisor=6, open=1, nigh=1)
...
def test_PremiseUnit_get_dict_ReturnsCorrectDictWithDvisiorAndOpen_Nigh():
    ...
    static_dict = {"branch": week_way, "open": 1, "nigh": 1, "divisor": 6}
...
def test_PremiseUnit_get_dict_ReturnsCorrectDictWithOpenAndNigh():
    ...
    week_str = "ced_week"
    week_way = create_way(timetech_way, week_str)
    week_premise = premiseunit_shop(branch=week_way, open=1, nigh=4)
    ...
    # THEN
    assert premise_dict is not None
    static_dict = {"branch": week_way, "open": 1, "nigh": 4}
    assert premise_dict == static_dict
...
def test_PremiseUnit_get_dict_ReturnsCorrectDictWithOnlyWayStr():
    ...
    week_str = "ced_week"
    week_way = create_way(timetech_way, week_str)
    week_premise = premiseunit_shop(branch=week_way)
...
def test_PremiseUnit_find_replace_way_casas():
    ...
    sunday_premise = premiseunit_shop(branch=old_sunday_way)
    print(sunday_premise)
    assert sunday_premise.branch == old_sunday_way
    ...
    assert sunday_premise.branch == new_sunday_way
...
def test_PremiseUnits_get_from_dict_CorrectlyBuildsObjFromIncompleteDict():
    ...
    weekday_str = "weekdays"
    weekday_way = create_way(root_tag(), weekday_str)
    static_dict = {weekday_way: {"branch": weekday_way}}
...
def test_PremiseUnitsUnit_set_bridge_SetsAttrsCorrectly():
    ...
    assert sun_premiseunit.branch == slash_sun_way
    ...
    assert sun_premiseunit.branch == star_sun_way
...
def test_way_find_replace_way_key_dict_ReturnsCorrectPremisesUnit_Scenario1():
    ...
    old_premise_x = premiseunit_shop(branch=old_seasons_way)
    old_premises_x = {old_premise_x.branch: old_premise_x}
    ...
    new_premise_x = premiseunit_shop(branch=new_seasons_way)
...
def test_way_find_replace_way_key_dict_ReturnsCorrectPremisesUnit_Scenario2():
    ...
    old_premiseunit = premiseunit_shop(branch=old_seasons_way)
    old_premiseunits = {old_premiseunit.branch: old_premiseunit}
    ...
    new_premiseunit = premiseunit_shop(branch=new_seasons_way)

...
def test_PremiseUnit_is_in_lineage_CorrectlyIdentifiesLineage():
    ...
    texas_fact = factheir_shop(base=usa_road, fpick=texas_road)

    # WHEN / THEN
    texas_premise = premiseunit_shop(need=texas_road)
    assert texas_premise.is_in_lineage(fact_fpick=texas_fact.fpick)

    # WHEN / THEN
    idaho_premise = premiseunit_shop(need=idaho_road)
    assert idaho_premise.is_in_lineage(fact_fpick=texas_fact.fpick) is False

    # WHEN / THEN
    usa_premise = premiseunit_shop(need=usa_road)
    assert usa_premise.is_in_lineage(fact_fpick=texas_fact.fpick)
    ...
    # THEN
    sea_fact = factheir_shop(base=sea_road, fpick=sea_road)
    assert sea_premise.is_in_lineage(fact_fpick=sea_fact.fpick)
    ...
    assert sea_premise.is_in_lineage(fact_fpick=seaside_fact.fpick) is False


def test_PremiseUnit_is_in_lineage_CorrectlyIdentifiesLineageWithNonDefaultBridge():
    # ESTABLISH
    slash_str = "/"
    nation_road = create_road(root_tag(), "Nation-States", bridge=slash_str)
    ...
    # "earth,seaside"
    # "earth,seaside,beach"
    sea_road = create_road("earth", "sea", bridge=slash_str)
    seaside_road = create_road("earth", "seaside", bridge=slash_str)
    seaside_beach_road = create_road(seaside_road, "beach", bridge=slash_str)
...
def test_PremiseUnit_get_range_segregate_status_ReturnsCorrectStatusBoolFor_is_rangePremise():
    # ESTABLISH
    yr_str = "ced_year"
    yr_road = create_road(root_tag(), yr_str)
    yr_premise = premiseunit_shop(need=yr_road, open=3, nigh=13)

    # WHEN / THEN
    yr_fact = factheir_shop(base=yr_road, fopen=5, fnigh=11, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact)

    yr_fact = factheir_shop(base=yr_road, fopen=1, fnigh=11, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact)

    yr_fact = factheir_shop(base=yr_road, fopen=8, fnigh=17, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact)

    yr_fact = factheir_shop(base=yr_road, fopen=0, fnigh=2, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact) is False

    yr_fact = factheir_shop(base=yr_road, fopen=15, fnigh=19, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact) is False

    yr_fact = factheir_shop(base=yr_road, fopen=1, fnigh=19, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact)

    # boundary tests
    yr_fact = factheir_shop(base=yr_road, fopen=13, fnigh=19, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact) is False

    yr_fact = factheir_shop(base=yr_road, fopen=0, fnigh=3, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact)

    yr_fact = factheir_shop(base=yr_road, fopen=0, fnigh=0, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact) is False
    yr_fact = factheir_shop(base=yr_road, fopen=3, fnigh=3, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact)
    yr_fact = factheir_shop(base=yr_road, fopen=13, fnigh=13, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact) is False
    yr_fact = factheir_shop(base=yr_road, fopen=17, fnigh=17, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact) is False

    yr_fact = factheir_shop(base=yr_road, fopen=20, fnigh=17, fpick=yr_road)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact) is False
...
def test_PremiseUnit_get_range_segregate_status_ReturnsCorrectStatusBoolForSegregatePremise():
    ...
    # WHEN / THEN
    yr_fact = factheir_shop(base=yr_road, fpick=yr_road, fopen=5, fnigh=5)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact)

    yr_fact = factheir_shop(base=yr_road, fpick=yr_road, fopen=6, fnigh=6)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact) is False

    yr_fact = factheir_shop(base=yr_road, fpick=yr_road, fopen=4, fnigh=6)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact)

    yr_fact = factheir_shop(base=yr_road, fpick=yr_road, fopen=3, fnigh=4)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact) is False
    ...
    # WHEN / THEN
    yr_fact = factheir_shop(base=yr_road, fpick=yr_road, fopen=2, fnigh=2)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact) is False

    yr_fact = factheir_shop(base=yr_road, fpick=yr_road, fopen=102, fnigh=102)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact) is False

    yr_fact = factheir_shop(base=yr_road, fpick=yr_road, fopen=1, fnigh=4)
    assert yr_premise._get_range_segregate_status(factheir=yr_fact)
...
def test_PremiseUnitUnit_is_range_or_segregate_ReturnsCorrectBool():
    # ESTABLISH
    wkday_str = "weekday"
    wkday_road = create_road(root_tag(), wkday_str)
...
def test_PremiseUnitUnit_get_premise_status_Returns_active_Boolean():
    ...
    # WHEN / THEN
    wkday_fact = factheir_shop(base=wkday_road, fpick=wkday_road)
    ...
    wkday_fact = factheir_shop(base=wkday_road, fpick=wkday_road, fopen=0, fnigh=2)
    assert wkday_premise._get_active(factheir=wkday_fact)
...
def test_PremiseUnitUnit_set_status_SetsAttr_status_OfSimple():
    ...
    bud_fact = factheir_shop(base=wkday_road, fpick=wed_road)
...
def test_PremiseUnit_set_status_SetsAttr_status_Scenario2():
    ...
    # WHEN
    wed_fact = factheir_shop(base=wkday_road, fpick=wed_road)
...
def test_PremiseUnit_set_status_SetsAttr_status_Scenario4():
    ...
    wed_premise = premiseunit_shop(need=wed_road)
    thu_fact = factheir_shop(base=wkday_road, fpick=thu_road)
    assert wed_premise._status is None
    assert wed_premise.is_in_lineage(fact_fpick=thu_fact.fpick) is False
...
def test_PremiseUnit_set_status_SetsAttr_status_Scenario5():
    ...
    # WHEN
    wed_rain_fact = factheir_shop(base=wkday_road, fpick=wed_rain_road)
...
def test_PremiseUnit_set_status_SetsStatus_status_ScenarioTime():
    ...
    timetech_road = create_road(root_tag(), timetech_str)
    hr24_str = "24hr"
...
def test_PremiseUnit_get_task_status_ReturnsObjWhen_status_IsFalse():
    # ESTABLISH
    hr24_str = "24hr"
    hr24_road = create_road(root_tag(), hr24_str)
    no_range_premise = premiseunit_shop(need=hr24_road)
    no_range_premise._status = False

    # WHEN / THEN
    no_range_fact = factheir_shop(base=hr24_road, fpick=hr24_road)
...
def test_PremiseUnit_get_task_status_ReturnsObjNotNull():
    ...
    # ESTABLISH
    factheir = factheir_shop(base=week_road, fpick=wed_road)
...
def test_PremiseUnit_set_status_SetsAttrs_Scenario03():
    ...
    # WHEN
    bud_fact = factheir_shop(base=hr24_road, fpick=hr24_road, fopen=8, fnigh=10)
...
def test_PremiseUnit_set_status_CorrectlySetCEDWeekStatusTrue():
    ...
    bud_fact = factheir_shop(base=week_road, fpick=week_road, fopen=7, fnigh=7)
...
def test_PremiseUnitsUnit_set_bridge_SetsAttrsCorrectly():
    ...
    sun_str = "Sunday"
    slash_str = "/"
    slash_week_road = create_road(root_tag(), week_str, bridge=slash_str)
    ...
    star_week_road = create_road(root_tag(), week_str, bridge=star_str)
...
def test_road_find_replace_road_key_dict_ReturnsCorrectPremisesUnit_Scenario1():
    # ESTABLISH
    casa_road = create_road(root_tag(), "casa")
...
def test_road_find_replace_road_key_dict_ReturnsCorrectPremisesUnit_Scenario2():
    ...
    casa_str = "casa"
    seasons_str = "seasons"
    old_casa_road = create_road(old_fisc_tag, casa_str)
    ...
    # WHEN
    new_fisc_tag = "Austin"
    new_casa_road = create_road(new_fisc_tag, casa_str)

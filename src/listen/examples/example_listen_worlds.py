...
def get_bud_explanation_world() -> WorldUnit:
    ...
    cat_text = "cat status"
    cat_road = sue_world.make_road(casa_road, cat_text)
    hun_n_text = "not hungry"
    hun_y_text = "hungry"
    clean_text = "cleaning"
    clean_road = sue_world.make_road(casa_road, clean_text)
    sweep_text = "sweep floor"
    dish_text = "clean dishes"
    sue_world.add_l1_idea(ideaunit_shop(casa_text, _weight=30))
    sue_world.add_idea(ideaunit_shop(cat_text, _weight=30), casa_road)
    sue_world.add_idea(ideaunit_shop(hun_n_text, _weight=30), cat_road)
    sue_world.add_idea(ideaunit_shop(hun_y_text, _weight=30), cat_road)
    sue_world.add_idea(ideaunit_shop(clean_text, _weight=30), casa_road)
    ...
    feed_text = "feed cat"
    idea_kid_feedcat = ideaunit_shop(feed_text, _weight=30, pledge=True)
    sue_world.add_l1_idea(idea_kid_feedcat)

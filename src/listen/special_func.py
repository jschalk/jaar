...
def create_pledge(
    ...
    if pledge_road is not None and get_terminus_node(pledge_road) != "":
        ...
        x_idea.pledge = True
        x_idea._doerunit.set_grouphold(x_grouphold)

        if x_grouphold is not None and x_bud.acct_exists(x_grouphold) is False:
            x_bud.add_acctunit(x_grouphold)
...
def add_voice_pledge(
    ...
    create_pledge(voice_bud, pledge_road, x_grouphold, reason_premise)

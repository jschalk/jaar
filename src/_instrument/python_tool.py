...
def create_filtered_csv_dict(
    ...
    for row in x_reader:
        ...
        new_csv_writer = csv_writer(pecun_owner_io, delimiter=",")
        new_csv_writer.writerow(row)
        place_obj_in_dict(io_dict, [row[0], row[1]], pecun_owner_io)

    x_dict = {}
    for pecun_id, owner_id_dict in io_dict.items():
        for owner_id, io_function in owner_id_dict.items():
            pecun_owner_csv = io_function.getvalue()
            pecun_owner_csv = pecun_owner_csv.replace("\r", "")
            place_obj_in_dict(x_dict, [pecun_id, owner_id], pecun_owner_csv)

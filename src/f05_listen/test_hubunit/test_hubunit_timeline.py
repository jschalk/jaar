...
def test_HubUnit_save_valid_purview_file_SavesFile(env_dir_setup_cleanup):
    ...
    t55_time_id = t55_purview.time_id
    assert os_path_exists(yao_hubunit.purview_file_path(t55_time_id)) is False

    # WHEN
    yao_hubunit._save_valid_purview_file(t55_purview)

    # THEN
    assert os_path_exists(yao_hubunit.purview_file_path(t55_time_id))
...
def test_HubUnit_purview_file_exists_ReturnsObj(env_dir_setup_cleanup):
    ...
    t55_purview = get_purviewepisode_55_example()
    t55_time_id = t55_purview.time_id
    assert yao_hubunit.purview_file_exists(t55_time_id) is False

    # WHEN
    yao_hubunit._save_valid_purview_file(t55_purview)

    # THEN
    assert yao_hubunit.purview_file_exists(t55_time_id)
...
def test_HubUnit_get_purview_file_ReturnsObj(env_dir_setup_cleanup):
    ...
    # WHEN / THEN
    assert yao_hubunit.get_purview_file(t55_time_id) == t55_purview
...
def test_HubUnit_delete_purview_file_DeletesFile(env_dir_setup_cleanup):
    ...
    # WHEN
    yao_hubunit.delete_purview_file(t55_time_id)
...
def test_HubUnit_get_purviewlog_ReturnsObj(env_dir_setup_cleanup):
    ...
    t55_time_id = t55_purview.time_id
    t66_time_id = t66_purview.time_id
    yao_hubunit._save_valid_purview_file(t55_purview)
    assert yao_hubunit.get_purviewlog().episode_exists(t55_time_id)
    assert yao_hubunit.get_purviewlog().episode_exists(t66_time_id) is False
    yao_hubunit._save_valid_purview_file(t66_purview)

    # WHEN / THEN
    assert yao_hubunit.get_purviewlog().episode_exists(t55_time_id)
    assert yao_hubunit.get_purviewlog().episode_exists(t66_time_id)
    assert yao_hubunit.get_purviewlog().get_episode(t66_time_id).get_net_purview("Sue")
...
def test_HubUnit_save_valid_budpoint_file_SavesFile(env_dir_setup_cleanup):
    ...
    t55_time_id = 55
    assert os_path_exists(yao_hubunit.budpoint_file_path(t55_time_id)) is False

    # WHEN
    yao_hubunit._save_valid_budpoint_file(t55_time_id, t55_budpoint)

    # THEN
    assert os_path_exists(yao_hubunit.budpoint_file_path(t55_time_id))
...
def test_HubUnit_budpoint_file_exists_ReturnsObj(env_dir_setup_cleanup):
    ...
    yao_hubunit = hubunit_shop(fiscals_dir(), fiscal_id(), yao_str)
    t55_time_id = 55
    assert yao_hubunit.budpoint_file_exists(t55_time_id) is False
    ...
    # THEN
    assert yao_hubunit.budpoint_file_exists(t55_time_id)
...
def test_HubUnit_get_budpoint_file_ReturnsObj(env_dir_setup_cleanup):
    ...
    # WHEN
    file_budpoint = yao_hubunit.get_budpoint_file(t55_time_id)
...
def test_HubUnit_delete_budpoint_file_DeletesFile(env_dir_setup_cleanup):
    ...
    # WHEN
    yao_hubunit.delete_budpoint_file(t55_time_id)
...
def test_HubUnit_calc_timepoint_purview_Sets_purview_file_Scenario0(
    ...
    t55_time_id = 55
    yao_hubunit = hubunit_shop(fiscals_dir(), fiscal_id(), yao_str)
    yao_hubunit._save_valid_budpoint_file(t55_time_id, get_budunit_3_acct())
    assert yao_hubunit.budpoint_file_exists(t55_time_id)
    assert yao_hubunit.purview_file_exists(t55_time_id) is False

    # WHEN
    yao_hubunit.calc_timepoint_purview(t55_time_id)

    # THEN
    assert yao_hubunit.budpoint_file_exists(t55_time_id)
    assert yao_hubunit.purview_file_exists(t55_time_id)
    t55_purview = yao_hubunit.get_purview_file(t55_time_id)
    assert t55_purview.time_id == t55_time_id
...
def test_HubUnit_calc_timepoint_purview_Sets_purview_file_Scenario1(
    ...
    t88_time_id = t88_purview.time_id
    yao_hubunit = hubunit_shop(fiscals_dir(), fiscal_id(), yao_str)
    yao_hubunit._save_valid_budpoint_file(t88_time_id, get_budunit_3_acct())
    yao_hubunit._save_valid_purview_file(t88_purview)
    assert yao_hubunit.budpoint_file_exists(t88_time_id)
    assert yao_hubunit.purview_file_exists(t88_time_id)
    before_t88_purview = yao_hubunit.get_purview_file(t88_time_id)
    assert before_t88_purview.time_id == t88_time_id
    ...
    # WHEN
    yao_hubunit.calc_timepoint_purview(t88_time_id)

    # THEN
    assert yao_hubunit.budpoint_file_exists(t88_time_id)
    yao_budpoint = yao_hubunit.get_budpoint_file(t88_time_id)
    assert yao_hubunit.purview_file_exists(t88_time_id)
    after_t88_purview = yao_hubunit.get_purview_file(t88_time_id)
    assert after_t88_purview.time_id == t88_time_id
...
def test_HubUnit_calc_timepoint_purview_RaisesException(env_dir_setup_cleanup):
    ...
    yao_hubunit = hubunit_shop(fiscals_dir(), fiscal_id(), yao_str)
    yao_hubunit._save_valid_purview_file(t88_purview)
    assert yao_hubunit.budpoint_file_exists(t88_time_id) is False
    assert yao_hubunit.purview_file_exists(t88_time_id)
    ...
    with pytest_raises(Exception) as excinfo:
        yao_hubunit.calc_timepoint_purview(t88_time_id)
    exception_str = (
        f"Cannot calculate timepoint {t88_time_id} purviews without saved BudPoint file"
    )
    assert str(excinfo.value) == exception_str
...
def test_HubUnit_calc_timepoint_purviews_Sets_purview_files_Scenario0(
    ...
    t88_time_id = t88_purview.time_id
    yao_hubunit = hubunit_shop(fiscals_dir(), fiscal_id(), yao_str)
    yao_hubunit._save_valid_budpoint_file(t66_time_id, get_budunit_3_acct())
    yao_hubunit._save_valid_budpoint_file(t88_time_id, get_budunit_3_acct())
    yao_hubunit._save_valid_purview_file(t66_purview)
    yao_hubunit._save_valid_purview_file(t88_purview)
    assert yao_hubunit.budpoint_file_exists(t66_time_id)
    assert yao_hubunit.budpoint_file_exists(t88_time_id)
    assert yao_hubunit.purview_file_exists(t66_time_id)
    assert yao_hubunit.purview_file_exists(t88_time_id)
    before_t66_purview = yao_hubunit.get_purview_file(t66_time_id)
    assert before_t66_purview.time_id == t66_time_id
    ...
    assert after_t88_purview.get_net_purview("Zia") == 165
    assert after_t88_purview._magnitude == 227
    after_t66_purview = yao_hubunit.get_purview_file(t66_time_id)
    assert after_t66_purview.time_id == t66_time_id

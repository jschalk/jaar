...
def test_HubUnit_timepoint_dir_ReturnsObj():
    ...
    # WHEN
    one_timepoint_dir = yao_hubunit.timepoint_dir(t88_time_fid)

    # THEN
    assert one_timepoint_dir == f"{yao_hubunit.timeline_dir()}/{t88_time_fid}"
...
def test_HubUnit_purview_file_path_ReturnsObj():
    ...
    # WHEN
    t88_purview_file_path = yao_hubunit.purview_file_path(t88_time_fid)

    # THEN
    x_file_path = (
        f"{yao_hubunit.timepoint_dir(t88_time_fid)}/{yao_hubunit.purview_file_name()}"
    )
...
def test_HubUnit_save_valid_purview_file_SavesFile(env_dir_setup_cleanup):
    ...
    t55_time_fid = t55_purview.time_fid
    assert os_path_exists(yao_hubunit.purview_file_path(t55_time_fid)) is False

    # WHEN
    yao_hubunit._save_valid_purview_file(t55_purview)

    # THEN
    assert os_path_exists(yao_hubunit.purview_file_path(t55_time_fid))
...
def test_HubUnit_purview_file_exists_ReturnsObj(env_dir_setup_cleanup):
    ...
    t55_purview = get_purviewepisode_55_example()
    t55_time_fid = t55_purview.time_fid
    assert yao_hubunit.purview_file_exists(t55_time_fid) is False

    # WHEN
    yao_hubunit._save_valid_purview_file(t55_purview)

    # THEN
    assert yao_hubunit.purview_file_exists(t55_time_fid)
...
def test_HubUnit_get_purview_file_ReturnsObj(env_dir_setup_cleanup):
    ...
    # WHEN / THEN
    assert yao_hubunit.get_purview_file(t55_time_fid) == t55_purview
...
def test_HubUnit_delete_purview_file_DeletesFile(env_dir_setup_cleanup):
    ...
    # WHEN
    yao_hubunit.delete_purview_file(t55_time_fid)
...
def test_HubUnit_get_purviewlog_ReturnsObj(env_dir_setup_cleanup):
    ...
    t55_time_fid = t55_purview.time_fid
    t66_time_fid = t66_purview.time_fid
    yao_hubunit._save_valid_purview_file(t55_purview)
    assert yao_hubunit.get_purviewlog().episode_exists(t55_time_fid)
    assert yao_hubunit.get_purviewlog().episode_exists(t66_time_fid) is False
    yao_hubunit._save_valid_purview_file(t66_purview)

    # WHEN / THEN
    assert yao_hubunit.get_purviewlog().episode_exists(t55_time_fid)
    assert yao_hubunit.get_purviewlog().episode_exists(t66_time_fid)
    assert yao_hubunit.get_purviewlog().get_episode(t66_time_fid).get_net_purview("Sue")
...
def test_HubUnit_budpoint_file_path_ReturnsObj():
    ...
    # WHEN
    t88_budpoint_file_path = yao_hubunit.budpoint_file_path(t88_time_fid)

    # THEN
    x_file_path = (
        f"{yao_hubunit.timepoint_dir(t88_time_fid)}/{yao_hubunit.budpoint_file_name()}"
    )
...
def test_HubUnit_save_valid_budpoint_file_SavesFile(env_dir_setup_cleanup):
    ...
    t55_time_fid = 55
    assert os_path_exists(yao_hubunit.budpoint_file_path(t55_time_fid)) is False

    # WHEN
    yao_hubunit._save_valid_budpoint_file(t55_time_fid, t55_budpoint)

    # THEN
    assert os_path_exists(yao_hubunit.budpoint_file_path(t55_time_fid))
...
def test_HubUnit_save_valid_budpoint_file_RaisesError(env_dir_setup_cleanup):
    ...
    # WHEN/THEN
    with pytest_raises(Exception) as excinfo:
        yao_hubunit._save_valid_budpoint_file(t55_time_fid, t_budpoint)
...
def test_HubUnit_budpoint_file_exists_ReturnsObj(env_dir_setup_cleanup):
    ...
    yao_hubunit = hubunit_shop(fiscals_dir(), fiscal_id(), yao_str)
    t55_time_fid = 55
    assert yao_hubunit.budpoint_file_exists(t55_time_fid) is False
    ...
    # THEN
    assert yao_hubunit.budpoint_file_exists(t55_time_fid)
...
def test_HubUnit_get_budpoint_file_ReturnsObj(env_dir_setup_cleanup):
    ...
    # WHEN
    file_budpoint = yao_hubunit.get_budpoint_file(t55_time_fid)
...
def test_HubUnit_delete_budpoint_file_DeletesFile(env_dir_setup_cleanup):
    ...
    # WHEN
    yao_hubunit.delete_budpoint_file(t55_time_fid)
...
def test_HubUnit_calc_timepoint_purview_Sets_purview_file_Scenario0(
    ...
    t55_time_fid = 55
    yao_hubunit = hubunit_shop(fiscals_dir(), fiscal_id(), yao_str)
    yao_hubunit._save_valid_budpoint_file(t55_time_fid, get_budunit_3_acct())
    assert yao_hubunit.budpoint_file_exists(t55_time_fid)
    assert yao_hubunit.purview_file_exists(t55_time_fid) is False

    # WHEN
    yao_hubunit.calc_timepoint_purview(t55_time_fid)

    # THEN
    assert yao_hubunit.budpoint_file_exists(t55_time_fid)
    assert yao_hubunit.purview_file_exists(t55_time_fid)
    t55_purview = yao_hubunit.get_purview_file(t55_time_fid)
    assert t55_purview.time_fid == t55_time_fid
...
def test_HubUnit_calc_timepoint_purview_Sets_purview_file_Scenario1(
    ...
    t88_time_fid = t88_purview.time_fid
    yao_hubunit = hubunit_shop(fiscals_dir(), fiscal_id(), yao_str)
    yao_hubunit._save_valid_budpoint_file(t88_time_fid, get_budunit_3_acct())
    yao_hubunit._save_valid_purview_file(t88_purview)
    assert yao_hubunit.budpoint_file_exists(t88_time_fid)
    assert yao_hubunit.purview_file_exists(t88_time_fid)
    before_t88_purview = yao_hubunit.get_purview_file(t88_time_fid)
    assert before_t88_purview.time_fid == t88_time_fid
    ...
    # WHEN
    yao_hubunit.calc_timepoint_purview(t88_time_fid)

    # THEN
    assert yao_hubunit.budpoint_file_exists(t88_time_fid)
    yao_budpoint = yao_hubunit.get_budpoint_file(t88_time_fid)
    assert yao_hubunit.purview_file_exists(t88_time_fid)
    after_t88_purview = yao_hubunit.get_purview_file(t88_time_fid)
    assert after_t88_purview.time_fid == t88_time_fid
...
def test_HubUnit_calc_timepoint_purview_RaisesException(env_dir_setup_cleanup):
    ...
    yao_hubunit = hubunit_shop(fiscals_dir(), fiscal_id(), yao_str)
    yao_hubunit._save_valid_purview_file(t88_purview)
    assert yao_hubunit.budpoint_file_exists(t88_time_fid) is False
    assert yao_hubunit.purview_file_exists(t88_time_fid)
    ...
    with pytest_raises(Exception) as excinfo:
        yao_hubunit.calc_timepoint_purview(t88_time_fid)
    exception_str = (
        f"Cannot calculate timepoint {t88_time_fid} purviews without saved BudPoint file"
    )
    assert str(excinfo.value) == exception_str
...
def test_HubUnit_calc_timepoint_purviews_Sets_purview_files_Scenario0(
    ...
    t88_time_fid = t88_purview.time_fid
    yao_hubunit = hubunit_shop(fiscals_dir(), fiscal_id(), yao_str)
    yao_hubunit._save_valid_budpoint_file(t66_time_fid, get_budunit_3_acct())
    yao_hubunit._save_valid_budpoint_file(t88_time_fid, get_budunit_3_acct())
    yao_hubunit._save_valid_purview_file(t66_purview)
    yao_hubunit._save_valid_purview_file(t88_purview)
    assert yao_hubunit.budpoint_file_exists(t66_time_fid)
    assert yao_hubunit.budpoint_file_exists(t88_time_fid)
    assert yao_hubunit.purview_file_exists(t66_time_fid)
    assert yao_hubunit.purview_file_exists(t88_time_fid)
    before_t66_purview = yao_hubunit.get_purview_file(t66_time_fid)
    assert before_t66_purview.time_fid == t66_time_fid
    ...
    assert after_t88_purview.get_net_purview("Zia") == 165
    assert after_t88_purview._magnitude == 227
    after_t66_purview = yao_hubunit.get_purview_file(t66_time_fid)
    assert after_t66_purview.time_fid == t66_time_fid

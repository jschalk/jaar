...
from src.f01_road.jaar_config import init_gift_id, get_test_cmty_title as cmty_title
...
def test_HubUnit_default_voice_bud_ReturnsCorrectObj():
    ...
    # THEN
    assert sue_default_voice.cmty_title == sue_hubunit.cmty_title
...
def test_HubUnit_delete_voice_file_DeletesvoiceFile(env_dir_setup_cleanup):
    # ESTABLISH
    sue_str = "Sue"
    sue_hubunit = hubunit_shop(env_dir(), cmty_title(), sue_str)
...
def test_HubUnit_initialize_gift_voice_files_CorrectlySavesvoiceFileAndGiftFile(
    ...
    sue_str = "Sue"
    seven_int = 25
    sue_hubunit = hubunit_shop(env_dir(), cmty_title(), sue_str, respect_bit=seven_int)
    assert sue_hubunit.voice_file_exists() is False
    init_gift_file_path = f"{sue_hubunit.gifts_dir()}/{init_gift_id()}.json"
    ...
    assert voice_bud.cmty_title == cmty_title()
...
def test_HubUnit_initialize_gift_voice_files_CorrectlySavesOnlygiftFile(
    ...
    # THEN
    assert sue_voice_bud.cmty_title == cmty_title()

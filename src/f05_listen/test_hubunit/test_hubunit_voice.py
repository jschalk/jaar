...
from src.f05_listen.examples.example_listen_favors import sue_2budatoms_favorunit
...
def test_HubUnit_default_voice_bud_ReturnsObj():
    ...
    assert sue_default_voice.last_favor_id == init_favor_id()
...
def test_HubUnit_create_initial_favor_files_from_default_CorrectlySavesFavorUnitFiles(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"
    sue_hubunit = hubunit_shop(env_dir(), fisc_title(), sue_str)
    init_favor_filename = sue_hubunit.favor_filename(init_favor_id())
    init_favor_file_path = create_path(sue_hubunit._favors_dir, init_favor_filename)
    assert os_path_exists(init_favor_file_path) is False
    assert sue_hubunit.voice_file_exists() is False

    # WHEN
    sue_hubunit._create_initial_favor_files_from_default()

    # THEN
    assert os_path_exists(init_favor_file_path)
    assert sue_hubunit.voice_file_exists() is False


def test_HubUnit_create_voice_from_favors_CreatesvoiceFileFromFavorFiles(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"
    ...
    # WHEN
    sue_hubunit._create_voice_from_favors()

    # THEN
    assert sue_hubunit.voice_file_exists()
    static_sue_voice = sue_hubunit._merge_any_favors(sue_hubunit.default_voice_bud())
    assert sue_hubunit.get_voice_bud().get_dict() == static_sue_voice.get_dict()


def test_HubUnit_create_initial_favor_and_voice_files_CreatesFavorFilesAndvoiceFile(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"
    ...
    # WHEN
    sue_hubunit._create_initial_favor_and_voice_files()
...
def test_HubUnit_create_initial_favor_files_from_voice_SavesOnlyFavorFiles(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"
    ...
    assert sue_hubunit.voice_file_exists()
    init_favor_file_path = create_path(
        sue_hubunit._favors_dir, f"{init_favor_id()}.json"
    )
    assert os_path_exists(init_favor_file_path) is False
...
def test_HubUnit_initialize_favor_voice_files_CorrectlySavesvoiceFileAndFavorFile(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"
...
def test_HubUnit_initialize_favor_voice_files_CorrectlySavesOnlyvoiceFile(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"
...
def test_HubUnit_initialize_favor_voice_files_CorrectlySavesOnlyfavorFile(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"
...
def test_HubUnit_append_favors_to_voice_file_AddsfavorsTovoiceFile(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_str = "Sue"
    ...
    # WHEN
    new_bud = sue_hubunit.append_favors_to_voice_file()

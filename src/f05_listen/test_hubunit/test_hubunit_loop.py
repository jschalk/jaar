from src.f01_road.jaar_config import get_test_deal_idea as deal_idea
...
def test_HubUnit_get_dw_perspective_bud_ReturnsBudWith_owner_nameSetToHubUnit_owner_name(
    ...
    bob_budunit = get_budunit_with_4_levels()
    bob_budunit.set_owner_name(bob_str)
    bob_hubunit = hubunit_shop(env_dir(), deal_idea(), bob_str)
...
def test_HubUnit_rj_perspective_bud_ReturnsBudWith_owner_nameSetToHubUnit_owner_name(
    ...
    nation_road = create_road(deal_idea(), nation_str)
    ...
    bob_iowa_hubunit = hubunit_shop(env_dir(), deal_idea(), bob_str, iowa_road)
    bob_iowa_hubunit.save_job_bud(yao_budunit)

    sue_str = "Sue"
    sue_hubunit = hubunit_shop(env_dir(), deal_idea(), sue_str, iowa_road)

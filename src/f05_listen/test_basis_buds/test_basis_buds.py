...
def test_create_empty_bud_ReturnsCorrectObj():
    ...
    slash_str = "/"
    penny_float = 0.7
    yao_soul = budunit_shop(yao_str, bridge=slash_str, penny=penny_float)
    yao_soul.set_l1_item(itemunit_shop("Iowa"))
    zia_str = "Zia"
    ...
    duty_zia_acctunit = yao_soul.get_acct(zia_str)
    ...
    zia_acctunit = yao_soul.get_acct(zia_str)
    zia_acctunit.add_membership(f"{slash_str}swimmers")
    yao_soul.set_credor_respect(zia_credor_pool)
    yao_soul.set_debtor_respect(zia_debtor_pool)

    # WHEN
    yao_empty_job = create_empty_bud(yao_soul, x_owner_name=zia_str)

    # THEN
    assert yao_empty_job.owner_name != yao_soul.owner_name
    assert yao_empty_job.owner_name == zia_str
    assert yao_empty_job.fiscal_title == yao_soul.fiscal_title
    assert yao_empty_job.last_gift_id is None
    assert yao_empty_job.get_acctunits_dict() == {}
    assert yao_empty_job.bridge == yao_soul.bridge
    assert yao_empty_job.fund_pool == yao_soul.fund_pool
    assert yao_empty_job.fund_coin == yao_soul.fund_coin
    assert yao_empty_job.respect_bit == yao_soul.respect_bit
    assert yao_empty_job.penny == yao_soul.penny
    assert yao_empty_job.credor_respect != yao_soul.credor_respect
    assert yao_empty_job.credor_respect == validate_respect_num()
    assert yao_empty_job.debtor_respect != yao_soul.debtor_respect
...
def test_create_listen_basis_ReturnsCorrectObj():
    ...
    assert yao_basis_job.fiscal_title == yao_duty.fiscal_title
...
def test_get_default_voice_bud_ReturnsCorrectObj():
    ...
    # WHEN
    default_voice_bud = get_default_voice_bud(sue_budunit)

    # THEN
    default_voice_bud.settle_bud()
    assert default_voice_bud.owner_name == sue_budunit.owner_name
    assert default_voice_bud.owner_name == sue_str
    assert default_voice_bud.fiscal_title == sue_budunit.fiscal_title
    assert default_voice_bud.fiscal_title == blue_str
    assert default_voice_bud.bridge == slash_str
    assert default_voice_bud.fund_pool == sue_acct_pool
    assert default_voice_bud.fund_coin == x_fund_coin
    assert default_voice_bud.respect_bit == x_respect_bit
    assert default_voice_bud.credor_respect == default_respect_num()
    assert default_voice_bud.debtor_respect == default_respect_num()
    assert default_voice_bud.max_tree_traverse == sue_max_tree_traverse
    assert len(default_voice_bud.get_acctunits_dict()) == 1
    assert len(default_voice_bud._item_dict) == 1

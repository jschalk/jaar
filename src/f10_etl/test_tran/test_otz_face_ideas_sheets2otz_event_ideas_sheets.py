...
from src.f10_etl.transformers import etl_otz_face_ideas_to_otz_event_otx_ideas
...
def test_etl_otz_face_ideas_to_otz_event_otx_ideas_CreatesFaceIdeaSheets_Scenario0_MultpleFaceNames(
    ...
    upsert_sheet(zia_br00003_filepath, cart_valid_str(), example_zia_df)
    ...
    assert sheet_exists(event3_br00003_filepath, cart_valid_str()) is False
    assert sheet_exists(event7_br00003_filepath, cart_valid_str()) is False
    ...
    assert sheet_exists(event7_br00003_filepath, cart_valid_str())
    assert sheet_exists(event9_br00003_filepath, cart_valid_str())

    gen_event3_df = pandas_read_excel(event3_br00003_filepath, cart_valid_str())
    gen_event7_df = pandas_read_excel(event7_br00003_filepath, cart_valid_str())
    gen_event9_df = pandas_read_excel(event9_br00003_filepath, cart_valid_str())

...
from src.f10_etl.transformers import etl_aft_event_ideas_to_fiscal_ideas
...
def test_WorldUnit_aft_event_ideas_to_fiscal_ideas_CreatesFaceIdeaSheets_Scenario0_MultpleFaceNames(
    ...
    # WHEN
    etl_aft_event_ideas_to_fiscal_ideas(x_faces_aft_dir)
    ...
    example_e3_m23_df = DataFrame([sue0, sue1], columns=idea_columns)
    example_e7_m23_df = DataFrame([zia0], columns=idea_columns)
    example_e9_m23_df = DataFrame([zia1], columns=idea_columns)
    example_e9_m55_df = DataFrame([zia2], columns=idea_columns)

...
from src.f10_etl.transformers import etl_event_bud_csvs_to_stand_json
from src.f10_etl.examples.etl_env import env_dir_setup_cleanup, get_test_etl_dir
from os.path import exists as os_path_exists


def test_WorldUnit_event_bud_csvs_to_stand_json_CreatesFiles_Scenario0_IgnoresCSV_budunit(
...
"""
    ...
    assert os_path_exists(e3_all_stand_path) is False

    # WHEN
    etl_event_bud_csvs_to_stand_json(fisc_mstr_dir)

    # THEN
    assert os_path_exists(e3_all_stand_path)
    expected_e3_stand = standunit_shop(bob_inx, None, a23_str, event_int=event3)
    e3_standunit = get_standunit_from_json(open_file(e3_all_stand_path))
    assert e3_standunit.event_int == expected_e3_stand.event_int
    expected_buddelta = expected_e3_stand._buddelta
    generated_e3_buddelta = e3_standunit._buddelta
    assert generated_e3_buddelta.budatoms == expected_buddelta.budatoms
    assert e3_standunit._buddelta == expected_e3_stand._buddelta
    assert e3_standunit == expected_e3_stand


def test_WorldUnit_event_bud_csvs_to_stand_json_CreatesFiles_Scenario1(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    sue_inx = "Suzy"
...
"""
    ...
    # print(f"{e3_stand_path=}")
    # print(f"{e7_stand_path=}")
    assert os_path_exists(e3_all_stand_path) is False
    assert os_path_exists(e7_all_stand_path) is False
    ...
    # print(f"{open_file(e3_stand_path)=}")
    # print(f"{open_file(e7_stand_path)=}")
    # stands_dir = create_path(fizz_world._fisc_mstr_dir, "stands")
    # atoms_dir = create_path(fizz_world._fisc_mstr_dir, "atoms")
    # e3_stand = standunit_shop(bob_inx, sue_inx, a23_str, stands_dir, atoms_dir, event3)
    # e7_stand = standunit_shop(bob_inx, sue_inx, a23_str, stands_dir, atoms_dir, event7)
    expected_e3_stand = standunit_shop(bob_inx, None, a23_str, event_int=event3)
    expected_e7_stand = standunit_shop(bob_inx, None, a23_str, event_int=event7)
    budacct_dimen = bud_acctunit_str()
    insert_str = atom_insert()
    expected_e3_stand._buddelta.add_budatom(
        budacct_dimen,
        insert_str,
        jkeys={acct_name_str(): bob_inx},
        jvalues={credit_belief_str(): credit77, debtit_belief_str(): None},
    )
    expected_e7_stand._buddelta.add_budatom(
        budacct_dimen,
        insert_str,
        jkeys={acct_name_str(): bob_inx},
        jvalues={credit_belief_str(): credit77, debtit_belief_str(): None},
    ...
    e3_standunit = get_standunit_from_json(open_file(e3_all_stand_path))
    e7_standunit = get_standunit_from_json(open_file(e7_all_stand_path))
    # print(f"{e7_standunit=}")
    assert e3_standunit.event_int == expected_e3_stand.event_int
    expected_buddelta = expected_e3_stand._buddelta
    generated_e3_buddelta = e3_standunit._buddelta
    assert generated_e3_buddelta.budatoms == expected_buddelta.budatoms
    assert e3_standunit._buddelta == expected_e3_stand._buddelta
    assert e3_standunit == expected_e3_stand
    e7_insert = e7_standunit._buddelta.budatoms.get("INSERT")
    expected_e7_insert = expected_e7_stand._buddelta.budatoms.get("INSERT")
    ...
    assert e7_standunit._buddelta == expected_e7_stand._buddelta
    assert e7_standunit == expected_e7_stand

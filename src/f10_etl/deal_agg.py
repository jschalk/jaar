...
class DealPrimeFilePaths:
    def __init__(self, deals_dir: str):
        ...
        self.deal_pact_path = create_path(deals_dir, "deal_pact_episode.xlsx")
...
class DealPrimeColumns:
    def __init__(self):
        ...
        self.deal_pact_staging_columns = [
            "source_br",
            "face_name",
            "event_int",
            "deal_idea",
...
def create_init_deal_prime_files(deals_dir: str):
    ...
    stage_deal_pact_df = DataFrame([], columns=xc.deal_pact_staging_columns)
    ...
    upsert_sheet(xp.deal_pact_path, "staging", stage_deal_pact_df)
    ...
    agg_deal_pact_df = DataFrame([], columns=xc.deal_pact_agg_columns)
    ...
    upsert_sheet(xp.deal_pact_path, "agg", agg_deal_pact_df)
...
def create_dealunit_jsons_from_prime_files(deals_dir: str):
    ...
    deal_pact_df = pandas_read_excel(xp.deal_pact_path, "agg")
    ...
    deals_pact_dict = dataframe_to_dict(deal_pact_df, ["deal_idea"])
    ...
    for deal_attrs in dealunits_dict.values():
        ...
        # _add_pactepisodes_from_df(dealunit, deal_cashbook_df)

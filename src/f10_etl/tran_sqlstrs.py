...
CREATE_BUD_ACCT_MEMBERSHIP_AGG_SQLSTR = """CREATE TABLE IF NOT EXISTS bud_acct_membership_agg (fiscal_title TEXT, owner_name TEXT, acct_name TEXT, group_label TEXT, credit_vote REAL, debtit_vote REAL)"""
...
CREATE_BUD_ACCTUNIT_AGG_SQLSTR = """CREATE TABLE IF NOT EXISTS bud_acctunit_agg (fiscal_title TEXT, owner_name TEXT, acct_name TEXT, credit_belief REAL, debtit_belief REAL)"""
...
CREATE_BUD_ITEM_AWARDLINK_AGG_SQLSTR = """CREATE TABLE IF NOT EXISTS bud_item_awardlink_agg (fiscal_title TEXT, owner_name TEXT, road TEXT, awardee_tag TEXT, give_force REAL, take_force REAL)"""
...
CREATE_BUD_ITEM_FACTUNIT_AGG_SQLSTR = """CREATE TABLE IF NOT EXISTS bud_item_factunit_agg (fiscal_title TEXT, owner_name TEXT, road TEXT, base TEXT, pick TEXT, fopen REAL, fnigh REAL)"""
...
CREATE_BUD_ITEM_HEALERLINK_AGG_SQLSTR = """CREATE TABLE IF NOT EXISTS bud_item_healerlink_agg (fiscal_title TEXT, owner_name TEXT, road TEXT, healer_name TEXT)"""
...
CREATE_BUD_ITEM_REASON_PREMISEUNIT_AGG_SQLSTR = """CREATE TABLE IF NOT EXISTS bud_item_reason_premiseunit_agg (fiscal_title TEXT, owner_name TEXT, road TEXT, base TEXT, need TEXT, nigh REAL, open REAL, divisor REAL)"""
...
CREATE_BUD_ITEM_REASONUNIT_AGG_SQLSTR = """CREATE TABLE IF NOT EXISTS bud_item_reasonunit_agg (fiscal_title TEXT, owner_name TEXT, road TEXT, base TEXT, base_item_active_requisite TEXT)"""
...
CREATE_BUD_ITEM_TEAMLINK_AGG_SQLSTR = """CREATE TABLE IF NOT EXISTS bud_item_teamlink_agg (fiscal_title TEXT, owner_name TEXT, road TEXT, team_tag TEXT)"""
...
CREATE_BUD_ITEMUNIT_AGG_SQLSTR = """CREATE TABLE IF NOT EXISTS bud_itemunit_agg (fiscal_title TEXT, owner_name TEXT, parent_road TEXT, item_title TEXT, begin REAL, close REAL, addin REAL, numor REAL, denom REAL, morph INTEGER, gogo_want REAL, stop_want REAL, mass REAL, pledge INTEGER, problem_bool INTEGER)"""
...
CREATE_BUDUNIT_AGG_SQLSTR = """CREATE TABLE IF NOT EXISTS budunit_agg (fiscal_title TEXT, owner_name TEXT, credor_respect REAL, debtor_respect REAL, fund_pool REAL, max_tree_traverse INTEGER, deal_time_int INTEGER, tally REAL, fund_coin REAL, penny REAL, respect_bit REAL)"""
...
BUDMEMB_INCONSISTENCY_SQLSTR = """SELECT fiscal_title, owner_name, acct_name, group_label
FROM bud_acct_membership_staging
GROUP BY fiscal_title, owner_name, acct_name, group_label
HAVING MIN(credit_vote) != MAX(credit_vote)
    OR MIN(debtit_vote) != MAX(debtit_vote)
"""
BUDACCT_INCONSISTENCY_SQLSTR = """SELECT fiscal_title, owner_name, acct_name
FROM bud_acctunit_staging
GROUP BY fiscal_title, owner_name, acct_name
HAVING MIN(credit_belief) != MAX(credit_belief)
    OR MIN(debtit_belief) != MAX(debtit_belief)
"""
BUDAWAR_INCONSISTENCY_SQLSTR = """SELECT fiscal_title, owner_name, road, awardee_tag
FROM bud_item_awardlink_staging
GROUP BY fiscal_title, owner_name, road, awardee_tag
HAVING MIN(give_force) != MAX(give_force)
    OR MIN(take_force) != MAX(take_force)
"""
BUDFACT_INCONSISTENCY_SQLSTR = """SELECT fiscal_title, owner_name, road, base
FROM bud_item_factunit_staging
GROUP BY fiscal_title, owner_name, road, base
HAVING MIN(pick) != MAX(pick)
    OR MIN(fopen) != MAX(fopen)
    OR MIN(fnigh) != MAX(fnigh)
"""
BUDHEAL_INCONSISTENCY_SQLSTR = """SELECT fiscal_title, owner_name, road, healer_name
FROM bud_item_healerlink_staging
GROUP BY fiscal_title, owner_name, road, healer_name
None
"""
BUDPREM_INCONSISTENCY_SQLSTR = """SELECT fiscal_title, owner_name, road, base, need
FROM bud_item_reason_premiseunit_staging
GROUP BY fiscal_title, owner_name, road, base, need
HAVING MIN(nigh) != MAX(nigh)
    OR MIN(open) != MAX(open)
    OR MIN(divisor) != MAX(divisor)
"""
BUDREAS_INCONSISTENCY_SQLSTR = """SELECT fiscal_title, owner_name, road, base
FROM bud_item_reasonunit_staging
GROUP BY fiscal_title, owner_name, road, base
HAVING MIN(base_item_active_requisite) != MAX(base_item_active_requisite)
"""
BUDTEAM_INCONSISTENCY_SQLSTR = """SELECT fiscal_title, owner_name, road, team_tag
FROM bud_item_teamlink_staging
GROUP BY fiscal_title, owner_name, road, team_tag
None
"""
BUDITEM_INCONSISTENCY_SQLSTR = """SELECT fiscal_title, owner_name, parent_road, item_title
FROM bud_itemunit_staging
GROUP BY fiscal_title, owner_name, parent_road, item_title
...
"""
BUDUNIT_INCONSISTENCY_SQLSTR = """SELECT fiscal_title, owner_name
FROM budunit_staging
GROUP BY fiscal_title, owner_name
...
SELECT fiscal_title, owner_name, acct_name, group_label
FROM bud_acct_membership_staging
GROUP BY fiscal_title, owner_name, acct_name, group_label
HAVING MIN(credit_vote) != MAX(credit_vote)
    OR MIN(debtit_vote) != MAX(debtit_vote)
...
SELECT fiscal_title, owner_name, acct_name
FROM bud_acctunit_staging
GROUP BY fiscal_title, owner_name, acct_name
HAVING MIN(credit_belief) != MAX(credit_belief)
    OR MIN(debtit_belief) != MAX(debtit_belief)
...
SELECT fiscal_title, owner_name, road, awardee_tag
FROM bud_item_awardlink_staging
GROUP BY fiscal_title, owner_name, road, awardee_tag
HAVING MIN(give_force) != MAX(give_force)
    OR MIN(take_force) != MAX(take_force)
...
SELECT fiscal_title, owner_name, road, base
FROM bud_item_factunit_staging
GROUP BY fiscal_title, owner_name, road, base
HAVING MIN(pick) != MAX(pick)
    OR MIN(fopen) != MAX(fopen)
...
SELECT fiscal_title, owner_name, road, healer_name
FROM bud_item_healerlink_staging
GROUP BY fiscal_title, owner_name, road, healer_name
None
)
...
SELECT fiscal_title, owner_name, road, base, need
FROM bud_item_reason_premiseunit_staging
GROUP BY fiscal_title, owner_name, road, base, need
HAVING MIN(nigh) != MAX(nigh)
    OR MIN(open) != MAX(open)
...
SELECT fiscal_title, owner_name, road, team_tag
FROM bud_item_teamlink_staging
GROUP BY fiscal_title, owner_name, road, team_tag
None
)
...
SELECT fiscal_title, owner_name, parent_road, item_title
FROM bud_itemunit_staging
GROUP BY fiscal_title, owner_name, parent_road, item_title
HAVING MIN(begin) != MAX(begin)
    OR MIN(close) != MAX(close)
...
SELECT fiscal_title, owner_name
FROM budunit_staging
GROUP BY fiscal_title, owner_name
HAVING MIN(credor_respect) != MAX(credor_respect)
    OR MIN(debtor_respect) != MAX(debtor_respect)
...
BUDMEMB_AGG_INSERT_SQLSTR = """INSERT INTO bud_acct_membership_agg (fiscal_title, owner_name, acct_name, group_label, credit_vote, debtit_vote)
SELECT fiscal_title, owner_name, acct_name, group_label, MAX(credit_vote), MAX(debtit_vote)
FROM bud_acct_membership_staging
WHERE error_message IS NULL
GROUP BY fiscal_title, owner_name, acct_name, group_label
;
"""
BUDACCT_AGG_INSERT_SQLSTR = """INSERT INTO bud_acctunit_agg (fiscal_title, owner_name, acct_name, credit_belief, debtit_belief)
SELECT fiscal_title, owner_name, acct_name, MAX(credit_belief), MAX(debtit_belief)
FROM bud_acctunit_staging
WHERE error_message IS NULL
GROUP BY fiscal_title, owner_name, acct_name
;
"""
BUDAWAR_AGG_INSERT_SQLSTR = """INSERT INTO bud_item_awardlink_agg (fiscal_title, owner_name, road, awardee_tag, give_force, take_force)
SELECT fiscal_title, owner_name, road, awardee_tag, MAX(give_force), MAX(take_force)
FROM bud_item_awardlink_staging
WHERE error_message IS NULL
GROUP BY fiscal_title, owner_name, road, awardee_tag
;
"""
BUDFACT_AGG_INSERT_SQLSTR = """INSERT INTO bud_item_factunit_agg (fiscal_title, owner_name, road, base, pick, fopen, fnigh)
SELECT fiscal_title, owner_name, road, base, MAX(pick), MAX(fopen), MAX(fnigh)
FROM bud_item_factunit_staging
WHERE error_message IS NULL
GROUP BY fiscal_title, owner_name, road, base
;
"""
BUDHEAL_AGG_INSERT_SQLSTR = """INSERT INTO bud_item_healerlink_agg (fiscal_title, owner_name, road, healer_name)
SELECT fiscal_title, owner_name, road, healer_name
FROM bud_item_healerlink_staging
WHERE error_message IS NULL
GROUP BY fiscal_title, owner_name, road, healer_name
;
"""
BUDPREM_AGG_INSERT_SQLSTR = """INSERT INTO bud_item_reason_premiseunit_agg (fiscal_title, owner_name, road, base, need, nigh, open, divisor)
SELECT fiscal_title, owner_name, road, base, need, MAX(nigh), MAX(open), MAX(divisor)
FROM bud_item_reason_premiseunit_staging
WHERE error_message IS NULL
GROUP BY fiscal_title, owner_name, road, base, need
;
"""
BUDREAS_AGG_INSERT_SQLSTR = """INSERT INTO bud_item_reasonunit_agg (fiscal_title, owner_name, road, base, base_item_active_requisite)
SELECT fiscal_title, owner_name, road, base, MAX(base_item_active_requisite)
FROM bud_item_reasonunit_staging
WHERE error_message IS NULL
GROUP BY fiscal_title, owner_name, road, base
;
"""
BUDTEAM_AGG_INSERT_SQLSTR = """INSERT INTO bud_item_teamlink_agg (fiscal_title, owner_name, road, team_tag)
SELECT fiscal_title, owner_name, road, team_tag
FROM bud_item_teamlink_staging
WHERE error_message IS NULL
GROUP BY fiscal_title, owner_name, road, team_tag
;
"""
BUDITEM_AGG_INSERT_SQLSTR = """INSERT INTO bud_itemunit_agg (fiscal_title, owner_name, parent_road, item_title, begin, close, addin, numor, denom, morph, gogo_want, stop_want, mass, pledge, problem_bool)
SELECT fiscal_title, owner_name, parent_road, item_title, MAX(begin), MAX(close), MAX(addin), MAX(numor), MAX(denom), MAX(morph), MAX(gogo_want), MAX(stop_want), MAX(mass), MAX(pledge), MAX(problem_bool)
FROM bud_itemunit_staging
WHERE error_message IS NULL
GROUP BY fiscal_title, owner_name, parent_road, item_title
;
"""
BUDUNIT_AGG_INSERT_SQLSTR = """INSERT INTO budunit_agg (fiscal_title, owner_name, credor_respect, debtor_respect, fund_pool, max_tree_traverse, deal_time_int, tally, fund_coin, penny, respect_bit)
SELECT fiscal_title, owner_name, MAX(credor_respect), MAX(debtor_respect), MAX(fund_pool), MAX(max_tree_traverse), MAX(deal_time_int), MAX(tally), MAX(fund_coin), MAX(penny), MAX(respect_bit)
FROM budunit_staging
WHERE error_message IS NULL
GROUP BY fiscal_title, owner_name
;

...
def test_CmtyPrimeColumns_Exists():
    # ESTABLISH / WHEN
    x_cmtyprimecolumns = CmtyPrimeColumns()
    ...
    cmty_timeline_hour_args = set(get_cmty_config_args(cmty_hour_str()).keys())
    cmty_timeline_month_args = set(get_cmty_config_args(cmty_month_str()).keys())
    cmty_timeline_weekday_args = set(get_cmty_config_args(cmty_weekday_str()).keys())
    cmtyunit_args = set(get_cmty_config_args(cmtyunit_str()).keys())
    assert set(x_cmtyprimecolumns.cmty_cashbook_agg_columns) == cmty_cashbook_args
    assert set(x_cmtyprimecolumns.cmty_deal_agg_columns) == cmty_deal_episode_args
    assert set(x_cmtyprimecolumns.cmty_hour_agg_columns) == cmty_timeline_hour_args
    assert set(x_cmtyprimecolumns.cmty_month_agg_columns) == cmty_timeline_month_args
    assert (
        set(x_cmtyprimecolumns.cmty_weekday_agg_columns) == cmty_timeline_weekday_args
    )
    assert set(x_cmtyprimecolumns.cmtyunit_agg_columns) == cmtyunit_args
    ...
    cmty_hour_staging = cmty_timeline_hour_args.union(staging_args)
    cmty_month_staging = cmty_timeline_month_args.union(staging_args)
    cmty_weekday_staging = cmty_timeline_weekday_args.union(staging_args)
    cmtyunit_staging = cmtyunit_args.union(staging_args)
    assert (
        set(x_cmtyprimecolumns.cmty_cashbook_staging_columns) == cmty_cashbook_staging
    )
    assert (
        set(x_cmtyprimecolumns.cmty_deal_staging_columns) == cmty_deal_episode_staging
    )
    assert set(x_cmtyprimecolumns.cmty_hour_staging_columns) == cmty_hour_staging
    assert set(x_cmtyprimecolumns.cmty_month_staging_columns) == cmty_month_staging
    assert set(x_cmtyprimecolumns.cmty_weekday_staging_columns) == cmty_weekday_staging
    assert set(x_cmtyprimecolumns.cmtyunit_staging_columns) == cmtyunit_staging

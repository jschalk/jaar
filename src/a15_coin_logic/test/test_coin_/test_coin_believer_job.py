...
def test_CoinUnit_rotate_job_ReturnsObj_Scenario1(env_dir_setup_cleanup):
    # ESTABLISH
    a23_str = "amy23"
    ...
    # WHEN
    sue_job = a23_coin.rotate_job(sue_str)

    # THEN
    example_believer = believerunit_shop(sue_str, a23_str)
    assert sue_job.coin_label == example_believer.coin_label
    assert sue_job.believer_name == example_believer.believer_name


def test_CoinUnit_rotate_job_ReturnsObj_Scenario2_EmptyPartnersCause_inallocable_partner_debt_points(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    coin_mstr_dir = get_module_temp_dir()
    ...
    save_job_file(coin_mstr_dir, init_sue_job)
    assert job_file_exists(coin_mstr_dir, a23_str, sue_str)
    assert job_file_exists(coin_mstr_dir, a23_str, yao_str) is False
    assert job_file_exists(coin_mstr_dir, a23_str, bob_str) is False
    assert job_file_exists(coin_mstr_dir, a23_str, zia_str) is False

    # WHEN
    rotated_sue_job = a23_coin.rotate_job(sue_str)
...
def a23_job(believer_name: str) -> BelieverUnit:
    coin_mstr_dir = get_module_temp_dir()
    return open_job_file(coin_mstr_dir, "amy23", believer_name)


def test_CoinUnit_rotate_job_ReturnsObj_Scenario3_job_ChangesFromRotation(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    coin_mstr_dir = get_module_temp_dir()
    ...
    save_job_file(coin_mstr_dir, init_sue_job)
    save_job_file(coin_mstr_dir, init_yao_job)
    save_job_file(coin_mstr_dir, init_bob_job)
    ...
    # WHEN / THEN
    assert len(a23_coin.rotate_job(sue_str).get_agenda_dict()) == 0
    assert len(a23_coin.rotate_job(yao_str).get_agenda_dict()) == 1
    assert len(a23_coin.rotate_job(bob_str).get_agenda_dict()) == 0


def test_CoinUnit_rotate_job_ReturnsObj_Scenario4_job_SelfReferenceWorks(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    coin_mstr_dir = get_module_temp_dir()
    ...
    # WHEN / THEN
    assert len(a23_coin.rotate_job(bob_str).get_agenda_dict()) == 1
    assert len(a23_coin.rotate_job(sue_str).get_agenda_dict()) == 0
    assert len(a23_coin.rotate_job(yao_str).get_agenda_dict()) == 1


def test_CoinUnit_generate_all_jobs_Scenario0_init_job_IsCreated(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    a23_str = "amy23"
    ...
    sue_dir = create_believer_dir_path(coin_mstr_dir, a23_str, sue_str)
    set_dir(sue_dir)
    assert gut_file_exists(coin_mstr_dir, a23_str, bob_str)
    ...
    # WHEN
    a23_coin.generate_all_jobs()

    # THEN
    assert gut_file_exists(coin_mstr_dir, a23_str, bob_str)
    assert gut_file_exists(coin_mstr_dir, a23_str, sue_str)
    assert job_file_exists(coin_mstr_dir, a23_str, bob_str)
    assert job_file_exists(coin_mstr_dir, a23_str, sue_str)


def test_CoinUnit_generate_all_jobs_Scenario1_jobs_rotated(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    coin_mstr_dir = get_module_temp_dir()
    a23_str = "amy23"
    a23_coin = coinunit_shop(a23_str, coin_mstr_dir, job_listen_rotations=1)
    bob_str = "Bob"
    sue_str = "Sue"
    yao_str = "Yao"
    bob_gut = believerunit_shop(bob_str, a23_str)
    ...
    assert not job_file_exists(coin_mstr_dir, a23_str, bob_str)
    assert not job_file_exists(coin_mstr_dir, a23_str, sue_str)
    assert not job_file_exists(coin_mstr_dir, a23_str, yao_str)
...
def test_CoinUnit_generate_all_jobs_Scenario2_jobs_rotated_InSortedOrder(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    coin_mstr_dir = get_module_temp_dir()
    ...
    save_gut_file(coin_mstr_dir, sue_gut)
    save_gut_file(coin_mstr_dir, yao_gut)
    save_gut_file(coin_mstr_dir, zia_gut)
    assert not job_file_exists(coin_mstr_dir, a23_str, bob_str)
    assert not job_file_exists(coin_mstr_dir, a23_str, sue_str)
    assert not job_file_exists(coin_mstr_dir, a23_str, yao_str)
    assert not job_file_exists(coin_mstr_dir, a23_str, zia_str)
...
def test_CoinUnit_generate_all_jobs_Scenario3_job_listen_rotation_AffectsJobs(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    coin_mstr_dir = get_module_temp_dir()
    ...
    assert not job_file_exists(coin_mstr_dir, a23_str, yao_str)
    assert not job_file_exists(coin_mstr_dir, a23_str, zia_str)
    assert a23_coin.job_listen_rotations == 1
    ...
    # WHEN
    a23_coin.job_listen_rotations = 2
    a23_coin.generate_all_jobs()

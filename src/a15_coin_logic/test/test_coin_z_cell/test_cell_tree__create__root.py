...
from src.a15_coin_logic.coin_main import _get_ote1_max_past_event_int, coinunit_shop
...
def test_CoinUnit_create_buds_root_cells_Scenaro0_BudEmpty(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    a23_str = "amy23"
    ...
    a23_json_path = create_coin_json_path(coin_mstr_dir, a23_str)
    save_file(a23_json_path, None, amy23_coin.get_json())
    print(f"{a23_json_path=}")
    a23_believers_path = create_coin_believers_dir_path(coin_mstr_dir, a23_str)
    assert count_dirs_files(a23_believers_path) == 0

    # WHEN
    amy23_coin.create_buds_root_cells({})

    # THEN
    assert count_dirs_files(a23_believers_path) == 0


def test_CoinUnit_create_buds_root_cells_Scenaro1_BudExists(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    ...
    timepoint37 = 37
    bud1_quota = 450
    amy23_coin.add_budunit(bob_str, timepoint37, bud1_quota)
    ...
    # WHEN
    amy23_coin.create_buds_root_cells(a23_ote1_dict)
...
def test_CoinUnit_create_buds_root_cells_Scenaro2_BudExistsButNoBelieverExistsInEventsPast(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    mstr_dir = get_module_temp_dir()
...
def test_CoinUnit_create_buds_root_cells_Scenaro3_BudExistsNotPerfectMatch_bud_time_event_int(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    mstr_dir = get_module_temp_dir()
    ...
    # Create CoinUnit with bob bud at time 37
    amy23_coin = coinunit_shop(a23_str, mstr_dir, penny=a23_penny)
    print(f"{amy23_coin.penny=}")
    bob_str = "Bob"
    timepoint37 = 37
    bud1_quota = 450
    bud1_celldepth = 3
    amy23_coin.add_budunit(bob_str, timepoint37, bud1_quota, celldepth=bud1_celldepth)
    a23_json_path = create_coin_json_path(mstr_dir, a23_str)
    save_file(a23_json_path, None, amy23_coin.get_json())
    assert os_path_exists(a23_json_path)

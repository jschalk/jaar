from src.a01_term_logic.test._util.a01_str import knot_str
from src.a02_finance_logic.test._util.a02_str import fund_iota_str, penny_str
from src.a03_group_logic.test._util.a03_str import respect_bit_str
from src.a06_believer_logic.test._util.a06_str import coin_label_str
from src.a07_timeline_logic.test._util.a07_str import cumulative_day_str, timeline_str
from src.a11_bud_logic.test._util.a11_str import amount_str, offi_time_str
from src.a12_hub_toolbox.test._util.a12_str import coin_mstr_dir_str


def brokerunits_str() -> str:
    return "brokerunits"


def coin_budunit_str() -> str:
    return "coin_budunit"


def coin_paybook_str() -> str:
    return "coin_paybook"


def coin_timeline_hour_str() -> str:
    return "coin_timeline_hour"


def coin_timeline_month_str() -> str:
    return "coin_timeline_month"


def coin_timeline_weekday_str() -> str:
    return "coin_timeline_weekday"


def coin_timeoffi_str() -> str:
    return "coin_timeoffi"


def coinunit_str() -> str:
    return "coinunit"


def cumulative_minute_str() -> str:
    return "cumulative_minute"


def hour_label_str() -> str:
    return "hour_label"


def job_listen_rotations_str() -> str:
    return "job_listen_rotations"


def month_label_str() -> str:
    return "month_label"


def paybook_str() -> str:
    return "paybook"


def weekday_label_str() -> str:
    return "weekday_label"


def weekday_order_str() -> str:
    return "weekday_order"

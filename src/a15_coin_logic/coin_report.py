...
def get_coin_guts_partners_dataframe(x_coin: CoinUnit) -> DataFrame:
    ...
    coin_believer_names = x_coin._get_believer_folder_names()
    # for all believers get gut
    gut_dfs = []
    for believer_name in coin_believer_names:
        gut_believer = open_gut_file(
            x_coin.coin_mstr_dir, x_coin.coin_label, believer_name
        )
...
def get_coin_jobs_partners_dataframe(x_coin: CoinUnit) -> DataFrame:
    ...
    job_dfs = []
    for believer_name in coin_believer_names:
        job = open_job_file(x_coin.coin_mstr_dir, x_coin.coin_label, believer_name)
        job.settle_believer()
...
def get_coin_jobs_agenda_dataframe(x_coin: CoinUnit) -> DataFrame:
    # get list of all believer paths
    job_dfs = []
    for x_believer_name in x_coin._get_believer_folder_names():

        job = open_job_file(x_coin.coin_mstr_dir, x_coin.coin_label, x_believer_name)
        job.settle_believer()

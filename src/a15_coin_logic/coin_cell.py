...
from src.a15_coin_logic.a15_path import BUD_MANDATE_FILENAME
from src.a15_coin_logic.coin_main import get_from_dict as coinunit_get_from_dict


def create_coin_believers_cell_trees(coin_mstr_dir, coin_label):
    ...
    for believer_name in get_level1_dirs(believers_dir):
        ...
        for bud_time in get_level1_dirs(buds_dir):
            create_cell_tree(coin_mstr_dir, coin_label, believer_name, bud_time)


def create_cell_tree(coin_mstr_dir, coin_label, bud_believer_name, bud_time):
    root_cell_json_path = create_cell_json_path(
        coin_mstr_dir, coin_label, bud_believer_name, bud_time
    )
    if os_path_exists(root_cell_json_path):
        _exists_create_cell_tree(coin_mstr_dir, coin_label, bud_believer_name, bud_time)


def _exists_create_cell_tree(coin_mstr_dir, coin_label, bud_believer_name, bud_time):
    root_cell_dir = create_cell_dir_path(
        coin_mstr_dir, coin_label, bud_believer_name, bud_time, []
    )
    cells_to_evaluate = [cellunit_get_from_dir(root_cell_dir)]
    believer_events_sets = collect_believer_event_dir_sets(coin_mstr_dir, coin_label)
    while cells_to_evaluate != []:
        ...
        believerevent = get_believerevent_obj(
            coin_mstr_dir, coin_label, cell_believer_name, e_int
        )
...
def load_cells_believerevent(coin_mstr_dir: str, coin_label: LabelTerm):
    coins_dir = create_path(coin_mstr_dir, "coins")
    coin_dir = create_path(coins_dir, coin_label)
    believers_dir = create_path(coin_dir, "believers")
    for believer_name in get_level1_dirs(believers_dir):
        ...
        for bud_time in get_level1_dirs(buds_dir):
            ...
            for dirpath, dirnames, filenames in os_walk(bud_time_dir):
                if CELLNODE_FILENAME in set(filenames):
                    _load_cell_believerevent(coin_mstr_dir, coin_label, dirpath)


def _load_cell_believerevent(coin_mstr_dir, coin_label, dirpath):
    ...
    believerevent = get_believerevent_obj(
        coin_mstr_dir, coin_label, cell_believer_name, event_int
    )
    x_cellunit.eval_believerevent(believerevent)
    cellunit_save_to_dir(dirpath, x_cellunit)


def set_cell_trees_found_facts(coin_mstr_dir: str, coin_label: LabelTerm):
    coins_dir = create_path(coin_mstr_dir, "coins")
    coin_dir = create_path(coins_dir, coin_label)
    believers_dir = create_path(coin_dir, "believers")
    for believer_name in get_level1_dirs(believers_dir):
...
def set_cell_trees_decrees(coin_mstr_dir: str, coin_label: str):
    coins_dir = create_path(coin_mstr_dir, "coins")
    coin_dir = create_path(coins_dir, coin_label)
    believers_dir = create_path(coin_dir, "believers")
    for believer_name in get_level1_dirs(believers_dir):
        ...
        for bud_time in get_level1_dirs(buds_dir):
            ...
            set_cell_tree_decrees(
                coin_mstr_dir, coin_label, believer_name, bud_time, bud_time_dir
            )
...
def set_cell_tree_decrees(
    ...
    # add nodes to to_evalute_cellnodes based on partner_agenda_fund_give believers
    root_cell = cellunit_get_from_dir(bud_time_dir)
    root_cell_dir = create_cell_dir_path(
        mstr_dir, coin_label, believer_name, bud_time, []
    )
    ...
    while to_evaluate_decreeunits != []:
        ...
        if x_cell := cellunit_get_from_dir(
            x_decree.cell_dir
        ) or generate_cell_from_decree(x_decree, mstr_dir, coin_label, believer_name):
...
def _add_child_decrees(
    ...
    for child_believer_name, child_mandate in x_cell._partner_mandate_ledger.items():
        child_cell_ancestors = x_decree.get_child_cell_ancestors(child_believer_name)
        child_dir = create_cell_dir_path(
            mstr_dir, coin_label, believer_name, bud_time, child_cell_ancestors
        )
...
def generate_cell_from_decree(
    x_decree: DecreeUnit, mstr_dir: str, coin_label: str, believer_name: BelieverName
) -> CellUnit:
    cell_believer_name = x_decree.cell_believer_name
    believers_downhill_events_ints = get_believers_downhill_event_ints(
        believer_events_sets=collect_believer_event_dir_sets(mstr_dir, coin_label),
    ...
    if downhill_event_int := believers_downhill_events_ints.get(cell_believer_name):
        believerevent_path = create_believerevent_path(
            mstr_dir, coin_label, cell_believer_name, downhill_event_int
        )
...
def set_cell_tree_cell_mandates(coin_mstr_dir: str, coin_label: str):
    coins_dir = create_path(coin_mstr_dir, "coins")
    coin_dir = create_path(coins_dir, coin_label)
    believers_dir = create_path(coin_dir, "believers")
    for believer_name in get_level1_dirs(believers_dir):
...
def create_bud_mandate_ledgers(coin_mstr_dir: str, coin_label: str):
    coin_json_path = create_coin_json_path(coin_mstr_dir, coin_label)
    coinunit = coinunit_get_from_dict(open_json(coin_json_path))
    for brokerunit in coinunit.brokerunits.values():
    ...
    save_json(coin_json_path, None, coinunit.to_dict())

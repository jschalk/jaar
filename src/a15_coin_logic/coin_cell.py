...
def create_coin_beliefs_cell_trees(coin_mstr_dir, coin_label):
    ...
    for belief_name in get_level1_dirs(beliefs_dir):
        ...
        for bud_time in get_level1_dirs(buds_dir):
            create_cell_tree(coin_mstr_dir, coin_label, belief_name, bud_time)


def create_cell_tree(coin_mstr_dir, coin_label, bud_belief_name, bud_time):
    root_cell_json_path = create_cell_json_path(
        coin_mstr_dir, coin_label, bud_belief_name, bud_time
    )
    if os_path_exists(root_cell_json_path):
        _exists_create_cell_tree(coin_mstr_dir, coin_label, bud_belief_name, bud_time)


def _exists_create_cell_tree(coin_mstr_dir, coin_label, bud_belief_name, bud_time):
    root_cell_dir = create_cell_dir_path(
        coin_mstr_dir, coin_label, bud_belief_name, bud_time, []
    )
    cells_to_evaluate = [cellunit_get_from_dir(root_cell_dir)]
    belief_events_sets = collect_belief_event_dir_sets(coin_mstr_dir, coin_label)
    while cells_to_evaluate != []:
        ...
        beliefevent = get_beliefevent_obj(
            coin_mstr_dir, coin_label, cell_belief_name, e_int
        )
...
def load_cells_beliefevent(coin_mstr_dir: str, coin_label: LabelTerm):
    coins_dir = create_path(coin_mstr_dir, "coins")
    coin_dir = create_path(coins_dir, coin_label)
    beliefs_dir = create_path(coin_dir, "beliefs")
    for belief_name in get_level1_dirs(beliefs_dir):
        ...
        for bud_time in get_level1_dirs(buds_dir):
            ...
            for dirpath, dirnames, filenames in os_walk(bud_time_dir):
                if CELLNODE_FILENAME in set(filenames):
                    _load_cell_beliefevent(coin_mstr_dir, coin_label, dirpath)


def _load_cell_beliefevent(coin_mstr_dir, coin_label, dirpath):
    x_cellunit = cellunit_get_from_dir(dirpath)
    cell_belief_name = x_cellunit.get_cell_belief_name()
    event_int = x_cellunit.event_int
    beliefevent = get_beliefevent_obj(
        coin_mstr_dir, coin_label, cell_belief_name, event_int
    )
    ...
    for belief_name in get_level1_dirs(beliefs_dir):
        ...
        for bud_time in get_level1_dirs(buds_dir):
            ...
            set_cell_tree_decrees(
                coin_mstr_dir, coin_label, belief_name, bud_time, bud_time_dir
            )
...
def set_cell_tree_decrees(
    ...
    # add nodes to to_evalute_cellnodes based on partner_agenda_fund_give beliefs
    root_cell = cellunit_get_from_dir(bud_time_dir)
    root_cell_dir = create_cell_dir_path(
        mstr_dir, coin_label, belief_name, bud_time, []
    )
    ...
    while to_evaluate_decreeunits != []:
        ...
        if x_cell := cellunit_get_from_dir(
            x_decree.cell_dir
        ) or generate_cell_from_decree(x_decree, mstr_dir, coin_label, belief_name):
...
def _add_child_decrees(
    ...
    for child_belief_name, child_mandate in x_cell._partner_mandate_ledger.items():
        child_cell_ancestors = x_decree.get_child_cell_ancestors(child_belief_name)
        child_dir = create_cell_dir_path(
            mstr_dir, coin_label, belief_name, bud_time, child_cell_ancestors
        )
...
def generate_cell_from_decree(
    x_decree: DecreeUnit, mstr_dir: str, coin_label: str, belief_name: BeliefName
) -> CellUnit:
    cell_belief_name = x_decree.cell_belief_name
    beliefs_downhill_events_ints = get_beliefs_downhill_event_ints(
        belief_events_sets=collect_belief_event_dir_sets(mstr_dir, coin_label),
    ...
    if downhill_event_int := beliefs_downhill_events_ints.get(cell_belief_name):
        beliefevent_path = create_beliefevent_path(
            mstr_dir, coin_label, cell_belief_name, downhill_event_int
        )

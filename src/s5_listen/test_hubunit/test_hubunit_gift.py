from src.s0_instrument.file import open_file, dir_files, delete_dir, set_dir, save_file
from src.s1_road.jaar_config import init_gift_id, get_test_fiscal_id as fiscal_id
from src.s4_gift.gift import giftunit_shop, get_json_filename
from src.s5_listen.hubunit import hubunit_shop
from src.s5_listen.examples.example_listen_atoms import get_atom_example_ideaunit_knee
from src.s5_listen.examples.example_listen_gifts import (
    get_sue_giftunit,
...
def test_HubUnit_save_gift_file_RaisesErrorIfGiftUnit_owner_id_IsWrong(
    ...
    assert (
        str(excinfo.value)
        == f"GiftUnit file cannot be saved because giftunit.owner_id is incorrect: {sue_giftunit.owner_id}. It must be {sue_str}."
    )
...
def test_HubUnit_default_giftunit_ReturnsObjWithCorrect_gift_id_WhenNogiftFilesExist(
    ...
    # THEN
    assert sue_giftunit.owner_id == sue_str
...
def test_HubUnit_merge_any_gifts_ReturnsObj_WithSinglegiftModifies_1atom(
    ...
    sue_hubunit.save_voice_bud(sue_hubunit.default_voice_bud())
    voice_bud = sue_hubunit.get_voice_bud()
    print(f"{voice_bud._fiscal_id=}")
    print(f"{sue_hubunit.fiscal_id=}")
    sports_str = "sports"

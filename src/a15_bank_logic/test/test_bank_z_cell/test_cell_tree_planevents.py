...
from src.a15_bank_logic.bank_cell import load_cells_planevent
from src.a15_bank_logic.test._util.a15_env import (
    env_dir_setup_cleanup,
    get_module_temp_dir,
)
from src.a15_bank_logic.test._util.example_banks import example_casa_clean_factunit
...
def test_load_cells_planevent_SetsFiles_Scenario0_NoFacts(
    ...
    event300 = 300
    time5 = 5
    save_arbitrary_planevent(bank_mstr_dir, a23_str, bob_str, event300)
    bob3_planevent_path = create_planevent_path(
        bank_mstr_dir, a23_str, bob_str, event300
    )
    print(f"{bob3_planevent_path=}")
    cellunit_add_json_file(bank_mstr_dir, a23_str, bob_str, time5, event300, [])
    bob5_cell_path = create_cell_json_path(bank_mstr_dir, a23_str, bob_str, time5)
    assert open_json(bob5_cell_path).get(planevent_facts_str()) == {}

    # WHEN
    load_cells_planevent(bank_mstr_dir, a23_str)
...
def test_load_cells_planevent_SetsFiles_Scenario1_WithFacts(
    ...
    save_arbitrary_planevent(bank_mstr_dir, a23_str, bob_str, event300, facts=x_facts)
    bob3_planevent_path = create_planevent_path(
        bank_mstr_dir, a23_str, bob_str, event300
    )
    print(f"{bob3_planevent_path=}")
...
def test_load_cells_planevent_SetsFiles_Scenario2_WithFacts_NotAtRoot(
    ...
    yao_str = "Yao"
    das = [yao_str, bob_str]
    cellunit_add_json_file(bank_mstr_dir, a23_str, bob_str, time5, event300, das)
    bob5_cell_path = create_cell_json_path(bank_mstr_dir, a23_str, bob_str, time5, das)
    assert open_json(bob5_cell_path).get(planevent_facts_str()) == {}

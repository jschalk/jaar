...
from src.a02_finance_logic.test._util.a02_str import celldepth_str, quota_str
...
from src.a15_bank_logic.bank import _get_ote1_max_past_event_int, bankunit_shop
...
def test_BankUnit_create_buds_root_cells_Scenaro0_BudEmpty(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    a23_str = "accord23"
    ...
    a23_json_path = create_bank_json_path(bank_mstr_dir, a23_str)
    save_file(a23_json_path, None, accord23_bank.get_json())
    print(f"{a23_json_path=}")
    a23_owners_path = create_bank_owners_dir_path(bank_mstr_dir, a23_str)
    assert count_dirs_files(a23_owners_path) == 0

    # WHEN
    accord23_bank.create_buds_root_cells({})

    # THEN
    assert count_dirs_files(a23_owners_path) == 0


def test_BankUnit_create_buds_root_cells_Scenaro1_BudExists(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    ...
    timepoint37 = 37
    bud1_quota = 450
    accord23_bank.add_budunit(bob_str, timepoint37, bud1_quota)
    ...
    # WHEN
    accord23_bank.create_buds_root_cells(a23_ote1_dict)
...
def test_BankUnit_create_buds_root_cells_Scenaro2_BudExistsButNoPlanExistsInEventsPast(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    mstr_dir = get_module_temp_dir()
...
def test_BankUnit_create_buds_root_cells_Scenaro3_BudExistsNotPerfectMatch_bud_time_event_int(
    env_dir_setup_cleanup,
):
    # ESTABLISH
    mstr_dir = get_module_temp_dir()
    ...
    # Create BankUnit with bob bud at time 37
    accord23_bank = bankunit_shop(a23_str, mstr_dir, penny=a23_penny)
    print(f"{accord23_bank.penny=}")
    bob_str = "Bob"
    timepoint37 = 37
    bud1_quota = 450
    bud1_celldepth = 3
    accord23_bank.add_budunit(

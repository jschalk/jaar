...
from src.a15_bank_logic.bank_cell import create_cell_tree
...
def test_create_cell_tree_Scenaro0_timepoint_Empty(env_dir_setup_cleanup):
    # ESTABLISH
    bank_mstr_dir = create_path(get_module_temp_dir(), "fizz_mstr")
    a23_str = "accord23"
    bob_str = "Bob"
    tp37 = 37

    a23_bob_tp37_path = cell_dir(bank_mstr_dir, a23_str, bob_str, tp37, [])
    print(f"{a23_bob_tp37_path=}")
    assert os_path_exists(a23_bob_tp37_path) is False

    # WHEN
    create_cell_tree(bank_mstr_dir, a23_str, bob_str, tp37)
...
def test_create_cell_tree_Scenaro1_LedgerDepth0(env_dir_setup_cleanup):
    ...
    bob37_root_cell_dir = cell_dir(bank_mstr_dir, a23_str, bob_str, tp37, [])
    cellunit_save_to_dir(bob37_root_cell_dir, x_cell)
    save_planevent(bank_mstr_dir, a23_str, bob_str, event56, [[yao_str], [bob_str]])
    assert (
...
def test_create_cell_tree_Scenaro2_LedgerDepth1(env_dir_setup_cleanup):
    ...
    bob37_dir = cell_dir(bank_mstr_dir, a23_str, bob_str, tp37, [])
    cellunit_save_to_dir(bob37_dir, x_cell)
    bob_accts = [[yao_str], [bob_str], [zia_str]]
    yao_accts = [[zia_str]]
    zia_accts = [[bob_str], [yao_str]]
    bob_e56_path = save_planevent(bank_mstr_dir, a23_str, bob_str, event56, bob_accts)
    yao_e56_path = save_planevent(bank_mstr_dir, a23_str, yao_str, event56, yao_accts)
    zia_e56_path = save_planevent(bank_mstr_dir, a23_str, zia_str, event56, zia_accts)
    assert os_path_exists(bob_e56_path)
    assert os_path_exists(yao_e56_path)
    assert os_path_exists(zia_e56_path)
    bob37_node_path = node_path(bank_mstr_dir, a23_str, bob_str, tp37, [])
    bob37_bob_node_path = node_path(bank_mstr_dir, a23_str, bob_str, tp37, [bob_str])
    bob37_yao_node_path = node_path(bank_mstr_dir, a23_str, bob_str, tp37, [yao_str])
    bob37_zia_node_path = node_path(bank_mstr_dir, a23_str, bob_str, tp37, [zia_str])
    ...
    bob37_bob_dir = cell_dir(bank_mstr_dir, a23_str, bob_str, tp37, [bob_str])
    bob37_yao_dir = cell_dir(bank_mstr_dir, a23_str, bob_str, tp37, [yao_str])
    bob37_zia_dir = cell_dir(bank_mstr_dir, a23_str, bob_str, tp37, [zia_str])
...
def test_create_cell_tree_Scenaro3_LedgerDepth1_MostRecentEvent(env_dir_setup_cleanup):
    ...
    yao_accts = [[zia_str]]
    zia_accts = [[bob_str], [yao_str]]
    bob_e55_path = save_planevent(bank_mstr_dir, a23_str, bob_str, event55, bob_accts)
    yao_e44_path = save_planevent(bank_mstr_dir, a23_str, yao_str, event44, yao_accts)
    yao_e33_path = save_planevent(bank_mstr_dir, a23_str, yao_str, event33, yao_accts)
    zia_e33_path = save_planevent(bank_mstr_dir, a23_str, zia_str, event33, zia_accts)
...
def test_create_cell_tree_Scenaro4_LedgerDepth1_OneOwnerHasNoPast_planevent(
    ...
    yao_e44_path = save_planevent(bank_mstr_dir, a23_str, yao_str, event44, yao_accts)
    yao_e33_path = save_planevent(bank_mstr_dir, a23_str, yao_str, event33, yao_accts)
    zia_e66_path = save_planevent(bank_mstr_dir, a23_str, zia_str, event66, zia_accts)
    assert os_path_exists(bob_e55_path)
    assert os_path_exists(yao_e44_path)

...
from src.a12_hub_toolbox.hub_path import create_bank_json_path, create_cell_dir_path
...
class BankUnit:
    ...
    # directory setup
    def _set_bank_dirs(self):
        banks_dir = create_path(self.bank_mstr_dir, "banks")
        self._bank_dir = create_path(banks_dir, self.bank_label)
        self._owners_dir = create_path(self._bank_dir, "owners")
        self._packs_dir = create_path(self._bank_dir, "packs")
        set_dir(x_path=self._bank_dir)
    ...
    def _set_all_healer_dutys(self, owner_name: OwnerName):
        x_gut = open_gut_file(self.bank_mstr_dir, self.bank_label, owner_name)
        x_gut.settle_plan()
    ...
    # job plan management
    def create_empty_plan_from_bank(self, owner_name: OwnerName) -> PlanUnit:
    ...
    def create_gut_file_if_none(self, owner_name: OwnerName) -> None:
        if not gut_file_exists(self.bank_mstr_dir, self.bank_label, owner_name):
            empty_plan = self.create_empty_plan_from_bank(owner_name)
            save_gut_file(self.bank_mstr_dir, empty_plan)
    ...
    def create_init_job_from_guts(self, owner_name: OwnerName) -> None:
        ...
        listen_to_agendas_create_init_job_from_guts(self.bank_mstr_dir, x_job)
        save_job_file(self.bank_mstr_dir, x_job)

    def rotate_job(self, owner_name: OwnerName) -> PlanUnit:
        x_job = open_job_file(self.bank_mstr_dir, self.bank_label, owner_name)
        x_job.settle_plan()
        ...
        return listen_to_debtors_roll_jobs_into_job(
            self.bank_mstr_dir, self.bank_label, owner_name
        )
    ...
    def generate_all_jobs(self) -> None:
        ...
        for _ in range(self.job_listen_rotations):
            for owner_name in owner_names:
                save_job_file(self.bank_mstr_dir, self.rotate_job(owner_name))

    def get_job_file_plan(self, owner_name: OwnerName) -> PlanUnit:
        return open_job_file(self.bank_mstr_dir, self.bank_label, owner_name)
    ...
    def add_budunit(
        ...
        if bud_time < self._offi_time_max and not allow_prev_to_offi_time_max_entry:
            exception_str = f"Cannot set budunit because bud_time {bud_time} is less than BankUnit._offi_time_max {self._offi_time_max}."
    ...
    def set_all_tranbook(self) -> None:
        ...
        x_tranbook = tranbook_shop(self.bank_label, x_tranunits)
    ...
    def _create_bud_root_cell(
        ...
        root_cell_dir = create_cell_dir_path(
            self.bank_mstr_dir, self.bank_label, owner_name, bud_time, []
        )
...
def _get_ote1_max_past_event_int(
    ...
    """Using the bank_ote1_agg grab most recent event int before a given bud_time"""
...
def bankunit_shop(
    ...
    x_bankunit = BankUnit(
        ...
        timeline=timeline,
        brokerunits={},
        paybook=tranbook_shop(bank_label),
...
def get_from_dict(bank_dict: dict) -> BankUnit:
    x_bank_label = bank_dict.get("bank_label")
    x_bank = bankunit_shop(
        bank_label=x_bank_label,
        bank_mstr_dir=bank_dict.get("bank_mstr_dir"),
        offi_times=set(bank_dict.get("offi_times")),
        knot=bank_dict.get("knot"),
        fund_iota=bank_dict.get("fund_iota"),
        respect_bit=bank_dict.get("respect_bit"),
        penny=bank_dict.get("penny"),
    )
    bank_dict_timeline_value = bank_dict.get("timeline")
    if bank_dict_timeline_value:
        x_bank.timeline = timelineunit_shop(bank_dict_timeline_value)
    else:
        x_bank.timeline = timelineunit_shop(None)
    x_bank.brokerunits = _get_brokerunits_from_dict(bank_dict.get("brokerunits"))
    x_bank.paybook = get_tranbook_from_dict(bank_dict.get("paybook"))
    return x_bank


def get_from_json(x_bank_json: str) -> BankUnit:
    return get_from_dict(get_dict_from_json(x_bank_json))


def get_from_default_path(bank_mstr_dir: str, bank_label: BankLabel) -> BankUnit:
    bank_json_path = create_bank_json_path(bank_mstr_dir, bank_label)
    x_bankunit = get_from_json(open_file(bank_json_path))
    x_bankunit.bank_mstr_dir = bank_mstr_dir
    x_bankunit._set_bank_dirs()
    return x_bankunit

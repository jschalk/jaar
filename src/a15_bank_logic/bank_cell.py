...
def create_bank_owners_cell_trees(bank_mstr_dir, bank_label):
    ...
    for owner_name in get_level1_dirs(owners_dir):
        ...
        for bud_time in get_level1_dirs(buds_dir):
            create_cell_tree(bank_mstr_dir, bank_label, owner_name, bud_time)


def create_cell_tree(bank_mstr_dir, bank_label, bud_owner_name, bud_time):
    root_cell_json_path = create_cell_json_path(
        bank_mstr_dir, bank_label, bud_owner_name, bud_time
    )
    if os_path_exists(root_cell_json_path):
        _exists_create_cell_tree(bank_mstr_dir, bank_label, bud_owner_name, bud_time)


def _exists_create_cell_tree(bank_mstr_dir, bank_label, bud_owner_name, bud_time):
    root_cell_dir = create_cell_dir_path(
        bank_mstr_dir, bank_label, bud_owner_name, bud_time, []
    )
    cells_to_evaluate = [cellunit_get_from_dir(root_cell_dir)]
    owner_events_sets = collect_owner_event_dir_sets(bank_mstr_dir, bank_label)
    while cells_to_evaluate != []:
        ...
        planevent = get_planevent_obj(bank_mstr_dir, bank_label, cell_owner_name, e_int)
...
def load_cells_planevent(bank_mstr_dir: str, bank_label: LabelTerm):
    banks_dir = create_path(bank_mstr_dir, "banks")
    bank_dir = create_path(banks_dir, bank_label)
    owners_dir = create_path(bank_dir, "owners")
    for owner_name in get_level1_dirs(owners_dir):
        ...
        for bud_time in get_level1_dirs(buds_dir):
            ...
            for dirpath, dirnames, filenames in os_walk(bud_time_dir):
                if CELLNODE_FILENAME in set(filenames):
                    _load_cell_planevent(bank_mstr_dir, bank_label, dirpath)


def _load_cell_planevent(bank_mstr_dir, bank_label, dirpath):
    x_cellunit = cellunit_get_from_dir(dirpath)
    cell_owner_name = x_cellunit.get_cell_owner_name()
    event_int = x_cellunit.event_int
    planevent = get_planevent_obj(bank_mstr_dir, bank_label, cell_owner_name, event_int)
    x_cellunit.eval_planevent(planevent)
    cellunit_save_to_dir(dirpath, x_cellunit)


def set_cell_trees_found_facts(bank_mstr_dir: str, bank_label: LabelTerm):
    banks_dir = create_path(bank_mstr_dir, "banks")
    bank_dir = create_path(banks_dir, bank_label)
    owners_dir = create_path(bank_dir, "owners")
    for owner_name in get_level1_dirs(owners_dir):
...
def set_cell_trees_decrees(bank_mstr_dir: str, bank_label: str):
    banks_dir = create_path(bank_mstr_dir, "banks")
    bank_dir = create_path(banks_dir, bank_label)
    owners_dir = create_path(bank_dir, "owners")
    for owner_name in get_level1_dirs(owners_dir):
        ...
        for bud_time in get_level1_dirs(buds_dir):
            ...
            set_cell_tree_decrees(
                bank_mstr_dir, bank_label, owner_name, bud_time, bud_time_dir
            )
...
def set_cell_tree_decrees(
    ...
    root_cell_dir = create_cell_dir_path(mstr_dir, bank_label, owner_name, bud_time, [])
    ...
    while to_evaluate_decreeunits != []:
        ...
        if x_cell := cellunit_get_from_dir(
            x_decree.cell_dir
        ) or generate_cell_from_decree(x_decree, mstr_dir, bank_label, owner_name):
...
def _add_child_decrees(
    ...
    for child_owner_name, child_mandate in x_cell._acct_mandate_ledger.items():
        ...
        child_dir = create_cell_dir_path(
            mstr_dir, bank_label, owner_name, bud_time, child_cell_ancestors
        )
...
def generate_cell_from_decree(
    x_decree: DecreeUnit, mstr_dir: str, bank_label: str, owner_name: OwnerName
) -> CellUnit:
    cell_owner_name = x_decree.cell_owner_name
    owners_downhill_events_ints = get_owners_downhill_event_ints(
        owner_events_sets=collect_owner_event_dir_sets(mstr_dir, bank_label),
    ...
    if downhill_event_int := owners_downhill_events_ints.get(cell_owner_name):
        planevent_path = create_planevent_path(
            mstr_dir, bank_label, cell_owner_name, downhill_event_int
        )
...
def set_cell_tree_cell_mandates(bank_mstr_dir: str, bank_label: str):
    banks_dir = create_path(bank_mstr_dir, "banks")
    bank_dir = create_path(banks_dir, bank_label)
    owners_dir = create_path(bank_dir, "owners")
    for owner_name in get_level1_dirs(owners_dir):
...
def create_bud_mandate_ledgers(bank_mstr_dir: str, bank_label: str):
    bank_json_path = create_bank_json_path(bank_mstr_dir, bank_label)
    bankunit = bankunit_get_from_dict(open_json(bank_json_path))
    for brokerunit in bankunit.brokerunits.values():
    ...
    save_json(bank_json_path, None, bankunit.get_dict())

...
def test_tranbook_shop_WithParametersReturnsObj():
    ...
    sue_str = "Sue"
    yao_str = "Yao"
    x_tranunits = {sue_str: {yao_str: {x_EpochInstant: x_fundnum}}}
...
def test_TranBook_set_tranunit_SetsAttrWithBlocktran_time_RaisesError():
    ...
    with pytest_raises(Exception) as excinfo:
        x_tranbook.set_tranunit(sue_yao_t55_tranunit, x_blocked_tran_times)
    exception_str = (
        f"Cannot set tranunit for tran_time={t55_t}, EpochInstant is blocked"
    )
...
def test_TranBook_set_tranunit_SetsAttrWithCurrenttran_time_RaisesError():
    ...
    exception_str = f"Cannot set tranunit for tran_time={t55_t}, EpochInstant is greater than current time={t55_t}"
    assert str(excinfo.value) == exception_str

    # WHEN / THEN
    with pytest_raises(Exception) as excinfo:
        x_tranbook.set_tranunit(sue_yao_t55_tranunit, offi_time_max=33)
    exception_str = f"Cannot set tranunit for tran_time={t55_t}, EpochInstant is greater than current time=33"
    assert str(excinfo.value) == exception_str
...
def test_TranBook_to_dict_ReturnsObj():
    ...
    sue_str = "Sue"
    yao_str = "Yao"
    all_tranunits = {sue_str: {yao_str: {x_EpochInstant: x_fundnum}}}
    ...
    assert sue_trans_dict.get(yao_str) == {x_EpochInstant: x_fundnum}

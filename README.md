...
# 0.0.1 "Banks" The foundation of jaar
For Levinas all of reality is born from the face to face encounter. The same (me) welcomes the other person through the Face. The Face of the other tells me it's suffering and it's suffering becomes my suffering. I then BANK to change who I am to ease that suffering. The suffering is infinitly deep and beyond my complete understanding so when I bank to respond to that suffering I am acting with confidence that I understand what the suffering is and that I know how to respond. That confidence stops the listening process, the Bank cuts the infinite into the finite and is the foundation for a world. When that Bank is created it can create a world. Worlds can hold a infinite amount of human experience. A small subset of that is logical systems. jaar is uses computers to build logic.

A Bank can create a world or change a current world. Each person can only make one bank at a time so a world that has been built by multiple banks implies each bank is from a different time. jaar describes the passage of time by *event_ints*. *event_int* is alropes an integer. 

For jaar all data must have *event_int*, *face_name*, *bank_label*. These are the required keys.
...
| event_int | face_name | bank_label | owner_name | acct_name | tran_time | amount |
|-----------|-----------|-----------|------------|-----------|-----------|--------|
|    77     | Emmanuel  | OxboxDean |  Emmanuel  |    Dean   |    891    |  7000  |

When jaar processes example 0.1.0 it creates a Bank labeled "OxboxDean" that contains owners Emmanuel and Dean and a single transaction of 7000 OxboxDean coins from Emmanuel to Dean. Here's the status metrics:
| bank_label | owner_name | bank_fund_amount | bank_fund_rank | bank_tasks |
|-----------|------------|-----------------|---------------|-----------|
